...
CALL %BEA_HOME%\weblogic92\server\bin\ant.bat -version 1>NUL 2>&1
IF %ERRORLEVEL% NEQ 0 (
...
ECHO;
SET ANT_HOME=%BEA_HOME%\weblogic92\server
SET ANT_HOME
...
SET CLASSPATH=%ANT_HOME%\lib\ant\junit.jar

@rem SET ANT_OPTS=-Xms256m -Xmx256m -XX:PermSize=128m -XX:MaxPermSize=128m
SET ANT_OPTS=
ECHO;
SET ANT_OPTS
