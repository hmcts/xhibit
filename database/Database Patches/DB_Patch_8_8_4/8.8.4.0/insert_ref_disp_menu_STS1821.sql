...
FROM XHB_COURT XC, 
      (SELECT xrdm.PARENT,
              'STS1821' DISPOSAL_CODE,
              'Serious Terrorism Sentence (18 to 21)' TITLE,
              NVL(MAX(xrdm.SEQ_NO),0)+1 NEXT_SEQ_NO 
         FROM XHB_REF_DISPOSAL_MENU xrdm 
        WHERE xrdm.PARENT = 100 -- 'IMC'
...
xrdm.TITLE
FROM XHB_REF_DISPOSAL_MENU xrdm
WHERE xrdm.DISPOSAL_CODE = 'STS1821'
...
FROM XHB_REF_DISPOSAL_MENU xrdm, 
     (SELECT 10 DIL_SEQ_NO,
      ...
      SELECT 20 DIL_SEQ_NO,
             NULL PROMPT,
             'N' INPUT_FLAG,
             'The court ordered that the defendant be sentenced to detention',
             NULL CONC_FLAG,
             'N' MANDATORY,
             'N' MULTIPLE_CHOICE,
             NULL DBDESTIN,
      ...
      SELECT 21 DIL_SEQ_NO,
             NULL PROMPT,
             'N' INPUT_FLAG,
             'under section 268a Sentencing Act 2020 as follows: An extended sentence of',
             NULL CONC_FLAG,
             'N' MANDATORY,
             'N' MULTIPLE_CHOICE,
             NULL DBDESTIN,
