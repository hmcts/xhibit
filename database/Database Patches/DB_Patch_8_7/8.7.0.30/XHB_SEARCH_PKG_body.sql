...
--    END log_entry;
    ...
    BEGIN
       ...
       ELSE
            ...
            lv_sql_string := 'SELECT ra.ref_advocate_id AS "id",
                   rlr.title,
                   rlr.first_name,
                   rlr.initials,
                   rlr.middle_name,
            ...
            if p_court_id_in IS NULL THEN
               lv_sql_string := lv_sql_string || ' and :1 is null';
            else
              lv_sql_string := lv_sql_string || ' and rlr.court_id=:1';
             end if;

            if l_adv_type_ind IS NULL THEN
               lv_sql_string := lv_sql_string || ' and :2 is null';
            else
              lv_sql_string := lv_sql_string || ' and UPPER(ra.adv_type_ind) LIKE :2';
             end if;

            if p_ref_legal_rep_id_in IS NULL THEN
               lv_sql_string := lv_sql_string || ' and :3 is null';
            else
              lv_sql_string := lv_sql_string || ' and ra.ref_legal_rep_id_in=:3';
             end if;

            if l_initials IS NULL THEN
               lv_sql_string := lv_sql_string || ' and :4 is null';
            else
              lv_sql_string := lv_sql_string || ' and UPPER(rlr.initials) LIKE :4';
             end if;

              if l_first_name IS NULL THEN
               lv_sql_string := lv_sql_string || ' and :5 is null';
            else
              lv_sql_string := lv_sql_string || ' and UPPER(rlr.first_name) LIKE :5';
             end if;

              if l_middle_name IS NULL THEN
               lv_sql_string := lv_sql_string || ' and :6 is null';
            else
              lv_sql_string := lv_sql_string || ' and UPPER(rlr.middle_name) LIKE :6';
             end if;

              if l_surname IS NULL THEN
               lv_sql_string := lv_sql_string || ' and :7 is null';
            else
              lv_sql_string := lv_sql_string || ' and UPPER(rlr.surname) LIKE :7';
             end if;

              if l_firm_name IS NULL THEN
               lv_sql_string := lv_sql_string || ' and :8 is null';
            else
              lv_sql_string := lv_sql_string || ' and UPPER(rc.firm_name) LIKE :8';
            end if;


            lv_sql_string := lv_sql_string || ' ORDER BY rlr.surname';

          OPEN p_results_out FOR
            lv_sql_string
          USING p_court_id_in, l_adv_type_ind, p_ref_legal_rep_id_in, l_initials, l_first_name, l_middle_name, l_surname, l_firm_name;
    ...
    BEGIN
        ...
        OPEN p_results_out FOR
            SELECT rsf.ref_solicitor_firm_id AS "id",
                   ...
                   '' as telephone_Number,
                   '' as fax_Number,
                   '' as nonsecure_Email_Address,
                   '' as secure_Email_Address
