...
**************************************************************************************/
  ...
  BEGIN
      ...
      and   xdhr.HEARING_END_DATE = (SELECT MAX(xdhr2.HEARING_END_DATE)
									...
									AND   xrsc2.CODE IN (SELECT regexp_substr(p_refSystemCode, '[^,]+', 1, LEVEL) FROM DUAL
...
FUNCTION is_dealtwith_RREC_Case (p_case_id    xhb_case.case_id%type 
	...
	CURSOR c_exported_with_end_1 IS
	Select count(doc.defendant_on_case_id)
	From   xhb_defendant_on_case doc
	...
	CURSOR c_exported_with_end_2 IS
	Select count(doc.defendant_on_case_id)
	From   xhb_defendant_on_case doc
...
BEGIN
	...
	ELSE
		...
		IF v_def_count > 0 THEN 
			...
			-- See if there are any defendants that have been exported
			IF p_end_date IS NULL THEN
				OPEN c_exported_no_end;
				FETCH c_exported_no_end INTO v_exported_count;
				CLOSE c_exported_no_end;
			ELSE
				OPEN c_exported_with_end_1;
				FETCH c_exported_with_end_1 INTO v_exported_count;
				CLOSE c_exported_with_end_1;
			END IF;
			
			IF v_def_count = v_exported_count THEN
				-- All defendants exported
				IF p_end_date IS NULL THEN
					-- Case is Dealt With
					v_retval := 'Y';
				ELSE
					-- Check at least one defendant has been exported within the reporting period
					OPEN c_exported_with_end_2;
					FETCH c_exported_with_end_2 INTO v_exp_in_period_cnt;
					CLOSE c_exported_with_end_2;
			...
			IF v_def_count > v_exported_count AND ( v_def_count - v_exported_count ) = v_bw_count THEN
