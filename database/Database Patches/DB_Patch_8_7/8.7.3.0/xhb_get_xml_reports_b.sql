  ...
  FUNCTION get_U_case_defendant(p_case_title IN XHB_CASE.CASE_TITLE%TYPE) RETURN XMLType IS
    ...
    BEGIN
      FOR array_no IN 1..3 LOOP
          ...
          -- Wordwrap the array cell (if required)
          IF xhb_formatting_pkg.wordwrapping_position(v_array(array_no),v_field_length) > 0 THEN  
             -- Trim the final word back to the previous space
             v_array(array_no) := xhb_formatting_pkg.get_wordwrapped_string(v_array(array_no),v_field_length);
             -- Reset the character number back to the previous space (for next array cell)
             v_char_no := v_char_no - (v_field_length - LENGTH(v_array(array_no)));
          END IF;
      END LOOP;
    END populate_array;
  BEGIN
    ...
    SELECT XMLELEMENT("cs:Defendant",
                XMLELEMENT("cs:PersonalDetails",
                   XMLFOREST(XMLFOREST(RTRIM(v_array(2)) AS "apd:CitizenNameForename",
                                       RTRIM(v_array(3)) AS "apd:CitizenNameForename",
                                       RTRIM(v_array(1)) AS "apd:CitizenNameSurname"
                                       ) AS "cs:Name"
                            ,'yes' AS "cs:IsMasked")
                          )--PersonalDetails
                      )--Defendant
