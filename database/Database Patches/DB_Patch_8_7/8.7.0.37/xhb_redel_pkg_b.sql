...
PROCEDURE find_future_fixtures (p_results_out OUT SYS_REFCURSOR,
  ...
  BEGIN
    ...
    OPEN p_results_out	FOR
      SELECT DISTINCT * from XHB_CASE_DIARY_FIXTURE cdf where cdf.LISTING_DATE > p_current_date and cdf.case_listing_entry_id in
...
/* PROCEDURE find_future_fixtures 
      (p_case_id IN xhb_case.case_id%TYPE, p_current_date date)*/
...
PROCEDURE find_future_listings (p_results_out OUT SYS_REFCURSOR,
  ...
  BEGIN
    ...
    -- 1
    OPEN p_results_out	FOR
    SELECT DISTINCT * from XHB_CASE_ON_LIST col where col.case_id = p_case_id and col.list_id in
...
/* PROCEDURE find_future_listings
      (p_case_id IN xhb_case.case_id%TYPE, p_current_date date)*/      
