CREATE OR REPLACE PACKAGE BODY XHB_LISTING_PKG AS
  ...
  FUNCTION case_on_list_exists_YN(p_case_id IN XHB_CASE_ON_LIST.CASE_ID%TYPE,
                                  p_date    IN XHB_CASE_ON_LIST.TIME_LISTED%TYPE)
    RETURN VARCHAR2 IS
    v_result VARCHAR2(1);
    CURSOR C_xcol IS
    SELECT 'Y'
  ...
  PROCEDURE find_cases_for_list_results( p_results_out   OUT SYS_REFCURSOR,
      ...
      BEGIN
		...
		SELECT	XC.CASE_ID,
				...
				CASE WHEN NVL(XSH.REF_CRACKED_EFFECTIVE_ID,0) != 0 THEN XRCE.DESCRIPTION
					WHEN XC.CASE_TYPE = 'T' AND XRHT.HEARING_TYPE_CODE IN ('TRL','TFL','TBK') THEN 'Missing'
