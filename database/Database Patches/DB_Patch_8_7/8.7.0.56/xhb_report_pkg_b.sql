...
'    SELECT notes.DIARY_NOTE_ENTRY_ID,
    ...
    WHERE xc.DEFAULT_HEARING_TYPE = xrht.REF_HEARING_TYPE_ID(+)
    AND   xrmc.REF_MONITORING_CATEGORY_ID = xc.MONITORING_CATEGORY_ID
            -- Next 3 lines are the only difference between Where clause of OUTC report and this one (UNLC). They contain no bind variables. 
...
FUNCTION is_dealtwith_RREC_Case (p_case_id    xhb_case.case_id%type 
	...
	Where  xv.case_id = p_case_id
	and    nvl(xv.obs_ind, 'N') != 'Y'
	and    xv.VERDICT_DATE >= p_start_date AND xv.VERDICT_DATE <= p_end_date
	and    xv.ref_verdict_id is not null
	AND    xrsc.ref_system_code_id = xv.ref_verdict_id
	AND    xrsc.code NOT IN ('NH','PH');
	
	CURSOR c_exported_no_end IS
	Select count(doc.defendant_on_case_id)
	From   xhb_defendant_on_case doc
	where  doc.date_exported < p_start_date
	and    NVL(doc.results_verified,'-') = 'E'
	and    doc.case_id = p_case_id;
	
	CURSOR c_exported_with_end IS
	Select count(doc.defendant_on_case_id)
	From   xhb_defendant_on_case doc
	where  doc.date_exported >= p_start_date AND doc.date_exported <= p_end_date
	and    NVL(doc.results_verified,'-') = 'E'
	and    doc.case_id = p_case_id;
	
	CURSOR c_bw_no_end IS
	SELECT COUNT(*)
	...
	AND NVL(xbh.obs_ind,'N') = 'N'
	AND xbh.bw_issue_date IS NOT NULL 
	AND xbh.bw_issue_date >= p_start_date AND xbh.bw_issue_date <= p_end_date
	AND (xbh.bw_end_date IS NULL OR xbh.bw_end_date > p_start_date);

	v_retval		         VARCHAR2(1) := 'N';
	v_case_type		       xhb_case.case_type%TYPE;
	v_sub_type		       xhb_case.case_sub_type%TYPE;
	v_date_trans_rec_to  xhb_case.date_trans_to%TYPE;
	v_verdict_count      number(8) := 0;
	v_not_exported_count number(8) := 0;
	v_def_count          number(8) := 0;
	v_bw_count           number(8) := 0;

BEGIN
	SELECT NVL(case_type, '-'), NVL(case_sub_type, '-'), nvl(date_trans_recorded_to, p_start_date)
	INTO v_case_type, v_sub_type, v_date_trans_rec_to
	FROM xhb_case
	WHERE case_id = p_case_id;
	
	IF p_end_date IS NOT NULL AND v_date_trans_rec_to < p_start_date THEN  -- Transferred out (not required for section 7 of RREC where end date is specified)
        v_retval := 'Y';
	ELSE    	
		...
		ELSE 
			...
			select count(*) into v_def_count from xhb_defendant_on_case where case_id = p_case_id;
		
			-- See if there are any defendants that have not been exported
			IF p_end_date IS NULL THEN
				OPEN c_exported_no_end;
				FETCH c_exported_no_end INTO v_not_exported_count;
				CLOSE c_exported_no_end;
			ELSE
				OPEN c_exported_with_end;
				FETCH c_exported_with_end INTO v_not_exported_count;
				CLOSE c_exported_with_end;
			END IF;
			
			IF v_def_count = v_not_exported_count THEN
				-- all defendants exported, case is Dealt With
				v_retval := 'Y';
			ELSE
				-- Check for Bench Warrant
				IF p_end_date IS NULL THEN
					OPEN c_bw_no_end;
					FETCH c_bw_no_end INTO v_bw_count;
					CLOSE c_bw_no_end;
				ELSE
					OPEN c_bw_with_end;
					FETCH c_bw_with_end INTO v_bw_count;
					CLOSE c_bw_with_end;
				END IF;
				
				IF ( v_def_count - v_not_exported_count ) = v_bw_count THEN
  ...
  **************************************************************************************/
    ...
    DBMS_OUTPUT.PUT_LINE('get_RREC_Summary: step ' || v_step || '  start date = ' || v_start_date || ', end date = ' || v_end_date);
      ...
      open p_resultset for
        ...
        sum(case when  (nvl(cas.creation_date, '01-Jan-1900')  < v_end_date    -- parameter here
                 and   nvl(cas.creation_date, '01-Jan-1900')   > v_start_date  
