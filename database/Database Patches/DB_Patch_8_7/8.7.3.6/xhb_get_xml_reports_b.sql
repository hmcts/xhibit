  ...
  BEGIN
                SELECT  XMLELEMENT("DLData"
                              ...
                              ,(SELECT XMLAgg(XMLELEMENT("DefendantOnCase", xc.CASE_TYPE || xc.CASE_NUMBER || '|' || xc.CASE_TYPE || '|' || xd.CREST_DEFENDANT_ID || '||' || ESCAPE_BROKER_RELEASE_CHARS(xdoc.PNC_ID) || '|' || ESCAPE_BROKER_RELEASE_CHARS(xdoc.DEFENDANT_NUMBER) || '|' || xdoc.IS_JUVENILE))
                                            ...
                                            AND xdoc.DEFENDANT_ID = xd.DEFENDANT_ID)
                              ,(SELECT XMLAgg(XMLELEMENT("ScheduledHearingDefendant", xd.CREST_DEFENDANT_ID || '|' || xc.CASE_TYPE || xc.CASE_NUMBER || '|' || xc.CASE_TYPE || '|' ||  NVL(xcr.CREST_COURT_ROOM_NO,1)  || '|' || NVL(xrj.CREST_JUDGE_ID,0) || '|' || ESCAPE_BROKER_RELEASE_CHARS(xrht.HEARING_TYPE_CODE) || '|' ||
  ...
  BEGIN
    SELECT XMLAgg(
                  XMLELEMENT("ScheduledHearing",xcol.SEQ_NO || '|' || CASE WHEN(xrsc.DE_CODE IS NOT NULL) THEN ESCAPE_BROKER_RELEASE_CHARS(xrsc.DE_CODE) ||  TO_CHAR(xcol.TIME_LISTED, 'HH24:MI am') END ||  '|' ||  TO_CHAR(xcol.TIME_LISTED, 'YYYY-MM-DDHH24:MI') || '|' || TO_CHAR(xsol.TIME_LISTED, 'HH24:MI am') || '|'
                                                ...
                                                || NVL(xcr.CREST_COURT_ROOM_NO, 1) || '|' || ESCAPE_BROKER_RELEASE_CHARS(xcs.COURT_SITE_CODE) || '|' || NVL(xrj.CREST_JUDGE_ID, 0) || '|' || NVL(xsol.SITTING_NUMBER, 0) || '|' ||  ESCAPE_BROKER_RELEASE_CHARS(xrht.HEARING_TYPE_DESC) || '|' || ESCAPE_BROKER_RELEASE_CHARS(xrht.CATEGORY)
