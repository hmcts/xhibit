    ...
    BEGIN
         ...
         UNION ALL
             SELECT
                 ...
                 getFaxNumber(xrsf.address_id) AS solicitor_firm_fax,
                 getEmailAddress(xrsf.address_id) AS solicitor_firm_email,
         ...
         UNION ALL
             SELECT
                 ...
                 RTRIM(NVL2(xa.address_1, xa.address_1 || ',', NULL) ||
                       ...
                       NVL2(xa.postcode, xa.postcode || ',', NULL) ||
                       NVL(xa.country, NULL), ',') AS solictior_firm_address,
                 getFaxNumber(xrpa.address_id) AS solicitor_firm_fax,
                 getEmailAddress(xrpa.address_id) AS solicitor_firm_email,
                 NULL AS recipient_last_update_date,
                 NULL AS recipient_creation_date,
                 NULL AS recipient_created_by,
                 NULL AS recipient_last_updated_by,
...
---------------------------------------------------------------------------
    ...
    ---------------------------------------------------------------------------
    --
    FUNCTION get_next_control_id(p_is_pilot IN NUMBER) RETURN XHB_WLL_CONTROL.wll_control_id%TYPE
    IS
    ...
    BEGIN
        ...
        IF p_is_pilot = 1 THEN
        BEGIN

            SELECT wll_control_id, G_PROCESSING_DOCUMENT
            INTO   l_wll_control_id, l_new_status
            FROM   XHB_WLL_CONTROL xcon, XHB_XML_DOCUMENT xdoc
            WHERE  xcon.status = 'DA'   
            AND xcon.xml_document_id=xdoc.XML_DOCUMENT_ID
            AND xdoc.court_id in(select court_id from xhb_court where is_pilot ='Y')
            AND    ROWNUM <= 1
            FOR UPDATE;
        ...
        EXCEPTION
            ...
            WHEN NO_DATA_FOUND THEN
                ...
                BEGIN
                    ...
                    INTO   l_wll_control_id, l_new_status
                    FROM   XHB_WLL_CONTROL xcon, XHB_XML_DOCUMENT xdoc
                    WHERE  xcon.status = 'PE'         -- Processing error...
                    AND xcon.xml_document_id=xdoc.XML_DOCUMENT_ID
                    AND xdoc.court_id in(select court_id from xhb_court where is_pilot ='Y')
                    AND    ROWNUM <= 1
                    FOR UPDATE;
    ...
    PROCEDURE SET_LIST_PUBLISH_STATUS(p_list_id           IN XHB_LIST.LIST_ID%TYPE,
                                      p_publish_date      IN XHB_LIST.PUBLISH_DATE%TYPE,
                                      p_publish_status    IN XHB_LIST.PUBLISH_STATUS%TYPE,
                                      p_publish_error_msg IN XHB_LIST.PUBLISH_ERROR_REASON%TYPE DEFAULT NULL) IS
      PRAGMA AUTONOMOUS_TRANSACTION;
    BEGIN
      IF p_publish_status = 'SUCCESS' THEN
          UPDATE xhb_list xl
             SET xl.list_number = NVL(xl.list_number,0) + CASE WHEN NVL(xl.publish_status,'~') IN ('SUCCESS','FAILURE') THEN 1 ELSE 0 END,
                 xl.publish_date = p_publish_date,
                 xl.publish_status = p_publish_status,
                 xl.publish_error_reason = NULL
           WHERE xl.list_id = p_list_id;
       ELSE
          UPDATE xhb_list xl
             SET xl.publish_status = p_publish_status,
                 xl.publish_error_reason = p_publish_error_msg
           WHERE xl.list_id = p_list_id;
       END IF;
       COMMIT;
    END SET_LIST_PUBLISH_STATUS;

    PROCEDURE PUBLISH_LIST(p_list_id XHB_LIST.LIST_ID%TYPE) IS
        v_return_xml CLOB;
        v_merc_xml CLOB;
        v_today_date VARCHAR2(10);
        v_list_row XHB_LIST%ROWTYPE;
        v_list_type VARCHAR2(32);
        v_list_name VARCHAR2(100);
        v_list_start_date VARCHAR2(10);
        v_list_end_date VARCHAR2(10);
        v_list_date_range VARCHAR2(50);
        v_list_version VARCHAR2(50);
        v_document_type_code VARCHAR2(3);
        ...
        v_formatted_list_date VARCHAR(128);
        l_validation_id XHB_VALIDATION.VALIDATION_ID%TYPE;
        v_unique_id VARCHAR(128);
    BEGIN
        SELECT *
        INTO v_list_row
        FROM XHB_LIST xl
        ...
        INTO v_list_court
        FROM XHB_COURT xc
        WHERE xc.COURT_ID = v_list_row.COURT_ID;

        SELECT XHB_VALIDATION_SEQ.NEXTVAL INTO l_validation_id FROM DUAL;

        --Determine the List Type
        SELECT xrld.REF_DATA_VALUE
        INTO v_list_type
        FROM XHB_REF_LISTING_DATA xrld
        WHERE xrld.REF_LISTING_DATA_ID = v_list_row.LIST_TYPE_ID
        AND xrld.REF_DATA_TYPE = 'LIST_TYPE';

        --Determine the date range and list version which are used in document titles, names and descriptions
        v_today_date := TO_CHAR(sysdate, 'DD/MM/YY');
        v_list_start_date := TO_CHAR(v_list_row.LIST_START_DATE, 'DD/MM/YY');
        v_list_end_date := TO_CHAR(v_list_row.LIST_END_DATE, 'DD/MM/YY');
        v_list_date_range := 'from ' || v_list_start_date || ' to ' || v_list_end_date;
        SELECT DECODE(v_list_row.DRAFT_OR_FINAL, 'D', 'DRAFT v', 'F', 'FINAL v') || v_list_row.LIST_NUMBER INTO v_list_version FROM DUAL;

        IF(v_list_type = 'Daily') THEN
          --If List is Daily List
          --Generate Daily Prison List XML and insert into validation and XML document tables
          v_document_type_code := 'DLP';
          v_unique_id := 'CSD' || v_document_type_code || LPAD(l_validation_id, 15, 0);
          v_return_xml:= XHB_GET_XML_REPORTS.GET_DAILY_PRISON_LIST(p_list_id, v_unique_id);
          v_list_name := 'Prison Daily List';
          v_document_title:= v_list_name || ' ' || v_list_version;
          v_document_name := v_document_type_code || ' ' || v_list_start_date || ' ' || v_list_version;
          v_document_description := v_list_name || ', ' || INITCAP(v_list_court.COURT_NAME) || ' on ' || v_list_start_date || ' '  || v_list_version;
          STORE_XML_DOCUMENT(v_return_xml, v_document_title, v_list_row.LIST_START_DATE, v_document_type_code, v_list_court.COURT_ID, v_list_court.COUNTRY, v_list_court.LANGUAGE);
          STORE_VALIDATION_DOCUMENT(v_return_xml, v_document_name, v_document_description, v_document_type_code, l_validation_id, v_unique_id, 16, v_list_court.CREST_COURT_ID);

          --Generate Daily Distribution List XML and insert into XML document tables
          SELECT XHB_VALIDATION_SEQ.NEXTVAL INTO l_validation_id FROM DUAL; -- We need a new validation id as we just used the last one for the Daily Prison List
          v_document_type_code := 'DLD';
          v_unique_id := 'CSD' || v_document_type_code || LPAD(l_validation_id, 15, 0);
          v_return_xml:= XHB_GET_DISTRIBUTION_LIST_XML.GET_DAILY_DISTRIBUTION_LIST(p_list_id, v_unique_id);
          v_list_name := 'Daily List for Distribution';
          v_document_title:= v_list_name || ' ' || v_list_version;
          -- CTX-3183  SET p_store_control parameter to True so record is inserted in WLL_CONTROL
          STORE_XML_DOCUMENT(v_return_xml, v_document_title, sysdate, v_document_type_code, v_list_court.COURT_ID, v_list_court.COUNTRY, v_list_court.LANGUAGE, true);

          --Generate Daily List XML
          SELECT XHB_VALIDATION_SEQ.NEXTVAL INTO l_validation_id FROM DUAL; -- We need a new validation id as we just used the last one for the Daily Distribution List
          v_document_type_code := 'DL';
          v_unique_id := 'CSD' || v_document_type_code || LPAD(l_validation_id, 15, 0);
          v_return_xml:= XHB_GET_XML_REPORTS.GET_DAILY_LIST(p_list_id, v_unique_id, 0);
          v_list_name := 'Daily List';
          v_document_title:= v_list_name || ' ' || v_list_version;
          v_document_name := v_document_type_code || ' ' || v_list_start_date || ' ' || v_list_version;
          v_document_description := v_list_name || ', ' || INITCAP(v_list_court.COURT_NAME) || ' on ' || v_list_start_date || ' '  || v_list_version;
          -- CTX-3183 remove p_store_control flag as WLL_CONTROL should not be written for DL
          STORE_XML_DOCUMENT(v_return_xml, v_document_title, v_list_row.LIST_START_DATE, v_document_type_code, v_list_court.COURT_ID, v_list_court.COUNTRY, v_list_court.LANGUAGE);
          STORE_VALIDATION_DOCUMENT(v_return_xml, v_document_name, v_document_description, v_document_type_code, l_validation_id, v_unique_id, 12, v_list_court.CREST_COURT_ID);

          --AND generated Daily List Mercator XML
          v_merc_xml:= XHB_GET_XML_REPORTS.GetInternalDailyListData(p_list_id);
          v_formatted_list_date:=  to_char(v_list_row.LIST_START_DATE, 'YYYY-MM-DD HH24:MI:SS');

          --Insert Mercator XML into MTBL_MERC_DL_STORAGE. Unless a daily list already exists for the list date and court ID, in which case overwrite that one
          MERGE INTO MTBL_MERC_DL_STORAGE USING DUAL ON (FROMID = v_list_court.CREST_COURT_ID AND ADDITIONAL3 = v_formatted_list_date)
                WHEN NOT MATCHED THEN  INSERT (NAME, MSGTYPE, MSGTIME, MSGDATE, ADDITIONAL1, ADDITIONAL3, FROMID, MESSAGE)
                   VALUES ('Daily List XML Extract Delivery', 'Daily_List_XML_Upload', to_char(sysdate, 'HH24:MI:SS'), trunc(sysdate), 'READY', v_formatted_list_date, v_list_court.CREST_COURT_ID, v_merc_xml)
                WHEN MATCHED THEN UPDATE SET MSGTIME = to_char(sysdate, 'HH24:MI:SS'), MSGDATE=trunc(sysdate), MESSAGE = v_merc_xml;

        ELSIF(v_list_type = 'Firm') THEN
          --If List is Firm List
          --Generate Firm Distribution List XML and insert into XML document tables
          v_document_type_code := 'FLD';
          v_unique_id := 'CSD' || v_document_type_code || LPAD(l_validation_id, 15, 0);
          v_return_xml:= XHB_GET_DISTRIBUTION_LIST_XML.GET_FIRM_DISTRIBUTION_LIST(p_list_id, v_unique_id);
          v_list_name := 'Firm List for Distribution';
          v_document_title:= v_list_name || '  ' || v_list_date_range || ' ' || v_list_version;
          -- CTX-3183  SET p_store_control parameter to True so record is inserted in WLL_CONTROL
          STORE_XML_DOCUMENT(v_return_xml, v_document_title, sysdate, v_document_type_code, v_list_court.COURT_ID, v_list_court.COUNTRY, v_list_court.LANGUAGE, true);

          SELECT XHB_VALIDATION_SEQ.NEXTVAL INTO l_validation_id FROM DUAL; -- We need a new validation id as we just used the last one for the Firm Distribution List
          v_document_type_code := 'FL';
          v_unique_id := 'CSD' || v_document_type_code || LPAD(l_validation_id, 15, 0);
          v_return_xml:= XHB_GET_XML_REPORTS.GET_FIRM_LIST_DATA(p_list_id, v_unique_id);
          v_list_name := 'Firm List';
          v_document_title:= v_list_name || '  ' || v_list_date_range || ' ' || v_list_version;
          v_document_name := v_document_type_code || ' ' || v_today_date || '  ' || v_list_date_range || ' ' || v_list_version;
          v_document_description := v_list_name || ', ' || INITCAP(v_list_court.COURT_NAME) || ' ' || INITCAP(v_list_court.COURT_PREFIX)
                                    || ' on ' || v_today_date || '  ' || v_list_date_range || ' ' || v_list_version;
          -- CTX-3183 remove p_store_control flag as WLL_CONTROL should not be written for FL
          STORE_XML_DOCUMENT(v_return_xml, v_document_title, v_list_row.LIST_START_DATE, v_document_type_code, v_list_court.COURT_ID, v_list_court.COUNTRY, v_list_court.LANGUAGE);
          STORE_VALIDATION_DOCUMENT(v_return_xml, v_document_name, v_document_description, v_document_type_code, l_validation_id, v_unique_id, 13, v_list_court.CREST_COURT_ID);

        ELSIF(v_list_type = 'Warned') THEN
          --If List is Warned List
          --Generate Warned Distribution List XML and insert into XML document tables
          v_document_type_code := 'WLD';
          v_unique_id := 'CSD' || v_document_type_code || LPAD(l_validation_id, 15, 0);
          v_return_xml:= XHB_GET_DISTRIBUTION_LIST_XML.GET_WARNED_DISTRIBUTION_LIST(p_list_id, v_unique_id);
          v_list_name := 'Warned List for Distribution';
          v_document_title:= v_list_name || ' ' || v_list_date_range || ' ' || v_list_version;
          -- CTX-3183  SET p_store_control parameter to True so record is inserted in WLL_CONTROL
          STORE_XML_DOCUMENT(v_return_xml, v_document_title, sysdate, v_document_type_code, v_list_court.COURT_ID, v_list_court.COUNTRY, v_list_court.LANGUAGE, true);

          SELECT XHB_VALIDATION_SEQ.NEXTVAL INTO l_validation_id FROM DUAL; -- We need a new validation id as we just used the last one for the Warned Distribution List
          v_document_type_code := 'WL';
          v_unique_id := 'CSD' || v_document_type_code || LPAD(l_validation_id, 15, 0);
          v_return_xml:= XHB_GET_XML_REPORTS.GET_WARNED_LIST(p_list_id, v_unique_id, 0, 1, 1, 1);
          v_list_name := 'Warned List';
          v_document_title:= v_list_name || ' ' || v_list_date_range || ' ' || v_list_version;
          v_document_name := v_document_type_code || ' ' || v_today_date || ' ' || v_list_date_range || ' ' || v_list_version;
          v_document_description := v_list_name || ', ' || INITCAP(v_list_court.COURT_NAME) || ' ' || INITCAP(v_list_court.COURT_PREFIX)
                                    || ' ' || v_list_date_range || ' ' || v_list_version;
          -- CTX-3183 remove p_store_control flag as WLL_CONTROL should not be written for WL
          STORE_XML_DOCUMENT(v_return_xml, v_document_title, v_list_row.LIST_START_DATE, v_document_type_code, v_list_court.COURT_ID, v_list_court.COUNTRY, v_list_court.LANGUAGE);
          STORE_VALIDATION_DOCUMENT(v_return_xml, v_document_name, v_document_description, v_document_type_code, l_validation_id, v_unique_id, 21, v_list_court.CREST_COURT_ID);
        END IF;

    END PUBLISH_LIST;

    ---
    -- get_cji_document_type_data
    ---
    -- Returns data that has been duplicated in XHB_MTBL_ADAPTOR_CONFIG from data stored in
    -- the CJIT.CJI_DOCUMENT_TYPE table. In LIVE that table is unreachable from this package
    -- because it is in a different database. Therefore, because there is a freeze on making
    -- more schema changes, the data has been duplicated in an existing XHIBIT table.
    ---
    FUNCTION get_cji_document_type_data(p_document_type_code IN VARCHAR2, p_column_name IN VARCHAR2)
        RETURN VARCHAR2 IS
        v_result VARCHAR2(255);
        v_sql VARCHAR2(1000);
    BEGIN

      v_sql := 'SELECT ' || p_column_name || ' FROM CJI_DOCUMENT_TYPE WHERE INTERNAL_CODE = ''' || p_document_type_code||'''';
      EXECUTE IMMEDIATE v_sql INTO v_result;

        RETURN v_result;
    END get_cji_document_type_data;

    --
    -- Store XML in the XHB_CLOB table and reference it in a new record in the XHB_XML_DOCUMENT table
    --
    PROCEDURE STORE_XML_DOCUMENT(p_return_xml CLOB, p_doc_title VARCHAR2, p_doc_date DATE, p_document_type_code VARCHAR2, p_court_id XHB_COURT.COURT_ID%TYPE,
          p_country XHB_XML_DOCUMENT.country%TYPE, p_language XHB_XML_DOCUMENT.language%TYPE,
          p_store_control boolean default false) as
          l_clob_id XHB_CLOB.clob_id%TYPE;
		  l_xml_document_id XHB_XML_DOCUMENT.xml_document_id%type;
    BEGIN
          -- Insert a new entry in the XHB_CLOB with the XML
          INSERT INTO XHB_CLOB(clob_id, clob_data)
          VALUES (XHB_CLOB_SEQ.NEXTVAL, p_return_xml)
          RETURNING clob_id
          INTO      l_clob_id;

          -- Insert a new entry into the xhb_xml_document table
          INSERT INTO XHB_XML_DOCUMENT ( date_created,
                                ...
                                VALUES ( p_doc_date,
                                         p_doc_title || ' ' || TO_CHAR(sysdate, 'YYYY-MM-DD HH24:MI:SS'),
                                         G_NEW_DOCUMENT,
                                         ...
                                         p_court_id,
                                         l_clob_id)
										 returning xml_document_id into l_xml_document_id ;

			IF p_store_control THEN
             INSERT INTO XHB_WLL_CONTROL(XML_DOCUMENT_ID,
										STATUS)
								VALUES(l_xml_document_id,
										'ND');
          END IF;

    END STORE_XML_DOCUMENT;

    --
    -- Store XML in the XHB_VALIDATION table as well as a new entry in the XHB_VALIDATION_CJI tables
    --
    PROCEDURE STORE_VALIDATION_DOCUMENT(p_return_xml CLOB, p_document_name VARCHAR2, p_document_description VARCHAR2, p_document_type_code VARCHAR2,
                                        p_validation_id XHB_VALIDATION.VALIDATION_ID%TYPE, p_unique_id VARCHAR, p_type_id NUMBER, p_crest_court_id XHB_COURT.CREST_COURT_ID%TYPE) as
          v_schema_name VARCHAR2(255);
          v_document_type_id NUMBER(8);
          v_stylesheet_name VARCHAR2(255);
    BEGIN
          -- Get the document type data which is copied into the XHIBIT schema
          -- from the CJIT schema because that schema is not reachable in LIVE
          v_schema_name := get_cji_document_type_data(p_document_type_code, 'SCHEMA_NAME');
          v_document_type_id := TO_NUMBER(get_cji_document_type_data(p_document_type_code, 'DOCUMENT_TYPE_ID'));
          v_stylesheet_name := get_cji_document_type_data(p_document_type_code, 'STYLESHEET_NAME');

          -- Insert a new entry into the xhb_validation table
          INSERT INTO XHB_VALIDATION (VALIDATION_ID, CODE, CLOB_DATA, SCHEMA_NAME, STATUS)
                                VALUES (p_validation_id, p_document_type_code, p_return_xml, v_schema_name, 'N');

          -- Insert a new entry into the xhb_validation_cji table
          INSERT INTO XHB_VALIDATION_CJI (VALIDATION_ID,
                                        EXT_DOCUMENT_ID,
                                        DOCUMENT_TYPE_ID,
                                        STATUS_ID,
                                        STYLESHEET_NAME,
                                        CREATION_DATE,
                                        BITS_REGISTERING_LOCATION,
                                        BITS_DOCUMENT_NAME,
                                        BITS_DOCUMENT_DESCRIPTION)
                                VALUES (p_validation_id,
                                        p_unique_id,
                                        v_document_type_id,
                                        1,
                                        v_stylesheet_name,
       ...
       INSERT INTO XHB_VALIDATION_EXISS (VALIDATION_ID,
                                        TYPE_ID,
                                        IDENTIFIER,
                                        CREST_COURT_ID,
                                        DESCRIPTION,
	...
	*
	* Returns an email address for a given address_id or NULL if none present.
	* Will return the Secure Email first, then the Non Secure Email
	**************************************************************************************/
	FUNCTION getEmailAddress(p_address_id  xhb_address.address_id%TYPE) RETURN xhb_contact_detail.contact_value%TYPE IS

		v_email   xhb_contact_detail.contact_value%TYPE;
	...
	BEGIN
		...
		BEGIN
			SELECT email
			INTO v_email
			FROM (SELECT  cdet.contact_value AS email, DECODE(contact_type, 'Secure Email', 1, 'Non Secure Email', 2, 3) AS ordering_col
				  FROM    xhb_contact_detail cdet
				  WHERE   cdet.contact_type IN('Secure Email', 'Non Secure Email')
	...
	* Will return the Secure Email first, then the Non Secure Email
	**************************************************************************************/
	FUNCTION getFaxNumber(p_address_id  xhb_address.address_id%TYPE) RETURN xhb_contact_detail.contact_value%TYPE IS

		v_fax   xhb_contact_detail.contact_value%TYPE;
   ...
   -- get_sub_wll_rec_by_court_id
    ----
    --      Returns all wll_recipients which match crest_sof_id and court_id
    ----
    FUNCTION get_wll_rec_crest_sof(
                                         p_court_id    IN     NUMBER,
                                         p_crest_sof_id IN    NUMBER
                                        )
                                        RETURN SYS_REFCURSOR
    IS
    ...
    BEGIN
         ...
         OPEN l_return_cursor FOR
            ...
            WHERE
               xwr.court_id = p_court_id
            AND
               xwr.crest_solicitor_firm_id = p_crest_sof_id;

         RETURN l_return_cursor;

    END get_wll_rec_crest_sof;
