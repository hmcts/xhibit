...
DECLARE
   ...
   BEGIN
      ...
      SELECT XHB_CONFIG_PROP_SEQ.NEXTVAL, 'FORMATTING_LIST_DELAY', 300 FROM DUAL
      WHERE NOT EXISTS (SELECT 1 FROM XHB_CONFIG_PROP X WHERE X.PROPERTY_NAME = 'FORMATTING_LIST_DELAY'); 
      COMMIT;
   END insert_record;
