    ...
    l_delete := 'DELETE xhb_plea
                 WHERE  defendant_on_offence_id IN (SELECT xdof.defendant_on_offence_id
                                                    ...
                                                    WHERE  xdof.defendant_on_case_id IN (SELECT xdoc.defendant_on_case_id
                                                           ...
                                                           OR xdof.offence_id IN (SELECT xoff.offence_id
...
/** 
 ...
 /************************************************************************************************
 *    PROCEDURE delete_case_ctx(p_hk_run_id, p_case_id)      CTX-2227
 ...
 **************************************************************************************************/
   PROCEDURE delete_case_ctx (p_case_id   IN XHB_CASE.CASE_ID%TYPE,
                              p_hk_run_id IN XHB_HK_ERROR_LOG.HK_RUN_ID%TYPE DEFAULT NULL)  IS
  ...
  BEGIN
    -- Set the case status to Inprogress 
    update_case_status(p_case_id     => p_case_id,
                       p_case_status => 'I');
    COMMIT;
    ...
    l_hk_run_id := CASE WHEN p_hk_run_id IS NOT NULL THEN p_hk_run_id ELSE get_next_hk_run_id END;
    ...
    delete_hearing(p_case_id   => p_case_id, p_log_delete => FALSE);
    delete_listing(p_case_id   => p_case_id, p_log_delete => FALSE);
    delete_defendant(p_case_id => p_case_id, p_log_delete => FALSE);
    delete_charge(p_case_id    => p_case_id, p_log_delete => FALSE);
    delete_case(p_case_id      => p_case_id, p_log_delete => FALSE);
