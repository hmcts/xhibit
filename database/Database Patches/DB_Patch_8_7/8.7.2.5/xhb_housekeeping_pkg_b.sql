 ...
 **************************************************************************************************/
 ...
 --   log_delete (p_table_name      => 'XHB_CASE_REFRESH_RESYNCH' );
	...
	-- Delete the XHB_LEO_ADV_LINK records (part 1)
    ...
    -- Delete the XHB_LEO_ADV_LINK records (part 2)
	...
	l_error_point := l_proc_name || ' 4.5 ';
 
    -- Now delete the XHB_LEGAL_AID_ORDER
    l_error_point := l_proc_name || ' 4.6 ';
    ...
    l_error_point := l_proc_name || ' 4.7 ';

    EXECUTE IMMEDIATE l_delete USING p_case_id;

	-- Now delete the XHB_DEF_ON_CASE_REF_SOL_FIRM
...
--    delete_table_ctx(' XHB_TIME ',  p_case_id);
    ...
    -- Added for CTX-3117 <Start>
    l_error_point := l_proc_name || ' 11.5 ';
    l_delete := 'DELETE xhb_defendant_charge
                 WHERE  defendant_on_case_id IN (SELECT defendant_on_case_id
                                                 FROM   xhb_defendant_on_case
                                                 WHERE  case_id = :1
    ...
    l_error_point := l_proc_name || ' 11.6 ';
	
    l_delete := REPLACE (l_delete, 'xhb', 'aud');
    EXECUTE IMMEDIATE l_delete USING p_case_id;
    -- Added for CTX-3117 <End>
   ...
   l_delete := 'DELETE xhb_disposal_line
                WHERE  disposal2_id IN (SELECT disposal2_id
                                        ...
                                        WHERE  defendant_on_case_id IN (SELECT defendant_on_case_id
                                                                        FROM   xhb_defendant_on_case
                                                                        WHERE  case_id = :1))
                OR disposal2_id IN (SELECT disposal2_id
                                        FROM xhb_disposal2 xd2,
                                        xhb_defendant_on_offence xdoo, 
                                    xhb_defendant_on_case xdoc
                                    WHERE xd2.defendant_on_offence_id = xdoo.defendant_on_offence_id
    ...
    l_delete := 'DELETE xhb_disposal2
                 ...
                 OR defendant_on_offence_id IN (SELECT xdoo.defendant_on_offence_id
                                  FROM xhb_defendant_on_offence xdoo, 
                                    xhb_defendant_on_case xdoc
                                    WHERE xdoo.defendant_on_case_id = xdoc.defendant_on_case_id
                                    and case_id = :1)';
    ...
    EXECUTE IMMEDIATE l_delete USING p_case_id, p_case_id;
    l_error_point := l_proc_name || ' 25.1 ';

    l_delete := REPLACE (l_delete,'xhb','aud');
    EXECUTE IMMEDIATE l_delete USING p_case_id, p_case_id;


    -- 26 XHB_DEFENDANT_ON_OFFENCE (part 1)
    ...
    EXECUTE IMMEDIATE l_delete USING p_case_id;
    l_error_point := l_proc_name || ' 26.1 ';

    l_delete := REPLACE (l_delete, 'xhb', 'aud');
    EXECUTE IMMEDIATE l_delete USING p_case_id;
	
	
	-- 26 XHB_DEFENDANT_ON_OFFENCE (part 2)
	l_error_point := l_proc_name || ' 26.2 ';
	l_delete := 'DELETE xhb_defendant_on_offence
                 WHERE  offence_id IN (SELECT offence_id
									   FROM xhb_offence
									   WHERE  charge_id IN (SELECT charge_id
	...
	EXECUTE IMMEDIATE l_delete USING p_case_id;
    l_error_point := l_proc_name || ' 26.3 ';

    l_delete := REPLACE (l_delete, 'xhb', 'aud');
    EXECUTE IMMEDIATE l_delete USING p_case_id;
	
    
    -- 27  XHB_DEFENDANT_ON_CASE
    ...
    -- 29  XHB_BREACH
    l_delete := ' DELETE FROM XHB_BREACH 
          WHERE charge_id IN (SELECT charge_id FROM xhb_charge 
                WHERE case_id = :1)';
                                      
    l_error_point := l_proc_name || ' 29 ';
    EXECUTE IMMEDIATE l_delete USING p_case_id;
    l_error_point := l_proc_name || ' 29.1 ';
