...
/** 
  ...
  **/  
  ...
  BEGIN
                SELECT  XMLELEMENT("DLData"
                              ...
                              ,(SELECT XMLAgg(XMLELEMENT("DefendantOnCase", xc.CASE_TYPE || xc.CASE_NUMBER || '|' || xc.CASE_TYPE || '|' || xd.CREST_DEFENDANT_ID || '||' || xdoc.PNC_ID || '|' || xdoc.DEFENDANT_NUMBER || '|' || xdoc.IS_JUVENILE))
                                            ...
                                            AND xdoc.DEFENDANT_ID = xd.DEFENDANT_ID)
                              ,(SELECT XMLAgg(XMLELEMENT("ScheduledHearingDefendant", xd.CREST_DEFENDANT_ID || '|' || xc.CASE_TYPE || xc.CASE_NUMBER || '|' || xc.CASE_TYPE || '|' ||  NVL(xcr.CREST_COURT_ROOM_NO,1)  || '|' || NVL(xrj.CREST_JUDGE_ID,0) || '|' || xrht.HEARING_TYPE_CODE || '|' ||
  ...
  BEGIN
    SELECT XMLAgg(
                  XMLELEMENT("ScheduledHearing",xcol.SEQ_NO || '|' || CASE WHEN(xrsc.DE_CODE IS NOT NULL) THEN xrsc.DE_CODE ||  TO_CHAR(xcol.TIME_LISTED, 'HH24:MI am') END ||  '|' ||  TO_CHAR(xcol.TIME_LISTED, 'YYYY-MM-DDHH24:MI') || '|' || TO_CHAR(xcol.TIME_LISTED, 'HH24:MI am') || '|'
  ...
  **/  
  FUNCTION GET_DAILY_LIST(p_list_id XHB_LIST.LIST_ID%TYPE, v_unique_id VARCHAR) RETURN CLOB AS
   ...
   CURSOR get_xml_c IS
       SELECT XMLAGG(XMLELEMENT("cs:DailyList"
                                ...
                                ,XMLELEMENT("cs:CourtLists",
                                              (SELECT XMLAgg( XMLELEMENT("cs:CourtList"
                                                                          ,GET_COURT_HOUSE(xcrts.COURT_SITE_ID)--CourtHouse
                                                                          ,XMLELEMENT("cs:Sittings",(get_sittings_in_courtsite(xcrts.COURT_SITE_ID, xl.LIST_ID, xl.list_start_date, 1)),
                                                                                                    (get_floating_cases(xcrts.COURT_SITE_ID, xl.LIST_ID, xl.list_start_date, 1)))--sittings
                                                                         )--CourtList
                                               ) FROM XHB_COURT_SITE xcrts
                                                  WHERE xcrts.COURT_ID = xcrt.COURT_ID 
                                                  AND (xcrts.obs_ind is null or xcrts.obs_ind <> 'Y'))
  ...
  **/
  ...
  SELECT XMLELEMENT("cs:RunningList" , XMLATTRIBUTES('http://www.courtservice.gov.uk/schemas/courtservice' AS "xmlns:cs"
                  ...
                  ,XMLELEMENT("cs:DocumentID", XMLFOREST('Running List ' AS "cs:DocumentName"
                    ...
                    ,XMLELEMENT("cs:ListHeader", XMLFOREST('Criminal' AS "cs:ListCategory"
                                                       ,to_xml_date_format(sysdate) AS "cs:StartDate"
                                                       ,1 AS "cs:Version"
                                                       ,to_xml_date_format(sysdate) AS "cs:PublishedTime"
                                                       )
                  ) --ListHeader
                      ...
                      (SELECT XMLAGG(
                                XMLELEMENT("cs:Case",
                                      ...
                                      ,(SELECT XMLAgg( XMLELEMENT("cs:Defendants", 
                                                          get_defendant(xdoc.DEFENDANT_ON_CASE_ID, 0, 0)))
  ...
  BEGIN
   SELECT XMLELEMENT("cs:CaseArrivedFrom"
                     , XMLELEMENT("cs:OriginatingCourt",XMLFOREST(xmcrt.COURT_TYPE AS "cs:CourtHouseType")
                          ,XMLELEMENT("cs:CourtHouseCode" ,XMLATTRIBUTES(xmcrt.COURT_SHORT_NAME AS "cs:CourtHouseShortName"))--CourtHouseCode
                           ,XMLELEMENT("cs:CourtHouseName", xmcrt.COURT_FULL_NAME)--CourtHouseName 
                           ,XMLELEMENT("cs:CourtHouseAddress", XMLFOREST(xaddm.ADDRESS_1 AS "p1:Line"
                                                                        ,xaddm.ADDRESS_2 AS "p1:Line"
                                                                        ,xaddm.ADDRESS_3 AS "p1:Line"
                                                                        ,xaddm.TOWN AS "p1:Line"
                                                                        ,xaddm.COUNTY AS "p1:Line"
                                                                        ,xaddm.POSTCODE AS "p1:PostCode")
                              )--CourtHouseAddress
  ...
  **/
  ...
  FUNCTION GET_WARNED_LIST(p_list_id XHB_LIST.LIST_ID%TYPE, v_unique_id VARCHAR,
  ...
  x_no_file    EXCEPTION;
   ...
   CURSOR get_xml_c IS
                       SELECT XMLAGG(XMLELEMENT("cs:WarnedList"
                                                ...
                                                , 'http://www.govtalk.gov.uk/people/bs7666' AS "xmlns:p2"
                                                , 'http://www.w3.org/2001/XMLSchema-instance' AS "xmlns:xsi"
                                                , 'http://www.courtservice.gov.uk/schemas/courtservice WarnedList.xsd' AS " xsi:schemaLocation")
                                  ...
                                  , GET_LIST_HEADER(xl.LIST_ID) --ListHeader
                                  , GET_CROWN_COURT(xl.COURT_ID) --CrownCourt
                                  --, XMLELEMENT("cs:ListingInstructions", XMLELEMENT("cs:ListingInstruction",'ListingInstructions - what field does this refer to?????')) --ListingInstructions           
                                  , XMLELEMENT("cs:CourtLists"
                                      ,(SELECT XMLAgg(XMLELEMENT("cs:CourtList"
                                                ...
                                                ,(SELECT XMLAgg(XMLELEMENT("cs:WithFixedDate",XMLATTRIBUTES(xrht.hearing_type_code AS "HearingType")
                                                  ,XMLAgg(XMLELEMENT("cs:Fixture"
                                                    ...
                                                    ,get_other_notes(xc.CASE_ID, p_include_standard_notes,  p_include_priority_notes,  p_include_restricted_notes)                                                          
                                                     ,XMLELEMENT("cs:Cases"
                                                        ,XMLELEMENT("cs:Case",
                                                                            XMLELEMENT("cs:CaseNumber",xc.CASE_TYPE || xc.CASE_NUMBER)
                                                                          , XMLELEMENT("cs:CaseArrivedFrom"
                                                                             , XMLELEMENT("cs:OriginatingCourt",XMLFOREST(originatingCourt.COURT_TYPE AS "cs:CourtHouseType")
                                                                                  ,XMLELEMENT("cs:CourtHouseCode" ,XMLATTRIBUTES(originatingCourt.COURT_SHORT_NAME AS "cs:CourtHouseShortName"))--CourtHouseCode
                                                                                   ,XMLELEMENT("cs:CourtHouseName", originatingCourt.COURT_FULL_NAME)--CourtHouseName 
                                                                              )--OriginatingCourt
                                                                            )--CaseArrivedFrom
                                                                           , XMLFOREST( XMLFOREST(xrht.hearing_type_desc AS "cs:HearingDescription"
                                                                                      ,xcdf.LISTING_DATE AS "cs:HearingDate"
                                                                                      ,get_highlight_note(xc.CASE_ID, p_include_standard_notes,  p_include_priority_notes,  p_include_restricted_notes) AS "cs:ListNote"
                                                                                      )"cs:Hearing")--Hearing
                                                                          ,(SELECT XMLAgg( XMLELEMENT("cs:Defendants", 
                                                                                              get_defendant(xdoc.DEFENDANT_ON_CASE_ID, 0, 0)))
                                                                            FROM XHB_DEFENDANT_ON_CASE xdoc
                                                                            WHERE xc.CASE_ID = xdoc.CASE_ID)--Defendants
                                                                          , (SELECT(XMLFOREST( xrpa.PROSECUTOR_NAME_3 AS "ProsecutingReference"
                                                                                ...
                                                                                AND  ((xc.CASE_TYPE = 'A' AND  NVL(xcpa.PROSECUTOR_TYPE, 'R') = 'R' AND NVL(xcpa.RESPONDENT_STATUS, 'R') = 'R') --If appeal case prosecutor is Respondent Type
                                                                                OR   (xc.CASE_TYPE <> 'A'))
                                                                                AND  xrpa.ADDRESS_ID = prosecutor_address.ADDRESS_ID (+))--Prosecution 
                                                                            ,get_non_available_dates(xc.CASE_ID, xl.LIST_START_DATE, xl.LIST_END_DATE)
                                                                          --DateOfInstigation
                                                                          ,XMLFOREST(FLOOR((trunc(sysdate) -  NVL(xc.SENT_FOR_TRIAL_DATE, NVL(xc.COMMITTAL_DATE, xc.APPEAL_LODGED_DATE))) / 7) || ' Week(s)'   as "cs:MethodOfInstigation" --Age of case to nearest complete week
                                                                          ,xc.CLASS_CODE AS "cs:CaseClassNumber")
                                                                    )--Case
                                                      )--Cases
                                                          ...
                                                          , XMLELEMENT("cs:Cases"
                                                            ,XMLELEMENT("cs:Case",
                                                                 ...
                                                                 , XMLFOREST(XMLFOREST(xrht.hearing_type_desc AS "cs:HearingDescription"
                                                                  ...
                                                                  ,(SELECT XMLAgg( XMLELEMENT("cs:Defendants", 
                                                                                              get_defendant(xdocol.DEFENDANT_ON_CASE_ID, 0, 0)))
  ...
  */
  ...
  SELECT XMLELEMENT("cs:Defendant",
                   XMLELEMENT("cs:PersonalDetails",
                             XMLFOREST(XMLFOREST(CASE WHEN (p_show_public_view = 1 AND xdoc.IS_MASKED = 'Y') THEN ' ' ELSE xd.FIRST_NAME END AS "p1:CitizenNameForename"
                                                ,CASE WHEN (p_show_public_view = 1 AND xdoc.IS_MASKED = 'Y') THEN xdoc.MASKED_NAME ELSE xd.SURNAME END AS "p1:CitizenNameSurname")"cs:Name"
                     ...
                     , XMLELEMENT("cs:Counsel"
                      , (SELECT XMLAgg(XMLELEMENT("cs:Solicitor"
                        ...
                        ,XMLFOREST(
                                    (get_original_charges(xdoc.DEFENDANT_ON_CASE_ID) )"cs:OriginalCharges"
                        )
                        --OriginalCharges
 ...
 FUNCTION get_highlight_note(p_case_id XHB_CASE.CASE_ID%TYPE, p_include_standard_notes INTEGER,  p_include_priority_notes INTEGER,  p_include_restricted_notes INTEGER) RETURN VARCHAR2 IS
  highlightNote VARCHAR2(400);
  BEGIN
  
  SELECT CASE WHEN((p_include_standard_notes = 1 AND freetext_noteclass.REF_DATA_VALUE = 'Standard') OR
                  (p_include_priority_notes = 1 AND freetext_noteclass.REF_DATA_VALUE = 'Priority') OR
                  (p_include_restricted_notes = 1 AND freetext_noteclass.REF_DATA_VALUE = 'Restricted')) THEN xdne_freetext.CREATION_DATE || ' ' || xdne_freetext.DIARY_NOTE_TEXT END
                  INTO highlightNote
  ...
  SELECT ( XMLFOREST( XMLAGG (XMLELEMENT (E, (CASE WHEN((p_include_standard_notes = 1 AND noteclass.REF_DATA_VALUE = 'Standard') OR
                                                        ...
                                                        (p_include_restricted_notes = 1 AND noteclass.REF_DATA_VALUE = 'Restricted')) 
  ...
  SELECT XMLAgg(XMLELEMENT("cs:CaseNumber", MAX(linked_case.CASE_TYPE) ||  MAX(linked_case.CASE_NUMBER) || 
                            CASE WHEN MAX(xrht_linked_fixture_type.HEARING_TYPE_DESC) IS NOT NULL THEN ' FIX ' || MAX(xrht_linked_fixture_type.HEARING_TYPE_DESC)  
                                 WHEN MAX(xrht_linked_hearing_type.HEARING_TYPE_DESC) IS NOT NULL THEN ' ' || MAX(xrht_linked_hearing_type.HEARING_TYPE_DESC) END  
                          )
                          ORDER BY CASE WHEN MAX(xrht_linked_fixture_type.HEARING_TYPE_DESC) IS NOT NULL THEN 0  
                                         WHEN MAX(xrht_linked_hearing_type.HEARING_TYPE_DESC) IS NOT NULL THEN 1 
                                         ELSE 2 END
    ...
    FROM  XHB_CASE xc,	XHB_CASE linked_case, XHB_CASE_DIARY_FIXTURE xcdf_linked_fixtures, XHB_CASE_LISTING_ENTRY xcle_linked_cases, XHB_REF_HEARING_TYPE xrht_linked_fixture_type
          ,XHB_CASE_ON_LIST xcol_linked_hearings, XHB_LIST xl_linked_caselist, XHB_REF_HEARING_TYPE xrht_linked_hearing_type, XHB_REF_LISTING_DATA xrld_linked_list_type
    ...
    AND xcol_linked_hearings.HEARING_TYPE_ID = xrht_linked_hearing_type.REF_HEARING_TYPE_ID (+)
