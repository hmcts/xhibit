create or replace PACKAGE BODY xhb_list_distribution_pkg AS
  ...
  PROCEDURE PUBLISH_LIST(p_list_id XHB_LIST.LIST_ID%TYPE) IS
   ...
   v_document_type CJIT.CJI_DOCUMENT_TYPE%ROWTYPE;
   ...
   --Determine the List Type
        ...
        IF(v_list_type = 'Daily') THEN
          --If List is Daily List
           ...
           SELECT * INTO v_document_type FROM CJIT.CJI_DOCUMENT_TYPE cdt WHERE cdt.INTERNAL_CODE = 'DLP';
           v_unique_id := 'CSD' || TRIM(v_document_type.INTERNAL_CODE) || LPAD(l_validation_id, 15, 0); 
           v_return_xml:= XHB_GET_XML_REPORTS.GET_DAILY_PRISON_LIST(p_list_id, v_unique_id);
           STORE_LIST(v_return_xml, 'Daily Prison List', v_list_row, v_document_type, l_validation_id , v_unique_id , v_list_court);
          ...
          SELECT * INTO v_document_type FROM CJIT.CJI_DOCUMENT_TYPE cdt WHERE cdt.INTERNAL_CODE = 'DLD';
          v_unique_id := 'CSD' || TRIM(v_document_type.INTERNAL_CODE) || LPAD(l_validation_id, 15, 0); 
          v_return_xml:= XHB_GET_DISTRIBUTION_LIST_XML.GET_DAILY_DISTRIBUTION_LIST(p_list_id, v_unique_id);
          STORE_LIST(v_return_xml, 'Daily List For Distribution', v_list_row, v_document_type, l_validation_id , v_unique_id , v_list_court);
          ...
          SELECT * INTO v_document_type FROM CJIT.CJI_DOCUMENT_TYPE cdt WHERE cdt.INTERNAL_CODE = 'DL';
          v_unique_id := 'CSD' || TRIM(v_document_type.INTERNAL_CODE) || LPAD(l_validation_id, 15, 0); 
        ...
        --If List is Firm List
          ...
          SELECT * INTO v_document_type FROM CJIT.CJI_DOCUMENT_TYPE cdt WHERE cdt.INTERNAL_CODE = 'FLD';
          v_unique_id := 'CSD' || TRIM(v_document_type.INTERNAL_CODE) || LPAD(l_validation_id, 15, 0); 
          v_return_xml:= XHB_GET_DISTRIBUTION_LIST_XML.GET_FIRM_DISTRIBUTION_LIST(p_list_id, v_unique_id);
          STORE_LIST(v_return_xml, 'Firm List For Distribution', v_list_row, v_document_type, l_validation_id , v_unique_id , v_list_court);
          ...
          SELECT * INTO v_document_type FROM CJIT.CJI_DOCUMENT_TYPE cdt WHERE cdt.INTERNAL_CODE = 'FL';
          v_unique_id := 'CSD' || TRIM(v_document_type.INTERNAL_CODE) || LPAD(l_validation_id, 15, 0);
        ...
        ELSIF(v_list_type = 'Warned') THEN
          ...
          SELECT * INTO v_document_type FROM CJIT.CJI_DOCUMENT_TYPE cdt WHERE cdt.INTERNAL_CODE = 'WLD';
          v_unique_id := 'CSD' || TRIM(v_document_type.INTERNAL_CODE) || LPAD(l_validation_id, 15, 0); 
          v_return_xml:= XHB_GET_DISTRIBUTION_LIST_XML.GET_WARNED_DISTRIBUTION_LIST(p_list_id, v_unique_id);
          STORE_LIST(v_return_xml, 'Warned List For Distribution', v_list_row, v_document_type, l_validation_id , v_unique_id , v_list_court);
          ...
          SELECT * INTO v_document_type FROM CJIT.CJI_DOCUMENT_TYPE cdt WHERE cdt.INTERNAL_CODE = 'WL';
          v_unique_id := 'CSD' || TRIM(v_document_type.INTERNAL_CODE) || LPAD(l_validation_id, 15, 0);
          v_return_xml:= XHB_GET_XML_REPORTS.GET_WARNED_LIST(p_list_id, v_unique_id, 1,1,1);
        END IF;
        
      STORE_LIST(v_return_xml, v_list_type || ' List', v_list_row, v_document_type, l_validation_id , v_unique_id , v_list_court);

  END PUBLISH_LIST;
  
  PROCEDURE STORE_LIST(p_return_xml CLOB, p_list_type VARCHAR2, p_list_row XHB_LIST%ROWTYPE, p_document_type CJIT.CJI_DOCUMENT_TYPE%ROWTYPE 
  ...
  INSERT INTO XHB_CLOB(clob_id, clob_data)
      ...
      INSERT INTO XHB_VALIDATION (VALIDATION_ID, CODE, CLOB_DATA, SCHEMA_NAME, STATUS)
                          VALUES (p_validation_id, p_document_type.INTERNAL_CODE, p_return_xml, p_document_type.SCHEMA_NAME, 'N');
      ...
      INSERT INTO XHB_VALIDATION_CJI (VALIDATION_ID, 
                              ...
                              VALUES (p_validation_id,
                                      p_unique_id,
                                      p_document_type.DOCUMENT_TYPE_ID,
                                      1,
                                      p_document_type.STYLESHEET_NAME,
                                      sysdate,
                                      p_list_court.CREST_COURT_ID,
                                      p_document_type.INTERNAL_CODE ||' ' || p_list_row.LIST_START_DATE || ' ' || DECODE(p_list_row.DRAFT_OR_FINAL, 'D', 'DRAFT v', 'F', 'FINAL v') || p_list_row.LIST_NUMBER,
