create or replace PACKAGE BODY "XHB_REPORT_PKG" AS
  ...
  *     725     PROCEDURE get_drsr_report
  *     772     PROCEDURE get_relcj_report
  *     816     PROCEDURE get_list_of_fixed_dates_report
  *     872     PROCEDURE update_case_diary_fix_run_date
  *     890     PROCEDURE get_run_date
  *     907     PROCEDURE get_prlis_report
  *     974     PROCEDURE publish_running_list
  *    1021     PROCEDURE get_ctlrp_report   
  *    1063     FUNCTION get_charge 
  *    1115     FUNCTION GET_SOLICITOR_ID
  *    1142     FUNCTION GET_MOST_RECENT_HEARING
  *    1163     PROCEDURE UPDATE_REPORT_LOG
  *    1181     Function getAddress
  *    1204     Function getPhoneNum
  *    1227     Function getAppellantAddress
  *    1274     PROCEDURE getAppealHearingNotifnRpt
  *    1427     PROCEDURE get_lod_report_between_dates
  *    1454     PROCEDURE get_obw_report
  *    1522     PROCEDURE getRUMO_Rpt   
  *    1634     FUNCTION GET_EARLIEST_LISTING_DATE 
  *    1667     PROCEDURE get_RAGE_report
  *    1716     FUNCTION GET_BENCH_WARRANT_DATE
  *    1736     FUNCTION GET_LINKED_CASES_LIST    
  *    1766     FUNCTION GET_JUVENILE_STATUS
  *    1786     function get_offences
  *    1816     function get_diary_notes_list
  *    1853     function get_list_history
  *    1937     PROCEDURE getRJS_Rpt
  *    2030     function  get_case_type
  *    2056     function  getCaseSubHdg
  *    2141     PROCEDURE get_RREC_detail
  *    2639     PROCEDURE get_RREC_summary
  *    2821     Function get_composite_casenum
  *    2877     Function get_trial_wait_time
  *    2938     PROCEDURE get_RRCA_detail
  *    3036     PROCEDURE get_RRCA_summary
  *    3141     PROCEDURE getCtRmSittingTimes
  *
  **************************************************************************************/
...
BEGIN
 ...
 --update PUB_RUNNING_LIST_ID on cases on the list 
  ...
  SELECT * INTO v_document_type FROM CJIT.CJI_DOCUMENT_TYPE cdt WHERE cdt.INTERNAL_CODE = 'RL';
  ...
  XHB_LIST_DISTRIBUTION_PKG.STORE_LIST(v_return_xml, 'Running', NULL, v_document_type, l_validation_id , v_unique_id , v_list_court);
...
BEGIN  
    ...
    and    case.court_id = ct1.court_id -- By restricting cases to those for the selected court, we ensure that any monetary orders found are issued by that court.
    ...
    CASE Upper(v_case_type) -- Normally is upper case but don't want to be tripped up by a rogue lower case entry 
        ...
        WHEN 'S' THEN  -- for Sentencing
            ...
            ELSE
                ...
                where  rsc.code_type = 'CASE_APPEAL_TYPE' 
 ...
 /* Procedure get_RREC_detail (p_resultset  OUT SYS_REFCURSOR 
    ...
    cursor c_cases_dealt_with is          -- Cases dealt with (closed). All defendants need a Date_Exported and at least one must be in the reporting period.
        ...
        and not exists(select doc.defendant_on_case_id    -- check there are no unexported defendants at the end-date (i.e. case is closed by then)
                       from xhb_defendant_on_case doc 
                       where nvl(doc.date_exported, '01-Jan-1900') > v_end_date   -- If a defendant's export date is after the end date then it was by implication not exported at the end date.
  ...
  BEGIN  
    ...
    v_step := '25';    
    OPEN   p_resultset FOR  
        SELECT rs.section_num, rs.court_site_name, rs.case_type, rs.case_number, rs.case_subhdg, rs.case_id  
...
--        sum(1) as section0,   -- in for dev only
        ...
        sum(case when exists(select doc.defendant_on_case_id  -- For a case to be open at the end of the period, it has to 
                             from  xhb_defendant_on_case doc  -- have at least one defendant unexported at the end date.
                             where nvl(doc.date_exported, v_end_date) >= v_end_date   -- We map null values to the end of the period 
...
END get_RRCA_Summary;  
  ...
  *
  * Returns for a given court and week, the sitting times for each court room for each 
  * day of the week, broken down by am / pm sessions.
  *
...
BEGIN          
    ...
    open p_resultset for
        select distinct ct.court_name, ctrm.crest_court_room_no, cts.court_site_name, ctrm.court_room_name, 
                        decode(seq.court_site_id, 1, 'am', 2, 'pm', '') as am_pm, 
                        NVL((select sum(case seq.court_site_id when 1 then ctu.am_time_hours + (ctu.am_time_mins/60) 
                                                               when 2 then ctu.pm_time_hours + (ctu.pm_time_mins/60) end)
                        ...
                        NVL((select sum(case seq.court_site_id when 1 then ctu.am_time_hours + (ctu.am_time_mins/60) 
                            ...
                            where ctu.sitting_date(+) between v_start_date and v_end_date 
                             -- join to outer query
                            and   ctrm.court_room_id = ctu.court_room_id), 0) as total 
        from  xhb_court ct, xhb_court_room ctrm, xhb_court_site cts, xhb_court_site seq  
        where cts.court_site_id = ctrm.court_site_id
        and   seq.court_site_id < 3
        and   cts.court_id = ct.court_id 
        and   ct.court_id = p_court_id   -- parameter here 
        order by cts.court_site_name, ctrm.crest_court_room_no, am_pm;
