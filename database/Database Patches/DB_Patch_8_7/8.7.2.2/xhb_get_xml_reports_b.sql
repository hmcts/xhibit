...
/** 
 ...
 FUNCTION GET_WARNED_LIST(p_list_id XHB_LIST.LIST_ID%TYPE, v_unique_id VARCHAR, p_annotated INTEGER,
  ...
  /*Get the number of charges against the case*/
  FUNCTION GetChargeLogCnt (p_case_id IN xhb_case.case_id%TYPE)
  RETURN NUMBER
  IS
   v_charge_cnt NUMBER :=0;
...
/* Returns a list of all charge logs for the XHB_DEFENDANT_ON_CASE.DEFENDANT_ON_CASE_ID passed in:
  <cs:Charge IndictmentCountNumber="1" >
      <cs:OffenceStatement>Person having charge procuring abandonment of animal</cs:OffenceStatement>
    </cs:Charge>*/
  FUNCTION get_charge_logs(p_defendant_on_case_id XHB_DEFENDANT_ON_CASE.DEFENDANT_ON_CASE_ID%TYPE) RETURN XMLType IS
  charges XMLType;
  BEGIN
  SELECT XMLAgg( XMLELEMENT("cs:Charge" ,XMLATTRIBUTES(xcl.SEQUENCE_NO AS "IndictmentCountNumber" ,
  '' AS "CJSoffenceCode" )
                           , XMLFOREST('' AS "cs:CRESTchargeID") 
                           , XMLFOREST(XHB_FORMATTING_PKG.parse_XML_string(xcl.CHARGES_INFO) AS "cs:OffenceStatement")
                          , XMLFOREST('tba' AS "cs:OffenceParticulars")

                  )) INTO charges
                  FROM XHB_DEFENDANT_ON_CASE xdoc, XHB_CHARGES_LOG xcl
                  WHERE xdoc.DEFENDANT_ON_CASE_ID = p_defendant_on_case_id
                  AND xdoc.CASE_ID = xcl.CASE_ID
                   ...
                   XMLELEMENT("cs:PersonalDetails",
                     ...
                     , XMLELEMENT("cs:Counsel"
                      , (SELECT XMLAgg(XMLELEMENT("cs:Solicitor"
                        ...
                        ,((CASE WHEN include_charge_log = 1 THEN 
                        XMLELEMENT("cs:Charges",XMLATTRIBUTES(GetChargeLogCnt(xdoc.CASE_ID) as "NumberOfCharges"),get_charge_logs(xdoc.DEFENDANT_ON_CASE_ID)) 
                        ELSE XMLELEMENT("cs:Charges",get_charges(xdoc.DEFENDANT_ON_CASE_ID)) END ))  --Charges
