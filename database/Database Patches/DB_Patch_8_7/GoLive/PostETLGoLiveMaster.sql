...
column filename new_value spool_filename
  select 'Xhibit_DB_Patch_8_7_0_PostETLGoLive_CourtID_'||'&&in_xhb_court_id'||'_'||
...
@@execute_xhb_dm_upd_dup_cpa.sql &&in_xhb_court_id;
select to_char(sysdate, 'DD-MON-YYYY HH24:MI:SS') from dual;
@@execute_xhb_dm_upd_case_listed.sql &&in_xhb_court_id;
select to_char(sysdate, 'DD-MON-YYYY HH24:MI:SS') from dual;
@@execute_xhb_dm_upd_dummy_sittings.sql &&in_xhb_court_id;
select to_char(sysdate, 'DD-MON-YYYY HH24:MI:SS') from dual;
@@disable_crest_link.sql &&in_xhb_court_id;
select to_char(sysdate, 'DD-MON-YYYY HH24:MI:SS') from dual;
@@execute_fix_deleted_crest_cases.sql &&in_xhb_court_id;
select to_char(sysdate, 'DD-MON-YYYY HH24:MI:SS') from dual;
COMMIT;
