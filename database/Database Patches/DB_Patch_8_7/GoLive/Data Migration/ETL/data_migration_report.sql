...
* INSTRUCTIONS
*               To run this script 
*               1. Put the script in a directory from where you can run sqlplus
*               2. Login to sqlplus as data_mig/data_mig
*               3. Type in @data_migration_report.sql    and then press the RETURN key
*               4. At the prompt you will see :-
*                       Enter XHIBIT Court ID: 
*                  So put in the value for the court being processed and press the RETURN key
*               5. Again at the prompt you will see :-
*                       Enter Crest Court ID: 
*                  So put in the value for the court being processed and press the RETURN key
*                   
*               6. The script should then run uninterropted to the end.
*               7. A file called DataMigReport_XXX_YYMMDD_HHMISS.txt will be generated in the same directory where this script was run from.
*               8. Exit sqlplus by typing in: exit
*
* VERSION HISTORY:
*
...
*
* 27/12/2019    S.Sethuraman     1.2        updated report out_put filename suffixed with crest_court_id and timestamp
*
* 08/03/2019    A Dennis         1.3        Updated to inlcude Brian H comments in CTX-3818
*
* 04/04/2019    S Sethuraman     1.4        Updated to inlcude Orphan record details
*
* 02/05/2019    S Sethuraman     1.5        Updates as per CTX-4168
*
* 05/06/2019    S Sethuraman     1.6        Updates as per CTX-4289
*
* 05/07/2019    S Sethuraman     1.7        Updates as per CTX-4289 - CASE_SUB_APPEARANCE - remove SYSDATE change
*
**/
...
ACCEPT xhibit_court_id CHAR PROMPT 'Enter XHIBIT Court ID: '
ACCEPT stg_crest_court_id CHAR PROMPT 'Enter Crest Court ID: '
...
PROMPT
PROMPT SELECT COUNT(*) FROM XHIBIT.XHB_DEFENDANT_ON_CASE WHERE CASE_ID IN (SELECT CASE_ID FROM XHIBIT.XHB_CASE WHERE COURT_ID = &xhibit_court_id) and LAST_UPDATED_BY = 'DATA_MIG';;
SELECT COUNT(*) FROM XHIBIT.XHB_DEFENDANT_ON_CASE WHERE CASE_ID IN (SELECT CASE_ID FROM XHIBIT.XHB_CASE WHERE COURT_ID = &xhibit_court_id) and LAST_UPDATED_BY = 'DATA_MIG';
