...
--have a variable for each of the fields to be created if they are no in xhb_ref_system_code
 ...
 AND nvl(obs_ind,'-') <> 'Y';
   ...
   PROCEDURE insert_record(v_court_id  IN XHB_REF_SYSTEM_CODE.COURT_ID%TYPE,
                           v_code      IN XHB_REF_SYSTEM_CODE.CODE%TYPE,
                           v_code_type IN XHB_REF_SYSTEM_CODE.CODE_TYPE%TYPE,
                           v_de_code   IN XHB_REF_SYSTEM_CODE.DE_CODE%TYPE) IS 
   BEGIN
     MERGE INTO xhb_ref_system_code USING DUAL ON (COURT_ID = v_court_id AND CODE_TYPE = v_code_type AND CODE = v_code)
     ...
     ,obs_ind)
     VALUES 
     (xhb_ref_system_code_seq.nextval  --ref_system_code_id
     ,v_code                           --code
   ...
   PROCEDURE misccaseappeal_code(v_court_id IN xhb_court.court_id%TYPE) IS
   BEGIN
     insert_record(v_court_id  => v_court_id,
                   v_code      => 'O',
                   v_code_type => 'MISCCASE_APPEAL_TYPE',
                   v_de_code   => 'OTHER APPEAL');
     COMMIT;
   END;
 ...
 IF p_court_id IS NULL THEN
  FOR courts_r IN courts_c
   LOOP
    misccaseappeal_code(courts_r.court_id);
 ...
 ELSE --court_id is specified so only run for that court
  misccaseappeal_code(p_court_id);
