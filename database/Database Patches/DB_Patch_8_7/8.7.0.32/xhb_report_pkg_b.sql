...
Select distinct 
                ...
                xc.CASE_TITLE,
                xc.CLASS_CODE,
                xdoc.CURRENT_BC_STATUS as BC_STATUS,
...
from   XHB_CASE_LISTING_ENTRY xcle
      ...
      ,(select doc.* 
          ...
          where doc.CURRENT_BC_STATUS IN ('B','C')
          and doc.CURRENT_BC_STATUS = NVL((SELECT DISTINCT 'C'
                                      FROM XHB_DEFENDANT_ON_CASE doc2
                                      WHERE doc2.CASE_ID = doc.CASE_ID
                                      and NVL(doc2.OBS_IND, '-') <> 'Y'
                                      and doc2.CURRENT_BC_STATUS = 'C'),'B')) xdoc
      ,XHB_CASE_DIARY_FIXTURE xcdf
      ,XHB_REF_HEARING_TYPE xrht
...
v_err_msg  VARCHAR2(1000); 
   BEGIN
     OPEN p_results_out FOR 
      ...
      TO_CHAR(xc.COMMITTAL_DATE,'dd/mm/yyyy') AS committaldate, TO_CHAR(xc.MAG_CONVICTION_DATE,'dd/mm/yyyy') AS convictiondate, 
...
PROCEDURE get_obw_report(p_results_out OUT SYS_REFCURSOR,
                             p_input_date IN XHB_BW_HISTORY.BW_ISSUE_DATE%TYPE,
                             p_court_id      IN XHB_CASE.COURT_ID%TYPE) AS
-- Local variables here
...
and bwh.bw_issue_date >= nvl(p_input_date, bwh.bw_issue_date)
and rsf.REF_SOLICITOR_FIRM_ID(+) = XHB_REPORT_PKG.GET_SOLICITOR_ID(doc.DEFENDANT_ON_CASE_ID)
