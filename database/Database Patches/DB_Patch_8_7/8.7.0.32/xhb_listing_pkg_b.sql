CREATE OR REPLACE PACKAGE BODY XHB_LISTING_PKG AS
	  ...
	  OPEN p_results_out FOR
		SELECT	XC.CASE_ID,
				...
				(	SELECT	DECODE(COUNT(*), 0, 'No', 'Yes') 
					...
					AND 	XCOL.TIME_LISTED > SYSDATE) AS FUTURE_HEARINGS_XCOL,
				(	SELECT 	DECODE(COUNT(*), 0, 'No', 'Yes') 
					FROM 	XHB_CASE_DIARY_FIXTURE XCDF, XHB_CASE_LISTING_ENTRY XCLE2
					WHERE 	XCLE2.CASE_ID = XC.CASE_ID 
					AND		XCDF.CASE_LISTING_ENTRY_ID = XCLE2.CASE_LISTING_ENTRY_ID
					AND		NVL(XCDF.OBS_IND,'N') = 'N'
					AND 	XCDF.LISTING_DATE > SYSDATE) AS FUTURE_HEARINGS_XCDF,
				XHB_CASE_PKG.DETERMINE_CASE_STATUS(XC.CASE_ID) AS CASE_STATUS, 
				XRHT.HEARING_TYPE_CODE AS HEARING_TYPE, 
				NULL AS EFFECTIVE_CRACKED_INEFFECTIVE,
				--DECODE( NVL(XSH.CRACKED_INEFFECTIVE_ID,0),
				--				0,'Not Applicable',
		...
		AND		(	(p_mode = 1 AND XS.COURT_SITE_ID IN (SELECT T.COURT_SITE_ID FROM XHB_COURT_SITE T WHERE T.COURT_ID = p_court_id))
				OR	(p_mode = 2 AND XS.IS_FLOATING = 1 AND XS.COURT_SITE_ID = p_site_id)
				OR	(p_mode = 3 AND XS.COURT_SITE_ID = p_site_id)
				OR	(p_mode = 4 AND XS.COURT_SITE_ID = p_site_id AND XS.COURT_ROOM_ID = p_room_id)
				)
		AND 	XCS.COURT_SITE_ID = XS.COURT_SITE_ID
		AND 	XCR.COURT_ROOM_ID = XS.COURT_ROOM_ID
		--AND   	XRCE.REF_CRACKED_EFFECTIVE_ID (+)= XSH.CRACKED_INEFFECTIVE_ID -- column on XHB_SCHEDULED_HEARING does not exist yet
		AND 	XRHT.REF_HEARING_TYPE_ID = XH.REF_HEARING_TYPE_ID
		ORDER BY XCS.COURT_SITE_CODE, XS.IS_FLOATING, XCR.CREST_COURT_ROOM_NO, XS.SITTING_SEQUENCE_NO;
  ...
  END find_cases_for_list_results;
	...
	PROCEDURE update_case_details (p_case_id IN XHB_CASE.CASE_ID%TYPE,
								  ...
								  p_user IN XHB_CASE.LAST_UPDATED_BY%TYPE) AS
								  
		n_count	NUMBER;
		n_case_listing_entry_id XHB_CASE_LISTING_ENTRY.CASE_LISTING_ENTRY_ID%TYPE := -1;
		n_directions_for_case_id XHB_DIRECTIONS_FOR_CASE.DIRECTIONS_FOR_CASE_ID%TYPE := -1;
								  
	BEGIN

		-- Update XHB_CASE record
		UPDATE XHB_CASE SET DEFAULT_HEARING_TYPE = DECODE(p_hearing_type, -1, NULL, p_hearing_type) WHERE CASE_ID = p_case_id;
		
		-- Double check that a XHB_CASE_LISTING_ENTRY record exists for the case
		n_case_listing_entry_id := p_case_listing_entry_id;
		SELECT COUNT(*) INTO n_count FROM xhb_case_listing_entry WHERE case_id = p_case_id;
		IF p_case_listing_entry_id = -1 AND n_count > 0 THEN
			-- Record did not exist when List Results screen retrieved data, but exists now so get the id
			SELECT case_listing_entry_id INTO n_case_listing_entry_id FROM xhb_case_listing_entry WHERE case_id = p_case_id;
		END IF;
		
		IF n_case_listing_entry_id = -1 THEN
			-- No XHB_CASE_LISTING_ENTRY record exists so create one
			INSERT INTO xhb_case_listing_entry (
				case_id,
				judge_id,
				created_by,
				last_updated_by,
		...
		ELSE
			-- Update existing XHB_CASE_LISTING_ENTRY record
			UPDATE xhb_case_listing_entry
			SET judge_id = DECODE(p_judge, -1, NULL, p_judge),
				last_updated_by = p_user
			WHERE case_listing_entry_id = n_case_listing_entry_id;
			
		END IF;
		
		-- Double check that a XHB_DIRECTIONS_FOR_CASE record exists for the case
		n_directions_for_case_id := p_directions_for_case_id;
		SELECT COUNT(*) INTO n_count FROM xhb_directions_for_case WHERE case_id = p_case_id;
		IF p_directions_for_case_id = -1 AND n_count > 0 THEN
			-- Record did not exist when List Results screen retrieved data, but exists now so get the id
			SELECT directions_for_case_id INTO n_directions_for_case_id FROM xhb_directions_for_case WHERE case_id = p_case_id;
		END IF;
		
		IF n_directions_for_case_id = -1 THEN
			-- No XHB_DIRECTIONS_FOR_CASE record exists so create one
			INSERT INTO xhb_directions_for_case (
				case_id,
				trial_time_unit,
				trial_time_estimate,
				last_updated_by,
			...
			VALUES (
				p_case_id,
				DECODE(p_time_unit, -1, NULL, p_time_unit),
				DECODE(p_time_est, -1, NULL, p_time_est),
				p_user,
				p_user
			);
		ELSE
			-- Update existing XHB_DIRECTIONS_FOR_CASE record
			UPDATE xhb_directions_for_case
			SET trial_time_unit = DECODE(p_time_unit, -1, NULL, p_time_unit),
				trial_time_estimate = DECODE(p_time_est, -1, NULL, p_time_est),
				last_updated_by = p_user
			WHERE directions_for_case_id = n_directions_for_case_id;
