...
from(select  o.* from Xhb_Formatting  o , Xhb_clob c
    where  o.court_Id =p_court_id
      ...
      and c.clob_data like concat('%<courtsitename>',concat(p_court_site_name,'</courtsitename>%')) order by o.date_in desc) where rownum=1;
  ...
  WHEN NO_DATA_FOUND THEN
    ...
    END get_latest_xhibit_clob_Id;
     ...
     FUNCTION get_doc_recipient_name(p_court_site_name IN VARCHAR2) RETURN VARCHAR2 IS
      v_courtsite_id XHB_COURT_SITE.COURT_SITE_ID%TYPE;
      v_amount NUMBER;
      v_name VARCHAR2(255);
     ...
     into v_courtsite_id
     from xhb_court_site
     where upper(court_site_name) = upper(p_court_site_name) and (obs_ind is null or obs_ind != 'Y');
     
     select count(court_satellite_id) into v_amount from xhb_court_satellite where court_site_id=v_courtsite_id and (obs_ind is null or obs_ind != 'Y');
     if(v_amount>0) then
        select internet_satellite_name into v_name from xhb_court_satellite where court_site_id=v_courtsite_id and (obs_ind is null or obs_ind != 'Y');
     else
     select internet_court_name into v_name from xhb_court where court_id = (select court_id from xhb_court_site where court_site_id=v_courtsite_id);
     end if;
     RETURN v_name;
     END get_doc_recipient_name;
