CREATE OR REPLACE PACKAGE BODY xhb_list_distribution_pkg AS
  ...
  PROCEDURE PUBLISH_LIST(p_list_id XHB_LIST.LIST_ID%TYPE) IS
   ...
   v_document_type VARCHAR(8);
   v_crest_court_code VARCHAR(3);
   ...
   --Determine the List Type
        ...
        IF(v_list_type = 'Daily') THEN
          ...
          v_return_xml:= XHB_GET_XML_REPORTS.GetDLData(p_list_id);
          
          --AND generated Daily List Mercator XML
          v_merc_xml:= XHB_GET_XML_REPORTS.GetInternalDailyListData(p_list_id);
          SELECT xc.CREST_COURT_ID INTO v_crest_court_code FROM XHB_COURT xc WHERE xc.COURT_ID = v_list_row.COURT_ID;
          v_formatted_list_date:=  to_char(v_list_row.LIST_START_DATE, 'YYYY-MM-DD HH24:MI:SS');
          
          MERGE INTO MTBL_MERC_DL_STORAGE USING DUAL ON (FROMID = v_crest_court_code AND ADDITIONAL3 = v_formatted_list_date)
                WHEN NOT MATCHED THEN  INSERT (NAME, MSGTYPE, MSGTIME, MSGDATE, ADDITIONAL1, ADDITIONAL3, FROMID, MESSAGE) 
                   VALUES ('Daily List XML Extract Delivery', 'Daily_List_XML_Upload', to_char(sysdate, 'HH24:MI:SS'), trunc(sysdate), 'READY', v_formatted_list_date, v_crest_court_code, v_merc_xml)
        ...
        ELSIF(v_list_type = 'Firm') THEN
          v_return_xml:= XHB_GET_XML_REPORTS.GET_FIRM_LIST_DATA(p_list_id);
          v_document_type:='FL';
        END IF;
        ...
        (clob_id, clob_data)
        VALUES
        (XHB_CLOB_SEQ.NEXTVAL, v_return_xml)
        RETURNING clob_id
        INTO      l_clob_id;
