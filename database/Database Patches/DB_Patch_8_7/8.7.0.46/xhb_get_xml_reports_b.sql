create or replace PACKAGE BODY XHB_GET_XML_REPORTS AS
  ...
  BEGIN    
    ...
    END IF;
    RETURN CONCAT(RTRIM(p_town)||RTRIM(v_delimiter),RTRIM(p_county));
...
/** 
  ...
  BEGIN
    SELECT XMLAgg(
                  XMLELEMENT("ScheduledHearing",xcol.SEQ_NO || '|' || CASE WHEN(xrsc.DE_CODE IS NOT NULL) THEN xrsc.DE_CODE ||  TO_CHAR(xcol.TIME_LISTED, 'HH24:MI am') END ||  '|' ||  TO_CHAR(xcol.TIME_LISTED, 'YYYY-MM-DDHH24:MI') || '|' || TO_CHAR(xcol.TIME_LISTED, 'HH24:MI am') || '|'
                                                || SUBSTR(xcol.LIST_NOTE_TEXT, 1, 78) || '|' || xc.CASE_TYPE || xc.CASE_NUMBER || '|' || xc.CASE_TYPE || '|' || xrht.HEARING_TYPE_CODE || '|'
                                                  ...
                                                  ,XMLAgg(XMLELEMENT("cs:Fixture"
                                                    ...
                                                    , XMLELEMENT("cs:Cases"
                                                        ,XMLELEMENT("cs:Case"
                                                                          ...
                                                                          ,(SELECT XMLAgg( XMLELEMENT("cs:Defendants", 
                                                                                          get_defendant(xdoc.DEFENDANT_ON_CASE_ID, 0, 1)))
                                                                            FROM XHB_DEFENDANT_ON_CASE xdoc
                                                                            WHERE xc.CASE_ID = xdoc.CASE_ID
                                                  ...
                                                  ,XMLAgg(XMLELEMENT("cs:Fixture" 
                                                          ...
                                                          , XMLELEMENT("cs:Cases"
                                                             ,XMLELEMENT("cs:Case"
                                                                 ...
                                                                 ,(SELECT XMLAgg( XMLELEMENT("cs:Defendants", 
                                                                                 get_defendant(xdocol.DEFENDANT_ON_CASE_ID, 0, 1)))
  ...
  */
  ...
  BEGIN
  SELECT XMLELEMENT("cs:ContactDetails",(SELECT XMLAgg(XMLFOREST(XMLFOREST(prosecutor_email.CONTACT_VALUE AS "EmailAddress") AS "Email")) 
 ...
 FUNCTION get_non_available_dates(p_case_id XHB_CASE.CASE_ID%TYPE, p_list_start_date XHB_LIST.LIST_START_DATE%TYPE, p_list_end_date XHB_LIST.LIST_END_DATE%TYPE) RETURN XMLType IS
  ...
  FROM XHB_DEF_ON_CASE_ON_LIST xdocol,  XHB_DEFENDANT_ON_CASE xdoc
    ...
    IF defendants IS NULL THEN 
       SELECT XMLELEMENT("cs:Defendant",
                XMLELEMENT("cs:PersonalDetails",
                   XMLFOREST(XMLFOREST(xc.case_title AS "p1:CitizenNameSurname") AS "cs:Name"
                            ,'yes' AS "cs:IsMasked")
                          )--PersonalDetails
                      )--Defendant 
       INTO   defendants
         ...
         ,XMLFOREST(xrpa.PROSECUTOR_NAME_3 AS "cs:ProsecutingReference"
                , XMLFOREST(xrpa.CPS_CODE AS "cs:OrganisationCode"
                           ...
                           , XMLFOREST(NVL(prosecutor_address.ADDRESS_1,'-') AS "p1:Line"
                                      ...
                                      ,NVL(prosecutor_address.ADDRESS_4,'-') AS "p1:Line"
                                      ,GET_TOWN_AND_COUNTY(prosecutor_address.TOWN,prosecutor_address.COUNTY) AS "p1:Line"
