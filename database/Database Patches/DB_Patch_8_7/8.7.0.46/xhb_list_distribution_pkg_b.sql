CREATE OR REPLACE PACKAGE BODY xhb_list_distribution_pkg AS
    ...
    BEGIN   
        ...
        IF(v_list_type = 'Daily') THEN
          ...
          v_unique_id := 'CSD' || v_document_type_code || LPAD(l_validation_id, 15, 0); 
          v_return_xml:= XHB_GET_XML_REPORTS.GET_DAILY_LIST(p_list_id, v_unique_id);
    ...
    FUNCTION get_cji_document_type_data(p_document_type_code IN VARCHAR2, p_column_name IN VARCHAR2)
        ...
        CURSOR c_xmac IS
        SELECT xmac.PARAMETERS
            FROM XHB_MTBL_ADAPTOR_CONFIG xmac
            WHERE xmac.ADAPTOR_TYPE = 'CJI' AND xmac.SERVER_TYPE = 'DOC_TYPE'
            AND xmac.DOCUMENT_NAME = p_document_type_code AND xmac.SERVER_ADDRESS = p_column_name;
    BEGIN
        OPEN c_xmac;
        FETCH c_xmac INTO v_result;
        CLOSE c_xmac;
        RETURN v_result;
