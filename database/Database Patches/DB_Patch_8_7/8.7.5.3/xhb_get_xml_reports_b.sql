...
FUNCTION GET_CROWN_COURT_RL(p_court_id XHB_COURT.COURT_ID%TYPE) RETURN XMLTYPE IS
 ...
 FUNCTION get_highlight_note(p_case_id XHB_CASE.CASE_ID%TYPE) RETURN VARCHAR2 IS
  ...
  otherNotes XMLTYPE;
  BEGIN
	SELECT XMLELEMENT("cs:Notes", XMLAgg( XMLELEMENT("cs:Note", (CASE WHEN((p_include_standard_notes = 1 AND NVL(noteclass.REF_DATA_VALUE,'Priority') = 'Standard') OR
																			(p_include_priority_notes = 1 AND NVL(noteclass.REF_DATA_VALUE,'Priority') = 'Priority') OR
																			(p_include_restricted_notes = 1 AND NVL(noteclass.REF_DATA_VALUE,'Priority') = 'Restricted') OR
															  ...
															  )ORDER BY DECODE(NVL(noteclass.REF_DATA_VALUE,'Priority'), 'Priority', 0, 'Restricted', 1, 2),
 ...
 FUNCTION get_non_available_dates(p_case_id XHB_CASE.CASE_ID%TYPE, p_list_start_date XHB_LIST.LIST_START_DATE%TYPE, p_list_end_date XHB_LIST.LIST_END_DATE%TYPE) RETURN XMLType IS
  ...
  BEGIN
    SELECT XMLAgg( XMLELEMENT("cs:Defendants",
                              get_defendant(xdoc.DEFENDANT_ON_CASE_ID, 0, 0, 1, 1)))
    INTO   defendants
