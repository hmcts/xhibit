		...
		IF xhb_rcs_tt IS NOT NULL AND xhb_rcs_tt.COUNT > 0 THEN
			FOR i IN xhb_rcs_tt.FIRST .. xhb_rcs_tt.LAST LOOP
				...
				IF 	v_curr_ref_judge_id = v_next_ref_judge_id AND 
					...
					v_curr_court_room_id = v_next_court_room_id THEN
				...
				ELSE             
					...
					IF v_ju_row_cnt = 0 
						...
						AND judge_usage_exists_YN(xhb_rcs_tt(i).court_room_id, xhb_rcs_tt(i).sitting_date, xhb_rcs_tt(i).ref_judge_id) = 'N' THEN
...
FUNCTION judge_usage_exists_YN (p_courtroom_id IN XHB_COURT_ROOM.COURT_ROOM_ID%TYPE
							   ,p_sittingdate  IN DATE
