 ...
 **************************************************************************************************/
  ...
  */         
  ...
  BEGIN
      ...
      SELECT PROPERTY_VALUE INTO v_number_of_years
         FROM XHB_CONFIG_PROP WHERE PROPERTY_NAME = 'XHB_HK_NO_OF_YEARS';        
   ...
   FOR i in eligible_B_U_cases loop 
      dbms_output.put_line(i.court_id||','||i.case_id||','||i.case_number||','||i.case_type||','||i.case_status||','||i.last_update_date||','||i.date_trans_to);
       UPDATE XHB_CASE 
          SET CASE_STATUS = 'H' 
        WHERE COURT_ID = i.court_id AND CASE_ID = i.CASE_ID;
   END LOOP;     
   COMMIT;
...
*/              
  ...
  BEGIN
       ...
       SELECT PROPERTY_VALUE INTO v_number_of_days
         FROM XHB_CONFIG_PROP WHERE PROPERTY_NAME = 'XHB_HK_NO_OF_DAYS_TO_HOUSEKEEP';
...
*/              
  ...
  CURSOR wr_def_history_recs (l_court_id xhb_case.court_id%type, l_case_id xhb_case.case_id%type) IS
   SELECT   xd.*
    FROM   xhb_Defendant  xd,xhb_defendant_on_Case xdc
    WHERE  xd.court_id = l_court_id 
    AND    xdc.case_id = l_case_id
  ...
  CURSOR wr_def_on_case_history_recs (l_court_id xhb_case.court_id%type, 
                                      l_case_id xhb_case.case_id%type,
                                      l_defendant_id xhb_defendant.defendant_id%type) IS
   SELECT  distinct xdc.defendant_number
    FROM   xhb_Defendant  xd,xhb_defendant_on_Case xdc
    WHERE  xd.court_id = l_court_id 
    AND    xdc.case_id = l_case_id
    AND    xd.defendant_id  = xdc.defendant_id  
    AND    xd.defendant_id = l_defendant_id;
    -- these would have never been written to def_on_case history as the case_id and defendant_ids
    -- are   written to 1st time    
    
  CURSOR wr_doc_history_recs_del_defs (l_court_id xhb_case.court_id%type, 
                                      l_case_id xhb_case.case_id%type,
                                      l_case_history_id xhb_case_history.case_history_id%type) IS
   SELECT  distinct xdc.defendant_number,xdh.defendant_history_id
    FROM   xhb_Defendant  xd,xhb_defendant_on_Case xdc,
           xhb_defendant_history xdh 
    WHERE  xd.court_id = l_court_id     
    AND    xd.court_id = xdh.court_id
    AND    xdc.case_id = l_case_id
    AND    xd.defendant_id  = xdc.defendant_id 
    AND    xd.defendant_id = xdh.defendant_id
    AND    xd.crest_defendant_id = xdh.crest_defendant_id
    -- Exclude those already written to DEF_ON_CASE_HISTORY
    AND NOT EXISTS (SELECT 'X' FROM XHB_DEFENDANT_ON_CASE_HISTORY XDCH 
                        WHERE XDCH.case_history_id = l_case_history_id AND 
                              XDCH.DEFENDANT_HISTORY_ID = XDH.DEFENDANT_HISTORY_ID AND
                              XDCH.DEFENDANT_NUMBER = XDC.DEFENDANT_NUMBER);     
  ...
  BEGIN
     ...
     BEGIN
      ...
      SELECT PROPERTY_VALUE INTO v_hk_period
         FROM XHB_CONFIG_PROP WHERE PROPERTY_NAME = 'XHB_HK_NO_OF_YEARS';
   ...
   LOOP 
      ...
      BEGIN 
       ...
       v_case_id := i.case_id;
       v_case_number := i.case_number;
       v_case_type := i.case_type;
      

       SELECT XHB_CASE_HISTORY_SEQ.NEXTVAL INTO v_case_history_id from dual;

       l_err_stage := 'CASE_HISTORY_INSERT - COURT_ID : '||v_court_id||' , CASE_NUMBER : '||v_case_number||' , CASE_TYPE : '||v_case_type;

       INSERT INTO XHB_CASE_HISTORY (
                                         CASE_HISTORY_ID,
                                         CASE_TYPE,
                                         CASE_NUMBER,
                                         PSD_CT_CODE,
                                ...
                                VALUES (
                                         ...
                                         i.CREST_CODE,
                                         i.COMMITTAL_DATE,
                                         '7 Year Case Housekeeping',
                                         i.CASE_TITLE,
                                         SYSDATE,
                                         i.SENT_FOR_TRIAL_DATE,
                                         i.COURT_ID,
                                         sysdate,
                                         sysdate,
                                         'WRITE_DEL_RECORD_TO_HISTORY',
                                         'WRITE_DEL_RECORD_TO_HISTORY'
                                        ); 

          FOR j in wr_def_history_recs (v_court_id,v_case_id)
          LOOP 
      
              v_defendant_id := j.defendant_id;
              
               SELECT XHB_DEFENDANT_HISTORY_SEQ.NEXTVAL INTO v_defendant_history_id from dual;

      
         l_err_stage := 'DEFENDANT_HISTORY_INSERT - COURT_ID : '||v_court_id||' , CASE_NUMBER : '||v_case_number||' , CASE_TYPE : '||v_case_type||' , DEFENDANT_ID : '||v_defendant_id;
               ...
               INSERT INTO XHB_DEFENDANT_HISTORY (
                 ...
                 -- write the def on case records for the defendants that are live and 
                 -- never been deleted with reference to other cases which are deleted before
                 FOR k in wr_def_on_case_history_recs (v_court_id,v_case_id,v_defendant_id)
                 LOOP

                    l_err_stage := 'DEF_ON_CASE_HISTORY_INSERT - COURT_ID : '||v_court_id||' , CASE_NUMBER : '||v_case_number||' , CASE_TYPE : '||v_case_type||' , DEFENDANT_ID : '||v_defendant_id;
            ...
            -- write the def on case records for the defendants that are already deleted
            -- with reference to other cases which are deleted before
            -- and whose defendant_history is already created i.e. not created in this run
            -- but link them to their def_on_case_history
            FOR m in wr_doc_history_recs_del_defs (v_court_id,v_case_id,v_case_history_id)
            LOOP

              v_defendant_history_id := m.defendant_history_id;
              v_defendant_number := m.defendant_number;
              
                    l_err_stage := 'DEL_DEF_ON_CASE_HISTORY_INSERT - COURT_ID : '||v_court_id||' , CASE_NUMBER : '||v_case_number||' , CASE_TYPE : '||v_case_type||' , DEFENDANT_HISTORY_ID : '||v_defendant_history_id||' , DEFENDANT_NO : '||v_defendant_number;
                    INSERT INTO XHB_DEFENDANT_ON_CASE_HISTORY (
                                         DEFENDANT_ON_CASE_HISTORY_ID,                    
                                         DEFENDANT_HISTORY_ID,
                                         CASE_HISTORY_ID,
    ...
    EXCEPTION  WHEN OTHERS THEN
        ...
        BEGIN
          UPDATE XHB_CASE 
           ...
           WHERE COURT_ID = v_court_id AND 
                 CASE_ID = v_case_id AND 
                 CASE_NUMBER = v_case_number AND 
  ...
  PROCEDURE process_cad_hk (p_age IN NUMBER DEFAULT 180) IS
	...
	SELECT l.dmi_cad_run_history_id, l.log_message
...
PROCEDURE del_courtel_list_data (p_upper_limit IN NUMBER DEFAULT 10000) IS
/*
...
*/              
  ...
  BEGIN
     ...
     BEGIN
      ...
      SELECT PROPERTY_VALUE INTO v_hk_clist_days
  ...
  --  DELETE from XHB_COURTEL_LIST
   ...
   LOOP
    ...
    EXCEPTION  WHEN OTHERS THEN
       ...
       l_err_message := 'XHB_HK_LIST_DELETE_COURTEL_LIST: courtel_list_id: '||v_clist_id||' , blob_id : '||v_blob_id;
          -- Rollback transaction and record error
          ROLLBACK;
