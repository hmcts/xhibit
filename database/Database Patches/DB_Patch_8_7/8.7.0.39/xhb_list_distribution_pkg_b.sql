create or replace PACKAGE BODY xhb_list_distribution_pkg AS
  ...
  PROCEDURE PUBLISH_LIST(p_list_id XHB_LIST.LIST_ID%TYPE) IS
   ...
   BEGIN   
        ...
        IF(v_list_type = 'Daily') THEN
           ...
           STORE_LIST(v_return_xml, 'Daily Prison List', v_list_row, v_document_type_code, l_validation_id , v_unique_id , v_list_court);
          ...
          STORE_LIST(v_return_xml, 'Daily List For Distribution', v_list_row, v_document_type_code, l_validation_id , v_unique_id , v_list_court);
        ...
        ELSIF(v_list_type = 'Firm') THEN
          ...
          STORE_LIST(v_return_xml, 'Firm List For Distribution', v_list_row, v_document_type_code, l_validation_id , v_unique_id , v_list_court);
        ...
        ELSIF(v_list_type = 'Warned') THEN
          ...
          STORE_LIST(v_return_xml, 'Warned List For Distribution', v_list_row, v_document_type_code, l_validation_id , v_unique_id , v_list_court);
          ...
          v_unique_id := 'CSD' || v_document_type_code || LPAD(l_validation_id, 15, 0);
          v_return_xml:= XHB_GET_XML_REPORTS.GET_WARNED_LIST(p_list_id, v_unique_id, 1,1,1);
        END IF;
        
      STORE_LIST(v_return_xml, v_list_type || ' List', v_list_row, v_document_type_code, l_validation_id , v_unique_id , v_list_court);
  ...
  PROCEDURE STORE_LIST(p_return_xml CLOB, p_list_type VARCHAR2, p_list_row XHB_LIST%ROWTYPE, p_document_type_code VARCHAR2 
                    ,p_validation_id XHB_VALIDATION.VALIDATION_ID%TYPE, p_unique_id VARCHAR, p_list_court XHB_COURT%ROWTYPE) as
  l_clob_id XHB_CLOB.clob_id%TYPE;
  ...
  INSERT INTO XHB_CLOB(clob_id, clob_data)
        ...
        INSERT INTO XHB_XML_DOCUMENT ( date_created,
                              ...
                              VALUES (TRUNC(SYSDATE),
                                       p_list_type ||' from ' || p_list_row.LIST_START_DATE || ' to ' || p_list_row.LIST_END_DATE || ' ' || DECODE(p_list_row.DRAFT_OR_FINAL, 'D', 'DRAFT v', 'F', 'FINAL v') || p_list_row.LIST_NUMBER || ' ' || sysdate,
                                       G_NEW_DOCUMENT,
                                       p_document_type_code,
                                       p_list_court.COURT_ID,
                                       l_clob_id
                                     );
        ...
        INSERT INTO XHB_VALIDATION_CJI (VALIDATION_ID, 
                              ...
                              VALUES (p_validation_id,
                                      ...
                                      v_stylesheet_name,
                                      sysdate,
                                      p_list_court.CREST_COURT_ID,
                                      p_document_type_code ||' ' || p_list_row.LIST_START_DATE || ' ' || DECODE(p_list_row.DRAFT_OR_FINAL, 'D', 'DRAFT v', 'F', 'FINAL v') || p_list_row.LIST_NUMBER,
                                      p_list_type ||', ' || p_list_court.COURT_NAME || ' on ' || p_list_row.LIST_START_DATE || ' ' || DECODE(p_list_row.DRAFT_OR_FINAL, 'D', 'DRAFT v', 'F', 'FINAL v') || p_list_row.LIST_NUMBER);
