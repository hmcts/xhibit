create or replace PACKAGE BODY        xhb_housekeeping_jkg AS
...
BEGIN
   l_error_point := l_proc_name || ' ' || p_table_name || ' 1';
   l_delete := 'DELETE ' || p_table_name
               || '  WHERE  case_id = :1';
...
END delete_table_ctx;
 ...
 /************************************************************************************************
 *    PROCEDURE delete_case_ctx(p_case_id)
 *
 ...
 **************************************************************************************************/
  ...
  BEGIN
    ...
    SELECT hk_run_id_seq.NEXTVAL, cas.case_number, cas.case_type 
    INTO   l_hk_run_id, v_case_no, v_case_type 
    FROM   XHB_CASE cas
