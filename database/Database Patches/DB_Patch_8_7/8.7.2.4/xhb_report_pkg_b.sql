...
v_err_msg  VARCHAR2(1000);
   BEGIN
     OPEN p_results_out FOR 
	  ...
	  AND (xdc.RESULTS_VERIFIED IS NULL OR xdc.RESULTS_VERIFIED <> 'E')
...
**************************************************************************************/
  ...
  BEGIN
      ...
      and  (xrsc.OBS_IND IS NULL OR xrsc.OBS_IND <> 'Y')
...
v_err_msg  VARCHAR2(1000);
   BEGIN
     ...
     CASE WHEN TRUNC(xdc.CUSTODY_TIME_LIMIT) BETWEEN TRUNC(SYSDATE) AND TRUNC(SYSDATE+7) THEN 'Y'
	  ...
	  AND (xdc.RESULTS_VERIFIED <> 'E' OR xdc.RESULTS_VERIFIED IS NULL)
...
BEGIN  
    ...
    and   (r.OBS_IND IS NULL OR r.OBS_IND <> 'Y')
...
BEGIN  
    ...
    JOIN
       ...
       AND (ref_system_code.OBS_IND IS NULL OR ref_system_code.OBS_IND <> 'Y')
    WHERE
