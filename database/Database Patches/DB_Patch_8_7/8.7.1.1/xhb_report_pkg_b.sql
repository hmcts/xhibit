create or replace PACKAGE BODY "XHB_REPORT_PKG" AS
  ...
  *    ----     -----------------------
  *      65     PROCEDURE get_ctlrl_report
  *     111     PROCEDURE update_case_reminder_printed
  *     139     FUNCTION GET_DEFENDANTS_LIST
  *     187     FUNCTION get_bc_status_ind
  *     204     PROCEDURE get_defendants_put_back_report
  *     284     PROCEDURE get_docar_report
  ...
  *    3969     PROCEDURE get_RRCA_detail
  *    4063     PROCEDURE get_RRCA_summary
  *    4209     PROCEDURE getCtRmSittingTimes
  *
  **************************************************************************************/
...
v_err_msg  VARCHAR2(1000);
   BEGIN
     OPEN p_results_out FOR 
      ...
      xdc.PTIURN AS urn,
      getPhoneNum(xa1.ADDRESS_ID) AS courttelephoneno,   -- ctx-2963 changed to use function getPhoneNum
	  ...
	  AND TRUNC(xdc.CUSTODY_TIME_LIMIT) BETWEEN TRUNC(sysdate) AND TRUNC(p_screen_time_limit)
...
**************************************************************************************/
  ...
  BEGIN
      ...
      or     ( NVL(xdoc.RESULTS_VERIFIED, 'x') <> 'R' --if there are subsequent hearings, make sure there are no disposals
      and      not exists (select 'x'-- joined to defendant on offence via defendant on case 
						  from  xhb_disposal2 xdisp, 
						        xhb_defendant_on_offence xdof
						  where xdisp.DEFENDANT_ON_OFFENCE_ID = xdof.DEFENDANT_ON_OFFENCE_ID 
						  AND   xdof.DEFENDANT_ON_CASE_ID = xdoc.DEFENDANT_ON_CASE_ID   -- join to the outer query making this subquery a correlated one
                          AND   nvl(xdisp.OBS_IND, 'N') <> 'Y'
						  )
		    ))
...
,             INITCAP(cxa.address_1 || ' ' || cxa.address_2 || ' ' || cxa.address_3 || ' ' || cxa.address_4) || ' ' || UPPER(cxa.postcode) AS crt_address_1
...
ORDER BY CASE_NO, FIXTURE_DATE, FIXTURE_ID, DEFENDANT_NAME;

    v_step  :=' 3';
		   
  -- We need to mark any Case Diary Fixture entries we have retrieved in this report as no longer needing notice 
  -- by setting XHB_CASE_DIARY_FIXTURE.FIXTURE_NOTICE_REQUIRED to 'N'.  Added for ctx-2895.
  UPDATE XHB_CASE_DIARY_FIXTURE SET FIXTURE_NOTICE_REQUIRED = 'N', last_updated_by = v_user, last_update_date = sysdate 
  WHERE case_diary_fixture_id IN (SELECT cdf.case_diary_fixture_id 
                                  FROM  XHB_CASE_DIARY_FIXTURE cdf 
                                      , xhb_case_listing_entry cle
									  , XHB_CASE c
                                  WHERE cdf.fixture_notice_required = 'Y'
								  AND   cle.case_listing_entry_id = cdf.case_listing_entry_id 
								  AND   trunc(cdf.listing_date) > trunc(sysdate)
								  AND   cle.case_id = c.case_id 
								  AND   c.court_id = p_court_id);
...
BEGIN
  ...
  OPEN p_results_out FOR
    SELECT  notes.DIARY_NOTE_ENTRY_ID,
            xc.CASE_TYPE || xc.CASE_NUMBER as CASE_NUMBER,
          DECODE(get_bc_status_ind(xc.CASE_ID), 'C', '*', 'B', ' ', 'N/A', ' ') as CUSTODY_CASE,
           xc.CASE_TITLE,
...
BEGIN
  ...
  OPEN p_results_out FOR
    ...
    WHERE xc.DEFAULT_HEARING_TYPE = xrht.REF_HEARING_TYPE_ID(+)
    AND   xrmc.REF_MONITORING_CATEGORY_ID = xc.MONITORING_CATEGORY_ID
    AND   NOT EXISTS (SELECT 1 FROM  XHB_CASE_ON_LIST xcol WHERE xcol.CASE_ID = XC.CASE_ID AND ROWNUM = 1 AND NVL(xcol.obs_ind,'N')='N' AND trunc (xcol.TIME_LISTED) > trunc(sysdate))
    AND   NOT EXISTS (SELECT 1 FROM  XHB_CASE_LISTING_ENTRY xcle, XHB_CASE_DIARY_FIXTURE xcdf 
                      WHERE xcle.CASE_ID = XC.CASE_ID  AND xcle.CASE_LISTING_ENTRY_ID = xcdf.CASE_LISTING_ENTRY_ID AND NVL(xcdf.obs_ind,'N')='N' AND ROWNUM = 1 AND trunc (xcdf.LISTING_DATE) > trunc(sysdate))
    AND   XHB_CASE_PKG.determine_case_status(xc.CASE_ID) =  'Open'
...
BEGIN
  OPEN p_results_out FOR
   select          
               ...
               cases.CASE_NUMBER
               FROM 
               (Select xsh.REF_CRACKED_EFFECTIVE_ID,
             ...
             ,        XHB_SCHEDULED_HEARING xsh 
			  ,		  XHB_REF_HEARING_TYPE xrht
                ...
                AND   cas.CASE_TYPE = 'T'
				AND   xrht.HEARING_TYPE_CODE  IN ( 'TRL','TFL','TBK')
                AND   to_char(xsh.NOT_BEFORE_TIME, 'Mon-YYYY')= p_MONTH_PERIOD||'-'||p_YEAR_PERIOD) cases
                , XHB_REF_CRACKED_EFFECTIVE xrce
                WHERE cases.REF_CRACKED_EFFECTIVE_ID(+) = xrce.REF_CRACKED_EFFECTIVE_ID
...
BEGIN
	OPEN p_results_out FOR
		...
		WHERE 	(xc.CASE_TYPE in ('S','T') OR (xc.CASE_TYPE = 'A' AND xc.CASE_SUB_TYPE in ('B', 'S', 'C')))
		AND 	xc.COURT_ID = p_court_id
		AND		xcle.CASE_ID = xc.CASE_ID
		...
		AND 	((p_run_again_ind = 1 AND TRUNC(xcdf.FXL_RUN_DATE) = TRUNC(p_run_date))
				OR
				(p_run_again_ind = 0 AND xcdf.FXL_RUN_DATE IS NULL))
		AND		xrht.REF_HEARING_TYPE_ID = xcdf.HEARING_TYPE_ID
		AND		xfda.CASE_DIARY_FIXTURE_ID = xcdf.CASE_DIARY_FIXTURE_ID
		AND		xfda.ATTENDING = 'Y'
		AND		NVL(xcdf.OBS_IND,'N') <> 'Y'
...
BEGIN  
	...
	select  cas.case_id,
            ...
            -- Appellant columns
            doc.defendant_number, INITCAP(df.first_name || ' ' || df.middle_name || ' ' || df.surname) as appellant_name, 
            ...
            -- Court columns 
            rct.court_full_name || chr(10) || getAddress(rct.address_id) as magistrate_address,      -- Changed ct to rct for CTX-2703
    ...
    and    r.code_type = 'CASE_APPEAL_TYPE'
...
v_err_msg  VARCHAR2(1000);
   BEGIN
     OPEN p_results_out FOR 
      ...
      xa2.ADDRESS_1 || '|' || xa2.ADDRESS_2 || '|' || xa2.ADDRESS_3 || '|' || xa2.TOWN || '|' || xa2.COUNTY || '|' || xa2.POSTCODE || '|' || xcd1.CONTACT_VALUE AS prosecutoraddress,
      xa3.ADDRESS_1 || '|' || xa3.ADDRESS_2 || '|' || xa3.ADDRESS_3 || '|' || xa3.TOWN || '|' || xa3.COUNTY || '|' || xa3.POSTCODE AS defaddress,
      xa4.ADDRESS_1 || '|' || xa4.ADDRESS_2 || '|' || xa4.ADDRESS_3 || '|' || xa4.TOWN || '|' || xa4.COUNTY || '|' || xa4.POSTCODE || '|' || xcd2.CONTACT_VALUE AS soladdress,
      NVL(xd.CURRENT_PRISON_STATUS,'N') as incustody,
	  xrc.COURT_FULL_NAME AS courtto,
	  INITCAP(xrc.COURT_FULL_NAME) AS courttobody,
      xhb_report_pkg.getPhoneNum(xa5.ADDRESS_ID) AS courtfromtelephoneno,    -- ctx-2963  changed to use getPhoneNum
      ...
      LEFT JOIN XHB_REF_SOLICITOR_FIRM xrs ON xhb_report_pkg.GET_SOLICITOR_ID(xdc.DEFENDANT_ON_CASE_ID) = xrs.REF_SOLICITOR_FIRM_ID
...
*
* Determines if a case is valid for the RECC report based upon case type and other
* properties
* Takes a case_id as input parameter.
*
**************************************************************************************/
...
BEGIN  
    ...
    OPEN   p_resultset FOR  
        ...
        where  cas.court_id = p_court_id
        and   XHB_REPORT_PKG.is_valid_RREC_Case(cas.case_id) = 'Y'
        ...
        and    exists(select doc.defendant_on_case_id  -- At least one date_exported must lie within the reporting period (and none after - see next subselect)
                      ...
                      and   doc.case_id = cas.case_id)   -- join to outer query  
  ...
  **************************************************************************************/
    ...
    DBMS_OUTPUT.PUT_LINE('get_RREC_Summary: step ' || v_step || '  start date = ' || v_start_date || ', end date = ' || v_end_date);
      ...
      open p_resultset for
        ...
        select 1 as sort_order, cs.court_site_name, XHB_REPORT_PKG.get_RREC_CaseType_Hdr(cas.case_id) as case_type, XHB_REPORT_PKG.get_RREC_CaseType_Subhdr(cas.case_id) as case_subhdg,
        ...
        0 as section6,         -- Section 6 is calculated from (6) = (2) + (3) + (4) - (5), but we just put in a dummy column
         sum(case when exists(select doc.defendant_on_case_id  -- Atleast one date_exported must lie within the reporting period
                             ...
                             and   nvl(doc.obs_ind, 'N') = 'N' and   doc.case_id = cas.case_id
                             and not exists(select doc.defendant_on_case_id    -- check there are no unexported defendants at the end-date (i.e. case is closed by then)
                                            from   xhb_defendant_on_case doc
                                            where  nvl(doc.date_exported, v_end_date + 1) > v_end_date   -- If a defendant's export date is after the end date then it was by implication not exported at the end date.
                                            and   doc.case_id = cas.case_id)
        ...
        group by cs.court_site_name, XHB_REPORT_PKG.get_RREC_CaseType_Hdr(cas.case_id), XHB_REPORT_PKG.get_RREC_CaseType_Subhdr(cas.case_id)
        UNION  -- The second part of the query calculates the totals for each case type for each court site
        select 1 as sort_order, cs.court_site_name, XHB_REPORT_PKG.get_RREC_CaseType_Hdr(cas.case_id) as case_type, 'Total' as case_subhdg,
        sum(case when (nvl(cas.received_date, v_start_date) < v_start_date)
                 and exists(select doc.defendant_on_case_id  -- For a case to be open at the start of the period, it has to
                            from  xhb_defendant_on_case doc  -- have at least one defendant unexported at the start date.
                            where nvl(doc.date_exported, v_end_date) > v_start_date   -- We map null values to a date after the start
		...
		AND	  (cs.court_site_code = 'A' OR EXISTS (SELECT NULL FROM xhb_court_satellite xcs WHERE xcs.court_site_id = cs.court_site_id AND NVL(xcs.obs_ind,'N') <> 'Y'))
        and   nvl(cas.received_date, v_start_date) <= v_end_date -- parameter here
        group by cs.court_site_name, XHB_REPORT_PKG.get_RREC_CaseType_Hdr(cas.case_id), 'Total'
		UNION	-- The third section returns the totals for each subtype for all sites at the court
		select 2 as sort_order, 'ALL' as court_site_name, XHB_REPORT_PKG.get_RREC_CaseType_Hdr(cas.case_id) as case_type, XHB_REPORT_PKG.get_RREC_CaseType_Subhdr(cas.case_id) as case_subhdg,
        sum(case when (nvl(cas.received_date, v_start_date) < v_start_date)
                 and exists(select doc.defendant_on_case_id  -- For a case to be open at the start of the period, it has to
                            from  xhb_defendant_on_case doc  -- have at least one defendant unexported at the start date.
                            where nvl(doc.date_exported, v_end_date) > v_start_date   -- We map null values to a date after the start
        ...
        and   nvl(cas.received_date, v_start_date) < v_end_date -- parameter here
        group by 'ALL', XHB_REPORT_PKG.get_RREC_CaseType_Hdr(cas.case_id), XHB_REPORT_PKG.get_RREC_CaseType_Subhdr(cas.case_id)
		UNION	-- The fourth section returns the totals for each type for all sites at the court
        select 2 as sort_order, 'ALL' as court_site_name, XHB_REPORT_PKG.get_RREC_CaseType_Hdr(cas.case_id) as case_type, 'Total' as case_subhdg,
        sum(case when (nvl(cas.received_date, v_start_date) < v_start_date)
                 and exists(select doc.defendant_on_case_id  -- For a case to be open at the start of the period, it has to
                            from  xhb_defendant_on_case doc  -- have at least one defendant unexported at the start date.
                            where nvl(doc.date_exported, v_end_date) > v_start_date   -- We map null values to a date after the start
		...
		and   NVL(cas.court_id_receiving_site,-1) in (
			...
			and	  (xcs.court_site_code = 'A' OR EXISTS (SELECT NULL FROM xhb_court_satellite sat WHERE sat.court_site_id = xcs.court_site_id AND NVL(sat.obs_ind,'N') <> 'Y')))
        and   nvl(cas.received_date, v_start_date) <= v_end_date -- parameter here
        group by 'ALL', XHB_REPORT_PKG.get_RREC_CaseType_Hdr(cas.case_id), 'Total'
...
END get_RREC_summary;
  ...
  **************************************************************************************/
  ...
  BEGIN
      -- We need to know if there are any Bench Warrants executed for this case before the end date
      select XHB_REPORT_PKG.get_rrca_age_band(p_case_id, p_end_date)
	  into   v_ageband
      from   dual;
...
BEGIN  
    ...
    SELECT ct.court_name, 
        ...
        XHB_REPORT_PKG.get_rrca_age_band_desc(ab.letter) AS age_band,
...
FUNCTION get_prosecutor_name(p_case_id      IN  XHB_CASE.CASE_ID%TYPE) RETURN VARCHAR2 AS
 ...
 BEGIN 
  SELECT xrpa.PROSECUTOR_NAME_1 || ' ' || xrpa.PROSECUTOR_NAME_2 || ' ' || xrpa.PROSECUTOR_NAME_3 INTO v_prosecutor_name
     FROM  XHB_CASE_PROSECUTOR_AGENCY xcpa,
