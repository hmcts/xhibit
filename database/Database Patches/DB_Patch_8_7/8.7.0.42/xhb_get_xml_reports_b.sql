...
/** 
  ...
  SELECT XMLELEMENT("cs:RunningList" , XMLATTRIBUTES('http://www.courtservice.gov.uk/schemas/courtservice' AS "xmlns:cs"
      ...
      /*************************************************Trial cases fields start here***************************************************/
          ...
          ,XMLFOREST((SELECT XMLAGG(
                      ...
                      AND xc.COURT_ID = p_court_id
                      AND  xc.PUB_RUNNING_LIST_ID = p_next_pub_running_list_id
           )"cs:CommitalCases")--Committal Cases
  ...
  BEGIN
   SELECT XMLELEMENT("cs:CaseArrivedFrom"
                     , XMLELEMENT("cs:OriginatingCourt"
                          ...
                          ,XMLELEMENT("cs:CourtHouseCode" ,XMLATTRIBUTES(xmcrt.COURT_SHORT_NAME AS "cs:CourtHouseShortName"), NVL(xmcrt.CREST_CODE, xmcrt.REF_COURT_ID))--CourtHouseCode
   ...
   BEGIN
                       SELECT XMLAGG(XMLELEMENT("cs:WarnedList"
                                  ...
                                  ,  XMLELEMENT("cs:CrownCourt", XMLConcat (XMLELEMENT( "cs:CourtHouseType", 'Crown Court') 
                                                                     ,XMLELEMENT("cs:CourtHouseCode", NVL(xcrt.CREST_COURT_ID, xcrt.COURT_ID))
                                  ...
                                  , XMLELEMENT("cs:CourtLists"
                                      ,(SELECT XMLAgg(XMLELEMENT("cs:CourtList"
                                                                ,XMLELEMENT("cs:CourtHouse", XMLConcat (XMLELEMENT( "cs:CourtHouseType", 'Crown Court') 
                                                                  ,XMLELEMENT("cs:CourtHouseCode", NVL(xcrts.SITE_GROUP, xcrts.COURT_SITE_ID))
                                                  ...
                                                  ,XMLAgg(XMLELEMENT("cs:Fixture"
                                                    ...
                                                    , XMLELEMENT("cs:Cases"
                                                        ,XMLELEMENT("cs:Case"
                                                            ...
                                                            )ORDER BY DECODE(xc.CASE_TYPE,'T', 3, 'S', 2, 'A', 1, 0), xc.CASE_NUMBER )--Fixture
                                                      )ORDER BY xrht.LIST_SEQUENCE)
                                                      FROM XHB_CASE_DIARY_FIXTURE xcdf, XHB_REF_HEARING_TYPE xrht, XHB_CASE_LISTING_ENTRY xcle, XHB_CASE xc, XHB_REF_LISTING_DATA xrld_predefined_note
                                               ...
                                               ,(SELECT XMLAgg(XMLELEMENT("cs:WithoutFixedDate",XMLATTRIBUTES(xrht.hearing_type_code AS "HearingType")
                                                  ,XMLAgg(XMLELEMENT("cs:Fixture" 
                                                          ...
                                                          , XMLELEMENT("cs:Cases"
                                                            ...
                                                            ,(get_linked_cases(xc.CASE_ID))AS "cs:LinkedCases"
                                                      )ORDER BY DECODE(xc.CASE_TYPE,'T', 3, 'S', 2, 'A', 1, 0), xc.CASE_NUMBER )
                                                  )ORDER BY xrht.LIST_SEQUENCE)
  ...
  */
  ...
  BEGIN
  SELECT XMLELEMENT("ContactDetails",(SELECT XMLAgg(XMLFOREST(XMLFOREST(prosecutor_email.CONTACT_VALUE AS "EmailAddress") AS "Email")) 
                  ...
                  OR   prosecutor_email.contact_type  = 'Secure Email')) 
              ,(SELECT XMLAgg(XMLFOREST(XMLFOREST(prosecutor_phone.CONTACT_VALUE AS "TelNationalNumber")AS "Telephone")) 
              ...
              ,(SELECT XMLAgg(XMLFOREST(XMLFOREST(prosecutor_fax.CONTACT_VALUE AS "FaxNationalNumber")  AS "Fax" )) 
                  FROM XHB_CONTACT_DETAIL prosecutor_fax 
                  WHERE prosecutor_fax.ADDRESS_ID = p_address_id
                  AND  prosecutor_fax.contact_type = 'Fax') 
            ) 
 ...
 FUNCTION get_highlight_note(p_case_id XHB_CASE.CASE_ID%TYPE) RETURN VARCHAR2 IS
  ...
  SELECT ( XMLFOREST( XMLAGG (XMLELEMENT (E, (CASE WHEN((p_include_standard_notes = 1 AND noteclass.REF_DATA_VALUE = 'Standard') OR
    ...
    WHERE p_case_id = xcle.CASE_ID
      ...
      AND xrld_note_type.REF_DATA_TYPE = 'NOTE_TYPE'
      AND xrld_note_type.REF_DATA_VALUE IN ('IN', 'CN')
 ...
 SELECT(XMLAGG(XMLELEMENT("cs:Prosecution"
         ,XMLFOREST(xrpa.PROSECUTOR_NAME_3 AS "cs:ProsecutingReference"
                , XMLFOREST(xrpa.CPS_CODE AS "cs:OrganisationCode"
                           ...
                           , XMLFOREST(NVL(prosecutor_address.ADDRESS_1,'-') AS "p1:Line"
                                      ,NVL(prosecutor_address.ADDRESS_2,'-') AS "p1:Line"
                                      ,NVL(prosecutor_address.ADDRESS_3,'-') AS "p1:Line"
   ...
   SELECT XMLELEMENT("cs:Hearing",XMLFOREST(xcol.SEQ_NO AS "cs:HearingSequenceNumber")
            ...
            -- ,XMLFOREST('CRESTHearingID' AS "CRESTHearingID")
               ...
               ,(SELECT XMLELEMENT("cs:CommittingCourt" 
                            ...
                            ,XMLELEMENT("cs:CourtHouseCode",XMLATTRIBUTES(xrcrt.COURT_SHORT_NAME AS "CourtHouseShortName"), NVL(xrcrt.CREST_CODE, xrcrt.REF_COURT_ID))
