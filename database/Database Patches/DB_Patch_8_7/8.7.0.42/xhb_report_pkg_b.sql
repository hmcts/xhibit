...
/* get the earliest _future_ listing date */
AND   trunc(xcdf.listing_date) = (SELECT MIN (trunc(f.listing_date))
                                  ...
                                  AND   f.listing_date > sysdate)   -- This line added for ctx-2895
AND EXISTS (SELECT 'x'
  ...
  WHERE case_diary_fixture_id IN (SELECT cdf.case_diary_fixture_id 
								  ...
								  AND   c.court_id = p_court_id
                                  AND   trunc(cdf.listing_date) = (SELECT MIN (trunc(f.listing_date))
                                                                   FROM   xhb_case_diary_fixture f
                                                                   WHERE  f.case_listing_entry_id = cdf.case_listing_entry_id)
								 );
...
v_err_msg  VARCHAR2(1000);
   BEGIN
     OPEN p_results_out FOR 
      ...
      getPhoneNum(xa1.ADDRESS_ID) AS courttotelephoneno,    -- ctx-2963  changed to use getPhoneNum
