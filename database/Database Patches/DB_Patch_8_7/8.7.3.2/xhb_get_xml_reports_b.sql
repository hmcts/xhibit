...
/**
  ...
  **/
  ...
  BEGIN
    SELECT XMLAgg(
                  XMLELEMENT("ScheduledHearing",xcol.SEQ_NO || '|' || CASE WHEN(xrsc.DE_CODE IS NOT NULL) THEN xrsc.DE_CODE ||  TO_CHAR(xcol.TIME_LISTED, 'HH24:MI am') END ||  '|' ||  TO_CHAR(xcol.TIME_LISTED, 'YYYY-MM-DDHH24:MI') || '|' || TO_CHAR(xsol.TIME_LISTED, 'HH24:MI am') || '|'
                                                || SUBSTR(ESCAPE_BROKER_RELEASE_CHARS(xcol.LIST_NOTE_TEXT || CASE WHEN(xcol.LIST_NOTE_TEXT IS NOT NULL AND xrld.REF_DATA_VALUE IS NOT NULL) THEN ',  ' END || xrld.REF_DATA_VALUE), 1, 78) || '|' || xc.CASE_TYPE || xc.CASE_NUMBER || '|' || xc.CASE_TYPE || '|' || ESCAPE_BROKER_RELEASE_CHARS(xrht.HEARING_TYPE_CODE) || '|'
                                     ...
                                     , XMLELEMENT("cs:Party"
                                       , XMLELEMENT("cs:Organisation"
                                         ...
                                         , XMLFOREST(xdocrsf.REP_ST_DATE AS "cs:StartDate"
										  ...
										  ,(CASE 
                      WHEN include_charge_log = 1 AND xc.case_type <> 'T' AND GetChargeCnt(xdoc.CASE_ID)>0 AND get_charges(xdoc.DEFENDANT_ON_CASE_ID) IS NOT NULL THEN
                        XMLELEMENT("cs:Charges",XMLATTRIBUTES(GetChargeCnt(xdoc.CASE_ID) as "NumberOfCharges"),get_charges(xdoc.DEFENDANT_ON_CASE_ID)) 
                        --show charges for all other list types for case = T apart from warned list i.e. 2
                      WHEN include_charge_log <>1 AND GetChargeCnt(xdoc.CASE_ID)>0 AND get_charges(xdoc.DEFENDANT_ON_CASE_ID) IS NOT NULL THEN 
                        XMLELEMENT("cs:Charges",XMLATTRIBUTES(GetChargeCnt(xdoc.CASE_ID) as "NumberOfCharges"),get_charges(xdoc.DEFENDANT_ON_CASE_ID)) 
						END)
