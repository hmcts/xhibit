...
if [ "$#" -ne 4 ]; then
   P_UPPER_LIMIT=0;
   P_COURTEL_PERIOD=0;
   P_RUNNING_OBS_PERIOD=0;
   P_RUNNING_DEL_PERIOD=0;
else
   P_UPPER_LIMIT=$1;
   P_COURTEL_PERIOD=$2;
...
set echo on
set feedback on
exec  xhb_housekeeping_pkg_ss.del_courtel_list_data($P_UPPER_LIMIT,$P_COURTEL_PERIOD);
exec  xhb_housekeeping_pkg_ss.housekeep_running_list($P_UPPER_LIMIT,$P_RUNNING_OBS_PERIOD,$P_RUNNING_DEL_PERIOD);
exit
endsql
