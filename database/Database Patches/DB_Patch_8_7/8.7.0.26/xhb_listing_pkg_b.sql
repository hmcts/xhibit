CREATE OR REPLACE PACKAGE BODY XHB_LISTING_PKG AS
  ...
  BEGIN
       OPEN p_results_out FOR
         SELECT xlluv.*, xlluv.list_id "ID" FROM XHB_LIST_LAST_UPDATED_V xlluv, XHB_REF_LISTING_DATA xrld
		  ...
		  ORDER BY xlluv.LIST_START_DATE DESC;
  END get_list;
  ...
  BEGIN
    ...
    OPEN p_results_out FOR
       ...
       (SELECT xl.*, xl.list_id "ID" FROM XHB_LIST_LAST_PUBLISHED_V xl
        WHERE xl.COURT_ID = p_court_id
          AND xl.LIST_TYPE_ID = v_list_type_id
          AND xl.LIST_START_DATE <= p_diary_date
        ORDER BY xl.LIST_START_DATE DESC) subquery
        WHERE ROWNUM = 1; 
  END get_latest_published_daily;
  ...
  BEGIN
    ...
    OPEN p_results_out FOR
       SELECT xlluv.*, xlluv.list_id "ID" FROM XHB_LIST_LAST_PUBLISHED_V xlluv
        WHERE xlluv.COURT_ID = p_court_id
          AND xlluv.LIST_TYPE_ID = v_list_type_id
          AND p_diary_date BETWEEN xlluv.LIST_START_DATE AND xlluv.LIST_END_DATE; 
  END get_published_list_for_date;
