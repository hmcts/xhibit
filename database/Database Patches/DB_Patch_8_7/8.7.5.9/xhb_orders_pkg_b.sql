...
BEGIN
      ...
      XMLELEMENT("AppealResultOrder",
                  ...
                  (SELECT XMLAGG(XMLELEMENT("Offence",
                                      XMLELEMENT("OffenceDescription", DECODE(xro.OFFENCE_CODE,'ZZ99999', xo.CREST_OFFENCE_FREETEXT, xro.OFFENCE_DESC || ' '  || xro.OFFENCE_DESC2) ),

                                      (SELECT XMLAGG(XMLELEMENT("OriginalSentence", get_disposal_lines(original_xdisp.DISPOSAL2_ID)) ORDER BY original_xdisp.DISPOSAL2_ID)
                                      ...
                                      CASE WHEN xrsc_main_verdict.CODE = 'AB' OR xrsc_main_verdict.CODE = 'AC'
                                        ...
                                        ELSE XMLELEMENT("AppealResults",
														XMLATTRIBUTES(xrar.APP_RESULT_CODE AS "AppealResultCode"),
                                                        XMLELEMENT("AppealResultDescription", xrar.APP_RESULT_DESCR1 || xrar.APP_RESULT_DESCR2 || DECODE(xv.APP_LESSER_OFF,NULL,'',' '||xv.APP_LESSER_OFF)   ),
                                                        (SELECT XMLAGG(XMLELEMENT("AppealResultSentence",
																		XMLATTRIBUTES(result_xdisp.COURT_TYPE AS "CourtType",
																		(SELECT DECODE(COUNT(*),0,'N','Y') FROM XHB_DISPOSAL2 tmp WHERE tmp.PSD_DISPOSAL2_ID IS NOT NULL AND tmp.PSD_DISPOSAL2_ID = result_xdisp.DISPOSAL2_ID AND NVL(tmp.OBS_IND, 'N') <> 'Y') AS "Varied"),
                                                                    get_disposal_lines(result_xdisp.DISPOSAL2_ID)) ORDER BY result_xdisp.DISPOSAL2_ID)
                                                        FROM XHB_DISPOSAL2 result_xdisp
                                                        WHERE result_xdisp.DEFENDANT_ON_OFFENCE_ID = xdoo.DEFENDANT_ON_OFFENCE_ID
														AND NVL(result_xdisp.OBS_IND,'N') <> 'Y')) END
                                ))--Offence
                  FROM XHB_CHARGE xch
                  ...
                  (SELECT XMLAGG(XMLELEMENT("OtherOrder",
                                            XMLATTRIBUTES(xdisp.COURT_TYPE AS "CourtType",
                                              (SELECT DECODE(COUNT(*),0,'N','Y') FROM XHB_DISPOSAL2 tmp WHERE tmp.PSD_DISPOSAL2_ID IS NOT NULL AND tmp.PSD_DISPOSAL2_ID = xdisp.DISPOSAL2_ID AND NVL(tmp.OBS_IND, 'N') <> 'Y') AS "Varied"),
                                            get_disposal_lines(xdisp.DISPOSAL2_ID)) ORDER BY xdisp.DISPOSAL2_ID)
...
v_disposal_lines VARCHAR2(10000);
 ...
 IS
  ...
  AND ( ref_data_view.FORM_PRINT = 'Y'
   ...
   ORDER BY  ref_data_view.DISPOSAL2_ID, DECODE(ref_data_view.DBDESTIN,'D15', 99999999, ref_data_view.DIL_SEQ_NO), xhb_disp_line.line_number;
BEGIN
...
BEGIN
IF p_val = 'V4' THEN
RETURN to_char(p_data,'L9,999,999,999.99');
ELSE
RETURN regexp_replace(p_data,chr(8),'');
END IF;
