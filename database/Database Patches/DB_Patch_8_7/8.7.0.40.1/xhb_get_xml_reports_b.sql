                                  ...
                                  , XMLELEMENT("cs:CourtLists"
                                      ,(SELECT XMLAgg(XMLELEMENT("cs:CourtList"
                                                                ,XMLELEMENT("cs:CourtHouse", XMLConcat (XMLELEMENT( "cs:CourtHouseType", 'Crown Court') 
                                                                  ...
                                                                  ,XMLELEMENT("cs:CourtHouseAddress", XMLFOREST(NVL(courtsite_address.ADDRESS_1, '-') AS "p1:Line"
                                                                                                          ,NVL(courtsite_address.ADDRESS_2, '-') AS "p1:Line"
                                                                                                          ,NVL(courtsite_address.ADDRESS_3, '-') AS "p1:Line"
  ...
  */
  ...
  SELECT XMLAgg( XMLELEMENT("cs:Charge" ,XMLATTRIBUTES(NVL(xcge.CREST_CHARGE_SEQ_NO, 1) AS "IndictmentCountNumber" , xro.OFFENCE_CODE AS "CJSoffenceCode")
                           ,XMLFOREST(xdof.crn_id AS "cs:CRN"
                                    ...
                                    , XMLFOREST(NVL(offence_address.ADDRESS_1, '-') AS "p1:Line"
                                               ,NVL(offence_address.ADDRESS_2, '-') AS "p1:Line"
                                               ,NVL(offence_address.ADDRESS_3, '-') AS "p1:Line"
  ...
  SELECT XMLELEMENT("cs:Defendant",
                   XMLELEMENT("cs:PersonalDetails",
                             XMLFOREST(XMLFOREST(CASE WHEN (p_show_public_view = 1 AND xdoc.IS_MASKED = 'Y') THEN ' ' ELSE xd.FIRST_NAME END AS "p1:CitizenNameForename"
                                     ...
                                     , XMLFOREST(NVL(xadd.ADDRESS_1, '-') AS "p1:Line"
                                                 ,NVL(xadd.ADDRESS_2, '-') AS "p1:Line"
                                                 ,NVL(xadd.ADDRESS_3, '-') AS "p1:Line"
  ...
  BEGIN
   SELECT XMLELEMENT("cs:Hearing",XMLFOREST(xcol.SEQ_NO AS "cs:HearingSequenceNumber")
               ...
               ,(SELECT XMLELEMENT("cs:CommittingCourt" 
                            ,XMLELEMENT("cs:CourtHouseType", xrcrt.COURT_TYPE)
                            ,XMLELEMENT("cs:CourtHouseCode",XMLATTRIBUTES(xrcrt.COURT_SHORT_NAME AS "CourtHouseShortName"))
