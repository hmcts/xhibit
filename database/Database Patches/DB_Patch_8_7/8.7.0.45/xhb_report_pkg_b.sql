...
BEGIN  
	...
	select  cas.case_id,
            ...
            rsf.solicitor_firm_name, getAppellantAddress(doc.defendant_on_case_id)  defendant_address, 
             -- Respondent columns
            INITCAP(rpa.prosecutor_name_1 || ' ' || rpa.prosecutor_name_2 || ' ' || rpa.prosecutor_name_3) as respondent_name, 
            getAddress(rpa.address_id) as respondent_address, 
            -- Court columns 
            getAddress(rct.address_id) as magistrate_address,      -- Changed ct to rct for CTX-2703
...
BEGIN
     OPEN p_results_out FOR 
	  ...
	  AND (((sysdate - (p_from_between*7)) <= (NVL(xc.committal_date,xc.sent_for_trial_date)) AND p_to_between IS NULL)
			OR (NVL(xc.committal_date,xc.sent_for_trial_date) BETWEEN (sysdate - (p_to_between*7)) AND (sysdate - (p_from_between*7)))) 
 ...
 Procedure get_RREC_summary(p_resultset  OUT SYS_REFCURSOR 
    ...
    DBMS_OUTPUT.PUT_LINE('get_RREC_Summary: step ' || v_step || '  start date = ' || v_start_date || ', end date = ' || v_end_date); 
      ...
      open p_resultset for 
        ...
        where cas.court_id = p_court_id 
        and   XHB_REPORT_PKG.is_valid_RREC_Case(cas.case_id) = 'Y'
		and   NVL(cas.court_id_receiving_site,-1) in (select xcs.court_site_id from xhb_court_site xcs where xcs.court_id = p_court_id and nvl(xcs.obs_ind,'N') <> 'Y')
        and   nvl(cas.received_date, v_start_date) < v_end_date -- parameter here 
...
BEGIN  
    ...
    order by cs.court_site_name, age_band_code, case_num;
...
BEGIN  
    ...
    select 1 as sort_order, 
           ...
           get_rrca_age_band_text(cas.case_id, v_end_date) as age_band_text, 
    ...
    and    cas.case_type = 'T' 
	and    nvl(cas.sent_for_trial_date, nvl(cas.committal_date, (v_end_date - 728))) <= v_end_date  -- Added for CTX-2964
    and    xhb_case_pkg.determine_case_status(cas.case_id) = 'Open'
    group by ct.court_name
           , cs.court_site_name
           , get_rrca_age_band(cas.case_id, v_end_date)   
           , get_rrca_age_band_text(cas.case_id, v_end_date)   
    UNION		   
    select 1 as sort_order, 
		   ct.court_name, 
           cs.court_site_name as court_site_name,     
	...
	select 2 as sort_order, 
           ...
           Sum(CASE get_bc_status_ind(cas.case_id) 
		           ...
		           WHEN 'C' THEN 1 Else 0 End) as total, 
           get_rrca_age_band(cas.case_id, v_end_date) as age_band_code		   
    from   xhb_court ct, xhb_case cas 
    where  cas.court_id  = ct.court_id 
    and    nvl(cas.court_id_receiving_site,-1) in (select xcs.court_site_id from xhb_court_site xcs where xcs.court_id = p_court_id and nvl(xcs.obs_ind,'N') <> 'Y')
    and    ct.court_id = p_court_id 
    and    cas.case_type = 'T' 
