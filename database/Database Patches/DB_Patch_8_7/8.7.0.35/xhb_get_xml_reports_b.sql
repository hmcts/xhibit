...
;
  ...
  **/
  ...
  FUNCTION GET_WARNED_LIST(p_list_id XHB_LIST.LIST_ID%TYPE, v_unique_id VARCHAR) RETURN CLOB AS
  ...
  x_no_file    EXCEPTION;
   ...
   CURSOR get_xml_c IS
                       SELECT XMLAGG(XMLELEMENT("cs:WarnedList"
                                  ...
                                  , XMLELEMENT("cs:CourtLists"
                                      ,(SELECT XMLAgg(XMLELEMENT("cs:CourtList"
                                                                ,GET_COURT_HOUSE(xcrts.COURT_SITE_ID)
                                          ,(SELECT XMLAgg(XMLELEMENT("cs:WithFixedDate",XMLATTRIBUTES(xrht.hearing_type_code AS "HearingType")
                                                  ,XMLAgg(XMLELEMENT("cs:Fixture"
                                                    ...
                                                    , XMLELEMENT("cs:Notes",xcdf.LIST_NOTE_TEXT)
                                                     ,XMLELEMENT("cs:Cases"
                                                        ,(SELECT XMLAgg(XMLELEMENT("cs:Case",
                                                                           ...
                                                                           , XMLFOREST( XMLFOREST(xrht.hearing_type_desc AS "cs:HearingDescription"
                                                                                      ...
                                                                                      ,xcdf.LIST_NOTE_TEXT AS "cs:ListNote"
                                                                                       )"cs:Hearing")--Hearing
                                                                          ,(SELECT XMLAgg( XMLELEMENT("cs:Defendants", 
                                                                                              get_defendant(xdoc.DEFENDANT_ON_CASE_ID)))
                                                                    ...
                                                                    FROM XHB_CASE_LISTING_ENTRY xcle, XHB_CASE xc
                                                                    WHERE xcdf.CASE_LISTING_ENTRY_ID = xcle.CASE_LISTING_ENTRY_ID
                                                                    AND xcle.CASE_ID = xc.CASE_ID)--Case
                                                                  )--Cases
                                                                  --,LinkedCases
                                                               ))--Fixture
                                                            ))
                                                            FROM XHB_CASE_DIARY_FIXTURE xcdf, XHB_REF_HEARING_TYPE xrht
                                               ...
                                               ,(SELECT XMLAgg(XMLELEMENT("cs:WithoutFixedDate",XMLATTRIBUTES(xrht.hearing_type_code AS "HearingType")
                                                  ,XMLAgg(XMLELEMENT("cs:Fixture"   
                                                          --           , XMLELEMENT("cs:FixedDate",xcdf.LISTING_DATE)
                                                          , XMLELEMENT("cs:Notes",xcol.LIST_NOTE_TEXT)
                                                          , XMLELEMENT("cs:Cases"
                                                            ,XMLELEMENT("cs:Case",
                                                                  XMLELEMENT("cs:CaseNumber", xc.CASE_TYPE || xc.CASE_NUMBER)
                                                                 /*, XMLELEMENT("CaseArrivedFrom"
                                                                 ...
                                                                 , XMLFOREST(XMLFOREST(xrht.hearing_type_desc AS "cs:HearingDescription"
                                                                            --,xcdf.LISTING_DATE AS "cs:HearingDate"
                                                                            ,xcol.LIST_NOTE_TEXT AS "cs:ListNote")"cs:Hearing")--Hearing
                                                  ...
                                                  FROM XHB_CASE_ON_LIST xcol, XHB_REF_HEARING_TYPE xrht, XHB_CASE xc
                                                  WHERE xcol.LIST_ID =xl.LIST_ID
                                                  AND (xcol.OBS_IND IS NULL OR xcol.OBS_IND <> 'Y')
                                                  AND xcol.HEARING_TYPE_ID = xrht.REF_HEARING_TYPE_ID
                                                  AND xrht.COURT_ID = xl.COURT_ID
                                                  ...
                                                  WHERE xcrts.COURT_ID = xcrt.COURT_ID 
                                                  AND (xcrts.obs_ind is null or xcrts.obs_ind <> 'Y')))--CourtLists  
                         )--WarnedList
             ) --header tag
             .getclobval() as xml_data
  ...
  */
  ...
  FUNCTION get_defendant(p_defendant_on_case_id XHB_DEFENDANT_ON_CASE.DEFENDANT_ON_CASE_ID%TYPE) RETURN XMLType IS
  defendant XMLType;
  BEGIN
  SELECT XMLELEMENT("cs:Defendant",
                   XMLELEMENT("cs:PersonalDetails",
                             XMLFOREST(XMLFOREST(xd.FIRST_NAME AS "p1:CitizenNameForename"
                                                ,xd.SURNAME AS "p1:CitizenNameSurname")"cs:Name"
          ...
          FROM XHB_DEFENDANT_ON_CASE xdoc, XHB_DEFENDANT xd, XHB_ADDRESS xadd, XHB_DEFENDANT_REFERENCE xdr
  ...
  defendants XMLType;
  BEGIN
  SELECT XMLAgg(get_defendant(xdoc.DEFENDANT_ON_CASE_ID)) INTO defendants
