...
BEGIN
     OPEN p_results_out FOR 
      ...
      xdc.IS_JUVENILE AS juvenile,
      TO_CHAR(xc.CLASS_CODE) AS classcode, 
      xdc.CURRENT_BC_STATUS AS bcstatus,
      TO_CHAR(NVL(xc.committal_date,xc.sent_for_trial_date),'dd-mm-yyyy') AS commitalsent,
      GET_LINKED_CASES_LIST(p_court_id,xdc.defendant_id,xc.CASE_GROUP_NUMBER) AS linkedcaseslist,
      ...
      WHERE xc.COURT_ID = p_court_id AND GET_CASE_STATUS(xc.CASE_ID) = 'Open' AND  ((p_class_code_list IS NULL AND xc.CLASS_CODE IS NULL) OR  (','||NVL(p_class_code_list,'')||',' LIKE '%,'||CAST(xc.CLASS_CODE AS VARCHAR(1000))||',%' AND p_class_code_list IS NOT NULL)) AND
      ...
      (p_bc_status IS NULL OR xdc.CURRENT_BC_STATUS = NVL(p_bc_status,''))
...
FUNCTION GET_LINKED_CASES_LIST( p_court_id IN XHB_CASE.court_id%TYPE,
                            p_defendant_id IN XHB_DEFENDANT_ON_CASE.DEFENDANT_ID%TYPE,
...
pos int;
    ...
    CURSOR linked_cases_list_c IS
      SELECT DISTINCT	xc.case_id
      FROM 	XHB_CASE xc
      WHERE (xc.case_id IN (
              SELECT xdc.case_id 
              FROM  XHB_DEFENDANT_ON_CASE xdc
              WHERE xdc.defendant_id = p_defendant_id
              AND 	(xdc.results_verified is null or xdc.results_verified != 'E')
              AND 	(xdc.obs_ind is null or xdc.obs_ind = 'N' or xdc.obs_ind = '')
            )
      OR  xc.case_group_number = p_group_number)
      AND xc.court_id = p_court_id
      AND ROWNUM <= 100;
    ...
    linked_cases_list:= linked_cases_list || linked_cases_list_row.case_id || '|';
...
FUNCTION GET_CASE_STATUS(p_case_id IN xhb_case.case_id%TYPE) 
	RETURN VARCHAR2 AS
 ...
 SELECT subquery.* FROM  
  ...
  FOR history_list_row IN history_list_c
   LOOP
    ...
    ELSE
      history_list:=history_list || history_list_row.DATE_OF_REMOVAL || ' ' || LIST_SUFFIX || ' ' || history_list_row.REASON_FOR_REMOVAL || '|';
    END IF;
   END LOOP;
