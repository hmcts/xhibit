  ...
  PROCEDURE PUBLISH_LIST(p_list_id XHB_LIST.LIST_ID%TYPE) IS
   ...
   --Determine the List Type
        ...
        ELSIF(v_list_type = 'Warned') THEN
          ...
          v_unique_id := 'CSD' || TRIM(v_document_type.INTERNAL_CODE) || LPAD(l_validation_id, 15, 0);
          v_return_xml:= XHB_GET_XML_REPORTS.GET_WARNED_LIST(p_list_id, v_unique_id);
          
        END IF;


        INSERT INTO XHB_CLOB(clob_id, clob_data)
        VALUES (XHB_CLOB_SEQ.NEXTVAL, v_return_xml)
        RETURNING clob_id
        INTO      l_clob_id;
        ...
        INSERT INTO XHB_XML_DOCUMENT ( date_created,
                              ...
                              VALUES (TRUNC(SYSDATE),
                                       v_list_type ||' List ' || DECODE(v_list_row.DRAFT_OR_FINAL, 'D', 'DRAFT v', 'F', 'FINAL v') || v_list_row.LIST_NUMBER || ' ' || v_list_row.LIST_START_DATE,
                                       G_NEW_DOCUMENT,
                                       v_document_type.INTERNAL_CODE,
                                       v_list_row.COURT_ID,
                                       l_clob_id
                                     );
      ...
      INSERT INTO XHB_VALIDATION (VALIDATION_ID, CODE, CLOB_DATA, SCHEMA_NAME, STATUS)
                          VALUES (l_validation_id, v_document_type.INTERNAL_CODE, v_return_xml, v_document_type.SCHEMA_NAME, 'N');
      ...
      INSERT INTO XHB_VALIDATION_CJI (VALIDATION_ID, 
                              ...
                              VALUES (l_validation_id,
                                      v_unique_id,
                                      v_document_type.DOCUMENT_TYPE_ID,
                                      1,
                                      v_document_type.STYLESHEET_NAME,
                                      sysdate,
                                      v_list_court.CREST_COURT_ID,
                                      v_document_type.INTERNAL_CODE ||' ' || v_list_row.LIST_START_DATE || ' ' || DECODE(v_list_row.DRAFT_OR_FINAL, 'D', 'DRAFT v', 'F', 'FINAL v') || v_list_row.LIST_NUMBER,
                                      v_list_type ||' List, ' || v_list_court.COURT_NAME || ' on ' || v_list_row.LIST_START_DATE || ' ' || DECODE(v_list_row.DRAFT_OR_FINAL, 'D', 'DRAFT v', 'F', 'FINAL v') || v_list_row.LIST_NUMBER);
