create or replace PACKAGE BODY xhb_ref_advocate_pkg AS
  ...
  PROCEDURE find_by_crest_chamber_id(p_count_out OUT NUMBER, p_crest_chamber_id IN NUMBER) AS 
  BEGIN
          SELECT COUNT(*) into p_count_out
          FROM
            xhb_ref_advocate xra
          WHERE
            xra.crest_chamber_id = p_crest_chamber_id
          AND 
            xra.obs_ind != 'Y';

  END find_by_crest_chamber_id;
...
/****************************************************************************************
*   Procedure to update the details for a chamber. Details are in tables
*   XHB_REF_CHAMBER and XHB_ADDRESS. They need a record (in XHB_REF_CHAMBER)for each court so
*   all of them for a given Chamber will need to be updated.  They share a common address record.
*   If the address fields are populated, the address record will be updated (unless the address_id
*   is null, in which case a new address record will be generated and its id used to populate
*   address_id in all instances of the Chamber record.     [CTX-3142]
*****************************************************************************************/
PROCEDURE update_chamber_details(p_crest_chamber_id  XHB_REF_CHAMBER.CREST_CHAMBER_ID%TYPE
                              ...
                              ,  p_secure_email      XHB_CONTACT_DETAIL.CONTACT_VALUE%TYPE) AS


-- Local Variables
v_step                    VARCHAR2(10);
v_proc_name               VARCHAR2(30) := 'update_chamber_details';
v_err_code                NUMBER;
v_err_msg                 VARCHAR2(1000);
v_address_id              XHB_ADDRESS.ADDRESS_ID%TYPE := NULL;

BEGIN
    v_step := '1';
	-- Although there is an instance of the Chamber record for every court, they all point to a common address record (i.e. the value of address_id is the same for each instance.)
	IF p_address_id IS NOT NULL THEN

	   v_address_id := p_address_id;

	    IF (p_address_1 IS NULL) AND (p_address_2 IS NULL) AND (p_address_3 IS NULL) AND (p_address_4 IS NULL) AND (p_town IS NULL) AND (p_county IS NULL) AND (p_postcode IS NULL) THEN
            NULL;  -- Do nothing
	    ELSE
            UPDATE XHB_ADDRESS SET ADDRESS_1 = p_address_1, ADDRESS_2 = p_address_2, ADDRESS_3 = p_address_3, ADDRESS_4 = p_address_4, TOWN = p_town, COUNTY = p_county, POSTCODE = p_postcode, COUNTRY = p_country, last_updated_by = p_user
            WHERE  ADDRESS_ID = p_address_id;
		END IF;
	ELSE
	    -- IF the address_id is null but the address parameters are populated, this could be a new address being added to an existing record.
		IF (p_address_1 IS NOT NULL) OR (p_address_2 IS NOT NULL) OR (p_address_3 IS NOT NULL) OR (p_address_4 IS NOT NULL) OR (p_town IS NOT NULL) OR (p_county IS NOT NULL) OR (p_postcode IS NOT NULL) THEN

		    -- Get a new ADDRESS_ID
		    SELECT XHB_ADDRESS_SEQ.NextVal INTO v_address_id
		    FROM   DUAL;

		    INSERT INTO XHB_ADDRESS(ADDRESS_ID,   ADDRESS_1,   ADDRESS_2,   ADDRESS_3,   ADDRESS_4,   TOWN,   COUNTY,   POSTCODE,   COUNTRY,   CREATED_BY, LAST_UPDATED_BY)
		                     VALUES(v_address_id, p_address_1, p_address_2, p_address_3, p_address_4, p_town, p_county, p_postcode, p_country, p_user,     p_user);

        END IF;
    END IF;

	-- Now we have to insert/update the 4 Contact Details fields phone_num, fax_num, email and secure_email
	-- We do not know whether any of these already exist.
	IF p_phone_num is not null then
	    update_contact_detail(v_address_id, 'Phone', p_phone_num, p_user);
    END IF;

	IF p_fax_num is not null then
	    update_contact_detail(v_address_id, 'Fax', p_fax_num, p_user);
    END IF;

	IF p_email is not null then
	    update_contact_detail(v_address_id, 'Non Secure Email', p_email, p_user);
    END IF;

	IF p_secure_email is not null then
	    update_contact_detail(v_address_id, 'Secure Email', p_secure_email, p_user);
    END IF;


    UPDATE 	XHB_REF_CHAMBER SET DX_REF = p_dx_ref, LOCATION_CODE = p_location_code, FIRM_NAME = p_firm_name, ADDRESS_ID = v_address_id, CLERK_NAME = p_clerk_name, LAST_UPDATED_BY = p_user   -- This should update multiple rows,
	WHERE   CREST_CHAMBER_ID = p_crest_chamber_id
	AND     nvl(OBS_IND, 'N') != 'Y';
...
PROCEDURE update_contact_detail(p_address_id    XHB_ADDRESS.ADDRESS_ID%TYPE
                              ...
                              , p_user          XHB_CONTACT_DETAIL.LAST_UPDATED_BY%TYPE) AS
-- Local variables
v_count         Number(8);

BEGIN

	SELECT COUNT(cd.contact_id)
	INTO   v_count
	from   XHB_CONTACT_DETAIL cd
	where  cd.address_id = p_address_id
	and    cd.contact_type = p_contact_type ;

	IF v_count > 0 THEN -- row exists
	    -- Update
		UPDATE XHB_CONTACT_DETAIL SET CONTACT_VALUE = p_contact_value, last_updated_by = p_user
		WHERE  CONTACT_TYPE = p_contact_type
		AND    ADDRESS_ID = p_address_id;
	ELSE
	    -- INSERT
		INSERT INTO XHB_CONTACT_DETAIL(CONTACT_ID,                     CONTACT_TYPE,   CONTACT_VALUE,   ADDRESS_ID,   CREATED_BY, LAST_UPDATED_BY)
		                        VALUES(XHB_CONTACT_DETAIL_SEQ.NextVal, p_contact_type, p_contact_value, p_address_id, p_user,     p_user);

	END IF;

END update_contact_detail;
