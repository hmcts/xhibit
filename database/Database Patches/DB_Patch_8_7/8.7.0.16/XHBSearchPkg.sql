--------------------------------------------------------
--  DDL for Package XHB_SEARCH_PKG
--------------------------------------------------------

  CREATE OR REPLACE PACKAGE "XHIBIT"."XHB_SEARCH_PKG" AS
	...
	/*PROCEDURE get_ref_calendar(p_results_out         OUT SYS_REFCURSOR,
                             p_court_id_in         IN  XHB_COURT.court_id%TYPE,
                             p_avail_in            IN  XHB_REF_CALENDAR.avail%TYPE,
                             p_sys_ac_avail_in     IN  XHB_REF_CALENDAR.sys_ac_avail%TYPE,
                             p_description_in      IN  XHB_REF_CALENDAR.description%TYPE);*/
...
--------------------------------------------------------
--  DDL for Package Body XHB_SEARCH_PKG
--------------------------------------------------------

  CREATE OR REPLACE PACKAGE BODY "XHIBIT"."XHB_SEARCH_PKG" AS
...
--    END log_entry;
    ...
    END get_solicitor;
	  ...
	  --
    -- Query the XHB_REF_CALENDAR table, but also with search criteria
    -- from XHB_COURT
    --
    PROCEDURE get_ref_calendar(p_results_out         OUT SYS_REFCURSOR,
                             ...
                             p_description_in      IN  XHB_REF_CALENDAR.description%TYPE)
    AS
        l_avail            CONSTANT XHB_REF_CALENDAR.avail%TYPE        := convert_value(p_avail_in);
        l_sys_ac_avail     CONSTANT XHB_REF_CALENDAR.sys_ac_avail%TYPE := convert_value(p_sys_ac_avail_in);
        l_description  	   CONSTANT XHB_REF_CALENDAR.description%TYPE  := convert_value(p_description_in);
    BEGIN
        --log_entry('get_ref_calendar');
