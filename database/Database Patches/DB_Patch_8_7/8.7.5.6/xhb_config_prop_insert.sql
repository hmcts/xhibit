...
DECLARE
   ...
   BEGIN
      ...
      SELECT XHB_CONFIG_PROP_SEQ.NEXTVAL, 'COURTEL_COURT_MOLD', 'MOLDC' FROM DUAL
      WHERE NOT EXISTS (SELECT 1 FROM XHB_CONFIG_PROP X WHERE X.PROPERTY_NAME = 'COURTEL_COURT_MOLD'); 
      COMMIT;
   END insert_record;
