...
FUNCTION GET_CROWN_COURT_RL(p_court_id XHB_COURT.COURT_ID%TYPE) RETURN XMLTYPE IS
  ...
  SELECT XMLELEMENT("cs:Defendant",
                   XMLELEMENT("cs:PersonalDetails",
                             XMLFOREST(XMLFOREST(CASE WHEN (p_show_public_view = 1 AND xdoc.IS_MASKED = 'Y') OR (p_show_defendant_name = 0) THEN ' ' ELSE xd.FIRST_NAME END AS "apd:CitizenNameForename"
												,CASE WHEN (p_show_public_view = 1 AND xdoc.IS_MASKED = 'Y') OR (p_show_defendant_name = 0) THEN ' ' ELSE xd.MIDDLE_NAME END AS "apd:CitizenNameForename"
