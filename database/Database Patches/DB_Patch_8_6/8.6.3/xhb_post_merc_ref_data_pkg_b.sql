CREATE OR REPLACE PACKAGE BODY xhb_post_merc_ref_data_pkg AS
  ...
  BEGIN
    ...
    INSERT INTO XHB_FORMB_RESULT(FORMB_RESULT_TYPE,REF_VERDICT_ID,RESULT_DESCRIPTION,COURT_ID) VALUES ('V', (SELECT REF_SYSTEM_CODE_ID FROM XHB_REF_SYSTEM_CODE WHERE CODE_TYPE = 'VERDICT' AND CODE = 'GJ' AND COURT_ID = p_court_id), 'Verdict', p_court_id);
    INSERT INTO XHB_FORMB_RESULT(FORMB_RESULT_TYPE,REF_VERDICT_ID,RESULT_DESCRIPTION,COURT_ID) VALUES ('V', (SELECT REF_SYSTEM_CODE_ID FROM XHB_REF_SYSTEM_CODE WHERE CODE_TYPE = 'VERDICT' AND CODE = 'GAOJ' AND COURT_ID = p_court_id), 'Verdict', p_court_id);
    INSERT INTO XHB_FORMB_RESULT(FORMB_RESULT_TYPE,REF_VERDICT_ID,RESULT_DESCRIPTION,COURT_ID) VALUES ('V', (SELECT REF_SYSTEM_CODE_ID FROM XHB_REF_SYSTEM_CODE WHERE CODE_TYPE = 'VERDICT' AND CODE = 'GLOJ' AND COURT_ID = p_court_id), 'Verdict', p_court_id);
  ...
  CURSOR xrd_co_data IS
    SELECT order_type_id,
           code
    FROM   XHB_ORDER_TYPE
    WHERE  code in ('RC', 'IMPO', 'COMY', 'CMPO', 'CMPRO', 'CRO', 'COMSENT', 'SUS', 'YRO');
  ...
  CURSOR linked_rs_data (v_order_code VARCHAR2) IS
    ...
    AND    (v_order_code = 'RC' AND disposal_code IN ('REMMENT','REMTREA')) OR
           ...
           (v_order_code = 'COMSENT' AND disposal_code IN ('CWRKCS', 'PSACTCS', 'PACOBCS','PRACTCS','CURFCS','EXCARCS',
                                                           'RESRQCS','MHTRTCS','DRGRQCS','ALTRTCS','SPVRQCS','ATCRQCS',                              'EXCURCS', 'FTPRCS', 'REHARCS'))
OR
           (v_order_code = 'YRO' AND disposal_code IN ('CWRKYR', 'PSACTYR', 'PACOYR','PRACTYR','CURFYR','EXCARYR',
                                                           'RESRQYR','MHTRTYR','DRGRQYR','LARYR','SPVRQYR','ATCRQYR',
                                                           'EDUCYR', 'FOSTYR', 'DTYR', 'ISTYR'))
 OR
           (v_order_code = 'CRO' AND disposal_code IN ('CRCR','CRDA','CRDC','CRHR','CRNH','CROCUR','CRODAR','CROEXC','CROG','CRRA')) OR
           (v_order_code = 'SUS' AND disposal_code IN ('CUSMSEC','CUSMCUR','CWRKSS','PSACTSS','PACOBSS','PRACTSS',
                                                        'CURFSS','EXCARSS','RESRQSS','MHTRTSS','DRGRQSS','ALTRTSS',
                                                        'SPVRQSS','ATCRRQSS','EXCURSS', 'CUSMSSE', 'FTPRSS', 'REHARSS' ));
