create or replace PACKAGE BODY        XHB_CREATE_DMI_CAD_FILE_PKG /* AUTHID CURRENT_USER */ AS
...
/**
  ...
  * 13/03/2019    R Joshi         4.0         Driving disposals removed to be displayed
  * 25/03/2020    M Harris        5.0         Make sure monetary amounts take precedence over duration
  * 16/04/2020    M Harris        6.0         Tidy up the run logs for coping with multiple courts
  * 09/02/2021    M Turton        7.0         CFRA-1121,CFRA-1130,XLC-43,XLC2-44 (XLC2-47)
  **/
...
PROCEDURE log_progress(p_run_id          IN XHB_DMI_CAD_RUN_HISTORY.DMI_CAD_RUN_HISTORY_ID%TYPE,
                       p_cad_ref_code_id IN XHB_DMI_CAD_RUN_HISTORY.DMI_CAD_REF_CODE_ID%TYPE,
                       p_run_end_time    IN XHB_DMI_CAD_RUN_HISTORY.END_RUN_TIME%TYPE DEFAULT NULL ) IS
BEGIN
    -- Update the existing record with progress
    UPDATE XHB_DMI_CAD_RUN_HISTORY xdcrh
       SET xdcrh.DMI_CAD_REF_CODE_ID = p_cad_ref_code_id,
           xdcrh.END_RUN_TIME = NVL(p_run_end_time, xdcrh.END_RUN_TIME)
     WHERE xdcrh.DMI_CAD_RUN_HISTORY_ID = p_run_id;

    -- If no record was found then create one
    IF SQL%ROWCOUNT = 0 THEN
      INSERT INTO XHB_DMI_CAD_RUN_HISTORY
...
PROCEDURE create_module_log(p_run_id          IN XHB_DMI_CAD_RUN_LOG.DMI_CAD_RUN_HISTORY_ID%TYPE,
                            p_cad_ref_code_id IN XHB_DMI_CAD_RUN_LOG.DMI_CAD_REF_CODE_ID%TYPE,
                            p_court_id        IN XHB_DMI_CAD_RUN_LOG.COURT_ID%TYPE,
                            p_module          IN XHB_DMI_CAD_RUN_LOG.MODULE_NAME%TYPE,
                            p_message         IN XHB_DMI_CAD_RUN_LOG.LOG_MESSAGE%TYPE) IS
BEGIN
     INSERT INTO xhb_dmi_cad_run_log
     (DMI_CAD_RUN_LOG_ID
       ,DMI_CAD_RUN_HISTORY_ID
       ,DMI_CAD_REF_CODE_ID
       ,COURT_ID
       ...
       ,START_TIME)
      VALUES
      (xhb_dmi_cad_run_log_seq.nextval
       ,p_run_id
       ,p_cad_ref_code_id
       ,p_court_id
       ,p_module
 ...
 WHERE dmi_cad_ref_code_short_name = p_cad_ref_code;
  ...
  IF upper(p_cad_ref_code) = 'S' --start the create process
   THEN
    ...
    log_progress(p_run_id          => v_run_hist_id,
                 p_cad_ref_code_id => v_cad_ref_code_id);

    --Create a detail row for the initial header row
    create_module_log(p_run_id          => v_run_hist_id,
                      p_cad_ref_code_id => v_cad_ref_code_id,
 ...
 CURSOR court_c IS
  ...
  /*If there is no court id passed in then the program will need to run for all courts.  If it is populated then it only needs to be run for one court*/
   ...
   log_event (p_cad_ref_code => 'C' --Complete
                     ,p_court_id => p_court_id
                     ,p_module   => 'xhb_create_dmi_cad_file_pkg.create_output'
                     ,p_message   => 'Process Complete.'||CASE WHEN x_clob IS NOT NULL THEN '  CLOB_ID in xhb_clob table: '||v_clob_id ELSE '' END
                    );
...
PROCEDURE build_clob  (p_date_from  IN  DATE DEFAULT NULL --date field not mandatory.  Will default to last calendar month if not populated
 ...
 ,FORCE_CODE xhb_ref_system_code.code%TYPE
 ...
 SELECT  DISTINCT xco.crest_court_id as court_code
        ,NVL(xhb_create_dmi_cad_file_pkg.get_appearance_date(xca.case_id, xdoc.defendant_on_case_id),'00000000') as APPEARANCE_DATE
...
--        ,NVL(to_char(xdoc.final_driving_licence_status),'0') final_driving_licence_status
        ,NVL(xhb_create_dmi_cad_file_pkg.get_disqualification('DISQ_FDLS',xdoc.defendant_on_case_id,xo.offence_id),'0') AS final_driving_licence_status
--MT XLC2-42 End        
        ,regexp_replace(xd.surname,'[^A-Za-z0-9]',' ')  surname -- remove alphanumeric characters from name
        ,regexp_replace(xd.initials,'[^A-Za-z0-9]','')  initials-- remove space and aplhanumeric characters from name
        ,(SELECT code FROM xhb_ref_system_code
           WHERE code_type='HO_POL_FORCE' AND ref_system_code_id = xco.police_force_code) AS force_code
        ,NVL(xd.ethnicity_self_defined,' ') AS SELF_ETHNICITY
       -- ,/*NVL(xd.ethnic_appearance_code,' ')*/ ' ' AS RSC_ETHNIC_CODE
        ,NVL(decode(ethnic_appearance_code,'W','1','B','2','A','3','O','4','N','9',' '),' ') AS RSC_ETHNIC_CODE
 ...
 AND   xca.case_type IN ('T','S') --only for trial and sentence case types? Remand b instruction only refers to these
   ...
   AND EXISTS ( SELECT 'x'
             ...
             -- AND   xhc1.ref_court_id = xcrt.ref_court_id
              ...
              AND  (xo1.obs_ind <> 'Y' OR xo1.obs_ind IS NULL)
               AND  EXISTS ((SELECT 1 FROM xhb_defendant_on_offence
                             WHERE defendant_on_case_id = xdoc1.defendant_on_case_id AND offence_id = xo1.offence_id
                              AND (obs_ind <>'Y' OR obs_ind IS NULL))
                             UNION
                            (SELECT 1 FROM xhb_defendant_charge
                             WHERE defendant_on_case_id = xdoc1.defendant_on_case_id AND charge_id=xc1.charge_id
                               AND (obs_ind <>'Y' OR obs_ind IS NULL))
                 )
              AND EXISTS ( -- This will remove disposal code 0 and Not Required disposal type values removed
                (  select 1 from xhb_disposal2 d, xhb_ref_disposal_type rdt, xhb_dmi_disposal_codes xdc
                where  D.REF_DISPOSAL_type_ID=RDT.REF_DISPOSAL_TYPE_ID and
                       RDT.DISPOSAL_CODE = xdc.crest_disposal_code and
--MT CFRA-1121         RDT.DISPOSAL_CODE NOT IN ('DISQVC','NRADD','DISTEST','DISPCCA','DDPAS','TDISQ','DISREM','DISQAO') 
                       xdc.disposal_type NOT IN('NR','DD')
                       )
                UNION
                ( select 1 from xhb_disposal2 d, xhb_ref_disposal_type rdt, xhb_dmi_disposal_codes xdc
                where  D.REF_DISPOSAL_type_ID=RDT.REF_DISPOSAL_TYPE_ID and
...
--MT CFRA-1121         RDT.DISPOSAL_CODE NOT IN ('DISQVC','NRADD','DISTEST','DISPCCA','DDPAS','TDISQ','DISREM','DISQAO') 
                       xdc.disposal_type NOT IN('NR','DD')
AND
                       d.defendant_on_offence_id IN (SELECT defendant_on_offence_id
                                         FROM xhb_defendant_on_offence
                                         WHERE defendant_on_case_id= xdoc.defendant_on_case_id
                                         AND offence_id= xo.offence_id)
 ...
 CURSOR line_cur (v_case_id IN xhb_case.case_id%TYPE
  ...
  SELECT NVL(xrf.ho_class,NVL(xo.crest_hoo_class_freetext,0)) HO_CLASS
        ...
        ,NVL(xcrt.crest_court_id,'0000') ref_court_id
        ...
        ,NVL(xhb_create_dmi_cad_file_pkg.get_disposal ('TEXT',xdoc.defendant_on_case_id,xo.offence_id,xhc.case_type,xc.charge_type,xc.charge_id,1),' ') as disposal1_text
        ,NVL(xhb_create_dmi_cad_file_pkg.get_disposal ('CODE',xdoc.defendant_on_case_id,xo.offence_id,xhc.case_type,xc.charge_type,xc.charge_id,1),'000') as disposal1_code
        ,NVL(xhb_create_dmi_cad_file_pkg.get_disposal ('AMOUNT',xdoc.defendant_on_case_id,xo.offence_id,xhc.case_type,xc.charge_type,xc.charge_id,1),'0000') as disposal1_amount
        ,NVL(xhb_create_dmi_cad_file_pkg.get_disposal ('UNIT',xdoc.defendant_on_case_id,xo.offence_id,xhc.case_type,xc.charge_type,xc.charge_id,1),'0') as disposal1_unit
        ,NVL(xhb_create_dmi_cad_file_pkg.get_disposal ('TEXT',xdoc.defendant_on_case_id,xo.offence_id,xhc.case_type,xc.charge_type,xc.charge_id,2),' ') as disposal2_text
        ,NVL(xhb_create_dmi_cad_file_pkg.get_disposal ('CODE',xdoc.defendant_on_case_id,xo.offence_id,xhc.case_type,xc.charge_type,xc.charge_id,2),'000') as disposal2_code
        ,NVL(xhb_create_dmi_cad_file_pkg.get_disposal ('AMOUNT',xdoc.defendant_on_case_id,xo.offence_id,xhc.case_type,xc.charge_type,xc.charge_id,2),'0000') as disposal2_amount
        ,NVL(xhb_create_dmi_cad_file_pkg.get_disposal ('UNIT',xdoc.defendant_on_case_id,xo.offence_id,xhc.case_type,xc.charge_type,xc.charge_id,2),'0') as disposal2_unit
        ,NVL(xhb_create_dmi_cad_file_pkg.get_disposal ('TEXT',xdoc.defendant_on_case_id,xo.offence_id,xhc.case_type,xc.charge_type,xc.charge_id,3),' ') as disposal3_text
        ,NVL(xhb_create_dmi_cad_file_pkg.get_disposal ('CODE',xdoc.defendant_on_case_id,xo.offence_id,xhc.case_type,xc.charge_type,xc.charge_id,3),'000') as disposal3_code
        ,NVL(xhb_create_dmi_cad_file_pkg.get_disposal ('AMOUNT',xdoc.defendant_on_case_id,xo.offence_id,xhc.case_type,xc.charge_type,xc.charge_id,3),'0000') as disposal3_amount
        ,NVL(xhb_create_dmi_cad_file_pkg.get_disposal ('UNIT',xdoc.defendant_on_case_id,xo.offence_id,xhc.case_type,xc.charge_type,xc.charge_id,3),'0')as disposal3_unit
        ,NVL(xhb_create_dmi_cad_file_pkg.get_disposal ('TEXT',xdoc.defendant_on_case_id,xo.offence_id,xhc.case_type,xc.charge_type,xc.charge_id,4),' ') as disposal4_text
        ,NVL(xhb_create_dmi_cad_file_pkg.get_disposal ('CODE',xdoc.defendant_on_case_id,xo.offence_id,xhc.case_type,xc.charge_type,xc.charge_id,4),'000') as disposal4_code
        ,NVL(xhb_create_dmi_cad_file_pkg.get_disposal ('AMOUNT',xdoc.defendant_on_case_id,xo.offence_id,xhc.case_type,xc.charge_type,xc.charge_id,4),'0000') as disposal4_amount
        ,NVL(xhb_create_dmi_cad_file_pkg.get_disposal ('UNIT',xdoc.defendant_on_case_id,xo.offence_id,xhc.case_type,xc.charge_type,xc.charge_id,4),'0') as disposal4_unit
        ,xdoc.defendant_on_case_id
        ,xdoc.case_id
        ,case when xhc.case_type='S' then '3'
              when xc.charge_type='B' then '3'
              else
                    NVL(xhb_create_dmi_cad_file_pkg.get_plea(XDOC.DEFENDANT_ON_CASE_ID,xc.charge_id,
                        xo.offence_id),' ')
               END as offence_plea
 --               NVL(xhb_create_dmi_cad_file_pkg.get_plea(XDOC.DEFENDANT_ON_CASE_ID,xc.charge_id,xo.offence_id)),' ') as offence_plea
  ...
  AND EXISTS (
              ( SELECT 1 FROM xhb_disposal2 d, xhb_ref_disposal_type rdt, xhb_dmi_disposal_codes xdc
                WHERE  D.REF_DISPOSAL_type_ID=RDT.REF_DISPOSAL_TYPE_ID AND
                       RDT.DISPOSAL_CODE = xdc.crest_disposal_code AND
                       d.defendant_on_case_id = xdoc.defendant_on_case_id
--MT CFRA-1121 Start
--                     AND RDT.DISPOSAL_CODE NOT IN ('DISQVC','NRADD','DISTEST','DISPCCA','DDPAS','TDISQ','DISREM','DISQAO') 
                       AND  xdc.disposal_type NOT IN('NR','DD')
--MT CFRA-1121 End
                       )
                UNION
                ( SELECT 1 FROM xhb_disposal2 d, xhb_ref_disposal_type rdt, xhb_dmi_disposal_codes xdc
                  WHERE  D.REF_DISPOSAL_type_ID=RDT.REF_DISPOSAL_TYPE_ID AND
                       RDT.DISPOSAL_CODE = xdc.crest_disposal_code 
--MT CFRA-1121 Start
--                     AND RDT.DISPOSAL_CODE NOT IN ('DISQVC','NRADD','DISTEST','DISPCCA','DDPAS','TDISQ','DISREM','DISQAO') 
                       AND  xdc.disposal_type NOT IN('NR','DD')
--MT CFRA-1121 End
         ...
         IF head_tt IS NOT NULL AND head_tt.COUNT > 0 THEN
          ...
          FOR i IN head_tt.FIRST..head_tt.LAST
           LOOP
              ...
              x_clob :=
              x_clob   ||( case when i >1 then CHR(10) end )--carriage return to start the detail line fee
                             ||RPAD('SCKA',4,' ')                                             --JOBNAME (1,4)
                             ...
                             ||LPAD(NVL(head_tt(i).force_code,' '),2,' ')                --FORCE (23,24)
              ...
              --FETCH line_cur BULK COLLECT INTO line_tt LIMIT 1000;
                ...
                -- IF line_tt IS NOT NULL AND line_tt.COUNT > 0 THEN
                 ...
                 --  LOOP
                    ...
                    x_clob := x_clob ||CHR(10) --carriage return to start the detail line feed
                                     ||RPAD('AA',2,' ')                   --Indicates start of line row (1,2)
  ...
  /*Get the initial disposal data based on the defendant on case id/ defendant on offence*/
   CURSOR disp_c
    ...
    SELECT distinct 'CHG' as TYPE
                   ...
                   ,xdc.ho_code_ind,rdl.dbdestin
                   ,rdl.dil_seq_no --MT Extended disposal change
                   ,last_value(ho_code) over (partition by d.disposal2_id order by ho_code_ind rows between unbounded preceding and unbounded following)
                    new_ho_code
                   ,count(*)
...
--MT CFRA-1121    AND RDT.DISPOSAL_CODE NOT IN ('DISQVC','NRADD','DISTEST','DISPCCA','DDPAS','TDISQ','DISREM','DISQAO') 
        AND rdt.menu_group IN('RS','XX')
        AND rdl.REF_DISPOSAL_LINE_ID = dl.ref_disposal_line_id
      --  AND rdl.prompt IS NOT NULL -- D17 for elecronic monitoring
        AND rdt.disposal_code = xdc.crest_disposal_code
        AND xdc.disposal_type NOT IN('NR','DD')
        AND( (NVL(xdc.ho_code_ind,'N') = DECODE(NVL(rdl.dbdestin,'N'),'D9', nvl(dl.data,'N'),
                                                                      'D10',nvl(dl.data,'N'),
                                                                      'D16',nvl(dl.data,'N'),
                                                                      'D17',nvl(dl.data,'N'),'N'))
             OR ((xdc.ho_code_ind) IN ('CHG')))
        AND (d.obs_ind <> 'Y' OR d.obs_ind IS NULL)
        AND (dl.obs_ind <> 'Y' OR dl.obs_ind IS NULL)
        GROUP BY rdt.disposal_code
               ,d.disposal2_id
               ,dl.data
               ,upper(rdl.prompt)
               ,d.defendant_on_offence_id
...
--MT CFRA-1121    AND RDT.DISPOSAL_CODE NOT IN ('DISQVC','NRADD','DISTEST','DISPCCA','DDPAS','TDISQ','DISREM','DISQAO') 
       ...
       SELECT type,disposal_code
           ...
           ,ho_code_ind
           ,dbdestin
           ,dil_seq_no --MT Extended disposal change
          -- ,DENSE_RANK() OVER (ORDER BY disposal2_id ASC) AS DISPOSAL_LINE
           ,DENSE_RANK () OVER (ORDER by to_number(new_ho_code),disposal2_id asc)  AS DISPOSAL_LINE
...
;
 ...
 -- new variables
 v_plea xhb_ref_system_code.code%type;
 v_data xhb_disposal_line.data%type := NULL;
 v_amount xhb_disposal_line.data%type := NULL;
 v_duration xhb_disposal_line.data%type := NULL;
  ...
  FOR disp_r IN disp_c
   LOOP
    ...
    ELSIF upper(p_field) = 'UNIT' and disp_r.disposal_type='LI' THEN
        x_return := 5;
        --MT Extended disposal change - start
    ELSIF upper(p_field) IN ('AMOUNT','UNIT') and disp_r.disposal_code in ('EXAO18','EXDO18') THEN
        IF disp_r.prompt LIKE '%DURATION%' AND disp_r.dil_seq_no = 140 THEN
           v_duration := disp_r.data;
        ELSIF disp_r.prompt LIKE '%UNIT%' AND disp_r.dil_seq_no = 150 THEN
            v_unit := disp_r.data;
        END IF;
    ELSIF upper(p_field) IN ('AMOUNT','UNIT') and disp_r.disposal_code in ('EXAU18','EXDU18') THEN
        IF disp_r.prompt LIKE '%DURATION%' AND disp_r.dil_seq_no = 100 THEN
           v_duration := disp_r.data;
        ELSIF disp_r.prompt LIKE '%UNIT%' AND disp_r.dil_seq_no = 110 THEN
            v_unit := disp_r.data;
        END IF;     
        --MT Extended disposal change - end
    ELSIF upper(p_field) IN ('AMOUNT','UNIT') THEN
        IF disp_r.prompt LIKE '%DURATION%' THEN
           v_duration := disp_r.data;
        ELSIF disp_r.prompt LIKE '%AMOUNT%' THEN
            v_amount := disp_r.data;
    ...
    ELSIF upper(p_field) = 'CODE'
     THEN
       ...
       IF (UPPER(disp_r.dbdestin) IN('D9','D10','D16','D17') and disp_r.data='Y')THEN
            x_return := disp_r.ho_code;
            EXIT;
       END IF;

       IF disp_r.disposal_code = 'FREE' THEN
          IF disp_r.type = 'COU' THEN
            BEGIN
           -- check the plea values and then return the disposal code acoordingly
              WITH PLEA as
                (SELECT --xp.plea_id,xp.breach_admitted,xp.ref_plea_id,
                             CASE WHEN xp.ref_plea_id IS NOT NULL THEN
                                ( SELECT xrsc.code
                                  FROM xhb_ref_system_code xrsc
                                  WHERE xrsc.ref_system_code_id=xp.ref_plea_id
                                  AND xrsc.code_type IN('PLEA','S41_PLEA'))
                            END OFFENCE_PLEA
                         FROM   xhb_plea xp,
                               xhb_defendant_on_offence xdoo
                        WHERE xdoo.defendant_on_case_id = p_doc_id
                    ...
                    SELECT offence_plea into v_plea
                    FROM plea;
               EXCEPTION WHEN OTHERS THEN v_plea := NULL;
               END;
              IF NVL(v_plea,'NG') in ('NG','AA','AC','P','CPNG','GAO','GLO','NPT') THEN
                 x_return:= 990;
              ELSE x_return:= 690;  END IF;

            ELSIF disp_r.type='CHG' THEN
               IF p_case_type = 'S' THEN x_return := 690;
               ELSE
                    IF  p_charge_type IN ('B','O') THEN
                    BEGIN
                     SELECT --xp.plea_id,xp.breach_admitted,xp.ref_plea_id,
                     ...
                     WHERE   xdc.defendant_on_case_id= p_doc_id--1381610--1889239
                      ...
                      EXCEPTION WHEN OTHERS then v_plea:=NULL;
                      END;
                      IF NVL(v_plea,2) = 2 THEN x_return := 990;
                      ELSE x_return := 690;
                      END IF;
                    ELSE x_return:= 990;
                    END IF;
   ...
   END LOOP; --disp_r
   -- v5.0 - Make sure monetary value takes precedence over duration
   v_data := NVL(v_amount,v_duration);
   -- If unit in hr, days, weeks, months and year then return amount as it is
   IF v_amount IS NULL AND UPPER(v_unit) IN ('HOUR','HOURS','DAY','DAYS','WEEK','WEEKS','MONTH','MONTHS','YEARS','YEARS') THEN
...
v_charge_type  xhb_charge.charge_type%TYPE;
BEGIN
    BEGIN
    ...
    IF n_ref_sys_code IS NULL THEN
        ...
        SELECT ref_system_code_id,charge_type INTO n_ref_sys_code, v_charge_type FROM xhb_charge
    ...
    ELSIF v_charge_type ='O' THEN
    -- O charge type is related S41summary offences which has ref_system_code NULL
        SELECT code INTO n_proc_type_no FROM xhb_ref_system_code
        WHERE code_type='HO_PROC_S41' and rownum=1;
    ELSE
       n_proc_type_no := NULL;
    END IF;
...
UNION
    SELECT --xp.plea_id,xp.breach_admitted,xp.ref_plea_id,
       ...
       THEN
            (SELECT CASE
                       WHEN xrsc.code IN (
                             'AC'--'Autrefois Convict'
                            ,'AA'--'Autrefois Acquit'
                            ,'CPGJ'--'Change of Plea: Not guilty to guilty (after jury sworn)'
                            ,'CPG'--'Change of Plea: Not guilty to guilty (no jury sworn)'
                            ,'G'--'Guilty'
                            ,'GAO'--'Guilty to alternative offence not charged namely'
                            ,'GLO'--'Guilty to lesser offence namely'
                            ) THEN '1'
                       WHEN xrsc.code IN (
                             'CPNG'--'Change of Plea: Guilty to not guilty'
                            ,'NG'--'NOT GUILTY'
                            ,'P'--'Pardon'
                            ) THEN '2'
                       WHEN xrsc.code IN (
                             'NPT'--'No plea taken'
                            ,'O'--'OTHER (ENTER AS FREE TEXT)'
                            ) THEN '3'
                       END PLEA
...
--                                  'DISQVC','DOSPAS','DISREM','LEND','LENDPP')
         AND RDT.DISPOSAL_CODE IN ('DISQ','DISTEST','DISTOA','DISPCCA','DDSPAS','DISREM','LEND','LENDPP',
                                   'DISOBLG','DISDISC','DISTOT','DISINT' )
--MT 1121 End
        AND rdl.REF_DISPOSAL_LINE_ID = dl.ref_disposal_line_id
        AND (d.obs_ind <> 'Y' OR d.obs_ind IS NULL)
        AND (dl.obs_ind <> 'Y' OR dl.obs_ind IS NULL)
        GROUP BY dl.line_number,dl.disposal_line_id
               ...
               ,upper(rdl.prompt)
               ,d.defendant_on_offence_id
               ,d.defendant_on_case_id, rdt.disposal_code,rdl.dil_seq_no
        UNION
        SELECT distinct dl.line_number,dl.disposal_line_id
                   ,d.disposal2_id
                   ,dl.data
...
--MT 1121 End
        ...
        GROUP BY dl.line_number,dl.disposal_line_id
               ...
               ,dl.line_number
               ,d.defendant_on_offence_id
               ,d.defendant_on_case_id,rdt.disposal_code,dil_seq_no )
       SELECT line_number,disposal_line_id
           ...
           ,defendant_on_offence_id
           ,defendant_on_case_id
           ,disposal_code, dil_seq_no --MT XLC2-47
...
--        x_return := CASE WHEN UPPER(disp_r.disposal_code) IN ('DISQ','DISTOA','DISPCCA','DISQAO','DISQVC','DDSPAS','DISREM') THEN 1
        x_return := CASE WHEN UPPER(disp_r.disposal_code) IN ('DISQ','DISTOA','DISPCCA','DDSPAS','DISREM',
                                                              'DISOBLG','DISDISC','DISTOT','DISINT') THEN 1
--MT XLC2-43 End
...
*/
--MT XLC2-43 Start
       IF UPPER(disp_r.disposal_code) IN ('DISQ','DISTOA','DISPCCA','DDSPAS','DISREM','DISINT') THEN x_return := 1;
       ELSIF (UPPER(disp_r.disposal_code) IN ('DISOBLG','DISDISC','DISTOT') AND nvl(disp_r.dil_seq_no,0) = 220 AND nvl(disp_r.data,'N') ='Y' ) THEN x_return := 2;
       ELSIF (UPPER(disp_r.disposal_code) IN ('DISOBLG','DISDISC','DISTOT') AND x_return is null ) THEN x_return := 1;
       ELSIF UPPER(disp_r.disposal_code) IN ('DISTEST') THEN x_return := 2;
       ELSIF UPPER(disp_r.disposal_code) IN ('LEND','LENDPP') THEN x_return := 3;
       END IF;
--MT XLC2-43 End   
...
--MT XLC2-47 Start
      ...
      ELSIF disp_r.disposal_code in ('DISQ','DISOBLG','DISDISC','DISTOT') AND disp_r.prompt like '%UNIT%' AND  disp_r.data='DELETED' THEN
        x_return := 12;
      ELSIF disp_r.disposal_code not in ('DISOBLG','DISDISC','DISTOT') AND disp_r.prompt LIKE '%DURATION%'  THEN
           v_duration := disp_r.data;
      ELSIF  disp_r.disposal_code not in ('DISOBLG','DISDISC','DISTOT') AND disp_r.prompt LIKE '%UNIT%'  THEN  --AND disp_r.disposal_code in ('DISOBLG','DISDISC','DISTOT'
            v_unit := disp_r.data;
      ELSIF disp_r.disposal_code in ('DISOBLG','DISDISC','DISTOT') AND disp_r.prompt LIKE '%DURATION%' AND disp_r.dil_seq_no = 100 THEN
           v_duration := disp_r.data;
      ELSIF  disp_r.disposal_code in ('DISOBLG','DISDISC','DISTOT') AND disp_r.prompt LIKE '%UNIT%' AND disp_r.dil_seq_no = 120 THEN  --AND disp_r.disposal_code in ('DISOBLG','DISDISC','DISTOT'
            v_unit := disp_r.data;
...
--      ELSIF disp_r.prompt LIKE '%UNIT%' THEN
 ...
 --MT XLC2-47 End
      END IF;  
--MT XLC2-42 Start
  ELSIF upper(p_field) = 'DISQ_FDLS' THEN 
       IF ltrim(disp_r.prompt) IN ('DURATION:') AND disp_r.disposal_code in ('DISTOT') THEN
          IF disp_r.prompt LIKE '%DURATION%' AND disp_r.dil_seq_no = 100 THEN
             v_duration := disp_r.data;
             x_return := to_number(v_duration);
             IF x_return = 0 THEN 
               x_return := 1;
             ELSE x_return := 2;
             END IF;
          END IF;
       ELSIF ltrim(disp_r.prompt) IN ('DURATION:') and disp_r.disposal_code in ('DISTOA') THEN
          IF disp_r.prompt LIKE '%DURATION%' AND disp_r.dil_seq_no = 120 THEN
             v_duration := disp_r.data;
             x_return := to_number(v_duration);
             IF x_return = 0 THEN 
               x_return := 1;
...
--    ELSIF UPPER(v_disp_code) IN ('DISQ','DISTOA','DISPCCA','DISQAO','DISQVC','DDSPAS','DISREM') THEN
    ELSIF UPPER(v_disp_code) IN ('DISQ','DISTOA','DISPCCA','DDSPAS','DISREM',
                                 'DISOBLG','DISDISC','DISTOT','DISINT') THEN
--MT XLC2-44 End
