...
FROM XHB_COURT XC, 
      (SELECT xrdm.PARENT,
              'IMPMESW' DISPOSAL_CODE,
              'Imprisonment for Life (Adult) for manslaughter of emergency service worker' TITLE,
              NVL(MAX(xrdm.SEQ_NO),0)+1 NEXT_SEQ_NO 
         FROM XHB_REF_DISPOSAL_MENU xrdm 
        WHERE xrdm.PARENT = 121 -- 'IMPR'
...
xrdm.TITLE
FROM XHB_REF_DISPOSAL_MENU xrdm
WHERE xrdm.DISPOSAL_CODE = 'IMPMESW'
...
FROM XHB_REF_DISPOSAL_MENU xrdm, 
     (SELECT 10 DIL_SEQ_NO,
      ...
      SELECT 20 DIL_SEQ_NO,
             NULL PROMPT,
             'N' INPUT_FLAG,
             'Imprisonment for Life (Adult) with minimum period of:',
             NULL CONC_FLAG,
             'N' MANDATORY,
             'N' MULTIPLE_CHOICE,
             NULL DBDESTIN,
      ...
      SELECT 21 DIL_SEQ_NO,
             NULL PROMPT,
             'N' INPUT_FLAG,
             '(Enter amount and units of imprisonment):',
             NULL CONC_FLAG,
             'N' MANDATORY,
             'N' MULTIPLE_CHOICE,
             NULL DBDESTIN,
