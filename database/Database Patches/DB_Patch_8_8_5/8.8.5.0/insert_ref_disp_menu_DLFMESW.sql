...
FROM XHB_COURT XC, 
      (SELECT xrdm.PARENT,
              'DLFMESW' DISPOSAL_CODE,
              'Detention for Life (Youth) for manslaughter of emergency service worker' TITLE,
...
xrdm.TITLE
FROM XHB_REF_DISPOSAL_MENU xrdm
WHERE xrdm.DISPOSAL_CODE = 'DLFMESW'
...
FROM XHB_REF_DISPOSAL_MENU xrdm, 
     (SELECT 10 DIL_SEQ_NO,
      ...
      SELECT 20 DIL_SEQ_NO,
             NULL PROMPT,
             'N' INPUT_FLAG,
             'Detention for Life (Youth) with minimum period of:',
             NULL CONC_FLAG,
             'N' MANDATORY,
             'N' MULTIPLE_CHOICE,
             NULL DBDESTIN,
      ...
      SELECT 21 DIL_SEQ_NO,
             NULL PROMPT,
             'N' INPUT_FLAG,
             '(Enter amount and units of detention):',
             NULL CONC_FLAG,
             'N' MANDATORY,
             'N' MULTIPLE_CHOICE,
             NULL DBDESTIN,
