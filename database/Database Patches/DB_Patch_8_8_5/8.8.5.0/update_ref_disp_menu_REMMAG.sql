...
SELECT
  ...
  xrdl.FORMAT,
  CASE WHEN 
    xrdl.DIL_SEQ_NO = 40 THEN 'Remitted to Magistrates'' court to be dealt with.'
...
FROM XHB_REF_DISPOSAL_MENU xrdm, 
     (SELECT 44 DIL_SEQ_NO,
        ...
        FROM DUAL
        UNION
        SELECT 47 DIL_SEQ_NO,
             NULL MCGROUP1,
             ' ' PROMPT,
             'N' INPUT_FLAG,
             'Summary Offence.' DATA,
             'Y' CONC_FLAG,
             'N' MANDATORY,
             'Y' MULTIPLE_CHOICE,
             NULL DBDESTIN,
        ...
        FROM DUAL
        UNION
        SELECT 57 DIL_SEQ_NO,
             NULL MCGROUP1,
             ' ' PROMPT,
             'N' INPUT_FLAG,
             'Remitted for Sentence.' DATA,
             'Y' CONC_FLAG,
             'N' MANDATORY,
             'Y' MULTIPLE_CHOICE,
             NULL DBDESTIN,
        ...
        (SELECT 'REMMAG' DISPOSAL_CODE, 
             4 TEMPLATE_VERSION 
        FROM DUAL) subqry2
WHERE xrdm.DISPOSAL_CODE = subqry2.DISPOSAL_CODE
  AND NOT EXISTS (SELECT NULL FROM XHB_REF_DISPOSAL_LINE xrdl2 
                   WHERE xrdl2.DISPOSAL_CODE = xrdm.DISPOSAL_CODE 
                     AND xrdl2.COURT_ID = xrdm.COURT_ID
                     AND xrdl2.TEMPLATE_VERSION = subqry2.TEMPLATE_VERSION
