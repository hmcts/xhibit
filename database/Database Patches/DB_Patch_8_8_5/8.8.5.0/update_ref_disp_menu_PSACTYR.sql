...
SELECT
  ...
  'N' OBS_IND,
  CASE WHEN 
    xrdl.DIL_SEQ_NO = 120 THEN 'V1'
	ELSE xrdl.VALIDATION END VALIDATION,
  xrdl.DBSOURCE,
  xrdl.CONC_FLAG,
  xrdl.FORMAT,
  CASE WHEN 
...
FROM XHB_REF_DISPOSAL_MENU xrdm, 
     (SELECT 123 DIL_SEQ_NO,
             NULL MCGROUP1,
             NULL PROMPT,
             'N' INPUT_FLAG,
             'Intensive Supervision and Surveillance (ISS)? (Y/N)' DATA,
             NULL CONC_FLAG,
             'N' MANDATORY,
             'N' MULTIPLE_CHOICE,
             NULL DBDESTIN,
        ...
        FROM DUAL 
	   UNION
      SELECT 124 DIL_SEQ_NO,
             NULL MCGROUP1,
             NULL PROMPT,
             'N' INPUT_FLAG,
             '(This is required only if Extended activity, Supervision requirement and Curfew with Electronic Monitoring is given)' DATA,
             NULL CONC_FLAG,
             'N' MANDATORY,
             'N' MULTIPLE_CHOICE,
             NULL DBDESTIN,
