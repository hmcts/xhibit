--
-- Filename:    DB_Patch_7_4_1.sql
--
-- System:      Pre-Prod,Production
--
...
-- ----         ---     ---------       -------
-- 14/03/05	SS			Database changes for Mercator load balancing
--
--
...
--
-- DESCRIPTION:   Installs database objects used for load balancing database
--                trigger events across multiple Event Servers.
--                The following database objects are created and/or altered:
--
...
--         
--                2. The m4ora8.sql script was already run.
--
--                3.
...
--                installation time.
--
--                The other privileges are necessary so that all Event Server
--                DB triggering users will operate within a secure and 
--                pre-defined Oracle environment.
--
-- INSTALL STEPS:
...
BEGIN
   ...
   dbms_output.put_line('Event Server DB triggering objects will be located in the following schema: ' || szSchema );
   szSpfx := szSchema || '.';
   ...
   EXCEPTION
      WHEN OTHERS THEN
         ...
         dbms_output.put_line('for Event Server database triggering to work.  ');
   ...
   IF nWarningCnt > 0 THEN
      ...
      END IF;
      szMessage      := szMessage || 'to be resolved before running Event Server DB triggering.  ';
...
VALUES ( 'XHIBIT', '7.4.1', sysdate , 'RELEASE', 'Database', 3); 
