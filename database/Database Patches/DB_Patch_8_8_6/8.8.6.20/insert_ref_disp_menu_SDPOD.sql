...
FROM XHB_COURT XC, 
      (SELECT xrdm.PARENT,
              'SDPOD' DISPOSAL_CODE,
              'Serious Disruption Prevention Order discharged (s28 POA 2023)' TITLE,
...
xrdm.TITLE
FROM XHB_REF_DISPOSAL_MENU xrdm
WHERE xrdm.DISPOSAL_CODE = 'SDPOD'
...
FROM XHB_REF_DISPOSAL_MENU xrdm, 
     (SELECT 10 DIL_SEQ_NO,
	  ...
	  SELECT 22 DIL_SEQ_NO,
             ...
             NULL PROMPT,
             'N' INPUT_FLAG,
             'discharged (under Section 28 of Public Order Act 2023).' DATA,
             NULL CONC_FLAG,
             'N' MANDATORY,
             'N' MULTIPLE_CHOICE,
             NULL DBDESTIN,
