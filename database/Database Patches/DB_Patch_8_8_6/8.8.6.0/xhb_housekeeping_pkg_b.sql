...
*/
  ...
  BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE(SPEECHMARKS || 'Date' || SPEECHMARKS || DELIMITER || SPEECHMARKS || 'Case Number' || SPEECHMARKS || DELIMITER || SPEECHMARKS || 'Case Retention Period Applied' || SPEECHMARKS || DELIMITER || SPEECHMARKS || 'Case Total Sentence' || SPEECHMARKS);
    ...
    LOOP
        ...
        DBMS_OUTPUT.PUT_LINE(SPEECHMARKS || TO_CHAR(v_crp_last_update_date, DATE_FORMAT) || SPEECHMARKS || DELIMITER || SPEECHMARKS || v_case_type || v_case_number || SPEECHMARKS || DELIMITER || SPEECHMARKS || v_policy_no || '-' || v_policy_description || SPEECHMARKS || DELIMITER || SPEECHMARKS || v_duration_years || 'Y ' || v_duration_months || 'M ' || v_duration_days || 'D' || SPEECHMARKS);
    END LOOP;
