create or replace PACKAGE "XHB_REPORT_PKG" AS
  ...
  * 14/03/2019    John Riley         1.6       CTX-3827 Amendments to RREC report following spec revision.
  *                                            Addition of fn is_dealtwith_RREC_case.
  *
  **************************************************************************************/
...
PROCEDURE MARK_FIXTURES_AS_REPORTED(p_fixtures IN CLOB);
...
FUNCTION GET_OBW_SOL_NAME(p_doc_id IN xhb_defendant_on_case.defendant_on_case_id%TYPE) RETURN XHB_REF_SOLICITOR_FIRM.SOLICITOR_FIRM_NAME%TYPE;
...
Function getAddressSingleLine(p_address_id  xhb_address.address_id%type) RETURN VARCHAR2;

FUNCTION getProsAgencySolAddress(p_case_pros_agency_id  xhb_case_prosecutor_agency.case_pros_agency_id%type) RETURN VARCHAR2;
FUNCTION getProsAgencySolName(p_case_pros_agency_id  xhb_case_prosecutor_agency.case_pros_agency_id%type) RETURN VARCHAR2;

Function getAppellantSolAddress(p_deft_on_case_id  xhb_defendant_on_case.defendant_on_case_id%type)
RETURN VARCHAR2;
...
PROCEDURE get_darts_report(p_results_out OUT SYS_REFCURSOR
								...
								,p_end_date IN XHB_CASE.CRP_LAST_UPDATE_DATE%TYPE);
...
FUNCTION unlc_check_case_on_list (p_case_id IN xhb_case.case_id%TYPE) RETURN VARCHAR2;
						  
FUNCTION get_exported_defendants (p_case_id IN xhb_case.case_id%TYPE
								 ,p_case_type IN xhb_case.case_type%TYPE
								 ,p_case_sub_type IN xhb_case.case_sub_type%TYPE) RETURN VARCHAR2;
...
PROCEDURE update_case_diary_fix_run_date(casediaryfixturelist IN CLOB);
...
PROCEDURE publish_running_list(p_cases_to_publish IN CLOB, p_court_id IN XHB_COURT.COURT_ID%TYPE);
...
PROCEDURE MARK_CASES_AS_NHA_REPORTED(p_cases IN CLOB);
...
FUNCTION get_rrca_composite_casenum(p_case_id XHB_CASE.case_id%TYPE, p_end_date DATE) Return VARCHAR2;
	  
FUNCTION get_rrca_waiting_from_date (p_case_id XHB_CASE.case_id%TYPE, p_end_date DATE) RETURN DATE;
	  
FUNCTION get_rrca_age_band_desc (p_age_band VARCHAR2) Return VARCHAR2;
	  
FUNCTION get_rrca_age_band(p_case_id XHB_CASE.case_id%TYPE, p_end_date DATE) Return VARCHAR2;
      
FUNCTION get_rrca_age_band_text(p_case_id XHB_CASE.case_id%TYPE, p_end_date DATE) Return VARCHAR2;
...
FUNCTION get_RREC_CaseType_Hdr(p_case_id  xhb_case.case_id%type) 
RETURN VARCHAR2 ;

FUNCTION is_valid_RREC_Case(p_case_id  xhb_case.case_id%type) 
RETURN VARCHAR2;
                           
FUNCTION is_dealtwith_RREC_Case(p_case_id  xhb_case.case_id%type
                              , p_start_date DATE
							  , p_end_date   DATE
							  , p_court_id   XHB_COURT.COURT_ID%TYPE) 
RETURN VARCHAR2;
...
FUNCTION get_RREC_CaseType_Subhdr(p_case_id  xhb_case.case_id%type) 
RETURN VARCHAR2;
...
FUNCTION   get_prosecutor (p_case_id xhb_case.case_id%type) 
RETURN VARCHAR2;
  
FUNCTION   get_prosecutor_name (p_case_id xhb_case.case_id%type) 
RETURN VARCHAR2; 

FUNCTION is_valid_case_status (p_case_status XHB_CASE.CASE_STATUS%TYPE) RETURN VARCHAR2;
