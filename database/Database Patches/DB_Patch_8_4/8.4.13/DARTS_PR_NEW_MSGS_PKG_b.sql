CREATE OR REPLACE PACKAGE BODY darts_pr_new_msgs_pkg
AS
...
** ===================================================
*/
   FUNCTION get_pr_max_id( p_min_id NUMBER )
      RETURN NUMBER
   IS
      v_max_id   NUMBER;
   BEGIN
   ...
   EXCEPTION
      WHEN OTHERS
      THEN
         DBMS_OUTPUT.put_line ('failure in get_max_id from DAR_PRIORITY_NEW_MESSAGES');
...
*/
   ...
   BEGIN
     ...
     FROM DAR_PRIORITY_NEW_MESSAGES;
               ...
               WHERE MESSAGE_ID IN
                 ( SELECT MESSAGE_ID FROM
                    ( SELECT MESSAGE_ID FROM          
                       ...
                       AND MESSAGE_ID < v_min_message_id + 200
                       ORDER BY MESSAGE_ID
                    )
         ...
         EXCEPTION
            ...
            THEN
               ...
               */
               ...
               v_tries := v_tries + 1;
               v_min_message_id := get_pr_max_id( v_min_message_id);

            WHEN OTHERS
            THEN
               DBMS_OUTPUT.put_line ('get_pr_darts_message_array failure');
               DBMS_OUTPUT.put_line (SQLERRM (SQLCODE));
               RAISE;
         END;
      END LOOP;
      ...
      */
      ...
      LOOP
         BEGIN
            DELETE FROM DAR_PRIORITY_NEW_MESSAGES
   ...
   END get_pr_darts_message_array;
 ...
 **                   time.
 ******************************************************************/
   FUNCTION insert_pr_darts_message_retry( message_id_in        IN DAR_NEW_MESSAGES.MESSAGE_ID%TYPE,
 ...
 BEGIN
    ...
    INSERT INTO DAR_PRIORITY_NEW_MESSAGES(MESSAGE_ID,
                                 XHIBIT_MESSAGE_CODE,
                                 EXISS_MESSAGE_CODE,
                                 PAYLOAD,
                                 RETRY_COUNT,
    ...
    EXCEPTION
      WHEN OTHERS THEN
         DBMS_OUTPUT.put_line ('dar_priority_new_messages INSERT failed.');
         DBMS_OUTPUT.put_line (SQLERRM (SQLCODE));
         RAISE;

   END insert_pr_darts_message_retry;
 ...
 ******************************************************************/
   FUNCTION get_pr_darts_message_refcur
   ...
   BEGIN
      ...
      get_pr_darts_message_array ( v_msg_array, v_count);
   ...
   EXCEPTION
      WHEN OTHERS THEN
         ...
         DBMS_OUTPUT.put_line (SQLERRM (SQLCODE));
         RAISE;
   END get_pr_darts_message_refcur;
