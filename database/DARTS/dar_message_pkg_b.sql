 ...
 */
  PROCEDURE get_range_from_string(p_string          IN  VARCHAR2, 
                                  po_min            OUT VARCHAR2,
                                  po_max            OUT VARCHAR2,
                                  p_range_delimiter IN  VARCHAR2 DEFAULT '-') IS
     v_range_pos NUMBER;
  BEGIN
     ...
     IF po_min IS NOT NULL AND po_max IS NOT NULL AND po_max < po_min THEN
  ...
  BEGIN
     ...
     IF v_dontrun_period IS NOT NULL THEN 
        ...
        ELSIF v_max IS NULL OR LENGTH(v_max)>MAX_LENGTH THEN
           ...
           RAISE INVALID_RANGE;
        ELSIF is_numeric_YN(v_min) = 'N' THEN 
           DBMS_OUTPUT.PUT_LINE('INVALID NUMBER: Min='||v_min);
           RAISE INVALID_RANGE;
        ELSIF is_numeric_YN(v_max) = 'N' THEN 
           DBMS_OUTPUT.PUT_LINE('INVALID NUMBER: Max='||v_max);
           RAISE INVALID_RANGE;   
        END IF;
 ...
 */
  ...
  EXCEPTION 
       WHEN OTHERS THEN 
            dbms_output.put_line('ERROR: message_id='||p_rec.message_id||' sqlerrm='||SQLERRM);
            RAISE;
