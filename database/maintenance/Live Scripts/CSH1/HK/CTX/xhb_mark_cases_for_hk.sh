...
declare -x ARCHIVE_LOGFILE=$SCRIPT_HOME/logs/xhb_mark_case_for_hk`date +"_%d_%m_%Y_%H_%M_%S"`.log
...
echo $AUDIT_MESSAGE >> $ARCHIVE_LOGFILE
echo ""
...
set echo on
set feedback on
exec  xhb_housekeeping_pkg.find_all_cases_eligible_for_hk($P_COURT_ID,$P_UPPER_LIMIT);
exit
endsql
