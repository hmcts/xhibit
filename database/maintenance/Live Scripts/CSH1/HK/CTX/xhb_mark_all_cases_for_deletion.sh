...
# Purpose                                                                           #
# ------------                                                                      #
# Identifies cases that are eligible for deletion,				    #
# which is normally indicated by a case that has been deleted manually or	    #
# been identified for housekeeping 365 days ago.				    #
...
declare -x ARCHIVE_LOGFILE=$SCRIPT_HOME/logs/xhb_mark_all_cases_for_deletion`date +"_%d_%m_%Y_%H_%M_%S"`.log
...
AUDIT_MESSAGE="`date +"%d/%m/%Y-%H:%M:%S:> "` EVENT_STATUS=Running PROCEDURE | DETAILS= find all HK Cases eligible for deletion in XHIBIT Database with parameters $P_COURT_ID, $P_UPPER_LIMIT";
echo $AUDIT_MESSAGE
echo ""
...
set echo on
set feedback on
exec  xhb_housekeeping_pkg.find_all_hk_cases_for_deletion($P_COURT_ID,$P_UPPER_LIMIT);
exit
endsql
...
if [ $err_code != 0 ]; then
     ...
     AUDIT_MESSAGE="`date +"%d/%m/%Y-%H:%M:%S:> "` EVENT_STATUS=Failure | DETAILS= find all HK Cases eligible for deletion in XHIBIT Database - SQL error while calling stored procedure";
     echo $AUDIT_MESSAGE
     exit 1
fi
