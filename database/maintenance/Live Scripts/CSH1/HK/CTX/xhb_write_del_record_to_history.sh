...
# Module ID: xhb_mark_all_cases_for_deletion                                        #
# File Name: "xhb_mark_all_cases_for_deletion.sh                                    #
# Author: Scott Atwell							  	    #
# Version: 0.1                                                                      #
# Date: 25-Nov-19                                                                   #
#                                                                                   #
# Purpose                                                                           #
# ------------                                                                      #
# Ensures that all cases marked in MTBL_CASE_HISTORY have history		    #
# information written about then in the appropriate XHB_*_HISTORY tables	    #
...
declare -x ARCHIVE_LOGFILE=$SCRIPT_HOME/logs/xhb_write_del_record_to_history`date +"_%d_%m_%Y_%H_%M_%S"`.log
...
AUDIT_MESSAGE="`date +"%d/%m/%Y-%H:%M:%S:> "` EVENT_STATUS=Start | DETAILS= write history record for cases to be deleted in XHIBIT Database";
echo $AUDIT_MESSAGE
...
AUDIT_MESSAGE="`date +"%d/%m/%Y-%H:%M:%S:> "` EVENT_STATUS=Running PROCEDURE | DETAILS= write history record for cases to be deleted in XHIBIT Database with parameters $P_COURT_ID, $P_UPPER_LIMIT";
echo $AUDIT_MESSAGE
echo ""
...
set echo on
set feedback on
exec  xhb_housekeeping_pkg.write_del_record_to_history($P_COURT_ID,$P_UPPER_LIMIT);
exit
endsql
...
if [ $err_code != 0 ]; then
     ...
     AUDIT_MESSAGE="`date +"%d/%m/%Y-%H:%M:%S:> "` EVENT_STATUS=Failure | DETAILS= write history record for cases to be deleted in XHIBIT Database - SQL error while calling stored procedure";
     echo $AUDIT_MESSAGE
     exit 1
fi
AUDIT_MESSAGE="`date +"%d/%m/%Y-%H:%M:%S:> "` EVENT_STATUS=Success | DETAILS= find all HK Cases eligible for deletion in XHIBIT Database";
echo $AUDIT_MESSAGE
echo ""
exit 0
