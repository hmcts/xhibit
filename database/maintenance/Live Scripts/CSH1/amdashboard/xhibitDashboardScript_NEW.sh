...
# 17-March-2020		MT	Add retest of Thin Client,MIDM4 and 5,Capacity	
#                               reporting
#
#
...
then
  RESPONSETHIN=`echo $THINCLIENT | grep 4430`
  RESPONSETHIN1=`echo $THINCLIENT1 | grep 4430`
  if [ `echo $RESPONSETHIN | wc -c` -eq 1 ] && [ `echo $RESPONSETHIN1 | wc
 -c` -eq 1 ]
  then
...
SMSLIMIT=3000
SMSCOUNT=`sqlplus -s cjit/cjit@csdbprd2 << END
...
# check CJIT
CJIPLIMIT=100
CJIPCOUNT=`sqlplus -s cjit/cjit@csdbprd2 << END
  set heading off feedback off
...
# check LAA
#LAALIMIT=500000
#LAACOUNT=`sqlplus -s gdgate/gdgate@csdbprd3 << END
#  set heading off feedback off
...
#DARTSRESULTS=`sqlplus -s darts/darts@csdbprd2 << END
#  set heading off feedback off
...
DARTSCOUNT=`sqlplus -s darts/darts@csdbprd2 << END
  set heading off feedback off
...
telnet_function $MIDM4_WL_IP 4430
        if [ "$FUNCTION_RETURN_VALUE" -ne "0" ]
        then
                THICK_CLIENT_SECURE_APPEND="$THICK_CLIENT_SECURE_APPEND MIDM4,"
        fi

telnet_function $MIDM5_WL_IP 4430
        if [ "$FUNCTION_RETURN_VALUE" -ne "0" ]
        then
                THICK_CLIENT_SECURE_APPEND="$THICK_CLIENT_SECURE_APPEND MIDM5,"
        fi
...
telnet_function $MIDM4_WL_IP 7071
        if [ "$FUNCTION_RETURN_VALUE" -ne "0" ]
        then
                THICK_CLIENT_NON_SECURE_APPEND="$THICK_CLIENT_NON_SECURE_APPEND
MIDM4,"
        fi

telnet_function $MIDM5_WL_IP 7071
        if [ "$FUNCTION_RETURN_VALUE" -ne "0" ]
        then
                THICK_CLIENT_NON_SECURE_APPEND="$THICK_CLIENT_NON_SECURE_APPEND
MIDM5,"
...
cat ./diskstats1.txt >> $FILEFORDASHBOARD
