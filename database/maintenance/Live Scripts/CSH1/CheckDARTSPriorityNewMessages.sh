...
SUCCESS=true
DAR_CHECK_FILENAME=$SCRIPT_HOME/output/number_of_priority_new_darts_messages.txt
LIMIT_BEFORE_WARNING=40
...
sqlplus <username>/<password>@${ORACLE_SID2} @$SCRIPT_HOME/sql/count_darts_priority_new_messages.sql $DAR_CHECK_FILENAME
...
OUTPUT_FILE=$SCRIPT_HOME/output/darts_priority_message_size.log
...
then
  ...
  SUBJECT='DARTS New Messages:'$CHK' Possible DARTS high volume of priority new messages in DAR_PRIORITY_NEW_MESSAGES. Please investigate.'
  EMAILCMD="$SCRIPT_HOME/../sendemail.pl -f xhibitcsh@justice.gov.uk -t $EMAIL_RECIPIENTS -u DARTS PRIORITY New Messages Alert -s $SMTP_SERVER -m $TODAY ; $SUBJECT "
