#!/bin/bash
#################################################################################
#   exiss_houskeeping2.sh                                                       #
# Automated job for housekeeping in the EXISS schema only			#
#                                                                               #
#                                                                               #
#                                                                               #
#                                                                               #
...
echo -e "Values entered: ${ARG2} ${ARG3}"
        ...
        num_check
	check_type	
echo -e "exec EXISS.exi_housekeeping2_pkg.initiate_run('${ARG2}',${ARG3});"	
	
${SQLPLUS} -s ${ORA3} >>${LOG_DIR}/${scriptname}.${LOGFILE}<<endsql
set serveroutput on
WHENEVER SQLERROR EXIT SQL.SQLCODE
exec EXISS.EXI_HOUSEKEEPING2_PKG.initiate_run('${ARG2}',${ARG3});
endsql
