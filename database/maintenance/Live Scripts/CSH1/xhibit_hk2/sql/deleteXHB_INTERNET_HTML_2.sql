...
--  select count(*) into rows_before from XHB_INTERNET_HTML t;
...
--  select max(xhb.creation_date) into last_date from XHB_INTERNET_HTML xhb;
  select count(*) into min_rows_to_keep from XHB_INTERNET_HTML xhb where xhb.creation_date<=(sysdate-7);
  ...
  --delete statement
   select min(internet_html_id) into minx from (select xhb.creation_date,xhb.internet_html_id from XHB_INTERNET_HTML xhb order by xhb.creation_date ASC) where rownum<=input_rows;
  
   select max(internet_html_id) into maxx from (select xhb.creation_date,xhb.internet_html_id from XHB_INTERNET_HTML xhb order by xhb.creation_date ASC) where rownum<=input_rows;

   delete from XHB_INTERNET_HTML xhb where xhb.internet_html_id>=minx and xhb.internet_html_id<=maxx;
