...
--  select max(exi.item_created) into last_date from EXISS.EXI_ITEM_OUTBOUND exi;
  ...
  --delete statement
 BEGIN
 -- select count(exi.item_created) into rows_to_delete from EXISS.EXI_ITEM_OUTBOUND exi where exi.item_created>=earliest_date and exi.item_created<=(earliest_date+input_days-1); 
 

   delete from EXISS.EXI_ITEM_OUTBOUND exi where exi.item_created>=earliest_date and exi.item_created<=(earliest_date+input_days-1);
