...
--  select count(*) into rows_before from XHB_CR_LIVE_INTERNET xhb;
 ...
 -- select max(xhb.creation_date) into last_date from XHB_CR_LIVE_INTERNET xhb;
  ...
  --delete statement
  BEGIN
  --  select count(*) into rows_to_delete from XHB_CR_LIVE_INTERNET xhb where xhb.creation_date<=(sysdate-7-input_days); 
    
   delete from XHB_CR_LIVE_INTERNET xhb where xhb.creation_date<=(sysdate-7-input_days);
