...
--  select max(xhb.creation_date) into last_date from XHB_EMAIL2 xhb;
  ...
  --delete statement
  BEGIN
  -- select count(*) into rows_to_delete from XHB_EMAIL2 xhb where xhb.creation_date<=(sysdate-7-input_days); 
   
   delete from XHB_EMAIL2 xhb where xhb.creation_date<=(sysdate-7-input_days);
