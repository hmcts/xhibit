...
DECLARE  
 ...
 BEGIN
  ...
  select count(*) into rowsbefore from XHB_DISPOSAL;

  DBMS_OUTPUT.put_line ('Current date: '||to_char(sysdate,'YYYY-MM-DD HH:MM:SS'));

  execute immediate 'truncate table XHB_DISPOSAL';

  select count(*) into rowsafter from XHB_DISPOSAL;
