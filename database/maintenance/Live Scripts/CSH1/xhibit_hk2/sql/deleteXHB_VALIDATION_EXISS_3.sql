...
--  select max(exi.item_created) into last_date from XHB_VALIDATION_EXISS exi;
  ...
  --delete statement
  BEGIN
--  select count(*) into rows_to_delete from XHB_VALIDATION_EXISS exi where exi.item_created<=(sysdate-7-input_days);
--  select count(*) into rows_to_delete_cji from XHB_VALIDATION_EXISS exi, XHB_VALIDATION_CJI cji where exi.validation_id=cji.validation_id and exi.item_created<=(sysdate-7-input_days);
  
  delete from XHB_VALIDATION_CJI cji where exists (select exi.validation_id from xhb_validation_exiss exi where cji.validation_id=exi.validation_id and exi.item_created<=(sysdate-7-input_days));
  actual_rows_deleted_cji := SQL%ROWCOUNT;
  DBMS_OUTPUT.put_line ('Rows deleted (cji): ' || actual_rows_deleted_cji);

   delete from XHB_VALIDATION_EXISS exi where exi.item_created<=(sysdate-7-input_days);
