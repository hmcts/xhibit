...
--  select max(exi.item_created) into last_date from EXISS.EXI_ITEM_OUTBOUND exi;
  select count(exi.item_created) into min_rows_to_keep from EXISS.EXI_ITEM_OUTBOUND exi where exi.item_created<=(sysdate-7);
  ...
  --delete statement
   select min(item_id) into minx from (select exi.item_created,exi.item_id from EXISS.EXI_ITEM_OUTBOUND exi order by exi.item_id ASC) where rownum<=input_rows;
  
   select max(item_id) into maxx from (select exi.item_created,exi.item_id from EXISS.EXI_ITEM_OUTBOUND exi order by exi.item_id ASC) where rownum<=input_rows;
 

   delete from EXISS.EXI_ITEM_OUTBOUND exi where exi.item_id>=minx and exi.item_id<=maxx;
 ...
 -- select count(txi.item_id) into rows_after_track from EXISS.EXI_ITEM_OUTBOUND_TRACKING txi;
