...
--  select count(*) into rows_before from CJI_AHM xhb;
 ...
 -- select max(xhb.sent_agg_date) into last_date from CJI_AHM xhb;
  ...
  --delete statement
 BEGIN
-- select count(*) into rows_to_delete from CJI_AHM xhb where xhb.sent_agg_date>=earliest_date and xhb.sent_agg_date<=(earliest_date+input_days-1); 
  
   delete from CJI_AHM xhb where xhb.sent_agg_date>=earliest_date and xhb.sent_agg_date<=(earliest_date+input_days-1);
