...
DECLARE  
 ...
 BEGIN
  ...
  select count(*) into rowsbefore from AUD_EMAIL2;

  DBMS_OUTPUT.put_line ('Current date: '||to_char(sysdate,'YYYY-MM-DD HH:MM:SS'));

  execute immediate 'truncate table AUD_EMAIL2';

 select count(*) into rowsafter from AUD_EMAIL2;
