...
DECLARE  
 ...
 BEGIN
  ...
  select count(*) into rowsbefore from GDG_TEMP_OUTBOUND;

  DBMS_OUTPUT.put_line ('Current date: '||to_char(sysdate,'YYYY-MM-DD HH:MM:SS'));

  execute immediate 'truncate table GDG_TEMP_OUTBOUND';

  select count(*) into rowsafter from GDG_TEMP_OUTBOUND;
