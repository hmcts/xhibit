...
DECLARE  
 ...
 BEGIN
  ...
  select count(*) into rowsbefore from AUD_CREST_IMPORT;

  DBMS_OUTPUT.put_line ('Current date: '||to_char(sysdate,'YYYY-MM-DD HH:MM:SS'));

  execute immediate 'truncate table AUD_CREST_IMPORT';

  select count(*) into rowsafter from AUD_CREST_IMPORT;
