...
DECLARE  
 ...
 BEGIN
  ...
  select count(*) into rowsbefore from WMB_MONITORING_BK;

  DBMS_OUTPUT.put_line ('Current date: '||to_char(sysdate,'YYYY-MM-DD HH:MM:SS'));

  execute immediate 'truncate table WMB_MONITORING_BK';

  select count(*) into rowsafter from WMB_MONITORING_BK;
