...
--  select max(xhb.creation_date) into last_date from XHB_CR_LIVE_INTERNET xhb;
  ...
  ELSE
   ...
   --delete statement
 BEGIN
 --select count(*) into rows_to_delete from XHB_CR_LIVE_INTERNET where creation_date>=earliest_date and creation_date<=(earliest_date+input_days-1); 
  
   delete from XHB_CR_LIVE_INTERNET xhb where xhb.creation_date>=earliest_date and xhb.creation_date<=(earliest_date+input_days-1);
