...
--  select count(exi.item_id) into rows_before from EXISS.EXI_ITEM_INBOUND exi;
 ...
 -- select max(exi.date_created) into last_date from EXISS.EXI_ITEM_INBOUND exi;
  ...
  --delete statement
 BEGIN
  --select count(exi.item_id) into rows_to_delete from EXISS.EXI_ITEM_INBOUND exi where exi.date_created>=earliest_date and exi.date_created<=(earliest_date+input_days-1);

   delete from EXISS.EXI_ITEM_INBOUND exi where exi.date_created>=earliest_date and exi.date_created<=(earliest_date+input_days-1);
