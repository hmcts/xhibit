...
DECLARE  
 ...
 BEGIN
  ...
  select count(*) into rowsbefore from AUD_INTERNET_HTML;

  DBMS_OUTPUT.put_line ('Current date: '||to_char(sysdate,'YYYY-MM-DD HH:MM:SS'));

  execute immediate 'truncate table AUD_INTERNET_HTML';

  select count(*) into rowsafter from AUD_INTERNET_HTML;
