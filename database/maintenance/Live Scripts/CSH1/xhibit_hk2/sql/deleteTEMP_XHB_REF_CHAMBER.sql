/* deleteTEMP_XHB_REF_CHAMBER.sql                                                 */
...
DECLARE  
 ...
 BEGIN
  ...
  select count(*) into rowsbefore from TEMP_XHB_REF_CHAMBER;

  DBMS_OUTPUT.put_line ('Current date: '||to_char(sysdate,'YYYY-MM-DD HH:MM:SS'));

  execute immediate 'truncate table TEMP_XHB_REF_CHAMBER';

  select count(*) into rowsafter from TEMP_XHB_REF_CHAMBER;
