/* deleteTMP_CASE_FOR_REFRESH.sql                                                */
...
DECLARE  
 ...
 BEGIN
  ...
  select count(*) into rowsbefore from TMP_CASE_FOR_REFRESH;

  DBMS_OUTPUT.put_line ('Current date: '||to_char(sysdate,'YYYY-MM-DD HH:MM:SS'));

  execute immediate 'truncate table TMP_CASE_FOR_REFRESH';

  select count(*) into rowsafter from TMP_CASE_FOR_REFRESH;
