...
DECLARE  
 ...
 BEGIN
  ...
  select count(*) into rowsbefore from AUD_ORDER_TYPE;

  DBMS_OUTPUT.put_line ('Current date: '||to_char(sysdate,'YYYY-MM-DD HH:MM:SS'));

  execute immediate 'truncate table AUD_ORDER_TYPE';

  select count(*) into rowsafter from AUD_ORDER_TYPE;
