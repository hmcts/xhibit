/* deleteGDG_INBOUND_MESSAGES.sql                                                */
...
DECLARE  
 ...
 BEGIN
  ...
  select count(*) into rowsbefore from GDG_INBOUND_MESSAGES;

  DBMS_OUTPUT.put_line ('Current date: '||to_char(sysdate,'YYYY-MM-DD HH:MM:SS'));

  execute immediate 'truncate table GDG_INBOUND_MESSAGES';

  select count(*) into rowsafter from GDG_INBOUND_MESSAGES;
