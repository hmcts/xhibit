/* deleteXHB_CR_LIVE_INTERNET.sql                                                */
...
--  select count(*) into rows_before from XHB_CR_LIVE_INTERNET xhb;
 -- select min(xhb.creation_date) into earliest_date from XHB_CR_LIVE_INTERNET xhb;
 -- select max(xhb.creation_date) into last_date from XHB_CR_LIVE_INTERNET xhb;
  select count(*) into min_rows_to_keep from XHB_CR_LIVE_INTERNET xhb where xhb.creation_date<=(sysdate-7);
  ...
  --delete statement
   select min(cr_live_internet_id) into minx from (select xhb.creation_date,xhb.cr_live_internet_id from XHB_CR_LIVE_INTERNET xhb order by xhb.creation_date ASC) where rownum<=input_rows;
  
   select max(cr_live_internet_id) into maxx from (select xhb.creation_date,xhb.cr_live_internet_id from XHB_CR_LIVE_INTERNET xhb order by xhb.creation_date ASC) where rownum<=input_rows;

   delete from XHB_CR_LIVE_INTERNET xhb where xhb.cr_live_internet_id>=minx and xhb.cr_live_internet_id<=maxx;
  ...
  -- count of rows after
  --select count(*) into rows_after from XHB_CR_LIVE_INTERNET xhb;
  --select min(xhb.creation_date) into post_earliest_date from XHB_CR_LIVE_INTERNET xhb;
  --select max(xhb.creation_date) into post_last_date from XHB_CR_LIVE_INTERNET xhb;
  -- get timings
