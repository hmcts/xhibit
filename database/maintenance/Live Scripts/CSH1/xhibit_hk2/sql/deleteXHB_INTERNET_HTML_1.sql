...
--  select count(*) into rows_before from XHB_INTERNET_HTML xhb;
 ...
 -- select max(xhb.creation_date) into last_date from XHB_INTERNET_HTML xhb;
  ...
  --delete statement
 BEGIN
-- select count(*) into rows_to_delete from XHB_INTERNET_HTML xhb where xhb.creation_date>=earliest_date and xhb.creation_date<=(earliest_date+input_days-1); 
  
   delete from XHB_INTERNET_HTML xhb where xhb.creation_date>=earliest_date and xhb.creation_date<=(earliest_date+input_days-1);
