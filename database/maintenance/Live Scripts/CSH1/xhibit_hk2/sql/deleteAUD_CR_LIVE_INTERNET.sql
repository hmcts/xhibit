/* deleteAUD_CR_LIVE_INTERNET.sql                                                */
...
DECLARE  
 ...
 BEGIN
  ...
  select count(*) into rowsbefore from AUD_CR_LIVE_INTERNET;

  DBMS_OUTPUT.put_line ('Current date: '||to_char(sysdate,'YYYY-MM-DD HH:MM:SS'));

  execute immediate 'truncate table AUD_CR_LIVE_INTERNET';

  select count(*) into rowsafter from AUD_CR_LIVE_INTERNET;
