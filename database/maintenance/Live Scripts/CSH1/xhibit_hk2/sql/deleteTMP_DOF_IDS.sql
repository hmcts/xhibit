...
DECLARE  
 ...
 BEGIN
  ...
  select count(*) into rowsbefore from TMP_DOF_IDS;

  DBMS_OUTPUT.put_line ('Current date: '||to_char(sysdate,'YYYY-MM-DD HH:MM:SS'));

  execute immediate 'truncate table TMP_DOF_IDS';

  select count(*) into rowsafter from TMP_DOF_IDS;
