...
--  select count(*) into rows_before from CJI_AHM xhb;
 ...
 -- select max(xhb.sent_agg_date) into last_date from CJI_AHM xhb;
  ...
  ELSE
   ...
   --delete statement
  BEGIN
  --  select count(*) into rows_to_delete from CJI_AHM xhb where xhb.sent_agg_date<=(sysdate-7-input_days); 
    
   delete from CJI_AHM where sent_agg_date<=(sysdate-7-input_days);
