...
DECLARE  
 ...
 BEGIN
  ...
  select count(*) into rowsbefore from AUD_REF_CHAMBER;

  DBMS_OUTPUT.put_line ('Current date: '||to_char(sysdate,'YYYY-MM-DD HH:MM:SS'));

  execute immediate 'truncate table AUD_REF_CHAMBER';

  select count(*) into rowsafter from AUD_REF_CHAMBER;
