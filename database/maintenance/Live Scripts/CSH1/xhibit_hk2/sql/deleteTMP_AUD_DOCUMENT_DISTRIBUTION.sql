/* deleteTMP_AUD_DOCUMENT_DISTRIBUTION.sql                                      */
...
DECLARE  
 ...
 BEGIN
  ...
  select count(*) into rowsbefore from TMP_AUD_DOCUMENT_DISTRIBUTION;

  DBMS_OUTPUT.put_line ('Current date: '||to_char(sysdate,'YYYY-MM-DD HH:MM:SS'));

  execute immediate 'truncate table TMP_AUD_DOCUMENT_DISTRIBUTION';

  select count(*) into rowsafter from TMP_AUD_DOCUMENT_DISTRIBUTION;
