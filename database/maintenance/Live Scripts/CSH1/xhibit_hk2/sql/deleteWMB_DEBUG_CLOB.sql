...
DECLARE  
 ...
 BEGIN
  ...
  select count(*) into rowsbefore from WMB_DEBUG_CLOB;

  DBMS_OUTPUT.put_line ('Current date: '||to_char(sysdate,'YYYY-MM-DD HH:MM:SS'));

  execute immediate 'truncate table WMB_DEBUG_CLOB';

  select count(*) into rowsafter from WMB_DEBUG_CLOB;
