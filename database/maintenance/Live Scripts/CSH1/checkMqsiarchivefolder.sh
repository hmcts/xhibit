...
echo "DIRECTORY DOES NOT EXIST"
  emailCmd="$EMAIL_SCRIPT -f $EMAIL_SENDER -t $EMAIL_RECIPIENTS -u MQSIARCHIVE FOLDER IS NOT PRESENT IN CSH1 -s $SMTP_SERVER -m MQSIARCHIVE FOLDER IS NOT PRESENT IN CSH1. CREATING FOLDER "
  `$emailCmd`
mkdir $ARCHIVE_DIRECTORY
chmod 777 $ARCHIVE_DIRECTORY
mv $LOGS_DIRECTORY/*.aud $ARCHIVE_DIRECTORY

CHECK_CMD=`ssh wmbroker@x.y.70.202 "/opt/moj/home/wmbroker/bin/cron/StopPublishWTXEventFlow_CSH1.sh;"`
echo $CHECK_CMD
count=`echo $CHECK_CMD|grep "Successful command completion"|wc -l`
if [ $count != "1" ]
then
  emailCmd="$EMAIL_SCRIPT -f $EMAIL_SENDER -t $EMAIL_RECIPIENTS -u WTXEvent flow might not be running -s $SMTP_SERVER -m A restart of BROKER on CSH1 might be required. Please investigate "
  `$emailCmd`

fi

CHECK_CMD1=`ssh wmbroker@x.y.70.202 "/opt/moj/home/wmbroker/bin/cron/StartPublishWTXEventFlow_CSH1.sh;"`
echo $CHECK_CMD1
