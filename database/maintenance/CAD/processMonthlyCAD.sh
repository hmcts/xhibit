...
# If no args are passed in then fine, set START_DATE and END_DATE accordingly
  if [ "${#}" = "0" ]
  then
    sqlplus xhibit/xhibit@${ORACLE_SID} @$SCRIPT_HOME/sql/getFirstDayOfLastMonth.sql $START_DATE_FILENAME
    sqlplus xhibit/xhibit@${ORACLE_SID} @$SCRIPT_HOME/sql/getLastDayOfLastMonth.sql $END_DATE_FILENAME
...
OUTPUT=`sqlplus -s xhibit/xhibit@csdbprd1 << END
SET SERVEROUTPUT ON;
...
# Get the run id
sqlplus xhibit/xhibit@${ORACLE_SID} @$SCRIPT_HOME/sql/getCADRunID.sql $CAD_RUN_ID_FILE
CAD_RUN_ID=`sed -n '4,4p' $CAD_RUN_ID_FILE`
echo Latest CADRunID is $CAD_RUN_ID >> ${LOG_FILE}

# Get and check for any errors
sqlplus xhibit/xhibit@${ORACLE_SID} @$SCRIPT_HOME/sql/checkForCADErrors.sql $ERRORS_FILE $CAD_RUN_ID
...
# Some string manipulation needed to convert the start date e.g. 01-MAY-19 which is part of a text file generated from SQL so that we can get the month name and finally
 convert into 2 digit month no for use with the
...
sqlplus xhibit/xhibit@${ORACLE_SID} @$SCRIPT_HOME/sql/getCADFile.sql $CAD_CLOB_FILE
