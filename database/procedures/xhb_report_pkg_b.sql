   ...
   BEGIN
     OPEN p_results_out FOR 
      ...
      CASE WHEN xrpa.DX_REF IS NOT NULL 
           THEN xrpa.DX_REF 
           ELSE xa2.ADDRESS_1 || chr(10) || xa2.ADDRESS_2 || chr(10) || xa2.ADDRESS_3 || chr(10) || xa2.ADDRESS_4 || chr(10) || xa2.TOWN || chr(10) || xa2.COUNTY || chr(10) || xa2.POSTCODE END AS prosecutor_address,
      ...
      WHERE xc.RECEIPT_TYPE in ('CT','TC','VB','ST','IO','EW') AND xc.COURT_ID = p_court_id AND xc.DATE_TRANS_TO IS NULL AND is_valid_case_status(xc.case_status) = 'Y'
...
v_err_code NUMBER;
v_err_msg  VARCHAR2(1000);
CURSOR c_get_case_ids_from_clob IS
SELECT regexp_substr(p_cases, '[^,]+', 1, level) case_id 
	FROM DUAL
    CONNECT BY regexp_substr(p_cases, '[^,]+', 1, level) IS NOT NULL;

BEGIN

  FOR case_rec IN c_get_case_ids_from_clob LOOP
   IF case_rec.case_id IS NULL OR LENGTH(case_rec.case_id) = 0 THEN
     EXIT;
   END IF;

   UPDATE XHB_CASE SET DATE_CTL_REMINDER_PRINTED  = sysdate 
   WHERE  case_id = TO_NUMBER(case_rec.case_id);
...
BEGIN
  SELECT /* get_bc_status_ind */ decode(min(DECODE(nvl(xdoc.CURRENT_BC_STATUS, 'N/A'), 'C', '1', 'J', '1', 'B', '2', 'N', '3', 'N/A', '3')), '1', 'C', '2', 'B', '3', 'N/A')
  INTO   v_bc_status
...
**************************************************************************************/
  ...
  BEGIN
      OPEN p_results_out FOR
      select    /* get_defendants_put_back_report */ 
      ...
      and   is_valid_case_status(xc.case_status) = 'Y'
  ...
  BEGIN
  OPEN p_results_out FOR
    SELECT /* get_docar_report */ c.CASE_TYPE, c.CASE_NUMBER, dof.DEFENDANT_NUMBER, d.SURNAME, d.FIRST_NAME, d.MIDDLE_NAME, dof.FORM_NG_SENT_DATE
    FROM XHB_DEFENDANT_ON_CASE dof, XHB_CASE c, XHB_DEFENDANT d
    WHERE dof.CASE_ID = c.CASE_ID
    AND is_valid_case_status(c.case_status) = 'Y'
...
,             CASE WHEN get_solicitor_id(xdoc.DEFENDANT_ON_CASE_ID) IS NULL 
                     THEN dxa.address_1 ||'|'||dxa.address_2||'|'||dxa.address_3||'|'||dxa.address_4 || '|' || dxa.town || '|' || dxa.county || '|' || dxa.postcode
                   ELSE (SELECT  CASE WHEN xrsf.DX_REF IS NOT NULL 
                                  THEN xrsf.DX_REF
                                  ELSE sxa.address_1 || '|' || sxa.address_2 || '|' || sxa.address_3 || '|' || sxa.address_4 || '|' || sxa.town || '|' || sxa.county || '|' || sxa.postcode END
...
,             xcrt.court_name AS COURT_NAME
,             cxa.address_1 || ' ' || cxa.address_2 || ' ' || cxa.address_3 || ' ' || cxa.address_4 || ' ' || cxa.town || ' ' || cxa.county || ' ' || UPPER(cxa.postcode) AS court_address
,             getAddressSingleLine(NVL(xcs.address_id,xcrt.address_id)) AS hearing_address
...
v_err_code NUMBER;
v_err_msg  VARCHAR2(1000);
CURSOR c_get_case_diary_ids_from_clob IS
	SELECT regexp_substr(p_fixtures, '[^,]+', 1, level) case_diary_fixture_id 
	FROM DUAL
    CONNECT BY regexp_substr(p_fixtures, '[^,]+', 1, level) IS NOT NULL;

BEGIN

-- We have to set the Fixture_Notice_Required. This prevents the letters being re-printed next time the report is run.
    FOR diary_rec IN c_get_case_diary_ids_from_clob LOOP
    
    IF diary_rec.case_diary_fixture_id IS NULL OR LENGTH(diary_rec.case_diary_fixture_id) = 0 THEN
        EXIT;
    END IF;
        
    UPDATE xhb_case_diary_fixture SET fixture_notice_required = 'N'
    WHERE  case_diary_fixture_id = TO_NUMBER(diary_rec.case_diary_fixture_id);
...
END MARK_FIXTURES_AS_REPORTED;
 ...
 PROCEDURE get_list_officers_diary_report(p_results_out OUT SYS_REFCURSOR
  ...
  v_err_msg   VARCHAR2(1000);
    BEGIN
      OPEN p_results_out FOR
        SELECT /* get_list_officers_diary_report */
        ROWNUM as ROW_ID, CASE_NUMBER, CASE_TITLE, NULL as DIARY_NOTE_TEXT, NULL as CREATION_DATE FROM(          
       ...
       (SELECT  xc.CASE_TYPE || xc.CASE_NUMBER as CASE_NUMBER, xc.CASE_TITLE as case_title, xdne.DIARY_NOTE_TEXT as diary_note_text, xdne.CREATION_DATE 
        ...
        LEFT JOIN XHB_CASE xc ON xdne.CASE_ID = xc.CASE_ID AND is_valid_case_status(xc.case_status) = 'Y'
 ...
 EXCEPTION
  ...
  BEGIN
  OPEN p_results_out FOR
Select /* get_cfix_report */
                xcdf.CASE_DIARY_FIXTURE_ID,
                xcdf.LISTING_DATE,
                to_char(xcdf.LISTING_DATE,'dd-Mon-yyyy') as HEARING_DATE,
                xc.CASE_TYPE||xc.CASE_NUMBER as CASE_NUMBER,
                xc.CASE_TITLE,
                xc.CLASS_CODE,
                get_bc_status_ind(xc.CASE_ID) as BC_STATUS,
                xrht.HEARING_TYPE_CODE as HEARING_TYPE,
                xdfc.TRIAL_TIME_ESTIMATE || decode(xdfc.TRIAL_TIME_ESTIMATE, NULL, NULL, decode(xdfc.TRIAL_TIME_UNIT,'1','H','2','D','3','W','4','M')) as EST,
                xrs.COURT_SITE_CODE as SITE,
                Highlight_Notes.DIARY_CREATION_DATE || ' ' || Highlight_Notes.DIARY_NOTE_TEXT as HIGHLIGHT_NOTE,
...
from   XHB_CASE_LISTING_ENTRY xcle
     ...
     , (SELECT xdne.CASE_LISTING_ENTRY_ID,
      ...
      ,(SELECT xdne.CASE_LISTING_ENTRY_ID, 
               nvl(xrld_pref.REF_DATA_VALUE, xdne.DIARY_NOTE_TEXT) DIARY_NOTE_TEXT,
               to_char(xdne.CREATION_DATE,'dd-Mon-yyyy') DIARY_CREATION_DATE
        FROM XHB_REF_LISTING_DATA xrld
            ,XHB_DIARY_NOTE_ENTRY xdne
            ,XHB_REF_LISTING_DATA xrld_pref      
        WHERE xrld.REF_DATA_TYPE = 'NOTE_TYPE'
...
'    SELECT  /* get_outc_report */ notes.DIARY_NOTE_ENTRY_ID,
            xc.CASE_TYPE || xc.CASE_NUMBER as CASE_NUMBER,
    ...
    FROM    XHB_CASE xc
          ...
          , (SELECT NVL(xcle.CASE_ID,xdne.CASE_ID) as CASE_ID,
                    ...
                    xdne.CREATION_DATE CREATION_DATE,
    ...
    END IF;
    v_where := v_where || ',CASE notes.note_type WHEN ''HN'' THEN 1 WHEN ''IN'' THEN 2 ELSE 99 END, notes.CREATION_DATE, notes.DIARY_NOTE_ENTRY_ID';
 
    v_report_sql :=  v_report_sql || v_where;

    -- Now we have assembled the dynamic SQL, run it.
    OPEN p_results_out FOR v_report_sql USING p_court_id;
...
'    SELECT /* get_unlc_report */ notes.DIARY_NOTE_ENTRY_ID,
           xc.CASE_TYPE || xc.CASE_NUMBER as CASE_NUMBER,
          DECODE(XHB_REPORT_PKG.get_bc_status_ind(xc.CASE_ID), ''C'', ''*'', ''B'', '' '', ''N/A'', '' '') as CUSTODY_CASE,
           xc.CASE_TITLE,
           NVL((SELECT decode(MAX(xdoc.is_juvenile), ''Y'', ''Juv'', '' '') 
    ...
    END IF;
    IF p_DEFAULT_HEARING_TYPE IS NOT NULL THEN 
        v_where := v_where || '   AND     xrht.HEARING_TYPE_CODE = ''' || p_DEFAULT_HEARING_TYPE || '''' ;
    END IF;
    IF p_TIME_EST_FROM IS NOT NULL THEN
        IF p_TIME_EST_TO IS NULL THEN
            v_where := v_where || '   AND      xdfc.TRIAL_TIME_ESTIMATE BETWEEN ' || to_char(p_TIME_EST_FROM) || ' AND NULL ' || '
...
FUNCTION unlc_check_case_on_list (p_case_id IN xhb_case.case_id%TYPE) RETURN VARCHAR2 IS
    ...
    SELECT /* unlc_check_case_on_list */ * FROM
...
BEGIN
  ...
  OPEN p_results_out FOR
    Select  /* get_drsr_report */
            to_char(xcol.TIME_LISTED, 'dd-mm-yyyy') as LISTING_DATE,
            xc.CASE_TYPE || xc.CASE_NUMBER as CASE_NUMBER,
...
PROCEDURE get_relcj_report(p_results_out OUT SYS_REFCURSOR
  ...
  BEGIN
    OPEN p_results_out FOR
        SELECT /* get_relcj_report */
            xc.DEFAULT_HEARING_TYPE as HEARING_TYPE_ID,
            xc.CASE_ID,
            xc.CASE_TYPE||xc.CASE_NUMBER as CASE_NUMBER,
            xc.CASE_TITLE,
            xc.CLASS_CODE, 
...
BEGIN
  OPEN p_results_out FOR
    select /* get_inftrpc_main_report */
     subqry.*,
...
BEGIN
  OPEN p_results_out FOR
   select /* get_inftrpc_casenumbers_report */ 
               xrce.CODE,
               xrce.DESCRIPTION, 
               0 AS TOTAL,
               to_char( LAST_DAY(to_date('01-'||p_MONTH_PERIOD||'-'||p_YEAR_PERIOD,'dd-Mon-YYYY')),'dd-Mon-YYYY') CALCMONTH,
           ...
           (Select xcol.CRACKED_INEFFECTIVE_ID,
            ...
            AND   xrht.REF_HEARING_TYPE_ID = xcol.HEARING_TYPE_ID
            AND   cas.CASE_TYPE = 'T'
            AND   is_valid_case_status(cas.case_status) = 'Y'
            AND   xrht.HEARING_TYPE_CODE  IN ( 'TRL','TFL','TBK')
...
BEGIN
    OPEN p_results_out FOR
        SELECT /* get_list_of_fixed_dates_report */
...
PROCEDURE get_darts_report(p_results_out out SYS_REFCURSOR
                                        ,p_court_id IN XHB_CASE.COURT_ID%TYPE DEFAULT NULL
                                        ,p_start_date IN XHB_CASE.CRP_LAST_UPDATE_DATE%TYPE
										,p_end_date IN XHB_CASE.CRP_LAST_UPDATE_DATE%TYPE) AS

 v_err_code NUMBER;
 v_err_msg  VARCHAR2(1000);
BEGIN
    OPEN p_results_out FOR
        SELECT xc.CRP_LAST_UPDATE_DATE,
		xc.case_type,
		xc.case_number,
		xc.court_id,
		xcc.court_name,
		xdarp.policy_no,
		xdarp.policy_description,
		xdrp.duration_days,
		xdrp.duration_months,
		xdrp.duration_years,
		xdrp.has_life
		FROM XHB_CASE xc,
			XHB_COURT xcc,
			XHB_DAR_RETENTION_POLICY xdrp,
			XHB_REF_DAR_RETENTION_POLICIES xdarp
		WHERE xc.CRP_LAST_UPDATE_DATE IS NOT NULL
		AND xc.CRP_LAST_UPDATE_DATE BETWEEN TRUNC(p_start_date) AND TRUNC(p_end_date + 1)-(1/(24*60*60))
		AND xdrp.DAR_RETENTION_POLICY_ID = xc.DAR_RETENTION_POLICY_ID 
		AND xdarp.REF_DAR_RETENTION_POLICY_ID = xdrp.REF_DAR_RETENTION_POLICY_ID
		AND xc.COURT_ID = NVL(p_court_id, xc.COURT_ID)
		AND xcc.COURT_ID = xc.COURT_ID
		ORDER BY TRUNC(xc.CRP_LAST_UPDATE_DATE), xc.case_type, xc.case_number;
  ...
  EXCEPTION
     WHEN OTHERS THEN
      ...
      v_err_msg  := SQLERRM;
      INSERT INTO XHB_REPORT_ERR_LOG(REPORTS_LOG_ID, error_message, run_date)
           VALUES (XHB_REPORTS_ERROR_LOG_SEQ.nextval, 'Exception handler raised when others in XHB_REPORT_PKG.GET_DARTS_REPORT '|| v_err_code || ' : ' ||v_err_msg, sysdate);
      RAISE;
END get_darts_report;
...
PROCEDURE update_case_diary_fix_run_date(casediaryfixturelist IN CLOB) AS
    ...
    v_err_msg  VARCHAR2(1000);
    CURSOR c_get_case_diary_ids_from_clob IS
	SELECT regexp_substr(casediaryfixturelist, '[^,]+', 1, level) case_diary_fixture_id 
	FROM DUAL
    CONNECT BY regexp_substr(casediaryfixturelist, '[^,]+', 1, level) IS NOT NULL;
...
BEGIN
  OPEN p_results_out FOR
   SELECT /* get_run_date */ MAX(xrl.DATE_LAST_RUN) AS rundate FROM XHB_REPORT_LOG xrl
...
CURSOR c_get_case_ids_from_clob IS
	SELECT regexp_substr(p_cases_to_publish, '[^,]+', 1, level) case_id 
	FROM DUAL
    CONNECT BY regexp_substr(p_cases_to_publish, '[^,]+', 1, level) IS NOT NULL;
BEGIN
...
v_err_msg  VARCHAR2(1000);
   BEGIN
     ...
     CASE WHEN TRUNC(xdc.CUSTODY_TIME_LIMIT) < TRUNC(SYSDATE) THEN 'Y'
      ELSE 'N'
      END AS expired 
      FROM XHB_CASE xc
      INNER JOIN XHB_DEFENDANT_ON_CASE xdc ON xc.CASE_ID = xdc.CASE_ID AND (xdc.OBS_IND <> 'Y' OR xdc.OBS_IND IS NULL)
      ...
      WHERE xc.RECEIPT_TYPE in ('CT','TC','VB','ST', 'IO', 'EW') AND xc.COURT_ID = p_court_id AND xc.DATE_TRANS_TO IS NULL AND is_valid_case_status(xc.case_status) = 'Y'
      AND TRUNC(xdc.CUSTODY_TIME_LIMIT) <= p_screen_time_limit
 ...
 PROCEDURE get_ctlrpex_report(p_results_out OUT SYS_REFCURSOR
                                ,p_court_id    IN XHB_CASE.COURT_ID%TYPE) AS
...
v_err_msg  VARCHAR2(1000);
   BEGIN
     OPEN p_results_out FOR 
      SELECT /* get_ctlrpex_report */
      xc.CASE_TYPE || xc.CASE_NUMBER || '-' || xdc.DEFENDANT_NUMBER AS casedefendantnumber,
      UPPER(xd.SURNAME) || ' ' || LOWER(xd.FIRST_NAME) AS defendantname,
      TO_CHAR(xdc.CUSTODY_TIME_LIMIT,'dd/mm/yyyy') AS custodytimelimit, 
      TO_CHAR(NVL(xc.committal_date, xc.sent_for_trial_date),'dd/mm/yyyy') AS committaldate, 
      (SELECT DECODE(COUNT(*),0,'N','Y')
        FROM XHB_CASE_ON_LIST xcol
        WHERE xcol.CASE_ID = xc.CASE_ID
        AND NVL(xcol.OBS_IND,'N') <> 'Y' ) AS listed, 
      DECODE(xrpa.PROSECUTOR_NAME_1,NUll,'',xrpa.PROSECUTOR_NAME_1 || ' ') || DECODE(xrpa.PROSECUTOR_NAME_2,NUll,'',xrpa.PROSECUTOR_NAME_2 || ' ') || xrpa.PROSECUTOR_NAME_3 AS prosecutorname,
      NVL(xdc.ctl_applies,' ') AS ctlapplies,
      xdc.current_bc_status AS currentbcstatus
      ...
      AND (xdc.RESULTS_VERIFIED <> 'E' OR xdc.RESULTS_VERIFIED IS NULL) AND xdc.DATE_EXPORTED IS NULL
      AND NVL(xdc.CTL_APPLIES,'Y') = 'Y'
      AND xdc.CUSTODY_TIME_LIMIT IS NULL
      ORDER BY xc.CASE_NUMBER, xdc.DEFENDANT_NUMBER;
        
    UPDATE XHB_REPORT_LOG SET DATE_LAST_RUN = sysdate WHERE CREST_REPORT_CODE = 'CTLRPEx' AND COURT_ID = p_court_id;
  ...
  EXCEPTION
     WHEN OTHERS THEN
      ...
      v_err_msg  := SQLERRM;
      INSERT INTO XHB_REPORT_ERR_LOG(REPORTS_LOG_ID, error_message, run_date)
           VALUES (XHB_REPORTS_ERROR_LOG_SEQ.nextval, 'Exception handler raised when others in XHB_REPORT_PKG.GET_CTRLPEX_REPORT '|| v_err_code || ' : ' ||v_err_msg, sysdate);
      RAISE;
        
 END get_ctlrpex_report;
...
SELECT xcl.charges_info
 ...
 END get_charges_info;
  ...
  solicitor_return XHB_REF_SOLICITOR_FIRM.REF_SOLICITOR_FIRM_ID%TYPE := NULL;
      BEGIN
    SELECT /* get_solicitor_id */ xrsf.REF_SOLICITOR_FIRM_ID
...
FUNCTION GET_OBW_SOL_NAME(p_doc_id IN xhb_defendant_on_case.DEFENDANT_ON_CASE_ID%TYPE) RETURN XHB_REF_SOLICITOR_FIRM.SOLICITOR_FIRM_NAME%TYPE IS
  solicitor_return XHB_REF_SOLICITOR_FIRM.SOLICITOR_FIRM_NAME%TYPE := NULL;
  doc_obs VARCHAR2(1);
  ref_obs VARCHAR2(1);
      BEGIN
    SELECT /* get_solicitor_firm_name */ xrsf.SOLICITOR_FIRM_NAME,xdocrsf.obs_ind,xrsf.obs_ind
        INTO solicitor_return, doc_obs, ref_obs
        FROM xhb_def_on_case_ref_sol_firm xdocrsf
      ,    xhb_ref_solicitor_firm xrsf
      WHERE p_doc_id  = xdocrsf.DEFENDANT_ON_CASE_ID (+)
      AND   xdocrsf.REF_SOLICITOR_FIRM_ID = xrsf.REF_SOLICITOR_FIRM_ID
      AND   (xdocrsf.REP_END_DATE IS NULL)
      AND rownum < 2;
      IF solicitor_return is not null and NVL(doc_obs, '-') <> 'Y' 
      AND   NVL(ref_obs, '-') <> 'Y' THEN
        RETURN solicitor_return;
      END IF;
      IF solicitor_return is not null and NVL(doc_obs, '-') <> 'Y' THEN
        RETURN solicitor_return || ' (X)';
      END IF;
      -- if this is still empty then return 'No Representation Recorded'
     RETURN 'No Representation Recorded';

    EXCEPTION
    WHEN NO_DATA_FOUND THEN
      RETURN 'No Representation Recorded'; -- If no solicitors found, return null

END GET_OBW_SOL_NAME;
...
BEGIN
 SELECT /* get_most_recent_hearing */ h2.HEARING_RECORD_ID  into most_recent_hearing_id from
...
BEGIN
    ...
    v_composite_address := v_adline1;   -- Address Line 1 is always populated
    IF v_adline2 IS NOT NULL THEN
        v_composite_address := v_composite_address || ', ' || v_adline2;
    END IF;
    IF v_adline3 IS NOT NULL THEN
        v_composite_address := v_composite_address || ', ' || v_adline3;
    END IF;
    IF v_adline4 IS NOT NULL THEN
        v_composite_address := v_composite_address || ', ' || v_adline4;
    END IF;
    IF v_town IS NOT NULL THEN
        v_composite_address := v_composite_address || ', ' || v_town;
    END IF;
    IF v_county IS NOT NULL THEN
        v_composite_address := v_composite_address || ', ' || v_county;
    END IF;
...
BEGIN  
    ...
    select  /* getAppealHearingNotifnRpt */
            cas.case_id,
            cas.case_type||cas.case_number as case_number,
            cas.case_sub_type,
            ct.crest_court_id,
            ct.court_name as court_name,
            ct.court_name as hearing_venue,
            ...
            getAddressSingleLine(ct.address_id) as hearing_address,
            ...
            csol.list_note_text as list_note_text,
            rld2.ref_data_value as predefined_list_note
    from    xhb_case cas, xhb_defendant df, xhb_defendant_on_case doc, xhb_ref_solicitor_firm rsf,   
            ...
            xhb_case_on_list csol, xhb_list lst, xhb_ref_listing_data rld, xhb_ref_listing_data rld2, 
    ...
    and    rld2.ref_listing_data_id(+) = csol.list_note_predefined_id
    and    rld2.ref_data_type(+) = 'PREDEFINED_LIST_NOTE'
    union 
    ...
    select  cas.case_id,
            ...
            ct.crest_court_id,
            ct.court_name as court_name,
            NVL(xcs.court_site_name,ct.court_name) as hearing_venue,
            INITCAP(rct.court_full_name) as mags_court_name,            
            -- Appellant columns
            doc.defendant_number, 
            INITCAP(DECODE(df.first_name,NUll,'',df.first_name || ' ') || DECODE(df.middle_name,NUll,'',df.middle_name || ' ') || df.surname) as appellant_name, 
            ...
            rct.court_full_name || chr(10) || CASE WHEN rct.DX_REF IS NOT NULL THEN rct.DX_REF ELSE getaddress(rct.address_id) END as magistrate_address,      -- Changed ct to rct for CTX-2703
            getAddressSingleLine(ct.address_id) as court_address, 
            getAddressSingleLine(NVL(xcs.address_id,ct.address_id)) as hearing_address,
            lower(Replace(r.de_code, 'APPEAL ')) as appeal_type, 'Clerk to Justice' as clerk_to_justice,
            cas.mag_conviction_date,  cas.orig_body_decision_date,
            cdf.listing_date,
            ...
            getPhoneNum(ct.address_id) as ct_phone_no,    -- ctx-2963  changed to use getPhoneNum
            NVL(df.CURRENT_PRISON_STATUS,'N') as in_custody,
            cdf.list_note_text as list_note_text,
            rld.ref_data_value as predefined_list_note
    from    xhb_case cas, xhb_defendant df, xhb_defendant_on_case doc, xhb_ref_solicitor_firm rsf,  
            xhb_case_prosecutor_agency cpa, xhb_ref_prosecutor_agency rpa, 
            -- tables unique to the 2nd query
            xhb_case_listing_entry cle, xhb_case_diary_fixture cdf, xhb_ref_listing_data rld,
            -- tables unique to the 2nd query (end)
            xhb_court ct, xhb_ref_system_code r, xhb_ref_court rct, xhb_court_site xcs           -- Added xhb_ref_court for CTX-2703
    where  df.defendant_id = doc.defendant_id 
    and    doc.case_id = cas.case_id 
    and    rsf.ref_solicitor_firm_id(+) = get_solicitor_id(doc.defendant_on_case_id) 
    and    cpa.CASE_PROS_AGENCY_ID = get_prosecutor (cas.CASE_ID)
    ...
    and    ct.court_id = r.court_id    -- Added for CTX-2703
    and    NVL(r.OBS_IND,'N') <> 'Y'
    and    rld.ref_listing_data_id(+) = cdf.list_note_pre_defined_id
    and    rld.ref_data_type(+) = 'PREDEFINED_LIST_NOTE'
    and    xcs.court_site_id(+) = cdf.court_site_id
    order by 1  ;
...
BEGIN  
    ...
    open p_resultset for
    -- This query considers cases which are listed in a future Firm list
    select  /* getNHAMiscAppealObjectors */
            cas.case_id,
            cas.case_type||cas.case_number as case_number,
...
BEGIN
    ...
    FOR case_rec IN c_get_case_ids_from_clob LOOP
        ...
        UPDATE xhb_case_diary_fixture SET fixture_notice_required = 'N'
        WHERE  case_listing_entry_id IN (SELECT case_listing_entry_id FROM xhb_case_listing_entry WHERE case_id = TO_NUMBER(case_rec.case_id));
...
BEGIN
  OPEN p_results_out FOR
   SELECT /* get_lod_report_between_dates */ xdne.DIARY_DATE as diary_date, xc.CASE_TYPE || xc.CASE_NUMBER as CASE_NUMBER, xc.CASE_TITLE as case_title, xdne.DIARY_NOTE_TEXT as diary_note_text, xdne.CREATION_DATE 
    FROM XHB_DIARY_NOTE_ENTRY xdne , xhb_case xc
   where xdne.case_id=xc.case_id(+) AND xhb_report_pkg.is_valid_case_status(xc.case_status) = 'Y'
   AND  ( (xdne.diary_date BETWEEN p_from_date AND p_to_date AND p_to_date IS NOT NULL)
...
v_err_msg       VARCHAR2(1000);
  ...
  BEGIN
    ...
    v_step := '2'; 
  OPEN p_results_out FOR
select /* get_obw_report */ cas4.case_id, cas4.case_type||cas4.case_number as case_number, 
doc.defendant_number,
def.surname|| ' ' ||def.initials as defendant_name,
cas4.class_code,
to_char(bwh.bw_issue_date,'DD-MON-YYYY') as bw_issue_date,
GET_OBW_SOL_NAME(doc.defendant_on_case_id) as solicitor_firm_name,
doc.ptiurn,
to_char (sysdate, 'DD-MON-YYYY') as today_date
from xhb_case cas4, xhb_defendant def, xhb_defendant_on_case doc, xhb_bw_history bwh
...
and is_valid_case_status(cas4.case_status) = 'Y'
...
BEGIN  
    ...
    select /* getRUMO_Rpt */
           mot.monetary_order_tracking_id, ct.court_name as court_name, getAddress(ct.address_id) as ct_address, getPhoneNum(ct.address_id) as ct_phone, ct2.court_full_name as collect_court_name, getAddress(ct2.address_id) as collect_court_address, 
           ...
           to_char(df.date_of_birth, 'DD-MON-YYYY') as dob,
           to_char(mot.order_date, 'DD-MON-YYYY') as order_date,
           case when mot.fined is null then '' else to_char(mot.fined,'L9,999,999,999.00') end  as fined,
           case when mot.compensation is null then '' else to_char(mot.compensation,'L9,999,999,999.00') end as compensation,
           case when mot.costs is null then '' else to_char(mot.costs,'L9,999,999,999.00')  end as costs,
    ...
    and    is_valid_case_status(case.case_status) = 'Y'
...
v_err_msg  VARCHAR2(1000);
   BEGIN
     OPEN p_results_out FOR 
      ...
      xa1.ADDRESS_1 || '|' || xa1.ADDRESS_2 || '|' || xa1.ADDRESS_3 || '|' || xa1.ADDRESS_4 || '|' || xa1.TOWN || '|' || xa1.COUNTY || '|' || xa1.POSTCODE AS courttoaddress,
      xa5.ADDRESS_1 || ' ' || xa5.ADDRESS_2 || '|' || xa5.ADDRESS_3 || '|' || xa5.ADDRESS_4 || '|' || xa5.TOWN || ' ' || xa5.COUNTY || ' ' || xa5.POSTCODE AS courtfromaddress,
      CASE WHEN xrpa.DX_REF IS NOT NULL
           THEN xrpa.DX_REF 
           ELSE xa2.ADDRESS_1 || '|' || xa2.ADDRESS_2 || '|' || xa2.ADDRESS_3 || '|' || xa2.ADDRESS_4 || '|' || xa2.TOWN || '|' || xa2.COUNTY || '|' || xa2.POSTCODE || '|' || xcd1.CONTACT_VALUE END AS prosecutoraddress,
      xa3.ADDRESS_1 || '|' || xa3.ADDRESS_2 || '|' || xa3.ADDRESS_3 || '|' || xa3.ADDRESS_4 || '|' || xa3.TOWN || '|' || xa3.COUNTY || '|' || xa3.POSTCODE AS defaddress,
      
      CASE WHEN xrs.DX_REF IS NOT NULL
           THEN xrs.DX_REF 
           ELSE xa4.ADDRESS_1 || '|' || xa4.ADDRESS_2 || '|' || xa4.ADDRESS_3 || '|' || xa4.ADDRESS_4 || '|' || xa4.TOWN || '|' || xa4.COUNTY || '|' || xa4.POSTCODE || '|' || xcd2.CONTACT_VALUE END AS soladdress,
...
FUNCTION GET_EARLIEST_LISTING_DATE(p_case_id IN xhb_case.CASE_ID%TYPE 
  ...
  BEGIN
   ...
   SELECT /* get_earliest_listing_date 1 */ 
          MIN(xcdf2.LISTING_DATE) INTO listing_date1
   ...
   SELECT /* get_earliest_listing_date 2 */
         MIN(xcol2.TIME_LISTED) INTO listing_date2
...
FUNCTION GET_DIARY_NOTES_LIST(p_case_id IN xhb_case.case_id%TYPE) RETURN CLOB IS
    ...
    SELECT /* get_diary_notes_list */ CASE xrld.REF_DATA_VALUE
        WHEN 'HN' THEN 1
        WHEN 'IN' THEN 2
        ELSE 3
    END AS sort_order,
    ...
    ORDER BY sort_order, xde.CREATION_DATE;
...
CURSOR history_list_c IS
 SELECT /* get_list_history */ subquery.* FROM  
         (SELECT xcol.CASE_ON_LIST_ID,
                NULL CASE_DIARY_FIXTURE_ID,
                xcol.TIME_LISTED as LIST_START_DATE,
                xlluv.LIST_END_DATE,
                xrld.REF_DATA_VALUE as LIST_TYPE,
                xrht.HEARING_TYPE_DESC as HEARING_DESC,
                xlluv.LIST_NUMBER,
                xlluv.LAST_UPDATE_DATE,
                TO_CHAR(xcol.DATE_OF_REMOVAL, 'DD-MON-YYYY') as DATE_OF_REMOVAL,
                NVL(xrsc.DE_CODE,xcol.REASON_FOR_REMOVAL) as REASON_FOR_REMOVAL
          FROM XHB_CASE_ON_LIST xcol,
               XHB_SITTING_ON_LIST xsol,
          ...
          AND xrsc.REF_SYSTEM_CODE_ID(+) = xcol.VACATION_PRE_DEFINED_RSON_ID
          UNION ALL
          SELECT NULL CASE_ON_LIST_ID,
                ...
                TO_CHAR(xcdf.DATE_VACATED, 'DD-MON-YYYY') as DATE_OF_REMOVAL,
  ...
  FOR history_list_row IN history_list_c
   LOOP
    IF history_list_row.DATE_OF_REMOVAL IS NULL AND history_list_row.LIST_TYPE <> 'Fixture' THEN
      history_list:=history_list || history_list_row.LIST_TYPE || ' ' || LIST_SUFFIX || ' ' || TO_CHAR(history_list_row.LIST_START_DATE, 'DD-MON-YYYY') || ' ' || history_list_row.HEARING_DESC || '|';
    ELSIF history_list_row.DATE_OF_REMOVAL IS NULL AND history_list_row.LIST_TYPE = 'Fixture' THEN
      history_list:=history_list || history_list_row.LIST_TYPE || ' ' || TO_CHAR(history_list_row.LIST_START_DATE, 'DD-MON-YYYY') || ' ' || history_list_row.HEARING_DESC || '|';
...
FUNCTION is_valid_RREC_Case (p_case_id  xhb_case.case_id%type) RETURN VARCHAR2 IS
    ...
    v_case_status   xhb_case.case_status%TYPE;

BEGIN
    SELECT NVL(case_type, '-'), NVL(class_code, 0), NVL(receipt_type, '-'), NVL(case_sub_type, '-'), case_status
    INTO v_case_type, v_class_code, v_receipt_type, v_sub_type, v_case_status
    FROM xhb_case
    WHERE case_id = p_case_id;
    
    IF is_valid_case_status(v_case_status) = 'Y' THEN
        -- Valid Case Status, check the case type specific rules
...
FUNCTION is_dealtwith_RREC_Case (p_case_id    xhb_case.case_id%type 
    ...
    Select /* is_dealtwith_RREC_Case */ count(xv.VERDICT_ID)
    ...
    CURSOR /* is_dealtwith_RREC_Case */ c_verdict_with_end IS
    ...
    CURSOR /* is_dealtwith_RREC_Case */ c_exported_at_start IS
    ...
    CURSOR /* is_dealtwith_RREC_Case */ c_exported_at_end IS
    Select count(doc.defendant_on_case_id)
    From   xhb_defendant_on_case doc
    ...
    CURSOR /* is_dealtwith_RREC_Case */ c_exported_in_period IS
    Select count(doc.defendant_on_case_id)
    From   xhb_defendant_on_case doc
    ...
    CURSOR /* is_dealtwith_RREC_Case */ c_bw_at_start IS
    SELECT COUNT(*)
    ...
    CURSOR /* is_dealtwith_RREC_Case */ c_bw_at_end IS
    SELECT COUNT(*)
    FROM xhb_defendant_on_case xdoc, xhb_bw_history xbh
    WHERE xdoc.case_id = p_case_id
    AND NVL(xdoc.obs_ind,'N') = 'N'
    ...
    CURSOR /* is_dealtwith_RREC_Case */ c_bw_in_period IS
    SELECT COUNT(*)
    FROM xhb_defendant_on_case xdoc, xhb_bw_history xbh
    WHERE xdoc.case_id = p_case_id
    AND NVL(xdoc.obs_ind,'N') = 'N'
...
FUNCTION is_benchwarrant_RREC_Case (p_case_id    xhb_case.case_id%type 
    ...
    SELECT /* is_benchwarrant_RREC_Case */ doc.defendant_on_case_id, doc.date_exported, bwh.bw_history_id, bwh.bw_issue_date, bwh.bw_end_date
...
BEGIN  
    ...
    ELSE 
        ...
        v_start_date   := v_end_date - 6;  -- The previous Sunday
    ...
    OPEN   p_resultset FOR  
        select /* get_rrec_detail */ 7 as section_num, cs.court_site_name, get_RREC_CaseType_Hdr(cas.case_id) as case_type, cas.case_number as case_number, get_RREC_CaseType_Subhdr(cas.case_id) as case_subhdg, cas.case_id
  ...
  **************************************************************************************/
    ...
    DBMS_OUTPUT.PUT_LINE('get_RREC_Summary: step ' || v_step || '  start date = ' || v_start_date || ', end date = ' || v_end_date);
      ...
      open p_resultset for
        ...
        select /* get_RREC_summary */ 1 as sort_order, cs.court_site_name, get_RREC_CaseType_Hdr(cas.case_id) as case_type, get_RREC_CaseType_Subhdr(cas.case_id) as case_subhdg,
...
END get_RREC_summary;
  ...
  **************************************************************************************/
  ...
  BEGIN
      ...
      select /* get_rrca_composite_casenum */ 
             count(doc.defendant_on_case_id) 
      ...
      IF v_deft_count > 0 THEN 
          ...
          SELECT /* get_rrca_composite_casenum */ 
                 '*' || cas.case_type || To_Char(cas.case_number) || ' - ' || Min(doc.defendant_number)
  ...
  **************************************************************************************/
  ...
  BEGIN
      ...
      -- We need to know if there are any Bench Warrants executed for this case before the end date
      SELECT /* get_rrca_waiting_from_date */
             COUNT(doc.defendant_on_case_id) 
      INTO   v_deft_count      -- For a case to be open at the start of the period, it has to 
      FROM   xhb_defendant_on_case doc, xhb_bw_history bwh           -- have at least one defendant unexported at the start date.
      WHERE  NVL(doc.date_exported, p_end_date) >= p_end_date        -- We map null values to the end of the period rather than deal with the nulls.
      ...
      IF v_deft_count > 0 THEN 
          ...
          SELECT /* get_rrca_waiting_from_date */
                 MAX(bwh.bw_end_date)                                    -- The most recent if there is more than one.
          INTO   v_return_date 
          FROM   xhb_defendant_on_case doc, xhb_bw_history bwh  
          WHERE  NVL(doc.date_exported, p_end_date) >= p_end_date        -- We map null values to the end of the period rather than deal with the nulls.
                                                                         -- We are looking for the date_exported to be null up to the end of the period.
...
BEGIN  
    ...
    SELECT  /* get_rrca_detail */
            ct.court_name, cs.court_site_name, NVL(cas.sent_for_trial_date, cas.committal_date) AS trial_date, get_bc_status_ind(cas.case_id) AS status, 
...
BEGIN          
    ...
    open p_resultset for   -- 1st query = mornings
        select /* getCtRmSittingTimes */ distinct ct.court_name as court_name, ctrm.crest_court_room_no as court_room_no, cts.court_site_code as court_site_code, cts.court_site_name as court_site_name, ctrm.court_room_name as court_room_name, 
                        'am' as am_pm, 
...
FUNCTION is_valid_case_status (p_case_status XHB_CASE.CASE_STATUS%TYPE) RETURN VARCHAR2 AS

    v_valid VARCHAR2(1) := 'Y';

BEGIN 
    
    IF NVL(p_case_status,'-') IN ('H','D','I','F') THEN
        v_valid := 'N';
    END IF;

    RETURN v_valid;

END is_valid_case_status;
