...
PROCEDURE get_ogrda_order (p_results_out 	OUT SYS_REFCURSOR
	...
	BEGIN
        ...
        NVL(xc.MAG_CONVICTION_DATE, xc.ORIG_BODY_DECISION_DATE) AS magconvictiondate,
				xco.court_name AS courtname,
...
---------------------------------------------------------------------------------------
-- Name: get_owrda_order
-- Description: Order Revoking Representation for Defendants and Appellants
---------------------------------------------------------------------------------------
PROCEDURE get_owrda_order (p_results_out 	OUT SYS_REFCURSOR
						  ,p_court_id		IN XHB_COURT.COURT_ID%TYPE
						  ,p_legal_aid_order_id	IN XHB_LEGAL_AID_ORDER.LEGAL_AID_ORDER_ID%TYPE) AS
	...
	BEGIN
		...
		OPEN p_results_out FOR
			SELECT
				...
				NVL(xd.current_prison_status,'N') AS in_custody,
				get_address(xd.address_id) AS defendant_address,
				INITCAP(TO_CHAR(nvl(xc.MAG_CONVICTION_DATE, xc.ORIG_BODY_DECISION_DATE), 'dd FMMONTH yyyy')) AS mag_conviction_date,
        INITCAP(xrc.COURT_FULL_NAME) AS court_full_name,
        INITCAP(TO_CHAR(xlao.DATE_OF_REVOCATION, 'dd FMMONTH yyyy')) AS date_of_revocation,
        xlao.REASON_FOR_REVOCATION_ID AS revocation_reason,
				INITCAP(xrs.solicitor_firm_name) AS solicitor_name,
				xrs.dx_ref AS solicitor_dx,
				CASE WHEN xrs.solicitor_firm_name IS NOT NULL THEN get_address(xrs.address_id) ELSE '' END AS solicitor_address,
				INITCAP(xrsc.de_code) AS prison_name,
        get_address(xco.ADDRESS_ID) AS court_address,
        XHB_REPORT_PKG.getPhoneNum(xco.ADDRESS_ID) AS COURT_TELEPHONE
			FROM
      ...
      LEFT OUTER JOIN xhb_ref_system_code xrsc 
        ...
        AND		xrsc.court_id = p_court_id
        AND 	NVL(xrsc.obs_ind,'N') <> 'Y'
      LEFT OUTER JOIN xhb_def_on_case_ref_sol_firm xdocrsf ON xdocrsf.DEF_ON_CASE_REF_SOL_FIRM_ID = get_prev_solicitor_on_order(xdoc.defendant_on_case_id, 0)
      LEFT OUTER JOIN xhb_ref_solicitor_firm xrs ON xrs.ref_solicitor_firm_id  = xdocrsf.REF_SOLICITOR_FIRM_ID 
      WHERE xlao.LEGAL_AID_ORDER_ID = p_legal_aid_order_id;
	...
	EXCEPTION
		WHEN OTHERS THEN
			...
			v_err_msg  := SQLERRM;
			INSERT INTO XHB_REPORT_ERR_LOG(REPORTS_LOG_ID, error_message, run_date)
			VALUES (XHB_REPORTS_ERROR_LOG_SEQ.NEXTVAL, 'Exception handler raised when others in XHB_PUBLIC_REP_PKG.get_owrda_order '|| v_err_code || ' : ' ||v_err_msg, SYSDATE);
			RAISE;
...
---------------------------------------------------------------------------------------
-- Name: get_owrr_order
-- Description: Order Revoking Representation for Respondents
---------------------------------------------------------------------------------------
PROCEDURE get_owrr_order (p_results_out 	OUT SYS_REFCURSOR
	...
	BEGIN
      ...
      INNER JOIN XHB_COURT xco ON xco.COURT_ID = xc.COURT_ID 
      LEFT OUTER JOIN XHB_REF_COURT xrc ON xrc.REF_COURT_ID = xc.REF_COURT_ID	
      LEFT OUTER JOIN XHB_PROSECUTOR_REF_SOL_FIRM xprsf ON xprsf.PROSECUTOR_REF_SOL_FIRM_ID = get_prev_respondent_on_order(xcpa.case_pros_agency_id, 0)
        AND NVL(xprsf.OBS_IND,'N') <> 'Y'
      LEFT OUTER JOIN XHB_REF_SOLICITOR_FIRM xrs ON xrs.REF_SOLICITOR_FIRM_ID = xprsf.REF_SOLICITOR_FIRM_ID
			WHERE xlao.LEGAL_AID_ORDER_ID = p_legal_aid_order_id;
	...
	EXCEPTION
		WHEN OTHERS THEN
			...
			v_err_msg  := SQLERRM;
			INSERT INTO XHB_REPORT_ERR_LOG(REPORTS_LOG_ID, error_message, run_date)
			VALUES (XHB_REPORTS_ERROR_LOG_SEQ.NEXTVAL, 'Exception handler raised when others in XHB_PUBLIC_REP_PKG.get_owrr_order '|| v_err_code || ' : ' ||v_err_msg, SYSDATE);
			RAISE;
