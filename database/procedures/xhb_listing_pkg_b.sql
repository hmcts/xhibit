CREATE OR REPLACE PACKAGE BODY XHB_LISTING_PKG AS
  ...
  FUNCTION get_list_type(p_list_type_id IN XHB_REF_LISTING_DATA.REF_LISTING_DATA_ID%TYPE)
    RETURN XHB_REF_LISTING_DATA.REF_DATA_VALUE%TYPE IS
    v_list_type XHB_REF_LISTING_DATA.REF_DATA_VALUE%TYPE;
    CURSOR C_xrld IS
    SELECT xrld.REF_DATA_VALUE
      FROM XHB_REF_LISTING_DATA xrld
     WHERE xrld.REF_DATA_TYPE = 'LIST_TYPE'
       AND xrld.REF_LISTING_DATA_ID = p_list_type_id;
  BEGIN
    OPEN C_xrld;
    FETCH C_xrld INTO v_list_type;
    CLOSE C_xrld;
    RETURN v_list_type;
  END get_list_type;
  ...
  PROCEDURE find_cases_for_list_results( p_results_out   OUT SYS_REFCURSOR,
    ...
    SELECT DISTINCT
				...
				CASE
           WHEN XCOL.CRACKED_INEFFECTIVE_ID != 0 AND subqry.CASE_TYPE = 'T' AND XRHT.HEARING_TYPE_CODE IN ('TRL','TFL','TBK') THEN XRCE.DESCRIPTION
           WHEN XCOL.case_on_list_id is null AND subqry.CASE_TYPE = 'T' AND XRHT.HEARING_TYPE_CODE IN ('TRL','TFL','TBK') THEN 'Not on List'
					 WHEN XCOL.case_on_list_id is not null AND subqry.CASE_TYPE = 'T' AND XRHT.HEARING_TYPE_CODE IN ('TRL','TFL','TBK') THEN 'Missing'
					 ELSE '' END AS EFFECTIVE_CRACKED_INEFFECTIVE,
    ...
    SELECT	XC.CASE_ID,
				...
				NVL(XSH.SEQUENCE_NO,-1) AS HEARING_SEQUENCE_NO,
				XS.IS_FLOATING,
		...
		AND		NVL(XC.CASE_STATUS,'-') NOT IN ('D','I','F','H')
		...
		AND		NVL(XCR.OBS_IND(+), 'N') = 'N') subqry,
      ...
      XHB_REF_CRACKED_EFFECTIVE XRCE
		...
		AND XCOL.HEARING_TYPE_ID(+) = subqry.REF_HEARING_TYPE_ID
		AND NVL(XCOL.OBS_IND(+),'N') = 'N'
    ...
    AND TRUNC(XCOL.TIME_LISTED(+)) = p_list_date
		ORDER BY subqry.COURT_SITE_CODE, subqry.IS_FLOATING, subqry.COURT_ROOM_NO, subqry.SITTING_SEQUENCE_NO, subqry.HEARING_SEQUENCE_NO;
  ...
  PROCEDURE get_cases_by_filter(p_results_out OUT SYS_REFCURSOR,
    ...
    END_OF_DAY DATE := TRUNC(SYSDATE)+(86399/86400);
  BEGIN
    OPEN p_results_out FOR
    SELECT  xc.case_id
    FROM  XHB_CASE xc,
    ...
    AND    xcle.case_id(+) = xc.case_id
    AND    NVL(xcle.OBS_IND(+),'N') = 'N'
    AND    NVL(fixtures_exist_YN(xcle.case_listing_entry_id,END_OF_DAY),'N') = 'N'
    AND    NVL(case_on_list_exists_YN(xc.case_id,END_OF_DAY),'N') = 'N'
        ...
        OPEN p_results_out FOR
         ...
         (SELECT xcol.CASE_ON_LIST_ID,
          ...
          SELECT NULL CASE_ON_LIST_ID,
                ...
                xcdf.LISTING_DATE as TIME_LISTED,
  ...
  END get_fixture_count;
	...
	**************************************************************************************/
	...
	BEGIN
		RETURN CASE WHEN SUBSTR(xhb_report_pkg.get_bc_status_ind(p_case_id => p_case_id),1,1) = p_bc_status THEN 'Y' ELSE 'N' END;
  ...
  FUNCTION get_existing_list_id(p_court_id     IN  XHB_LIST.COURT_ID%TYPE,
    ...
    RETURN XHB_LIST.LIST_ID%TYPE IS
    v_result     XHB_LIST.LIST_ID%TYPE;
    v_list_type  XHB_REF_LISTING_DATA.REF_DATA_VALUE%TYPE;
    v_ref_cursor SYS_REFCURSOR;
  BEGIN
    -- Get the list type
    v_list_type := get_list_type(p_list_type_id => p_list_type_id);

    -- Get the ref cursor
    IF v_list_type IN ('Daily') THEN
       get_latest_list(
                      p_results_out  => v_ref_cursor,
                      p_court_id     => p_court_id,
                      p_list_type    => v_list_type,
                      p_diary_date   => p_start_date);
    ELSE
       get_list_in_date_range(
                      p_results_out  => v_ref_cursor,
                      p_court_id     => p_court_id,
                      p_list_type    => v_list_type,
    ...
    -- Get the list_id from the ref cursor
    FOR rec IN (SELECT xl.list_id, xl.list_start_date
                  FROM xhb_list xl
                 WHERE xl.list_id IN (
                       SELECT EXTRACTVALUE(x.COLUMN_VALUE, 'node()') rc_value
                         FROM TABLE(XMLSEQUENCE(EXTRACT(
                        (SELECT COLUMN_VALUE
                         FROM TABLE(XMLSEQUENCE(V_REF_CURSOR))),'/ROW/node()'))) x
                       WHERE x.COLUMN_VALUE.getrootelement() = 'LIST_ID')) LOOP
        -- Daily must be a specific date, firm/warned just in the range
        IF v_list_type NOT IN ('Daily') OR rec.list_start_date = p_start_date THEN
           v_result := rec.list_id;
           EXIT;
        END IF;
    END LOOP;

    RETURN v_result;
  END get_existing_list_id;
  ...
  FUNCTION get_existing_def_on_case_id(p_list_id       IN XHB_CASE_ON_LIST.LIST_ID%TYPE,
    ...
    RETURN XHB_DEF_ON_CASE_ON_LIST.DEF_ON_CASE_ON_LIST_ID%TYPE IS
      PRAGMA AUTONOMOUS_TRANSACTION;
      v_id XHB_DEF_ON_CASE_ON_LIST.DEF_ON_CASE_ON_LIST_ID%TYPE;
      CURSOR C_docol IS
      SELECT xdocol.def_on_case_on_list_id
         FROM xhb_def_on_case_on_list xdocol, xhb_case_on_list xcol
       WHERE xcol.case_id = p_case_id
            AND xcol.list_id = p_list_id
            AND xcol.time_listed = p_list_date
            AND xdocol.defendant_on_case_id = p_defendant_on_case_id
           AND xdocol.case_on_list_id =  xcol.case_on_list_id
           AND NVL(xdocol.obs_ind,'N') = 'N'
           AND NVL(xcol.obs_ind,'N') = 'N';
    BEGIN
       OPEN C_docol;
       FETCH C_docol INTO v_id;
       CLOSE C_docol;
       RETURN v_id;
    END get_existing_def_on_case_id;

	FUNCTION is_already_on_list_YN(
			p_parent_case_on_list_id	IN	XHB_CASE_ON_LIST.PARENT_CASE_ON_LIST_ID%TYPE,
			p_list_type					IN	XHB_REF_LISTING_DATA.REF_DATA_VALUE%TYPE)
	RETURN VARCHAR2 IS
		v_result_YN VARCHAR2(1);
		CURSOR C_xcol IS
		SELECT 'Y'
		  FROM xhb_case_on_list xcol_p
		 WHERE xcol_p.case_on_list_id = p_parent_case_on_list_id
		   AND EXISTS -- child exists on the required list type
			   (SELECT 1 FROM xhb_case_on_list xcol_c, xhb_list xl_c, xhb_ref_listing_data xrld
				 WHERE xcol_c.parent_case_on_list_id = xcol_p.case_on_list_id
				   AND NVL(xcol_c.obs_ind,'N') = 'N'
				   AND xl_c.list_id = xcol_c.list_id
				   AND NVL(xl_c.obs_ind,'N') = 'N'
				   AND xrld.ref_listing_data_id = xl_c.list_type_id
				   AND xrld.ref_data_type = 'LIST_TYPE'
				   AND xrld.ref_data_value = p_list_type
				   AND EXISTS -- child list is in the parent lists range
					   (SELECT 1 FROM xhb_list xl_p
						 WHERE xl_p.list_id = xcol_p.list_id
						   AND xl_c.LIST_START_DATE BETWEEN xl_p.LIST_START_DATE AND xl_p.LIST_END_DATE
						   AND NVL(xl_p.obs_ind,'N') = 'N'
						   AND ROWNUM = 1)
				   AND ROWNUM = 1)
		   AND ROWNUM = 1;
	BEGIN
		OPEN C_xcol;
		FETCH C_xcol INTO v_result_YN;
		CLOSE C_xcol;
		RETURN NVL(v_result_YN,'N');
	END is_already_on_list_YN;

	PROCEDURE get_unlisted_cases_on_list(
			p_results_out	OUT	SYS_REFCURSOR,
			p_list_id		IN	XHB_CASE_ON_LIST.LIST_ID%TYPE,
			p_reserved		IN	XHB_CASE_ON_LIST.RESERVED%TYPE) AS
	BEGIN
		OPEN p_results_out FOR
		SELECT xcol.case_on_list_id AS id,
			   xcol.time_listed time_listed_date,
			   TO_CHAR(xcol.time_listed,'HH24') time_listed_hour,
			   TO_CHAR(xcol.time_listed,'MM') time_listed_minute,
			   xcol.*
		  FROM XHB_CASE_ON_LIST xcol
		WHERE xcol.LIST_ID = p_list_id
		  AND (p_reserved IS NULL OR NVL(xcol.reserved,'N') = p_reserved)
		  AND NVL(xcol.obs_ind,'N') = 'N'
		  AND is_already_on_list_YN(xcol.case_on_list_id, 'Daily') = 'N';
	END get_unlisted_cases_on_list;
