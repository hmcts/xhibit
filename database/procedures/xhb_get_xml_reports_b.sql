...
create or replace PACKAGE BODY XHB_GET_XML_REPORTS AS
  ...
  FUNCTION get_U_case_defendant(p_case_title IN XHB_CASE.CASE_TITLE%TYPE) RETURN XMLType IS
    v_xml XMLType;
    v_field_length CONSTANT NUMBER := 35;
    TYPE v_table_type IS TABLE OF VARCHAR2(35);
    v_array v_table_type := v_table_type();
    
    PROCEDURE populate_array IS
      v_char_no NUMBER := 0;
    BEGIN
      FOR array_no IN 1..3 LOOP
          -- Add a cell to the array
          v_array.EXTEND;
          
          -- Populate the array cell
          FOR i IN 1..v_field_length LOOP
              v_char_no := v_char_no + 1;
              v_array(array_no) := v_array(array_no) || SUBSTR(p_case_title,v_char_no,1);
          END LOOP;
  ...
  BEGIN
    ...
    SELECT XMLELEMENT("cs:Defendant",
                XMLELEMENT("cs:PersonalDetails",
                   XMLFOREST(XMLFOREST(v_array(2) AS "apd:CitizenNameForename",
                                       v_array(3) AS "apd:CitizenNameForename",
                                       v_array(1) AS "apd:CitizenNameSurname"
                                       ) AS "cs:Name"
                            ,'no' AS "cs:IsMasked")
                          )--PersonalDetails
                      )--Defendant
      ...
      FROM DUAL;
    RETURN v_xml;
  END get_U_case_defendant;

  FUNCTION get_prosecutor_authority(p_prosecutor_name IN XHB_REF_PROSECUTOR_AGENCY.PROSECUTOR_NAME_3%TYPE) 
    RETURN VARCHAR2 IS 
  BEGIN
    IF  INSTR(p_prosecutor_name,'CPS') > 0 THEN 
       RETURN 'Crown Prosecution Service'; 
    ELSE 
       RETURN 'Other Prosecutor'; 
    END IF;
  END get_prosecutor_authority;
  ...
  BEGIN
    ...
    END IF;
    RETURN NVL(RTRIM(SUBSTR(CONCAT(RTRIM(p_town)||RTRIM(v_delimiter),RTRIM(p_county)), 1, 35)),'-');
  ...
  **/
  ...
  SELECT XMLAgg( XMLELEMENT("cs:CourtList", XMLATTRIBUTES(to_char(court_dates.C_DATE, 'yyyy-mm-dd') AS "SittingDate")
   ...
   (SELECT   trunc(xcol.TIME_LISTED) C_DATE , xcol.COURT_SITE_ID C_COURT_SITE_ID
       ...
       UNION
SELECT  trunc(xsol.time_listed) C_DATE , xsol.court_site_id C_COURT_SITE_ID
from xhb_sitting_on_list xsol
       WHERE xsol.LIST_ID = p_list_id
       AND  (xsol.obs_ind is null or xsol.obs_ind <> 'Y')
      GROUP BY  trunc(xsol.TIME_LISTED), xsol.COURT_SITE_ID) court_dates -- A list of all court dates on each court site for this list
  ...
  FUNCTION REPLACE_ESCAPED_XML_ENTITIES(p_string_to_escape CLOB) RETURN CLOB AS
    v_escaped_string CLOB;
    BEGIN
  ...
  END REPLACE_ESCAPED_XML_ENTITIES;
  ...
  **/
  ...
  BEGIN
                SELECT  XMLELEMENT("DLData"
                              ...
                              ,(SELECT XMLAgg(XMLELEMENT("CourtSite", ESCAPE_BROKER_RELEASE_CHARS(xcrts.COURT_SITE_CODE) || '|' || ESCAPE_BROKER_RELEASE_CHARS(xcrts.COURT_SITE_NAME)))
                                        ...
                                        AND xcrts.COURT_SITE_ID IN (SELECT xcol.COURT_SITE_ID
                                                                    ...
                                                                    AND (xcol.obs_ind is null or xcol.obs_ind <> 'Y')))
                              ,(SELECT XMLAgg(XMLELEMENT("CourtRoom", ESCAPE_BROKER_RELEASE_CHARS(xcs.COURT_SITE_CODE) || '|' || ESCAPE_BROKER_RELEASE_CHARS(xcr.CREST_COURT_ROOM_NO)))
                              ...
                              ,(SELECT XMLAgg(XMLELEMENT("Case", xc.CASE_TYPE || xc.CASE_NUMBER || '|' || xc.CASE_TYPE || '|' || xrc.CREST_CODE || '|' || xc.PROS_AGENCY_REFERENCE || '|' || ESCAPE_BROKER_RELEASE_CHARS(xc.INDICTMENT_INFO_1) || '|' || ESCAPE_BROKER_RELEASE_CHARS(xc.INDICTMENT_INFO_2) || '|' ||
                                                                 ESCAPE_BROKER_RELEASE_CHARS(xc.INDICTMENT_INFO_3) || '|' || ESCAPE_BROKER_RELEASE_CHARS(xc.INDICTMENT_INFO_4) || '|' || ESCAPE_BROKER_RELEASE_CHARS(xc.INDICTMENT_INFO_5) || '|' || ESCAPE_BROKER_RELEASE_CHARS(xc.INDICTMENT_INFO_6) || '|' ||
                                                                 ESCAPE_BROKER_RELEASE_CHARS(CASE WHEN xc.CASE_TYPE = 'B' THEN xc.BAIL_MAG_CODE ELSE xc.CASE_TITLE END) || '|' || xc.MAGISTRATES_CASE_REF || '|' || ESCAPE_BROKER_RELEASE_CHARS(xc.CASE_TITLE) || '|' || xc.CASE_SUB_TYPE))
                              ...
                              ,(SELECT XMLAgg(XMLELEMENT("Hearing", xc.CASE_TYPE || xc.CASE_NUMBER || '|' || xc.CASE_TYPE || '|' || xrht.HEARING_TYPE_CODE || '|' || ESCAPE_BROKER_RELEASE_CHARS(xrht.HEARING_TYPE_DESC) || '|' || xrht.CATEGORY ))
                              ...
                              ,(SELECT XMLAgg(XMLELEMENT("Sitting",'0|F||' || TO_CHAR(xl.LIST_START_DATE, 'YYYY-MM-DDHH24:MI') || '||||||' || xl.LIST_ID || '|0|' || min_xcr.CREST_COURT_ROOM_NO || '|' || xcs.COURT_SITE_CODE ))
                                              FROM XHB_COURT_SITE xcs,
                                                   (SELECT MIN(xcr.CREST_COURT_ROOM_NO) AS CREST_COURT_ROOM_NO, xcr.COURT_SITE_ID
                                                    FROM XHB_COURT_ROOM xcr
                                                    WHERE (xcr.obs_ind is null or xcr.obs_ind <> 'Y')
                                                    GROUP BY xcr.COURT_SITE_ID) min_xcr
                                              WHERE xcs.COURT_SITE_ID IN  (SELECT xcol.COURT_SITE_ID FROM XHB_CASE_ON_LIST xcol
                                                                            WHERE xcol.LIST_ID = xl.LIST_ID
                                                                            AND  xcol.FLOATER_CASE = 'Y'
                                                                            AND (xcol.obs_ind is null or xcol.obs_ind <> 'Y'))
                                              AND min_xcr.COURT_SITE_ID = xcs.COURT_SITE_ID)
                              , GET_INTERNAL_DL_SCH_HEARINGS(xl.LIST_ID)
                              ,(SELECT XMLAgg(XMLELEMENT("Defendant",xd.CREST_DEFENDANT_ID || '|' || ESCAPE_BROKER_RELEASE_CHARS(xd.FIRST_NAME) || '|' || ESCAPE_BROKER_RELEASE_CHARS(xd.MIDDLE_NAME) || '|' || ESCAPE_BROKER_RELEASE_CHARS(xd.SURNAME) || '|' || ESCAPE_BROKER_RELEASE_CHARS(xd.INITIALS) || '|' || xd.DATE_OF_BIRTH || '|' || xd.GENDER || '|' || xd.PRISON_ID || '|' ||
                                                                    ESCAPE_BROKER_RELEASE_CHARS(defendant_address.ADDRESS_1) || '|' || ESCAPE_BROKER_RELEASE_CHARS(defendant_address.ADDRESS_2) || '|' || ESCAPE_BROKER_RELEASE_CHARS(defendant_address.ADDRESS_3) || '|' || ESCAPE_BROKER_RELEASE_CHARS(defendant_address.ADDRESS_4) || '|' || ESCAPE_BROKER_RELEASE_CHARS(defendant_address.POSTCODE) || '|' || ESCAPE_BROKER_RELEASE_CHARS(defendant_address.TOWN) || '|' || ESCAPE_BROKER_RELEASE_CHARS(defendant_address.COUNTY)))
                              ...
                              ,(SELECT XMLAgg(XMLELEMENT("DefendantOnCase", xc.CASE_TYPE || xc.CASE_NUMBER || '|' || xc.CASE_TYPE || '|' || xd.CREST_DEFENDANT_ID || '||' || ESCAPE_BROKER_RELEASE_CHARS(xdoc.PNC_ID) || '|' || ESCAPE_BROKER_RELEASE_CHARS(xdoc.DEFENDANT_NUMBER) || '|' || xdoc.IS_JUVENILE))
                                            ...
                                            AND xdoc.DEFENDANT_ID = xd.DEFENDANT_ID)
                              ,(SELECT XMLAgg(XMLELEMENT("ScheduledHearingDefendant", xd.CREST_DEFENDANT_ID || '|' || xc.CASE_TYPE || xc.CASE_NUMBER || '|' || xc.CASE_TYPE || '|' ||  NVL(TO_CHAR(xcr.CREST_COURT_ROOM_NO), min_xcr.CREST_COURT_ROOM_NO) || '|' || NVL(xrj.CREST_JUDGE_ID,0) || '|' || ESCAPE_BROKER_RELEASE_CHARS(xrht.HEARING_TYPE_CODE) || '|' ||
                                                                                      ESCAPE_BROKER_RELEASE_CHARS(xcs.COURT_SITE_CODE) || '|' || NVL(xsol.SITTING_NUMBER,0) || '|' || ESCAPE_BROKER_RELEASE_CHARS(xrht.HEARING_TYPE_DESC) || '|' || ESCAPE_BROKER_RELEASE_CHARS(xrht.CATEGORY)))
  ...
  IF v_xml_length = 0 THEN
    RAISE x_no_file;
  ELSE
    RETURN REPLACE_ESCAPED_XML_ENTITIES(v_return_xml);
  END IF;
  ...
  BEGIN
    SELECT XMLAgg(
                  XMLELEMENT("Sitting", xsol.SITTING_NUMBER || '|' || 'T' || '|' || TO_CHAR(xsol.TIME_LISTED, 'HH24:MI am') || '|' || TO_CHAR(xsol.TIME_LISTED, 'YYYY-MM-DDHH24:MI') ||  '|'
                                       || ESCAPE_BROKER_RELEASE_CHARS(xsol.LIST_NOTE_TEXT) || '|' || ESCAPE_BROKER_RELEASE_CHARS(xsol.JP1) || '|' || ESCAPE_BROKER_RELEASE_CHARS(xsol.JP2) || '|' || ESCAPE_BROKER_RELEASE_CHARS(xsol.JP3) || '|' || ESCAPE_BROKER_RELEASE_CHARS(xsol.JP4) || '|'
  ...
  BEGIN
    SELECT XMLAgg(
                  XMLELEMENT("ScheduledHearing",xcol.SEQ_NO || '|' || CASE WHEN(xrsc.DE_CODE IS NOT NULL) THEN ESCAPE_BROKER_RELEASE_CHARS(xrsc.DE_CODE) ||  TO_CHAR(xcol.TIME_LISTED, 'HH24:MI am') END ||  '|' ||  TO_CHAR(xcol.TIME_LISTED, 'YYYY-MM-DDHH24:MI') || '|' || 
                  CASE WHEN to_char(xcol.time_listed,'HH24:MI') = '00:00' OR xcol.time_listed is null THEN to_char(xsol.time_listed,'HH24:MI am')
                ELSE  to_char(xcol.time_listed,'HH24:MI am')
                END || '|'
                                                || SUBSTR(ESCAPE_BROKER_RELEASE_CHARS(xcol.LIST_NOTE_TEXT) || CASE WHEN(xcol.LIST_NOTE_TEXT IS NOT NULL AND xrld.REF_DATA_VALUE IS NOT NULL) THEN ',  ' END || ESCAPE_BROKER_RELEASE_CHARS(xrld.REF_DATA_VALUE), 1, 78) || '|' || xc.CASE_TYPE || xc.CASE_NUMBER || '|' || xc.CASE_TYPE || '|' || xrht.HEARING_TYPE_CODE || '|'
                                                || NVL(TO_CHAR(xcr.CREST_COURT_ROOM_NO), min_xcr.CREST_COURT_ROOM_NO) || '|' || ESCAPE_BROKER_RELEASE_CHARS(xcs.COURT_SITE_CODE) || '|' || NVL(xrj.CREST_JUDGE_ID, 0) || '|' || NVL(xsol.SITTING_NUMBER, 0) || '|' ||  ESCAPE_BROKER_RELEASE_CHARS(xrht.HEARING_TYPE_DESC) || '|' || ESCAPE_BROKER_RELEASE_CHARS(xrht.CATEGORY)
  ...
  **/
  ...
  SELECT  '<?xml version="1.0" encoding="UTF-8"?>' ||
              XMLELEMENT("cs:RunningList" , XMLATTRIBUTES('http://www.courtservice.gov.uk/schemas/courtservice' AS "xmlns:cs"
                  ...
                  , GET_CROWN_COURT_RL(p_court_id) --CrownCourt
      ...
      /*************************************************Trial cases fields start here***************************************************/
                , XMLFOREST((SELECT XMLAGG(
                                XMLELEMENT("cs:Case",
                                      ...
                                      , CASE WHEN xc.PRELIMINARY_DATE_OF_HEARING IS NOT NULL THEN
                                          XMLELEMENT("cs:Hearing",
                                                    XMLFOREST('Preliminary Hearing' AS "cs:HearingDescription"
                                                              , xc.PRELIMINARY_DATE_OF_HEARING AS "cs:HearingDate")) END
   ...
   SELECT XMLELEMENT("cs:CaseArrivedFrom"
                     , XMLELEMENT("cs:OriginatingCourt"
                          ...
                          ,XMLELEMENT("cs:CourtHouseCode" ,XMLATTRIBUTES(NVL(xmcrt.COURT_SHORT_NAME, '---') AS "CourtHouseShortName"), LPAD(xmcrt.CREST_CODE, 4, 0))--CourtHouseCode
                           ...
                           ,XMLELEMENT("cs:CourtHouseAddress", XMLFOREST(NVL(RTRIM(xaddm.ADDRESS_1), '-') AS "apd:Line"
                                                                        ,NVL(RTRIM(xaddm.ADDRESS_2), '-') AS "apd:Line"
                                                                        ,NVL(RTRIM(xaddm.ADDRESS_3), '-') AS "apd:Line"
                                                                        ,NVL(RTRIM(xaddm.ADDRESS_4), '-') AS "apd:Line"
                                                                        ,GET_TOWN_AND_COUNTY(xaddm.TOWN,xaddm.COUNTY) AS "apd:Line")
                              )--CourtHouseAddress
                             ,XMLFOREST(xmcrt.dx_ref AS "cs:CourtHouseDX")
 ...
 FUNCTION GET_WARNED_LIST(p_list_id XHB_LIST.LIST_ID%TYPE, v_unique_id VARCHAR, p_annotated INTEGER,
   ...
   BEGIN
                       SELECT '<?xml version="1.0" encoding="UTF-8"?>' ||
                              XMLELEMENT("cs:WarnedList"
                                  ...
                                  , GET_LIST_HEADER(xl.LIST_ID) --ListHeader
                                  , GET_CROWN_COURT_RL(xl.COURT_ID)
                                  ...
                                  , XMLELEMENT("cs:CourtLists"
                                      ,(SELECT XMLAgg(XMLELEMENT("cs:CourtList"
                                                                ,XMLELEMENT("cs:CourtHouse", XMLConcat (XMLELEMENT( "cs:CourtHouseType", 'Crown Court')
                                                                    ,XMLELEMENT("cs:CourtHouseCode", NVL(xcrts.CREST_COURT_ID, xcrt.CREST_COURT_ID))
                                                                    ,XMLELEMENT("cs:CourtHouseName", 'at ' || xcrts.LIST_NAME)
                                                                    ,XMLFOREST(NVL(xcrts.SITE_GROUP, xcrts.COURT_SITE_ID) AS "cs:Description")) --CrownCourt                                                                   
                                                                )--CourtHouse
                                                ,(SELECT XMLAgg(XMLELEMENT("cs:WithFixedDate",XMLATTRIBUTES(xrht.HEARING_TYPE_CODE AS "HearingType")
                                                    ...
                                                    , XMLELEMENT("cs:Cases"
                                                        ,XMLELEMENT("cs:Case"
                                                                          ...
                                                                          , XMLELEMENT("cs:Hearing"
                                                                                      ...
                                                                                      ,CASE WHEN p_annotated = 1 THEN get_highlight_note(xc.CASE_ID)
                                                                                            WHEN p_annotated = 0 THEN to_char(xrht.LIST_SEQUENCE) END AS "cs:ListNote"
																					  ,CASE WHEN p_annotated = 1 THEN get_interpreter_note(xc.CASE_ID) END AS "cs:InterpreterNote"
                                                                                      ))--Hearing
                                                                          ,(SELECT XMLAgg( XMLELEMENT("cs:Defendants",
                                                                                          get_defendant(xdoc.DEFENDANT_ON_CASE_ID, 0, 1, 1))order by xdoc.defendant_number)
																		  ...
																		  ,CASE WHEN p_annotated = 1 AND xc.case_type = 'T' THEN get_charges_info(xc.case_id) END	-- charge information for T cases
																		  ,CASE WHEN p_annotated = 1 THEN get_required_judge(xc.case_id) END

																		 ,XMLFOREST(xc.CLASS_CODE AS "cs:CaseClassNumber")
                                                                    )--Case
                                                            )--Cases
                                                          ,CASE WHEN p_annotated = 1 THEN get_fixture_note(xc.CASE_ID,xl.LIST_START_DATE, xl.LIST_END_DATE)  END
                                               ...
                                               ,(SELECT XMLAgg(XMLELEMENT("cs:WithoutFixedDate",XMLATTRIBUTES(xrht.HEARING_TYPE_CODE AS "HearingType")
                                                  ,XMLAgg(XMLELEMENT("cs:Fixture"
                                                          , CASE WHEN p_annotated = 1 THEN get_other_notes(xc.CASE_ID, p_include_standard_notes, p_include_priority_notes, p_include_restricted_notes)
                                                          ...
                                                          , XMLELEMENT("cs:Cases"
                                                             ,XMLELEMENT("cs:Case"
                                                                 ...
                                                                 ,(SELECT XMLAgg( XMLELEMENT("cs:Defendants",
                                                                                 get_defendant(xdocol.DEFENDANT_ON_CASE_ID, 0, 1, 1)) order by xdoc.defendant_number)
                                                          ...
                                                          ,CASE WHEN p_annotated = 1 THEN get_default_note(xc.CASE_ID)  END
                                                            ,(get_linked_cases(xc.CASE_ID))AS "cs:LinkedCases"
  ...
  FUNCTION GetChargeCount (p_case_id IN xhb_case.case_id%TYPE)
  RETURN NUMBER
  IS
  ...
  AND (obs_ind is null or obs_ind <> 'Y')
  AND charge_type<>'G'
  ;

   RETURN v_charge_count;
  ...
  */
  ...
  <cs:CrownCourt>
		...
		<cs:CourtHouseAddress>
			<apd:Line>747 Snaresbrook Drive</apd:Line>
			<apd:Line>87 Dirrapuwd Dcrr</apd:Line>
			<apd:Line>Isnershire</apd:Line>
  ...
  BEGIN
   SELECT XMLELEMENT("cs:CrownCourt", XMLConcat (XMLELEMENT( "cs:CourtHouseType", 'Crown Court')
                                                                     ,XMLELEMENT("cs:CourtHouseCode",XMLATTRIBUTES(NVL(xcrt.SHORT_NAME, '---') AS "CourtHouseShortName"), xcrt.CREST_COURT_ID)
                                                                     ,XMLELEMENT("cs:CourtHouseName", xcrt.court_name)
                                                                     , XMLELEMENT("cs:CourtHouseAddress", XMLFOREST(NVL(RTRIM(xadc.ADDRESS_1), '-') AS "apd:Line"
                                                                                                                ,NVL(RTRIM(xadc.ADDRESS_2), '-') AS "apd:Line"
                                                                                                                ,NVL(RTRIM(xadc.ADDRESS_3), '-') AS "apd:Line"
                                                                                                                ,NVL(RTRIM(xadc.ADDRESS_4), '-') AS "apd:Line"
                                                                                                                ,GET_TOWN_AND_COUNTY(xadc.TOWN,xadc.COUNTY) AS "apd:Line"
                                                                                                                ,validate_postcode(xadc.POSTCODE) AS "apd:PostCode"
                                                                                                                )
                                                                                 )
    ...
    FROM xhb_court xcrt
     ...
     AND   (xcdp.contact_type(+)) = 'TEL'
     AND   xadc.address_id = xcdd.address_id (+)
     AND   (xcdd.contact_type(+)) = 'FAX';

     RETURN crownCourt;
  END GET_CROWN_COURT;

/*
Used for Running list as court house code should also have the shortname and various
other changes that aren't on other lists */
FUNCTION GET_CROWN_COURT_RL(p_court_id XHB_COURT.COURT_ID%TYPE) RETURN XMLTYPE IS
  crownCourt XMLTYPE;
  BEGIN
   SELECT XMLELEMENT("cs:CrownCourt", XMLConcat (XMLELEMENT( "cs:CourtHouseType", 'Crown Court')
                                                                     ,XMLELEMENT("cs:CourtHouseCode",XMLATTRIBUTES(NVL(xcrt.SHORT_NAME, '---') AS "CourtHouseShortName"), xcrt.CREST_COURT_ID)
                                                                    ...
                                                                    , XMLFOREST(xcrt.DX_REF as "cs:CourtHouseDX"
                                                                              , xcdp.contact_value AS "cs:CourtHouseTelephone"
                                                                              , xcdd.contact_value AS "cs:CourtHouseFax"
                                                                              ,GET_SATELLITE_COURT_NAMES(xcrt.COURT_ID) AS "cs:Description") --CrownCourt 
    ))INTO crownCourt
    FROM xhb_court xcrt
     ,    xhb_address xadc --addressfor court
     ,    xhb_contact_detail xcdp --contact detail phone
  ...
  SELECT XMLAgg( XMLELEMENT("cs:Charge" ,XMLATTRIBUTES(NVL(xcge.CREST_CHARGE_SEQ_NO, 1) AS "IndictmentCountNumber" , xro.OFFENCE_CODE AS "CJSoffenceCode")
                           ,XMLFOREST(
                           CASE
                            WHEN xdof.crn_id is not null THEN xdof.crn_id
                            WHEN xdoc.ASN is not null THEN concat(xdoc.ASN,LPAD(xo.CREST_OFFENCE_SEQ_NO, 3, 0))
                            ELSE null
                          END AS "cs:CRN"
                                     ,CONCAT(CONCAT(xco.CREST_COURT_ID,'S'),LPAD(xo.CREST_OFFENCE_ID, 8,0)) AS "cs:CRESTchargeID"
                                     , xc.CASE_TYPE || xc.CASE_NUMBER AS "cs:CaseNumber"
                                    , XHB_FORMATTING_PKG.parse_XML_string(xro.OFFENCE_DESC) AS "cs:OffenceStatement"
                  ...
                  XHB_DEFENDANT_ON_CASE xdoc, XHB_COURT xco
                  WHERE xdof.DEFENDANT_ON_CASE_ID = p_defendant_on_case_id
                   ...
                   AND xdoc.CASE_ID = xc.CASE_ID
                   AND xc.COURT_ID = xco.COURT_ID
                   AND xdoc.DEFENDANT_ON_CASE_ID=p_defendant_on_case_id
  ...
  FUNCTION get_defendant(p_defendant_on_case_id XHB_DEFENDANT_ON_CASE.DEFENDANT_ON_CASE_ID%TYPE, p_show_public_view IN INTEGER, p_show_prison_information IN INTEGER, p_show_defendant_name INTEGER) RETURN XMLType IS
     v_return_xml XMLTYPE;
  BEGIN
    SELECT GET_DEFENDANT(p_defendant_on_case_id, p_show_public_view, p_show_prison_information, p_show_defendant_name, 0)
                          INTO v_return_xml
                          FROM DUAL;
    RETURN v_return_xml;
  END get_defendant;

  FUNCTION get_defendant(p_defendant_on_case_id XHB_DEFENDANT_ON_CASE.DEFENDANT_ON_CASE_ID%TYPE, p_show_public_view IN INTEGER, p_show_prison_information IN INTEGER, p_show_defendant_name INTEGER, include_charge_log INTEGER) RETURN XMLType IS
  defendant XMLType;
  BEGIN
  SELECT XMLELEMENT("cs:Defendant",
                   XMLELEMENT("cs:PersonalDetails",
                             XMLFOREST(XMLFOREST(CASE WHEN (p_show_public_view = 1 AND xdoc.IS_MASKED = 'Y') OR (p_show_defendant_name = 0) THEN ' ' ELSE INITCAP(xd.FIRST_NAME) END AS "apd:CitizenNameForename"
												,CASE WHEN (p_show_public_view = 1 AND xdoc.IS_MASKED = 'Y') OR (p_show_defendant_name = 0) THEN ' ' ELSE INITCAP(xd.MIDDLE_NAME) END AS "apd:CitizenNameForename"
                                                ,CASE WHEN (p_show_public_view = 1 AND xdoc.IS_MASKED = 'Y') THEN xdoc.MASKED_NAME WHEN (p_show_defendant_name = 0) THEN ' ' ELSE xd.SURNAME END AS "apd:CitizenNameSurname") "cs:Name"
                                     , xdoc.MASKED_NAME AS "cs:MaskedName"
                                     , DECODE(xdoc.is_masked, 'Y', 'yes', 'no') AS "cs:IsMasked"
                                     , XMLFOREST(xd.date_of_birth AS "apd:BirthDate"
                                                 ,CASE WHEN xd.date_of_birth IS NOT NULL THEN 'not verified' ELSE NULL END AS "apd:VerifiedBy") "cs:DateOfBirth"
                                     -- , xd.AGE
                                     , DECODE(xd.gender, 1, 'male', 2, 'female', 'unknown') AS "cs:Sex"
                                     , XMLFOREST(NVL(RTRIM(xa.ADDRESS_1), '-') AS "apd:Line"
                                                                                       ,NVL(RTRIM(xa.ADDRESS_2), '-') AS "apd:Line"
                                                                                       ,NVL(RTRIM(xa.ADDRESS_3), '-') AS "apd:Line"
                                                                                       ,NVL(RTRIM(xa.ADDRESS_4), '-') AS "apd:Line"
                                                                                       ,GET_TOWN_AND_COUNTY(xa.TOWN,xa.COUNTY) AS "apd:Line"
                                                                                       ,validate_postcode(xa.POSTCODE) AS "apd:PostCode")
                                                                                        "cs:Address"
                     ...
                     --CRO
                     ,xc.magistrates_case_ref AS "cs:MagistratesCourtRefNumber"--magistratestcourtrefnumber
                     ...
                     ,DECODE(NVL(xdoc.CURRENT_BC_STATUS,'N'), 'B', 'On bail', 'C', 'In custody', 'J', 'In custody', 'Not applicable') AS "cs:CustodyStatus")
                      ...
                      , (SELECT XMLAgg(XMLELEMENT("cs:Solicitor"
                                     , XMLELEMENT("cs:Party"
                                       , XMLELEMENT("cs:Organisation"
                                         ...
                                         , XMLELEMENT("cs:OrganisationAddress",XMLFOREST(NVL(RTRIM(solicitor_address.ADDRESS_1), '-') AS "apd:Line"
                                                                                       ,NVL(RTRIM(solicitor_address.ADDRESS_2), '-') AS "apd:Line"
                                                                                       ,NVL(RTRIM(solicitor_address.ADDRESS_3), '-') AS "apd:Line"
                                                                                       ,NVL(RTRIM(solicitor_address.ADDRESS_4), '-') AS "apd:Line"
                                                                                       ,GET_TOWN_AND_COUNTY(solicitor_address.TOWN,solicitor_address.COUNTY) AS "apd:Line")
                                                     )--OrganisationAddress
                                         ...
                                         , XMLFOREST(xdocrsf.REP_ST_DATE AS "cs:StartDate"
										  ...
										  ,(CASE
                      WHEN include_charge_log = 1 AND xc.case_type <> 'T' AND GetChargeCount(xdoc.CASE_ID)>0 AND get_charges(xdoc.DEFENDANT_ON_CASE_ID) IS NOT NULL THEN
                        XMLELEMENT("cs:Charges",XMLATTRIBUTES(GetChargeCount(xdoc.CASE_ID) as "NumberOfCharges"),get_charges(xdoc.DEFENDANT_ON_CASE_ID))
                        --show charges for all other list types for case = T apart from warned list i.e. 2
                      WHEN include_charge_log <>1 AND GetChargeCount(xdoc.CASE_ID)>0 AND get_charges(xdoc.DEFENDANT_ON_CASE_ID) IS NOT NULL THEN
                        XMLELEMENT("cs:Charges",XMLATTRIBUTES(GetChargeCount(xdoc.CASE_ID) as "NumberOfCharges"),get_charges(xdoc.DEFENDANT_ON_CASE_ID))
						END)
                        ,XMLFOREST((get_original_charges(xdoc.DEFENDANT_ON_CASE_ID) )"cs:OriginalCharges") --OriginalCharges

                        --AdditionalNotes
                        ,(CASE WHEN include_charge_log = 1 AND xc.case_type ='T' THEN
                       get_charges_info(xdoc.CASE_ID)
                       END)
          ...
          FROM XHB_DEFENDANT_ON_CASE xdoc, XHB_DEFENDANT xd, XHB_DEFENDANT_REFERENCE xdr, XHB_DEFENDANT_REFERENCE defPrisonLocation
		       ,XHB_REF_SYSTEM_CODE prisonLoc, XHB_CASE xc, XHB_ADDRESS xa
           WHERE xdoc.DEFENDANT_ON_CASE_ID = p_defendant_on_case_id
            ...
            and   xdoc.case_id=xc.case_id
            AND   xd.ADDRESS_ID = xa.ADDRESS_ID(+)
            AND ROWNUM = 1;
    RETURN defendant;
 ...
 FUNCTION get_highlight_note(p_case_id XHB_CASE.CASE_ID%TYPE) RETURN VARCHAR2 IS
  ...
  FUNCTION get_default_note(p_case_id XHB_CASE.CASE_ID%TYPE) RETURN XMLTYPE IS
  defaultNote XMLTYPE;
  BEGIN

 select XMLELEMENT("cs:DefaultNotes",
XMLAGG(XMLELEMENT("cs:Note", note.DIARY_NOTE_TEXT)))
    INTO defaultNote
    FROM (
    SELECT xdne_freetext.DIARY_NOTE_TEXT
    FROM XHB_CASE_LISTING_ENTRY xcle, XHB_DIARY_NOTE_ENTRY xdne_freetext, XHB_REF_LISTING_DATA xrld_note_type, XHB_REF_LISTING_DATA freetext_noteclass
    WHERE p_case_id = xcle.CASE_ID(+)
      AND xcle.CASE_LISTING_ENTRY_ID = xdne_freetext.CASE_LISTING_ENTRY_ID(+)
      AND NVL(xdne_freetext.OBS_IND(+), '-') <> 'Y'
      ...
      AND xdne_freetext.DIARY_NOTE_TEXT(+) IS NOT NULL
      AND xrld_note_type.REF_DATA_TYPE = 'NOTE_TYPE'
      AND xrld_note_type.REF_DATA_VALUE = 'DCN'
      AND xdne_freetext.NOTE_CLASSIFICATION_ID =  freetext_noteclass.REF_LISTING_DATA_ID (+)
       UNION
      SELECT predef_text.REF_DATA_VALUE
    FROM XHB_CASE_LISTING_ENTRY xcle, 
    XHB_DIARY_NOTE_ENTRY xdne_freetext, 
    XHB_REF_LISTING_DATA xrld_note_type, 
    XHB_REF_LISTING_DATA freetext_noteclass,
    XHB_REF_LISTING_DATA predef_text
    WHERE p_case_id = xcle.CASE_ID(+)
      AND xcle.CASE_LISTING_ENTRY_ID = xdne_freetext.CASE_LISTING_ENTRY_ID(+)
      AND NVL(xdne_freetext.OBS_IND(+), '-') <> 'Y'
      AND xdne_freetext.NOTE_TYPE_ID = xrld_note_type.REF_LISTING_DATA_ID
      AND xdne_freetext.DIARY_NOTE_PRE_DEFINED_ID IS NOT NULL
      AND xrld_note_type.REF_DATA_TYPE = 'NOTE_TYPE'
      AND xrld_note_type.REF_DATA_VALUE = 'DCN'
      AND xdne_freetext.NOTE_CLASSIFICATION_ID =  freetext_noteclass.REF_LISTING_DATA_ID (+)
      AND xdne_freetext.DIARY_NOTE_PRE_DEFINED_ID= predef_text.ref_listing_data_id(+))note;
  return defaultNote;
  END get_default_note;
  
 FUNCTION get_fixture_note(p_case_id XHB_CASE.CASE_ID%TYPE, p_list_start_date XHB_LIST.LIST_START_DATE%TYPE, p_list_end_date XHB_LIST.LIST_END_DATE%TYPE) RETURN XMLTYPE IS
  fixtureNote XMLTYPE;
  BEGIN

 select XMLELEMENT("cs:FixtureNotes",XMLAGG(
XMLFOREST(note.DIARY_NOTE_TEXT AS "cs:Note")),
XMLAGG(XMLFOREST(note.LIST_NOTE_TEXT AS "cs:Note")))
INTO fixtureNote
    FROM (
SELECT 
        xrld.REF_DATA_VALUE AS DIARY_NOTE_TEXT  ,  xcdf.LIST_NOTE_TEXT      
...
AND   xrld.REF_DATA_TYPE(+) = 'PREDEFINED_LIST_NOTE'
AND xcdf.LISTING_DATE >= p_list_start_date
AND xcdf.LISTING_DATE <= p_list_end_date
AND NVL(xcle.OBS_IND, '-') <> 'Y'
AND NVL(xcdf.OBS_IND, '-') <> 'Y'
AND NVL(xrld.OBS_IND, '-') <> 'Y')note;
return fixtureNote;
  END get_fixture_note;
 
  
  
  FUNCTION get_interpreter_note(p_case_id XHB_CASE.CASE_ID%TYPE) RETURN VARCHAR2 IS
  interpreterNote VARCHAR2(400);
  BEGIN

  SELECT xdne_freetext.CREATION_DATE || ' ' || xdne_freetext.DIARY_NOTE_TEXT
    INTO interpreterNote
    FROM XHB_CASE_LISTING_ENTRY xcle, XHB_DIARY_NOTE_ENTRY xdne_freetext, XHB_REF_LISTING_DATA xrld_note_type, XHB_REF_LISTING_DATA freetext_noteclass
    WHERE p_case_id = xcle.CASE_ID(+)
      AND xcle.CASE_LISTING_ENTRY_ID = xdne_freetext.CASE_LISTING_ENTRY_ID(+)
      AND NVL(xdne_freetext.OBS_IND(+), '-') <> 'Y'
      ...
      AND xdne_freetext.DIARY_NOTE_TEXT(+) IS NOT NULL
      AND xrld_note_type.REF_DATA_TYPE = 'NOTE_TYPE'
      AND xrld_note_type.REF_DATA_VALUE = 'IN'
      AND xdne_freetext.NOTE_CLASSIFICATION_ID =  freetext_noteclass.REF_LISTING_DATA_ID (+);
  return interpreterNote;
  END get_interpreter_note;
  ...
  otherNotes XMLTYPE;
  BEGIN
	SELECT XMLELEMENT("cs:Notes", XMLAgg( XMLELEMENT("cs:Note", (xdne.CREATION_DATE || ' ' || NVL(xrld_predefined_note.REF_DATA_VALUE, xdne.DIARY_NOTE_TEXT) ||
                                      CASE WHEN xdne.DIARY_DATE IS NOT NULL THEN ' ['||xdne.DIARY_DATE||']' END) ) ORDER BY xdne.CREATION_DATE))
    INTO otherNotes
    ...
    WHERE ((xdne.CASE_LISTING_ENTRY_ID IS NOT NULL AND
	  ...
	  AND ( CASE  
			WHEN NVL(noteclass.REF_DATA_VALUE,'Priority') = 'Priority' AND p_include_priority_notes = 1 THEN 'Y'
			WHEN NVL(noteclass.REF_DATA_VALUE,'Priority') = 'Restricted' AND p_include_restricted_notes = 1 THEN 'Y' 
			WHEN NVL(noteclass.REF_DATA_VALUE,'Priority') = 'Standard' AND p_include_standard_notes = 1 THEN 'Y'
			ELSE 'N' 
			END) = 'Y';
  ...
  SELECT XMLAgg(XMLELEMENT("cs:CaseNumber", MAX(linked_case.CASE_TYPE) ||  MAX(linked_case.CASE_NUMBER) ||
    ...
    AND linked_case.court_id = xc.court_id
 ...
 FUNCTION get_non_available_dates(p_case_id XHB_CASE.CASE_ID%TYPE, p_list_start_date XHB_LIST.LIST_START_DATE%TYPE, p_list_end_date XHB_LIST.LIST_END_DATE%TYPE) RETURN XMLType IS
  ...
  BEGIN
    SELECT XMLAgg( XMLELEMENT("cs:Defendants",
                              get_defendant(xdoc.DEFENDANT_ON_CASE_ID, 0, 0, 1, 1)) order by xdoc.defendant_number)
    INTO   defendants
    ...
    IF defendants IS NULL THEN
       SELECT XMLELEMENT("cs:Defendants",
				XMLELEMENT("cs:Defendant",
					XMLELEMENT("cs:PersonalDetails",
					   XMLFOREST(XMLFOREST('**No Deft/Applt**' AS "apd:CitizenNameSurname") AS "cs:Name"
								,'yes' AS "cs:IsMasked")
							  )--PersonalDetails
						  )--Defendant
				)--Defendants
  ...
  SELECT CASE WHEN NVL(xcol.IS_COURT_ROOM_LIST_ENTRY,'N') = 'Y' OR p_show_court_list = 0 THEN 1 ELSE 0 END INTO show_defendant_name
  FROM XHB_CASE_ON_LIST xcol
  ...
  SELECT XMLAgg(get_defendant(xdoc.DEFENDANT_ON_CASE_ID, 0, p_show_prison_information,
	CASE WHEN p_show_court_list = 0 THEN 1	-- Always display name if not the court list
		 ...
		 WHEN p_show_court_list = 1 AND show_defendant_name = 1 AND NVL(xdocol.IS_COURT_ROOM_LIST_ENTRY,'Y') = 'N' THEN 0	-- Court List and case court room list indiciator is selected but defendant is not so no name
		 ELSE 1	-- All other scenarios display name
	END)order by xdoc.defendant_number) INTO defendants
  FROM XHB_DEF_ON_CASE_ON_LIST xdocol,  XHB_DEFENDANT_ON_CASE xdoc
    ...
    IF defendants IS NULL THEN
       SELECT get_U_case_defendant(xc.case_title)
       INTO   defendants
...
/*<cs:Sitting>
					...
					<cs:Judiciary>
						<cs:Judge>
							<apd:CitizenNameTitle>MRS</apd:CitizenNameTitle>
							<apd:CitizenNameForename>GINA</apd:CitizenNameForename>
							<apd:CitizenNameSurname>SIMS</apd:CitizenNameSurname>
							<apd:CitizenNameRequestedName>GINA</apd:CitizenNameRequestedName>
  ...
  SELECT XMLAgg( XMLELEMENT("cs:Sitting",
                            XMLFOREST(xcr.crest_court_room_no AS "cs:CourtRoomNumber"
                                      ...
                                      ,XMLELEMENT("cs:Judiciary",
                                            XMLELEMENT("cs:Judge",XMLFOREST(xrj.TITLE AS "apd:CitizenNameTitle"
                                                                      , xrj.FIRST_NAME AS "apd:CitizenNameForename"
                                                                      , NVL(xrj.SURNAME, 'N/A') AS "apd:CitizenNameSurname"
                                                                      , xrj.HONOURS as "apd:CitizenNameSuffix"
                                                                      , NVL(SUBSTR(xrj.FULL_LIST_TITLE1||xrj.FULL_LIST_TITLE2||xrj.FULL_LIST_TITLE3,0,70),'N/A') AS "apd:CitizenNameRequestedName"
                                                                      , xrj.CREST_JUDGE_ID AS "cs:CRESTjudgeID"
                                                                     )
                                                )--judge
                                                ,XMLFOREST( XMLFOREST(xsol.JP1 AS "apd:CitizenNameSurname", xsol.JP1 AS "apd:CitizenNameRequestedName") "cs:Justice")
                                                ,XMLFOREST( XMLFOREST(xsol.JP2 AS "apd:CitizenNameSurname", xsol.JP2 AS "apd:CitizenNameRequestedName") "cs:Justice")
                                                ,XMLFOREST( XMLFOREST(xsol.JP3 AS "apd:CitizenNameSurname", xsol.JP3 AS "apd:CitizenNameRequestedName") "cs:Justice")
                                                ,XMLFOREST( XMLFOREST(xsol.JP4 AS "apd:CitizenNameSurname", xsol.JP4 AS "apd:CitizenNameRequestedName") "cs:Justice")
                                       )--judiciary
...
FUNCTION get_floating_cases(p_court_site_id XHB_COURT_SITE.COURT_SITE_ID%TYPE, p_list_id XHB_LIST.LIST_ID%TYPE,
  ...
  SELECT (XMLELEMENT("cs:Sitting",
             XMLFOREST(99 AS "cs:CourtRoomNumber"
                      ...
                      ,XMLELEMENT("cs:Judiciary",
                                  XMLELEMENT("cs:Judge",XMLFOREST('N/A' AS "apd:CitizenNameSurname"
                                                                  , 'N/A' AS "apd:CitizenNameRequestedName"
 ...
 SELECT(XMLAGG(XMLELEMENT("cs:Prosecution"
         , XMLATTRIBUTES(get_prosecutor_authority(xrpa.PROSECUTOR_NAME_3) AS "ProsecutingAuthority")
         , XMLFOREST(REPLACE(xrpa.PROSECUTOR_NAME_3, 'CPS', 'Crown Prosecution Service') AS "cs:ProsecutingReference"
                , XMLFOREST(xrpa.CPS_CODE AS "cs:OrganisationCode"
          ,CASE WHEN (xrpa.PROSECUTOR_NAME_3 = 'CPS') THEN
            'Crown Prosecution Service' 
            ELSE
            xrpa.PROSECUTOR_NAME_1 || ' ' || xrpa.PROSECUTOR_NAME_2 || ' ' || xrpa.PROSECUTOR_NAME_3
            END AS "cs:OrganisationName" 
                   ) "cs:ProsecutingOrganisation"
         )
  ))) INTO prosecution
...
/*Generate the Hearing XML for the specified XHB_CASE_ON_LIST_ID
        ...
        <cs:Prosecution>
          <cs:ProsecutingOrganisation>
            ...
            <cs:OrganisationAddress>
              <apd:Line>TH</apd:Line>
              <apd:Line>SGF</apd:Line>
              <apd:Line>H</apd:Line>
              <apd:Line>GFHFDGH</apd:Line>
            </cs:OrganisationAddress>
  ...
  BEGIN
   SELECT XMLELEMENT("cs:Hearing",XMLFOREST(xcol.SEQ_NO AS "cs:HearingSequenceNumber")
               ...
               ,XMLFOREST(p_case_on_list_id AS "cs:CRESThearingID")
               ,XMLFOREST( CASE WHEN xrsc.DE_CODE IS NOT NULL THEN xrsc.DE_CODE || ' ' || to_char(xcol.TIME_LISTED, 'HH:MI am') ELSE ' ' END AS "cs:TimeMarkingNote")
               ,XMLFOREST(xc.CASE_TYPE || xc.CASE_NUMBER AS "cs:CaseNumber")
               , get_prosecution(xc.CASE_ID)--Prosecution
               ,(SELECT XMLELEMENT("cs:CommittingCourt"
							...
							, XMLELEMENT("cs:CourtHouseAddress", XMLFOREST(NVL(RTRIM(xadc.ADDRESS_1), '-') AS "apd:Line"
                              ...
                              , XMLFOREST(xrcrt.DX_REF as "cs:CourtHouseDX"
                              , xcdp.contact_value AS "cs:CourtHouseTelephone"
							))
               FROM XHB_REF_COURT xrcrt, XHB_ADDRESS xadc, XHB_CONTACT_DETAIL xcdp
               WHERE xc.REF_COURT_ID = xrcrt.REF_COURT_ID
			   AND   xrcrt.address_id = xadc.address_id
			   AND   xadc.address_id = xcdp.address_id (+)
			   AND   upper(xcdp.contact_type(+)) = 'PHONE') -- "CommittingCourt"
...
FUNCTION validate_postcode(p_postcode XHB_ADDRESS.POSTCODE%TYPE) RETURN VARCHAR2 IS
BEGIN
IF REGEXP_LIKE(p_postcode,'(GIR 0AA)|^((([A-Z][0-9][0-9]?)|(([A-Z][A-HJ-Y][0-9][0-9]?)|"
			+ "(([A-Z][0-9][A-Z])|([A-Z][A-HJ-Y][0-9]?[A-Z])))) [0-9][A-Z]{2})$')

  THEN return p_postcode;
END IF;

return null;
END validate_postcode;

FUNCTION get_charges_info (p_case_id IN xhb_case.case_id%TYPE) RETURN XMLType IS
    v_xml XMLType := null;
...
ORDER BY SEQUENCE_NO;
 ...
 BEGIN
  ...
  LOOP
       charges_return := charges_return|| get_charge_row.charges_info; --For each loop iteration concatenate the charges
    END LOOP;
    if(charges_return is not null) THEN
       SELECT XMLELEMENT("cs:AdditionalNotes",charges_return)
       INTO  v_xml 
       FROM dual;
        END IF;
      RETURN v_xml;
 ...
 END get_charges_info;
	...
	FUNCTION get_required_judge (p_case_id IN xhb_case.case_id%TYPE) RETURN XMLType IS
		v_xml XMLType := null;
		judge_details VARCHAR2(400) := NULL;

	BEGIN
	
		SELECT xrj.surname || ' (' || xrj.judge_type || ')' INTO judge_details
		FROM xhb_ref_judge xrj, xhb_case_listing_entry xcle
		WHERE xcle.case_id = p_case_id
		AND NVL(xcle.obs_ind, 'N') <> 'Y'
		AND xrj.ref_judge_id = xcle.judge_id;
		
		IF judge_details IS NOT NULL THEN
			SELECT XMLELEMENT("cs:RequiredJudge",judge_details)
			INTO  v_xml 
			FROM dual;
		END IF;
		RETURN v_xml;
