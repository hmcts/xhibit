  ...
  BEGIN
   SELECT XMLELEMENT("cs:CaseArrivedFrom"
                     , XMLELEMENT("cs:OriginatingCourt"
                          ...
                          ,XMLELEMENT("cs:CourtHouseCode" ,XMLATTRIBUTES(xmcrt.COURT_SHORT_NAME AS "cs:CourtHouseShortName"),  LPAD(xmcrt.CREST_CODE, 4,0))--CourtHouseCode
                           ,XMLELEMENT("cs:CourtHouseName", xmcrt.COURT_FULL_NAME)--CourtHouseName
                           ,XMLELEMENT("cs:CourtHouseAddress", XMLFOREST(NVL(RTRIM(xaddm.ADDRESS_1),'-') AS "Line"
                                                                        ,NVL(RTRIM(xaddm.ADDRESS_2),'-') AS "Line"
                                                                        ,NVL(RTRIM(xaddm.ADDRESS_3),'-') AS "Line"
                                                                        ,NVL(RTRIM(xaddm.ADDRESS_4),'-') AS "Line"
                                                                        ,XHB_GET_XML_REPORTS.GET_TOWN_AND_COUNTY(xaddm.TOWN,xaddm.COUNTY) AS "Line"
                                                                        ,XHB_GET_XML_REPORTS.validate_postcode(xaddm.POSTCODE) AS "PostCode")
                              )--CourtHouseAddress
            ...
            SELECT'<?xml version="1.0" encoding="UTF-8"?>' ||
                    XMLELEMENT("cs:DailyList"
                                  ...
                                  ,XMLELEMENT("cs:DocumentID", XMLFOREST('Daily Distribution List ' || DECODE(xl.DRAFT_OR_FINAL, 'D', 'DRAFT', 'F', 'FINAL') || ' v' || xl.LIST_NUMBER || ' ' || xl.LIST_START_DATE AS "cs:DocumentName"
                                                                         ...
                                                                         ,XHB_GET_XML_REPORTS.to_xml_date_format(systimestamp) AS "cs:TimeStamp"
                                                                        )
                                   )
...
from xhb_sitting_on_list xsol
       ...
       GROUP BY  trunc(xsol.TIME_LISTED), xsol.COURT_SITE_ID
       ORDER BY C_DATE
       ) court_dates -- A list of all court dates on each court site for this list
   ...
   SELECT '<?xml version="1.0" encoding="UTF-8"?>' ||
            XMLELEMENT("cs:WarnedList"
                          ...
                          , XMLELEMENT("cs:WarnedListDetail"
                                        ,XMLELEMENT("cs:Text", xcrt.WL_FREE_TEXT)
                                        ,XMLFOREST('Any representation about the listing of a case should be made to the Listing Officer ' ||
                        ...
                        , XMLELEMENT("cs:CourtLists"
                                    ,(SELECT XMLAgg(XMLELEMENT("cs:CourtList"
                                                    ...
                                                    ,XMLELEMENT("cs:CourtHouse", XMLConcat (
                                                          ...
                                                          ,XMLELEMENT("cs:CourtHouseAddress", XMLFOREST(NVL(RTRIM(courtsite_address.ADDRESS_1), '-') AS "Line"
                                                                                                  ,NVL(RTRIM(courtsite_address.ADDRESS_2), '-') AS "Line"
                                                                                                  ,NVL(RTRIM(courtsite_address.ADDRESS_3), '-') AS "Line"
                                                                                                  ,NVL(RTRIM(courtsite_address.ADDRESS_4), '-') AS "Line"
                                                                                                  ,XHB_GET_XML_REPORTS.GET_TOWN_AND_COUNTY(courtsite_address.TOWN,courtsite_address.COUNTY) AS "Line"
                                                                                                  )
                                                            )
                                        ...
                                        ,(SELECT XMLAgg(XMLELEMENT("cs:WithFixedDate",XMLATTRIBUTES(xrht.hearing_type_code AS "id", xrht.hearing_type_code AS "HearingType")
                                          ,XMLAgg(XMLELEMENT("cs:Fixture"
                                            ...
                                            , XMLFOREST(xrld_predefined_note.REF_DATA_VALUE || CASE WHEN(xcdf.LIST_NOTE_TEXT IS NOT NULL AND xrld_predefined_note.REF_DATA_VALUE IS NOT NULL) THEN ',  ' END || xcdf.LIST_NOTE_TEXT AS "cs:Notes")                                                        
                                             ,XMLELEMENT("cs:Cases"
                                                ,XMLELEMENT("cs:Case"
                                                                  ...
                                                                  , XMLELEMENT("cs:Defendants"
                                                                              ,(SELECT XMLAgg(get_defendant(xdoc.DEFENDANT_ON_CASE_ID, 0, 0) order by xdoc.defendant_number)
                                              ...
                                              FROM XHB_CASE_DIARY_FIXTURE xcdf, XHB_REF_HEARING_TYPE xrht, XHB_CASE_LISTING_ENTRY xcle, XHB_CASE xc, XHB_REF_COURT originatingCourt, XHB_REF_LISTING_DATA xrld_predefined_note
                                       ...
                                       ,(SELECT XMLAgg(XMLELEMENT("cs:WithoutFixedDate",XMLATTRIBUTES(xrht.hearing_type_code AS "id", xrht.hearing_type_code AS "HearingType")
                                          ,XMLAgg(XMLELEMENT("cs:Fixture"
                                                   ...
                                                   , XMLFOREST(xrld_predefined_note.REF_DATA_VALUE || CASE WHEN(xcol.LIST_NOTE_TEXT IS NOT NULL AND xrld_predefined_note.REF_DATA_VALUE IS NOT NULL) THEN ',  ' END || xcol.LIST_NOTE_TEXT AS "cs:Notes") 
                                                   , XMLELEMENT("cs:Cases"
                                                      ,XMLELEMENT("cs:Case"
                                                         , XMLATTRIBUTES(xc.CASE_ID AS "id")
                                                         , XMLELEMENT("cs:CaseNumber", xc.CASE_TYPE || xc.CASE_NUMBER)
                                                         ...
                                                         , XMLFOREST(XMLFOREST(xrht.hearing_type_desc AS "cs:HearingDescription"
                                                          ...
                                                          ,XMLELEMENT("cs:Defendants",
                                                                      (SELECT XMLAgg(get_defendant(xdocol.DEFENDANT_ON_CASE_ID, 0, 0)order by xdoc.defendant_number)
                                                                        ...
                                                                        AND xdoc.DEFENDANT_ON_CASE_ID = xdocol.DEFENDANT_ON_CASE_ID
                                                                        AND (xdoc.OBS_IND IS NULL OR xdoc.OBS_IND <> 'Y')))--Defendants
                                                        ,GET_PROSECUTION(xc.CASE_ID)--Prosecution
                                                         -- AppealDate
                                                        --DateOfInstigation
                                          ...
                                          FROM XHB_CASE_ON_LIST xcol, XHB_REF_HEARING_TYPE xrht, XHB_CASE xc, XHB_REF_COURT originatingCourt, XHB_REF_LISTING_DATA xrld_predefined_note
  ...
  */
  ...
  BEGIN
    SELECT XMLELEMENT("cs:ListHeader",
                      XMLFOREST('Criminal' AS "cs:ListCategory"
                               ...
                               ,XHB_GET_XML_REPORTS.to_xml_date_format(NVL(xl.PUBLISH_DATE, systimestamp)) AS "cs:PublishedTime"
  ...
  BEGIN
   SELECT XMLELEMENT("cs:CrownCourt", XMLConcat (XMLELEMENT( "cs:CourtHouseType", 'Crown Court')
                                                                     ...
                                                                     , XMLELEMENT("cs:CourtHouseAddress", XMLFOREST(NVL(RTRIM(xadc.ADDRESS_1), '-') AS "Line"
                                                                                                                ,NVL(RTRIM(xadc.ADDRESS_2), '-') AS "Line"
                                                                                                                ,NVL(RTRIM(xadc.ADDRESS_3), '-') AS "Line"
                                                                                                                ,NVL(RTRIM(xadc.ADDRESS_4), '-') AS "Line"
                                                                                                                ,XHB_GET_XML_REPORTS.GET_TOWN_AND_COUNTY(xadc.TOWN,xadc.COUNTY) AS "Line"
                                                                                                                ,XHB_GET_XML_REPORTS.validate_postcode(xadc.POSTCODE) AS "PostCode"
                                                                                                                )
                                                                                 )
                                                                    , XMLFOREST(xcrt.DX_REF as "cs:CourtHouseDX"
  ...
  SELECT XMLELEMENT("cs:Defendant"
                   ...
                   , XMLELEMENT("cs:PersonalDetails",
                             XMLFOREST(XMLFOREST(CASE WHEN (p_show_public_view = 1 AND xdoc.IS_MASKED = 'Y') THEN ' ' ELSE xd.FIRST_NAME END AS "CitizenNameForename"
                                     ...
                                     , XMLFOREST(NVL(RTRIM(xadd.ADDRESS_1), '-') AS "Line"
                                                 ,NVL(RTRIM(xadd.ADDRESS_2), '-') AS "Line"
                                                 ,NVL(RTRIM(xadd.ADDRESS_3), '-') AS "Line"
                                                 ,NVL(RTRIM(xadd.ADDRESS_4), '-') AS "Line"
                                                 ,XHB_GET_XML_REPORTS.GET_TOWN_AND_COUNTY(xadd.TOWN,xadd.COUNTY) AS "Line"
                                                 ,XHB_GET_XML_REPORTS.validate_postcode(xadd.POSTCODE) AS "PostCode"
                                                  ) "cs:Address" )
                                )--PersonalDetails
                      ...
                      , (SELECT XMLAgg(XMLELEMENT("cs:Solicitor"
                                           ...
                                           , XMLELEMENT("cs:Party"
                                             , XMLELEMENT("cs:Organisation"
                                               ...
                                               , XMLELEMENT("cs:OrganisationAddress",XMLFOREST(NVL(RTRIM(solicitor_address.ADDRESS_1), '-') AS "Line"
                                                                                             ,NVL(RTRIM(solicitor_address.ADDRESS_2), '-') AS "Line"
                                                                                             ,NVL(RTRIM(solicitor_address.ADDRESS_3), '-') AS "Line"
                                                                                             ,NVL(RTRIM(solicitor_address.ADDRESS_4), '-') AS "Line"
                                                                                             ,XHB_GET_XML_REPORTS.GET_TOWN_AND_COUNTY(solicitor_address.TOWN,solicitor_address.COUNTY) AS "Line"
                                                                                             ,XHB_GET_XML_REPORTS.validate_postcode(solicitor_address.POSTCODE) AS "PostCode")
                                                           )--OrganisationAddress
                                                         , XMLFOREST(xrsf.DX_REF AS "cs:OrganisationDX")
							 , XHB_GET_XML_REPORTS.get_contact_details( xrsf.ADDRESS_ID) --"cs:ContactDetails"
                                               )--Organisation
                                            )--Party
                                         , XMLFOREST(xdocrsf.REP_ST_DATE AS "cs:StartDate"
                                                  , xdocrsf.REP_END_DATE AS "cs:EndDate"
                                                  , xrsf.SOLICITOR_FIRM_NAME AS "cs:FirmName")
                                          )--Solicitor
                                          ...
                                          AND xdocrsf.REF_SOLICITOR_FIRM_ID = XHB_REPORT_PKG.GET_SOLICITOR_ID(xdocrsf.DEFENDANT_ON_CASE_ID)                                          AND xdocrsf.REF_SOLICITOR_FIRM_ID = xrsf.REF_SOLICITOR_FIRM_ID
  ...
  defendants XMLType;
  BEGIN
  SELECT XMLAgg(get_defendant(xdocol.DEFENDANT_ON_CASE_ID, 0, p_show_prison_information)order by xdoc.defendant_number) INTO defendants
  FROM XHB_DEF_ON_CASE_ON_LIST xdocol, XHB_DEFENDANT_ON_CASE xdoc
                            ...
                            ,XMLFOREST(xcr.crest_court_room_no AS "cs:CourtRoomNumber"
                                      ...
                                      ,XMLELEMENT("cs:Judiciary",
                                            XMLELEMENT("cs:Judge",XMLFOREST(xrj.TITLE AS "CitizenNameTitle"
                                                                      ...
                                                                      , NVL(SUBSTR(xrj.FULL_LIST_TITLE1||xrj.FULL_LIST_TITLE2||xrj.FULL_LIST_TITLE3,0,70),'N/A') AS "CitizenNameRequestedName"
 ...
 SELECT(XMLAGG(XMLELEMENT("cs:Prosecution"
         ...
         ,XMLFOREST(xrpa.PROSECUTOR_NAME_3 AS "cs:ProsecutingReference"
                , XMLFOREST(xrpa.CPS_CODE AS "cs:OrganisationCode"
                           ...
                           , XMLFOREST(NVL(RTRIM(prosecutor_address.ADDRESS_1), '-') AS "Line"
                                      ,NVL(RTRIM(prosecutor_address.ADDRESS_2), '-') AS "Line"
                                      ,NVL(RTRIM(prosecutor_address.ADDRESS_3), '-') AS "Line"
                                      ,NVL(RTRIM(prosecutor_address.ADDRESS_4), '-') AS "Line"
                                      ,XHB_GET_XML_REPORTS.GET_TOWN_AND_COUNTY(prosecutor_address.TOWN,prosecutor_address.COUNTY) AS "Line"
                                      ,XHB_GET_XML_REPORTS.validate_postcode(prosecutor_address.POSTCODE) AS "PostCode"
