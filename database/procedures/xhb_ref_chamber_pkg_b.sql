...
BEGIN
 ...
 PROCEDURE delete_ref_chamber (p_ref_chamber_id IN xhb_ref_chamber.ref_chamber_id%TYPE, p_user_name IN XHB_REF_CHAMBER.created_by%TYPE) AS
	...
	CURSOR c_get_associated_counsel IS
	SELECT DISTINCT xra.crest_advocate_id
	FROM xhb_ref_advocate xra
	WHERE xra.crest_chamber_id = p_ref_chamber_id
	AND NVL(xra.obs_ind,'N') != 'Y';
 
 BEGIN
 
	-- Start by removing any associated counsel
	FOR counsel_rec IN c_get_associated_counsel LOOP
		xhb_ref_advocate_pkg.delete_counsel_details(counsel_rec.crest_advocate_id, p_user_name);
    END LOOP;
 
	-- Obsolete the chamber
	...
	EXCEPTION
		WHEN OTHERS THEN
			ROLLBACK;
			DBMS_OUTPUT.PUT_LINE('!!! AN ERROR OCCURRED delete_ref_chamber:  ERROR: '||SUBSTR(SQLERRM,1,110));
			RAISE;
