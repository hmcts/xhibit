create or replace PACKAGE xhb_list_distribution_pkg AS
    ...
    PROCEDURE STORE_XML_DOCUMENT(p_return_xml CLOB, p_doc_title VARCHAR2, p_doc_date DATE, p_document_type_code VARCHAR2, p_court_id XHB_COURT.COURT_ID%TYPE,
                                 p_country XHB_XML_DOCUMENT.country%TYPE,
                                 p_language XHB_XML_DOCUMENT.language%TYPE,
                                 p_store_control boolean default false);
    ...
    PROCEDURE STORE_VALIDATION_DOCUMENT(p_return_xml CLOB, p_document_name VARCHAR2, p_document_description VARCHAR2, p_document_type_code VARCHAR2,
                                        p_validation_id XHB_VALIDATION.VALIDATION_ID%TYPE, p_unique_id VARCHAR, p_type_id NUMBER, p_crest_court_id XHB_COURT.CREST_COURT_ID%TYPE);
	...
	FUNCTION getEmailAddress(p_address_id  xhb_address.address_id%TYPE) RETURN xhb_contact_detail.contact_value%TYPE;
	FUNCTION getFaxNumber(p_address_id  xhb_address.address_id%TYPE) RETURN xhb_contact_detail.contact_value%TYPE;


  -- get_wll_rec_by_court_and_crest_sol_firm_id
  ----
  --      Returns all wll_recipients which match crest_sof_id
  ----
  ...
  PROCEDURE SET_LIST_PUBLISH_STATUS(p_list_id           IN XHB_LIST.LIST_ID%TYPE,
                                    ...
                                    p_publish_error_msg IN XHB_LIST.PUBLISH_ERROR_REASON%TYPE DEFAULT NULL);
