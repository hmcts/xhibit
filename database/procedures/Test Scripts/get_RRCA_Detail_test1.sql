...
*
*
*		This test looks for: open cases by age by court_id = 81 (Snaresbrook, single-site)
*                            open cases by age by court_id = 80 (Swansea, multi-site)
* Columns:      ct.court_name, court_site_name, status, waiting_time, case_num, age_band
*
*************************************************************************************
*
*       
...
DECLARE
    ...
    v_message_filename  VARCHAR2(100)   := 'get_RRCA_Detail_test1';
    ...
    v_court_site             XHB_COURT_SITE.COURT_SITE_NAME%TYPE;
...
--    v_waiting_time           DATE;
    ...
    v_last_age_band          VARCHAR2(10);
    v_error                  VARCHAR2(255);
        
    
BEGIN

    DBMS_OUTPUT.PUT_LINE('--======================== Test 1 get_RRCA_Detail START ============================');
    ...
    XHB_REPORT_PKG_JR.get_RRCA_Detail( p_resultset      =>  v_resultset
                                     , p_date           =>  v_rpt_date
...
-- l 58
  ...
  -- check message is correct
    ...
    DBMS_OUTPUT.PUT_LINE('Court Name      Court Site      Trial Date      Status  Waiting Time  Case Num     Age Band');
/*  * Columns:      ct.court_name, trial date, status, waiting_time, case_num, age_band
 */    
    LOOP        
        v_step := ' 5';
        FETCH v_resultset INTO v_court_full_name, v_court_site, v_trial_date, v_status, v_waiting_time , v_case_number, v_age_band;
        ...
        IF (v_row < 11) OR (v_age_band != v_last_age_band) THEN
            DBMS_OUTPUT.PUT_LINE(rpad(v_court_full_name, 16, ' ') || rpad(v_court_site, 16, ' ') || rpad(v_trial_date, 17, ' ') || rpad(v_status, 10, ' ') ||  rpad(v_waiting_time, 10, ' ') || rpad(v_case_number, 16, ' ') || rpad(v_age_band, 12, ' '));
        END IF;
...
-- l 141       
    ...
    DBMS_OUTPUT.PUT_LINE('         ------------------- Test 1 get_RRCA_Detail  END --------------------------' || chr(10));
