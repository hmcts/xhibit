...
DECLARE
    ...
    v_case_class        XHB_CASE.CLASS_CODE%TYPE    := 1;   -- Can be 1, 2, 3, null
	...
	v_reqd_judge_type   xhb_case_listing_entry.ref_judge_type_id%TYPE := NULL;    -- Can be 'R', 'AR', 'CJ', 'HJ', 'D', 'DH', 'DJ' or null
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('--======================== Test 2 ' || v_message_filename || ' START ============================');
...
--                            number                          
    ...
    IF v_row != 621 THEN 
        v_test_fail_msg := v_test_fail_msg || 'Rows retrieved: expected 621, got ' || v_row  || Chr(10); 
    END IF;
...
--    DBMS_OUTPUT.PUT_LINE('output message file in utl_file_hold directory = ' || v_message_filename || chr(10));
    ...
    DBMS_OUTPUT.PUT_LINE('         ------------------- Test 2 ' || v_message_filename || '  END --------------------------' || chr(10));
