...
DECLARE
	...
	v_reqd_judge_type   xhb_case_listing_entry.ref_judge_type_id%TYPE := 111101 ;    -- Can be 111101, 112536, 114008, 111073 or null
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('--======================== Test 6 ' || v_message_filename || ' START ============================');
...
--                            number                          
    ...
    IF v_row != 10 THEN 
        v_test_fail_msg := v_test_fail_msg || 'Rows retrieved: expected 10, got ' || v_row  || Chr(10); 
    END IF;
...
--    DBMS_OUTPUT.PUT_LINE('output message file in utl_file_hold directory = ' || v_message_filename || chr(10));
    ...
    DBMS_OUTPUT.PUT_LINE('         ------------------- Test 6 ' || v_message_filename || '  END --------------------------' || chr(10));
