...
*
* MODULE      : getUNLC_Rpt_test1
*
* DESCRIPTION : This script tests the get_unlc_report stored procedure. 
* 				get_unlc_report accepts 16 user-selectable parameters.
*
...
* ----------    -------         --------    ----------------------------------------
* 30/10/2018    J Riley         1.0         First Version-
**/
...
DECLARE
    ...
    v_row               NUMBER(8)   := 0;
	v_exp_row           NUMBER(8)   := 609;

    v_message_filename  VARCHAR2(100)   := 'get_unlc_report_test1';
	...
	v_df_hearing_type   XHB_REF_HEARING_TYPE.HEARING_TYPE_CODE%TYPE := NULL;
	...
	v_reqd_judge_type   xhb_case_listing_entry.ref_judge_type_id%TYPE := NULL;
	v_units_weeks       NUMBER(8);
	v_secure_ctrm       XHB_CASE.SECURE_COURT%TYPE  := 'N';   -- 'N' selects both options, 'Y' selects only secure court cases
	v_juvenile_only     xhb_defendant_on_case.is_juvenile%TYPE := 'N';  -- 'N' selects both options, 'Y' selects only cases where at least one defendant is a juvenile
	...
	v_sortby            VARCHAR2(20);-- Can be 'CASENUMBER' or null/anything else (defaults to trial date)
...
*/
    ...
    XHB_REPORT_PKG_JR.get_unlc_report( p_results_out          =>  v_resultset
...
*               CLASS_CODE, HEARING_TYPE, monitoring_category_code, LOEST, CASE_GROUP_NUMBER, 
*               First_NAD, NOTE_CLASSIFICATION, DIARY_NOTE_TEXT, NOTE_TYPE   */
    ...
    LOOP        
        ...
        FETCH v_resultset INTO v_diary_note_entry_id, v_case_number, v_custody_case, v_case_title, v_juv_flg, v_committed_sent, v_class_code, v_hearingtype, v_mon_cat_cd, v_loest, v_case_no_grp, v_1st_NAD, v_note_class, v_diary_note_txt, v_note_type ;
    ...
    IF v_row != v_exp_row THEN 
        v_test_fail_msg := v_test_fail_msg || 'Rows retrieved: expected ' || v_exp_row || ', got ' || v_row  || Chr(10); 
    END IF;
