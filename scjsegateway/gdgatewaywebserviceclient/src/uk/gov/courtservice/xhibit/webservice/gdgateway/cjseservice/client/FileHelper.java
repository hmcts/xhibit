...
public class FileHelper
{
    ...
    private static final String WS_CLIENT_LOG_DIR_NAME = "WS_CLIENT_LOG_DIR_NAME";
    private static final String DEFAULT_WS_CLIENT_LOG_DIR_NAME = "stub";
    
    private static final String INBOX    = System.getProperty("file.separator", "/") + getWSClientFileLoggingDirName() + "_inbox";
    private static final String OUTBOX   = System.getProperty("file.separator", "/") + getWSClientFileLoggingDirName() + "_outbox";
    private static final String ERRORBOX = System.getProperty("file.separator", "/") + getWSClientFileLoggingDirName() + "_errorbox";
	                                       ...
	                                       String directoryName)
	{
		...
		try
		{
	        ...
	        log.debug("<< Write message to fileOS >>");
	        ...
	        log.debug("<< Completed Writing message to fileOS >>");
		}
		catch(Exception e)
		{
    ...
    public static File createXmlDirectory(String transactionName, String directoryName, Calendar calendar) {
        ...
        {
            boxType = OUTBOX;
            log.debug("<< Directory will be outbox >>: " + boxType);
        }
        ...
        {
            log.debug("<< Directory will be inbox >>: " + boxType);
        }

        if(System.getProperty("log4j.log.dir") == null ||
           System.getProperty("log4j.log.dir").trim().equals(""))
        {
            log.debug("<< Writing gdgateway web service client log to directory called >>: " + boxType);
            logDir = new File(boxType);
        }
        else
        {
            log.debug("<< Writing gdgateway web service client log to directory called>>: " + System.getProperty("log4j.log.dir") + boxType);
            logDir = new File((System.getProperty("log4j.log.dir") + boxType));
        }

        if (!logDir.exists()) {
            log.debug("<< Creating gdgateway web service client logging directory of type >>: " + boxType);
            boolean isDirCreated = logDir.mkdir();
            if (!isDirCreated) {
                log.debug("<< gdgateway web service client logging directory not created:" + boxType);
            }
        }
    ...
    /**
     * Returns the generic directory name used for the client file logging 
     *
     * @return String
     */
    public static String getWSClientFileLoggingDirName()
    {
        String wSClientFileLoggingDirName =
            System.getProperty(WS_CLIENT_LOG_DIR_NAME,DEFAULT_WS_CLIENT_LOG_DIR_NAME);

        log.debug("<< wSClientFileLoggingDirName: " + wSClientFileLoggingDirName + " >>");

        return wSClientFileLoggingDirName;
    }
}