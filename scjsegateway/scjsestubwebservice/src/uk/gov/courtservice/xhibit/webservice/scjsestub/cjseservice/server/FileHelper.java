...
{
    private static final String INBOX    = System.getProperty("file.separator", "/") + "stub_inbox";
    private static final String OUTBOX   = System.getProperty("file.separator", "/") + "stub_outbox";
    private static final String ERRORBOX = System.getProperty("file.separator", "/") + "stub_errorbox";
	                                       ...
	                                       String directoryName)
	{
		...
		try
		{
	        ...
	        log.debug("<< Write Stub Web Service message to fileOS >>");
	        ...
	        log.debug("<< Completed Writing Stub Web Service message to fileOS >>");
		}
		catch(Exception e)
		{
		...
		finally
		{
			...
			{
				log.debug("<< logDataToXmlFile bufferedOutputStream close Exception >>: " + e);
			}
    ...
    public static File createXmlDirectory(String transactionName, String directoryName, Calendar calendar) {
        ...
        {
            log.debug("<< Writing gdgateway web service log to directory called >>: " + boxType);
            logDir = new File(boxType);
        }
        else
        {
            log.debug("<< Writing gdgateway web service log to directory called>>: " + System.getProperty("log4j.log.dir") + boxType);
            logDir = new File((System.getProperty("log4j.log.dir") + boxType));
        }

        if (!logDir.exists()) {
            log.debug("<< Creating gdgateway web service logging directory of type >>: " + boxType);
            boolean isDirCreated = logDir.mkdir();
            if (!isDirCreated) {
                log.debug("<< gdgateway web service logging directory not created:" + boxType);
            }
        }
