...
{
    private static final String INBOX    = ".." + System.getProperty("file.separator", "/") + "inbox";
    private static final String OUTBOX   = ".." + System.getProperty("file.separator", "/") + "outbox";
    private static final String ERRORBOX = ".." + System.getProperty("file.separator", "/") + "errorbox";
	                                       ...
	                                       String directoryName)
	{
		...
		try
		{
	        ...
	        out("<< Write Echo Message to fileOS >>");
	        ...
	        out("<< Completed Writing Echo Message to fileOS >>");
		}
		catch(Exception e)
		{
    ...
    public static File createXmlDirectory(String transactionName, String directoryName, Calendar calendar) {
        ...
        if(System.getProperty("echo.wsclient.log") == null ||
           System.getProperty("echo.wsclient.log").trim().equals(""))
        {
            out("<< Writing gdgateway web service client log to directory called >>: " + boxType);
            logDir = new File(boxType);
        }
        else
        {
            out("<< Writing gdgateway web service client log to directory called>>: " + System.getProperty("echo.wsclient.log") + boxType);
            logDir = new File((System.getProperty("echo.wsclient.log") + boxType));
        }

        if (!logDir.exists()) {
            out("<< Creating echo client logging directory of type >>: " + boxType);
            boolean isDirCreated = logDir.mkdir();
            if (!isDirCreated) {
                out("<< echo client logging directory not created:" + boxType);
            }
        }
        ...
        directoryName = directoryName + "_" + "client" + "_" + dayOfMonth + "-" + month
                + "-" + year;
