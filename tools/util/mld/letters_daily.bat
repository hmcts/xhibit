...
IF NOT DEFINED XML_FILE (
    ECHO usage daily_letters.bat dl-xml
    EXIT /B 1
)
...
CALL %LETTERS_SCRIPT% uk.gov.courtservice.xhibit.business.xmlbinding.listdistribution.listprocessor.DailyListProcessor %XML_FILE%
IF %ERRORLEVEL% NEQ 0 (
    EXIT /B %ERRORLEVEL%
)
