...
IF NOT DEFINED XML_FILE (
    ECHO usage warned_letters.bat wl-xml
    EXIT /B 1
)
...
CALL %LETTERS_SCRIPT% uk.gov.courtservice.xhibit.business.xmlbinding.listdistribution.listprocessor.WarnedListProcessor %XML_FILE%
IF %ERRORLEVEL% NEQ 0 (
    EXIT /B %ERRORLEVEL%
)
