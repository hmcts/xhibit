...
public class CrestFormBFFormFXMLHelper extends CrestFormBFXmlHelper {
    // Only Criminal type charges 'C' apply to this type of Crest Form
    private String chargeType = this.CRIMINAL_APPEAL;
     ...
     */
    public CrestFormBFFormFXMLHelper(String typeCode) {
        super(typeCode);
    }
    ...
    protected void populateSchema(CrestForm crestForm, XhbDefendantOnCase doc, String courtClerkName,
        ...
        CrestFormBFAppealCaseReferenceHelper
                .populateAppealCaseReference(crestForm.getCrestFormF().getAppealCase(), doc);

        // populate the court clerk
        if (courtClerkName != null) {
            crestForm.getCrestFormF().getCourtClerk().setClerkName(courtClerkName);
        }

        // populate the defendant details
        CrestFormBFDefendantHelper.populateDefendant(crestForm.getCrestFormF().getDefendant(), doc);

        // populate the charge details for the given charge type
        crestForm.getCrestFormF().setCharges(
                CrestFormBFChargeHelper2.populateCharges(crestForm.getCrestFormF().getCharges(), doc, chargeType));

        // populate the unrelated disposals
        crestForm.getCrestFormF().setUnrelatedDisposals(
                CrestFormBFUnrelatedDisposalHelper2.populateUnrelatedDisposals(crestForm.getCrestFormF()
    ...
    public Object getCrestForm(CrestForm crestForm) {
        return crestForm.getCrestFormF();
    }
