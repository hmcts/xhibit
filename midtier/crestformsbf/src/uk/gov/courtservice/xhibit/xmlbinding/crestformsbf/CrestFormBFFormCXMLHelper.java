...
public class CrestFormBFFormCXMLHelper extends CrestFormBFXmlHelper {
    // Only Section_41 type charges 'O' apply to this type of Crest Form
    private String chargeType = this.SECTION_41;
     ...
     */
    public CrestFormBFFormCXMLHelper(String typeCode) {
        super(typeCode);
    }
    ...
    protected void populateSchema(CrestForm crestForm, XhbDefendantOnCase doc, String courtClerkName,
        ...
        CrestFormBFCaseReferenceHelper.populateCaseReference(crestForm.getCrestFormC().getCase(), doc);

        // populate the court clerk
        if (courtClerkName != null) {
            crestForm.getCrestFormC().getCourtClerk().setClerkName(courtClerkName);
        }

        // populate the defendant details
        CrestFormBFDefendantHelper.populateDefendant(crestForm.getCrestFormC().getDefendant(), doc);

        // populate the charge details for the given charge type
        crestForm.getCrestFormC().setCharges(
                CrestFormBFChargeHelper2.populateCharges(crestForm.getCrestFormC().getCharges(), doc, chargeType));
    }
    ...
    public Object getCrestForm(CrestForm crestForm) {
        return crestForm.getCrestFormC();
    }
