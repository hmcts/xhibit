...
public class CrestFormBFFormEXMLHelper extends CrestFormBFXmlHelper {

    // Only Breach type charges 'B' apply to this type of Crest Form
    private String chargeType = this.BREACH;
     ...
     */
    public CrestFormBFFormEXMLHelper(String typeCode) {
        super(typeCode);
    }
    ...
    protected void populateSchema(CrestForm crestForm, XhbDefendantOnCase doc, String courtClerkName,
        ...
        CrestFormBFCaseReferenceHelper.populateCaseReference(crestForm.getCrestFormE().getCase(), doc);

        // populate the court clerk
        if (courtClerkName != null) {
            crestForm.getCrestFormE().getCourtClerk().setClerkName(courtClerkName);
        }

        // populate the defendant details
        CrestFormBFDefendantHelper.populateDefendant(crestForm.getCrestFormE().getDefendant(), doc);

        // populate the charge details for the given charge type
        crestForm.getCrestFormE().setBreachCharges(
                CrestFormBFBreachChargeHelper2.populateBreachCharges(crestForm.getCrestFormE().getBreachCharges(), doc,
                        chargeType));
    }
    ...
    public Object getCrestForm(CrestForm crestForm) {
        return crestForm.getCrestFormE();
    }
