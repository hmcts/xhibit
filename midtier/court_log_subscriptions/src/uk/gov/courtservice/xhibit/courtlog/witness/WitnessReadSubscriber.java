...
/**
 * This is a category subscriber used for handling witness read events
 * 
 * @author tz0d5m
 * @version $Revision: 1.1 $
 */
public class WitnessReadSubscriber extends Subscriber {
    /**
     * Pre-creation logic for witness read events, used to perform the required
     * business validation.
     * 
     * @param context
    ...
    public void preCreate(OperationContext context) throws CourtLogBusinessException {
        log.debug("Start - preCreate for witness read subscriber");
        ...
        if (WitnessHelper.isTrialWitnessEvent(eventId)) {
            ...
            WitnessHelper.validateTrialWitnessReadId(context.getCrudValue());
        ...
        log.debug("End - preCreate for witness read subscriber");
    }

    /**
     * Pre-update logic for witness read events, used to perform the required
     * business validation.
     * 
     * @param context
     *            The subscription context for shared state
    ...
    public void preUpdate(OperationContext context) throws CourtLogBusinessException {
        log.debug("Start - preUpdate for witness read subscriber");
        Integer eventId = context.getCrudValue().getEventType();
        ...
        log.debug("End - preUpdate for witness read subscriber");
    }
     ...
     * @throws WitnessReadException
     *             if there was a problem swearing in a witness
     * @see uk.gov.courtservice.xhibit.courtlog.subscriptions.Subscriber
     *      #preDelete(uk.gov.courtservice.xhibit.courtlog.OperationContext)
     */
    public void preDelete(OperationContext context) throws WitnessReleasedException {
        log.debug("Start - preDelete for witness read subscriber");

        //Integer eventId = context.getCrudValue().getEventType();

        // Commented out pre delete section that was copied from WitnessSworn - as not needed for WitnessRead
/*        if (WitnessHelper.isTrialWitnessEvent(eventId)) {
        ...
        log.debug("End - preDelete for witness read subscriber");
    }
}
