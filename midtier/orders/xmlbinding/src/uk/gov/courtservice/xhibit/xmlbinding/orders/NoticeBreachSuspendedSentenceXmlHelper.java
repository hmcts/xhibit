 ...
 * 'NoticeBreachSuspendedSentence ' order.
 * </p>
 * <p>
 * Description:
 * </p>
 * <p>
 * Instance of XmlHelper that populates the Order Castor object for a 'NoticeBreachSuspendedSentence' order.
 * </p>
 * <p>
...
//
public class NoticeBreachSuspendedSentenceXmlHelper extends OrderXMLHelper {

  
    /**
     * Simple constructor instantiating a functional instance of XML helper for
     * a 'NoticeBreachSuspendedSentence ' order.
     * 
     * @param typeCode
     *            the type code for this particular order.
     */
    public NoticeBreachSuspendedSentenceXmlHelper(String typeCode) {
        super(typeCode);
    }
    ...
    protected void populateSchema(Order order, DefendantOnCase docEntity) throws OrderXMLException {
        // Populate the header.
        OrderHeaderHelper.populateHeader(order.getOrderData().getNoticeBreachSuspendedSentence().getOrderHeader(), docEntity, this.getTypeCode());
        order.getOrderData().getNoticeBreachSuspendedSentence().setAssociatedCases(
                AssociatedCasesHelper.populateAssociatedCases(order.getOrderData().getNoticeBreachSuspendedSentence()
    ...
    protected void logicalValidation(Integer defendantOnCaseID, ValidationHelper helper) {
        NoticeBreachSuspendedSentenceOrderHelper.logicalValidateNoticeBreachSuspendedSentence(helper.getOrderToBeValidated().getOrderData().getNoticeBreachSuspendedSentence(), this.getTypeCode(), helper);
    }

    protected String structuralValidation(ValidationHelper helper) throws ValidationException, MarshalException,
            OrderXMLException {
        return NoticeBreachSuspendedSentenceOrderHelper.structuralValidateNoticeBreachSuspendedSentence(helper.getOrderToBeValidated().getOrderData().getNoticeBreachSuspendedSentence(), this.getTypeCode(), helper);
    }
}