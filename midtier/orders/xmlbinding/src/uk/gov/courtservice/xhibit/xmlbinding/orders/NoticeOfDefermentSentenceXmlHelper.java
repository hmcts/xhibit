 ...
 * 'NoticeOfDefermentSentence ' order.
 * </p>
 * <p>
 * Description:
 * </p>
 * <p>
 * Instance of XmlHelper that populates the Order Castor object for a 'NoticeOfDefermentSentence' order.
 * </p>
 * <p>
...
//
public class NoticeOfDefermentSentenceXmlHelper extends OrderXMLHelper {
  
    /**
     * Simple constructor instantiating a functional instance of XML helper for
     * a 'NoticeOfDefermentSentence ' order.
     * 
     * @param typeCode
     *            the type code for this particular order.
     */
    public NoticeOfDefermentSentenceXmlHelper(String typeCode) {
        super(typeCode);
    }
    ...
    protected void populateSchema(Order order, DefendantOnCase docEntity) throws OrderXMLException {
        // Populate the header.
        OrderHeaderHelper.populateHeader(order.getOrderData().getNoticeOfDefermentSentenceOrder().getOrderHeader(), docEntity, this.getTypeCode());
        order.getOrderData().getNoticeOfDefermentSentenceOrder().setAssociatedCases(
                AssociatedCasesHelper.populateAssociatedCases(order.getOrderData().getNoticeOfDefermentSentenceOrder()
    ...
    protected void logicalValidation(Integer defendantOnCaseID, ValidationHelper helper) {
        NoticeOfDefermentSentenceOrderHelper.logicalValidateNoticeOfDefermentSentence(helper.getOrderToBeValidated().getOrderData().getNoticeOfDefermentSentenceOrder(), this.getTypeCode(), helper);
    }

    protected String structuralValidation(ValidationHelper helper) throws ValidationException, MarshalException,
            OrderXMLException {
        return NoticeOfDefermentSentenceOrderHelper.structuralValidateNoticeOfDefermentSentence(helper.getOrderToBeValidated().getOrderData().getNoticeOfDefermentSentenceOrder(), this.getTypeCode(), helper);
    }
}