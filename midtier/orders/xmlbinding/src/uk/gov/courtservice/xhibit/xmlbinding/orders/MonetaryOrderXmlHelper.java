 ...
 * </p>
 * <p>
 * Instance of XmlHelper that populates the Order Castor object for a 'Monetary' order.
 * </p>
 * <p>
...
//
public class MonetaryOrderXmlHelper extends OrderXMLHelper {

    private static Logger log = CSServices.getLogger(MonetaryOrderXmlHelper.class);
     ...
     */
    public MonetaryOrderXmlHelper(String typeCode) {
        super(typeCode);
    }
    ...
    protected void populateSchema(Order order, DefendantOnCase docEntity) throws OrderXMLException {
        // Populate the header.
        OrderHeaderHelper.populateHeader(order.getOrderData().getMonetaryOrder().getOrderHeader(), docEntity, this.getTypeCode());
        
        // Populate the body of the order.
        MonetaryOrderStructure mos = order.getOrderData().getMonetaryOrder();
        MonetaryOrderHelper.populateMonetaryOrder(mos, docEntity, this.getTypeCode());
    }
    ...
    protected void logicalValidation(Integer defendantOnCaseID, ValidationHelper helper) {
        MonetaryOrderHelper.logicalValidateMonetaryOrder(helper.getOrderToBeValidated().getOrderData().getMonetaryOrder(),
    ...
    protected String structuralValidation(ValidationHelper helper) throws ValidationException, MarshalException,
            OrderXMLException {
        return MonetaryOrderHelper.structuralValidateMonetaryOrder(helper.getOrderToBeValidated().getOrderData()
                .getMonetaryOrder(), this.getTypeCode(), helper);
    }
}