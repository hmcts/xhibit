 ...
 * </p>
 * <p>
 * Description: This helper class helps populate the XML for the Youth Rehabilitation Order.
 * </p>
 * <p/> 
 ...
 * 
 */
public class YROXmlHelper extends OrderXMLHelper {

    private static final Logger log = CSServices.getLogger(YROXmlHelper.class);
    
    public YROXmlHelper(String typeCode) {
        super(typeCode);
    }

    protected void populateSchema(Order order, DefendantOnCase docEntity) throws OrderXMLException {
        YouthRehabilitationOrderStructure yrOrder = null;
        
        yrOrder = order.getOrderData().getYouthRehabilitationOrder();
        
        // Populate the header.
        if (log.isDebugEnabled()){log.debug("Populating header XML for YR order.");}
        
        if (order.getOrderData().getYouthRehabilitationOrder() == null)
        {
            log.error("Order data could not be retrieved for YR order.");
            throw new OrderXMLException("order.dataentry.error" ,"Order data could not be retrieved for YR order.");
        }
        OrderHeaderHelper.populateHeader(order.getOrderData().getYouthRehabilitationOrder().getOrderHeader(), docEntity, this
                .getTypeCode());
        // Add place holder for Post code if it has been removed by the order helper.
        rebuildBlankAddressLines(order);
        if (log.isDebugEnabled()){log.debug("Populating body XML for YR order.");}
        // Populate the body of the order.
        YROrderHelper.populateYouthRehabilitationOrder(order.getOrderData().getYouthRehabilitationOrder(), docEntity);

        // Use the LinkedOffenceHelper to populate the Linked Offences
        // for the Youth Rehabilitation order. We're using a back-population technique here
        // that ensures that the LinkedOffences remain null if there are truly
        // no LinkedOffences
        Integer courtId = docEntity.getCaze().getCourtId();
        order.getOrderData().getYouthRehabilitationOrder().setLinkedOffences(
                LinkedOffenceHelper.populateLinkedOffences(order.getOrderData().getYouthRehabilitationOrder().getLinkedOffences(),
        ...
        // no AssociatedCases/
        yrOrder.setAssociatedCases(
                AssociatedCasesHelper.populateAssociatedCases(
                        yrOrder.getAssociatedCases(), docEntity.getCaze()));
    }
    ...
    protected void logicalValidation(Integer defendantOnCaseID, ValidationHelper helper) {
        XhibitYROrderValidationHelper.logicalValidateYouthRehabilitationOrder(helper.getOrderToBeValidated().getOrderData()
                .getYouthRehabilitationOrder(), this.getTypeCode(), helper);
    }
    ...
    private void rebuildBlankAddressLines(Order order){
        ...
        uk.gov.courtservice.xhibit.xmlbinding.generated.orders.Address defAdd = 
            order.getOrderData().getYouthRehabilitationOrder().getOrderHeader().getDefendant().getPersonalDetails().getAddress();
