 ...
 * 'ActionConditionalDischarge ' order.
 * </p>
 * <p>
 * Description:
 * </p>
 * <p>
 * Instance of XmlHelper that populates the Order Castor object for a 'ActionConditionalDischarge' order.
 * </p>
 * <p>
...
//
public class ActionConditionalDischargeXmlHelper extends OrderXMLHelper {
 
    /**
     * Simple constructor instantiating a functional instance of XML helper for
     * a 'ActionConditionalDischarge ' order.
     * 
     * @param typeCode
     *            the type code for this particular order.
     */
    public ActionConditionalDischargeXmlHelper(String typeCode) {
        super(typeCode);
    }
    ...
    protected void populateSchema(Order order, DefendantOnCase docEntity) throws OrderXMLException {
        // Populate the header.
        OrderHeaderHelper.populateHeader(order.getOrderData().getActionConditionalDischargeOrder().getOrderHeader(), docEntity, this.getTypeCode());
        order.getOrderData().getActionConditionalDischargeOrder().setAssociatedCases(
                AssociatedCasesHelper.populateAssociatedCases(order.getOrderData().getActionConditionalDischargeOrder()
    ...
    protected void logicalValidation(Integer defendantOnCaseID, ValidationHelper helper) {
        ActionConditionalDischargeOrderHelper.logicalValidateActionConditionalDischarge(helper.getOrderToBeValidated().getOrderData().getActionConditionalDischargeOrder(), this.getTypeCode(), helper);
    }

    protected String structuralValidation(ValidationHelper helper) throws ValidationException, MarshalException,
            OrderXMLException {
        return ActionConditionalDischargeOrderHelper.structuralValidateActionConditionalDischarge(helper.getOrderToBeValidated().getOrderData().getActionConditionalDischargeOrder(), this.getTypeCode(), helper);
    }
}