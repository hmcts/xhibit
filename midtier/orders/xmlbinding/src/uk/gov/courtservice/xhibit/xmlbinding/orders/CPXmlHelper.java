 ...
 * @version 1.0
 */
public class CPXmlHelper extends OrderXMLHelper {
     ...
     */
    public CPXmlHelper(String typeCode) {
        super(typeCode);
    }
    ...
    protected void populateSchema(Order order, DefendantOnCase docEntity) throws OrderXMLException {
        // Populate the header.
        OrderHeaderHelper.populateHeader(order.getOrderData().getCPOrder().getOrderHeader(), docEntity, this
                .getTypeCode());

        // Populate the body of the order.
        XhibitCommunityOrderHelper.populateXhibitCommunityOrder(order.getOrderData().getCPOrder(), docEntity, true,
                false, this.getTypeCode());
        ...
        // no AssociatedCases/
        order.getOrderData().getCPOrder().setAssociatedCases(
                AssociatedCasesHelper.populateAssociatedCases(order.getOrderData().getCPOrder().getAssociatedCases(),
                        docEntity.getCaze()));
    }
    ...
    protected void logicalValidation(Integer defendantOnCaseID, ValidationHelper helper) {
        XhibitCommunityOrderValidationHelper.logicalValidateCommunityOrder(helper.getOrderToBeValidated()
                .getOrderData().getCPOrder(), this.getTypeCode(), helper);
    }

    protected String structuralValidation(ValidationHelper helper) throws ValidationException, MarshalException,
            OrderXMLException {
        XhibitCommunityOrderValidationHelper.structuralValidateCommunityOrder(helper.getOrderToBeValidated()
                .getOrderData().getCPOrder(), this.getTypeCode(), helper);
