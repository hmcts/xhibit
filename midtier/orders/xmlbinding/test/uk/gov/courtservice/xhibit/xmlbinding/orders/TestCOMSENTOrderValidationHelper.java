...
import uk.gov.courtservice.xhibit.xmlbinding.generated.orders.COOrder;
...
public class TestCOMSENTOrderValidationHelper extends TestCase
{
	...
	public TestCOMSENTOrderValidationHelper() {
		super();
	}
	
	public void testSuccess() throws OrderXMLException {
		CommunityOrderStructure orderStructure = getOrder().getOrderData().getCOOrder();
		
		try {
			XhibitCOMSENTOrderValidationHelper.logicalValidateCommunityOrder(orderStructure, null);
		} catch (Exception e) {
			fail();
		}
	}
    ...
    public void testLocalJusticeAreaFail() {
    	CommunityOrderStructure orderStructure = getOrder().getOrderData().getCOOrder();
    	orderStructure.getPettySessionalArea().setCourtHouseName(EMPTY_STRING);
		// LOCAL_JUSTICE_AREA_EMPTY
		expectFailure(orderStructure);
    }   
	...
	private void expectFailure(CommunityOrderStructure orderStructure) {
		try {
			XhibitCOMSENTOrderValidationHelper.logicalValidateCommunityOrder(orderStructure, null);
			fail();
	...
	private OrderData getOrderData() {
		...
		COOrder orderData = new COOrder();
		result.setCOOrder(orderData);
