...
import uk.gov.courtservice.xhibit.common.publicdisplay.renderdata.AllCourtStatusThinValue;
 ...
 * 
 */
class AllCourtStatusThinProcessor extends PublicDisplayRowProcessor {
    /** Cache of value objects */
    protected Map<String, AllCourtStatusThinValue> values = new HashMap<String, AllCourtStatusThinValue>();

    /** Need to store in the array list so that sort order is maintained */
    protected ArrayList<AllCourtStatusThinValue> arrayValues = new ArrayList<AllCourtStatusThinValue>();
    ...
    public void processRow(Row row) {
        ...
        AllCourtStatusThinValue value = values.get(siteRoomName);
        if (value == null) {
            // Create a new value object add to the cache
            value = new AllCourtStatusThinValue();
     ...
     */
    Collection<AllCourtStatusThinValue> getData() {
        return arrayValues;
    }
     ...
     */
    protected void populateData(Row row, AllCourtStatusThinValue value) {
        ...
        value.setEvent((BranchEventXMLNode) EventXMLNodeHelper.buildEventNode(row.getString(COURT_LOG_ENTRY)));
