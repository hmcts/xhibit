 ...
 * @author pznwc5
 */
public class AllCourtStatusThinQuery extends PublicDisplayQuery {
    /** The stored procedure to execute */
    private static final String SQL_PROCEDURE = "{ call XHB_PUBLIC_DISPLAY_PKG.GET_ALL_COURT_STATUS_THIN(?,?,?,?) }";

    /**
     * Constructor compiles the query
     */
    public AllCourtStatusThinQuery() {
        super(SQL_PROCEDURE);
    ...
    public Collection getData(Date date, int courtId, int[] courtRoomIds) {
        return execute(date, courtId, courtRoomIds, new AllCourtStatusThinProcessor());
    }
    // WELSH the below may be able to be removed.
    // public Collection getData(Date date, int courtId, int[] courtRoomIds)
    // {
    // return execute(date, courtId, courtRoomIds, new
    // AllCourtStatusThinProcessor());
    // }
}
