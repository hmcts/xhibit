...
public class AllCourtStatusQuery extends PublicDisplayQuery {
    ...
    private static final String SQL_PROCEDURE = "{ call XHB_PUBLIC_DISPLAY_PKG.GET_ALL_COURT_STATUS(?,?,?,?) }";

    /**
     * Constructor compiles the query
     */
    public AllCourtStatusQuery() {
        super(SQL_PROCEDURE);
    ...
    public Collection getData(Date date, int courtId, int[] courtRoomIds) {
        return execute(date, courtId, courtRoomIds, new AllCourtStatusProcessor());
    }
}
