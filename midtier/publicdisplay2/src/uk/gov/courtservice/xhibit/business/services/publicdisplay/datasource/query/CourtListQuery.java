...
public class CourtListQuery extends PublicDisplayQuery {
    ...
    private static final String SQL_PROCEDURE = "{ call XHB_PUBLIC_DISPLAY_PKG.GET_COURT_LIST(?,?,?,?) }";

    /**
     * Constructor compiles the query
     */
    public CourtListQuery() {
        super(SQL_PROCEDURE);
    ...
    public Collection getData(Date date, int courtId, int[] courtRoomIds) {
        return execute(date, courtId, courtRoomIds, new CourtListProcessor());
    }
}
