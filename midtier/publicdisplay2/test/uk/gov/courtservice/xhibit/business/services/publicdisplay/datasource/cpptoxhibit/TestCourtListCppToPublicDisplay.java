...
public class TestCourtListCppToPublicDisplay extends TestAbstractCppToPublicDisplay {
	...
	public void setUp() throws Exception {
		MockitoAnnotations.initMocks(TestCourtListCppToPublicDisplay.class);	
	}

	public TestCourtListCppToPublicDisplay() {
		super(new CourtListCppToPublicDisplay(new Date(), courtId, courtRoomIds) );
	}
	
	public TestCourtListCppToPublicDisplay(CourtListCppToPublicDisplay courtListCppToPublicDisplay) {
		super(courtListCppToPublicDisplay);
	}
	...
	/**
	 * Tests that for a valid single courtroom CPP XML CLOB, the data is assigned to the CourtListValue as expected
	 * @throws Exception
	 */
	...
	public void testSingleCourtroomXML() throws Exception {
		...
		List<CourtListValue> cppData = (List<CourtListValue>) cppToPublicDisplay.getCppData();
		
		// Check basic information
		assertEquals("Error in cppData size", 2, cppData.size());
		assertEquals("Error CourtListValue.getCourtRoomName()", "Court Room 11", cppData.get(0).getCourtRoomName());
		assertEquals("Error CourtListValue.getCaseNumber()", "PDCase123", cppData.get(0).getCaseNumber());
		assertEquals("Error CourtListValue.getNotBeforeTimeAsString()", "09:30", cppData.get(0).getNotBeforeTimeAsString());
		
		assertEquals("Error CourtListValue.getCourtRoomName()", "Court Room 11", cppData.get(1).getCourtRoomName());
		assertEquals("Error CourtListValue.getCaseNumber()", "PDCase456", cppData.get(1).getCaseNumber());
		assertEquals("Error CourtListValue.getNotBeforeTimeAsString()", "10:00", cppData.get(1).getNotBeforeTimeAsString());
		
		// Check Defendants
		assertEquals("Error CourtListValue.getDefendantNames()", 2, cppData.get(1).getDefendantNames().size());
	...
	/**
	 * Tests that for a valid multiple courtroom CPP XML CLOB, the data is assigned to the CourtListValue as expected
	 * @throws Exception
	 */
	@SuppressWarnings("static-access")
	@Test
	public void testMultipleCasesinDifferentRoomXML() throws Exception {
		...
		// Check basic information
		// should only return data from court room 11 - the one record
		assertEquals("Error in cppData size", 1, cppData.size());
		assertEquals("Error CourtListValue.getCourtRoomName()", "Court Room 11", cppData.get(0).getCourtRoomName());
		assertEquals("Error CourtListValue.getCaseNumber()", "PDCase123", cppData.get(0).getCaseNumber());
		assertEquals("Error CourtListValue.getNotBeforeTimeAsString()", "10:00", cppData.get(0).getNotBeforeTimeAsString());
		
		// Check Defendants
		assertEquals("Error CourtListValue.getDefendantNames()", 2, cppData.get(0).getDefendantNames().size());
	 ...
	 * removed, the resulting collection of CourtListValue values is the correct size and has been sorted
	 * as expected.
	 * @throws Exception
	 */
	@SuppressWarnings("static-access")
	...
	public void testSortXML() throws Exception {
		...
		clob.setClobData("<?xml version=\"1.0\" encoding=\"UTF-8\"?><?xml-stylesheet type=\"text/xsl\" href=\"InternetWebPageTemplate.xsl\"?><currentcourtstatus xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\">" +
				"<court><courtname>SNARESBROOK</courtname><courtsites><courtsite><courtsitename>SNARESBROOK</courtsitename><courtrooms><courtroom><cases><caseDetails><cppurn>PDCase123</cppurn><activecase>0</activecase><hearingtype>Plea and Trial Preparation</hearingtype>" +
				"<hearingprogress>0</hearingprogress><judgename>Before: Stig of the Dump</judgename><notbeforetime>10:30</notbeforetime><timestatusset>09:30</timestatusset><defendants>" +
				"<defendant><firstname>SAD</firstname><middlename></middlename><lastname>KEN</lastname><reportingrestrictions>0</reportingrestrictions></defendant>" +
				"<defendant><firstname>NOGGIN</firstname><middlename>THE</middlename><lastname>NOG</lastname><reportingrestrictions>0</reportingrestrictions></defendant></defendants>" +
				"<publicnotices><publicnotice>No Trousers on a Thursday</publicnotice><publicnotice>Rubber Chickens are Strictly Prohibited</publicnotice></publicnotices>" +
				"<currentstatus><event><time>09:30</time><date>18/12/19</date><free_text>The Judge has soiled himself</free_text><process_linked_cases/><defendant_on_case_id/><type>CPP</type><defendant_name>SAD KEN</defendant_name></event></currentstatus>" +
				"</caseDetails><caseDetails><cppurn>PDCase456</cppurn><activecase>1</activecase><hearingtype>For Mention</hearingtype>" +
				"<judgename>Before: Stig of the Dump</judgename><notbeforetime>10:00</notbeforetime><timestatusset>09:30</timestatusset><defendants>" +
				"<defendant><firstname>HOMER</firstname><lastname>SIMPSON</lastname><reportingrestrictions>0</reportingrestrictions></defendant>" +
				"<defendant><firstname>MARGE</firstname><lastname>SIMPSON</lastname><reportingrestrictions>0</reportingrestrictions></defendant></defendants>" +
				"<publicnotices><publicnotice>Anyone called Mark must wear a clown costume</publicnotice></publicnotices>" +
				"<currentstatus><event><time>09:30</time><date>18/12/19</date><free_text>Witness 1 Sworn in</free_text><process_linked_cases/><defendant_on_case_id/><type>CPP</type><defendant_name/></event></currentstatus>" +
		...
		@SuppressWarnings("unchecked")
		List<CourtListValue> xhibitData = (List<CourtListValue>)getXhibitData();
		...
		@SuppressWarnings("unchecked")
		List<CourtListValue> processedData = (List<CourtListValue>) CppDataSourceFactory.postProcessing(CppDataSourceFactory.DataType.COURTLIST_TYPE, xhibitData);
		...
		assertEquals("Error in cppData size", 5, processedData.size());
		
		// First CourtListValue object
		assertEquals("Error CourtListValue.getCourtRoomName()", "Court Room 11", processedData.get(0).getCourtRoomName());
		assertEquals("Error CourtListValue.getCaseNumber()", "T20190014", processedData.get(0).getCaseNumber());		
		// Second CourtListValue object
		assertEquals("Error CourtListValue.getCourtRoomName()", "Court Room 11", processedData.get(1).getCourtRoomName());
		assertEquals("Error CourtListValue.getCaseNumber()", "T20190013", processedData.get(1).getCaseNumber());			
		// Third CourtListValue object
		assertEquals("Error CourtListValue.getCourtRoomName()", "Court Room 11", processedData.get(2).getCourtRoomName());
		assertEquals("Error CourtListValue.getCaseNumber()", "T20190015", processedData.get(2).getCaseNumber());
		// Fourth CourtListValue object
		assertEquals("Error CourtListValue.getCourtRoomName()", "Court Room 11", processedData.get(3).getCourtRoomName());
		assertEquals("Error CourtListValue.getCaseNumber()", "PDCase456", processedData.get(3).getCaseNumber());
		// Fifth CourtListValue object
		assertEquals("Error CourtListValue.getCourtRoomName()", "Court Room 11", processedData.get(4).getCourtRoomName());
		assertEquals("Error CourtListValue.getCaseNumber()", "PDCase123", processedData.get(4).getCaseNumber());
	...
	protected CourtStructureValue getCourtStructureValue() {
		...
		XhbCourtRoomBasicValue[] roomArray = {room11};
	...
	/**
	 * Protected method that returns a mock list of Xhibit CourtListValue objects
	 * @return
	 */
	protected List<?> getXhibitData() {
		List<CourtListValue> data = new ArrayList<CourtListValue>();	
		
		CourtListValue room11a = new CourtListValue();
		...
		room11a.setNotBeforeTime(Timestamp.valueOf("2019-12-18 11:30:00.0"));
		...
		CourtListValue room11b = new CourtListValue();
		room11b.setCourtRoomName("Court Room 11");
		room11b.setCourtSiteCode("A");
		room11b.setCourtSiteName("SNARESBROOK");
		room11b.setCrestCourtRoomNo(11);
		room11b.setNotBeforeTime(Timestamp.valueOf("2019-12-18 09:00:00.0"));
		...
		CourtListValue room11c = new CourtListValue();
		...
		room11c.setNotBeforeTime(Timestamp.valueOf("2019-12-18 10:00:00.0"));
