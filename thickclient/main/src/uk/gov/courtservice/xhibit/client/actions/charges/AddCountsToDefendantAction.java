...
public class AddCountsToDefendantAction extends XAction {
    public AddCountsToDefendantAction() {
        populateFromBundle("AddCountsToDefendant");
    }
    ...
    public void xActionPerformed(ActionEvent e) throws CSRecoverableException {
        ...
        /*
         * CCN0257 Field is now optional //Check if additional Offence End date
         * present before continuing if
         ...
         * getString("addCountToDefendant.title"),
         * JOptionPane.INFORMATION_MESSAGE); throw new UserCancelException(); }
         */

        if (ccm.getOffenceValue().getOffenceEndDateTime() != null) {
            ...
            if (notOnOffence.size() > 0) {
                AddDefendantsToOffenceDialog addDefendantsToOffenceDialog = new AddDefendantsToOffenceDialog(xac, true,
                        AddDefendantsToOffenceDialog.TITLE.COUNT_TO_DEFENDANT, notOnOffence,
                        ChargesControllerHelper.MODE.ADD);
                addDefendantsToOffenceDialog.setVisible(true);

                if (ccm.isCancelClicked())
                    throw new UserCancelException();

                // Refresh Charges tab
                chargesController.loadCharges();
            } else {
                JOptionPane.showMessageDialog(xac, getString("addCountToDefendant.noDefendants.msg"),
                        getString("addCountToDefendant.noDefendants.title"), JOptionPane.INFORMATION_MESSAGE);
            }
        } else {
            /*
             * CCN0257 Field is now optional JOptionPane.showMessageDialog(xac,
             * getString("addDefendantOnOffence.noEndDate.msg"),
             * getString("addDefendantsToCount.title"),
