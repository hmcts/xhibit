...
/**
 * Factory for creating all tree nodes for firm lists.
 * 
 * @author uphillj
 * @amend groenm - added the list date a room is associated with.
 *
 */
public class FirmListTreeNodeFactory extends AbstractTreeNodeFactory {

	public FirmListTreeNodeFactory(XDialog listDialog, ListModel listModel, Outline listOutline, DefaultTreeModel listTreeModel) {
		super(listDialog, listModel, listOutline, listTreeModel);
	}

	@Override
	public DefaultMutableTreeNode createDate(Calendar listDate) {
		return createDateTreeNode(listDate);
	}
	...
	public DefaultMutableTreeNode createCaseParent(CaseOnListType parentType, Object... params) {
		...
		else if (CaseOnListType.Reserve.equals(parentType)) {
			treeNode = createReserveCasesTreeNode();
		}
		else {
