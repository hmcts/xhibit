...
/**
 * <p>
 * Title: PleaBailActTableModel
 * </p>
 * <p>
 * Description: The table model that describes the sctructure for the Bail Act Offence pleas
 * that are held on the Failure to Appear Charge.
 * </p>
 * 
 * <p>
 * Company: Logica
 ...
 * @version 1.0
 */
public class PleaBailActTableModel extends XHIBITDefaultTableModel {

    private static final long serialVersionUID = 102L;
    
    public static final int COLUMN_OFFENCE_DESC = 0;
    public static final int COLUMN_DEFENDANT = 1;
    public static final int COLUMN_DATEPUT = 2;
    public static final int COLUMN_PLEA = 3;
    public static final int COLUMN_ALTERED = 4;

    private static final Logger log = CSServices.getLogger(PleaBailActTableModel.class);

    private String emptyStr = "";
    private ResultsRowValue dataRow;
    private PleaControllerModel model;

    public PleaBailActTableModel(Object[] tableRows, PleaControllerModel model) {
        super();
        setupTableHeaders();
    ...
    public Object getValueAt(int row, int col) {
        ...
        switch (col) {
        case COLUMN_OFFENCE_DESC:
            cellValue = dataRow.getChargeSequenceNumber() + ": " + dataRow.getBreachOffencesText();
            break;
    ...
    private void setupTableHeaders() {
        String[] columnNames = { getResource("bailAct.table.column.offencesdesc"), 
                                 getResource("bailAct.table.column.defendant"),
                                 getResource("bailAct.table.column.dateput"),
                                 getResource("bailAct.table.column.pleadescription"), 
                                 getResource("bailAct.table.column.altered") };
    ...
    public void setValueAt(Object obj, int row, int col) {
        ...
        if (col == COLUMN_DATEPUT) {
            ...
            if (obj == null && 
                (dataRow.getPleaValue() == null 
                  || 
                 (dataRow.getPleaValue() != null && dataRow.getPleaValue().getDatePut() == null))) 
            {
                return;
            }
        ...
        if (col == COLUMN_PLEA && !PleaHelper.isBailActPleaValid(obj)) {
            return;
        }
        dataRow.setOperationalBailActPlea();

        try {
            switch (col) {
            case COLUMN_PLEA:
                dataRow.processBailActPlea(obj);
                break;
            case COLUMN_DATEPUT:
                dataRow.setBailActDatePut(obj);
                break;
            default:
                break;
            }
