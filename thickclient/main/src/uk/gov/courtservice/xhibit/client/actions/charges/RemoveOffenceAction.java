 ...
 * 
 * 53941 15-08-2003 AW Daley Modified to load error box titles and messages from
 * a resource file.
 */
public class RemoveOffenceAction extends XAction {
    private ResourceBundle resources = XHIBITConstant.getResourceBundle(XhibitBundles.MaintainCharges);

    public RemoveOffenceAction() {
        populateFromBundle("RemoveOffence");
    }
    ...
    public void xActionPerformed(ActionEvent e) throws CSRecoverableException {
        ...
        boolean messageBoxReply = XMessageBox.alert(xac, XHIBITConstant.getResource(resources,
                "RemoveOffence.Query.Title"), true, XMessageBox.ICONQUESTION, XHIBITConstant.getResource(resources,
                "RemoveOffence.Query.Message"), XMessageBox.YESNO, XMessageBox.DEFAULTNO);
        ...
        if (messageBoxReply) {
            ...
            try {
                ...
                String message = XHIBITConstant.getResource(resources, "RemoveOffence.Results.Query.Message");
                if (ChargeTypes.isBreachChargeType(ccm.getChargeValue().getChargeType())) {
                    //If charge is a breach then explain that breaches always have a plea to avoid confusion
                    message = XHIBITConstant.getResource(resources, "RemoveOffence.Results.Query.MessageBreach");                    
                }
                
                resultsFoundMBReply = XMessageBox.alert(xac, XHIBITConstant.getResource(resources,
                        "RemoveOffence.Results.Query.Title"), true, XMessageBox.ICONQUESTION, 
                        message, XMessageBox.YESNO,
                ...
                if (resultsFoundMBReply) {
                    try {
                        ...
                        XHIBITConstant.debug(XHIBITConstant.getResource(resources,
                                "RemoveOffence.ResultsFoundException2"));
                    }
                }
            }
            if (offenceRemoved) {
