...
public class IdentificationPanel extends XDirectionsForDefendantPanel {
    ...
    private final String pdLookup = "E" + getEventType() + "_Identification";

    private final String borderTitle = "IdentificationBorder";

    private DirectionsForDefendantValue[] model;

    // private DirectionsForDefendantBasicValue dfv;

    public IdentificationPanel(DirectionsForDefendantValue[] model) throws CSRecoverableException {
        setModel(model);
        init();
        // stepActivate();
    }
    ...
    public Integer getEventType() {
        return PDHConstants.DEF_IDENTIFICATION;
    }
    ...
    public void moveModelToScreen() {
        if (model != null) {
            ...
            if (dfv != null) {
                String xmlKey = PDHConstants.getKeyForDb(dfv.getIsIdentified(), getRestrictions());
        ...
        if (model != null && getModified()) {
            ...
            if (selectedRB != null && selectedRB.getClientProperty(PDHConstants.dbCode) != null) {
                for (int i = 0; i < model.length; i++) {
                    ...
                    dfv.setIsIdentified((String) selectedRB.getClientProperty(PDHConstants.dbCode));
                }
            }
        }
    }
    ...
    public void populateCRUD(HashMap crud, Integer defOnCaseId) {
        if (model != null) {
            ...
            for (int i = 0; i < model.length; i++) {
                XhbDirectionsForDefendantBasicValue dfv = model[i].getDirectionsForDefendantBasicValue();
                if (dfv.getDefendantOnCaseId().equals(defOnCaseId) && dfv.getIsIdentified() != null) {
                    crud.put(pdLookup, PDHConstants.getKeyForDb(dfv.getIsIdentified(), getRestrictions()));
                }
            }
        }
    }
    ...
    private Collection getRestrictions() {
        if (thisRestriction == null) {
            thisRestriction = RestrictionFinder.getRestrictingValues(PDHConstants.DEF_IDENTIFICATION.toString()
                    + ".xsd", pdLookup);
        }
    ...
    private boolean isModelsIdentical() {
        ...
        for (int i = 0; i < model.length - 1; i++) {
            ...
            if ((dfd1.getIsIdentified() == null && dfd2.getIsIdentified() != null)
                    || (dfd1.getIsIdentified() != null && !dfd1.getIsIdentified().equals(dfd2.getIsIdentified()))) {
                return false;
            }
        }
        return true;
