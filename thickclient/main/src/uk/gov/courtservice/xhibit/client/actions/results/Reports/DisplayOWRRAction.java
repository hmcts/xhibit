...
public class DisplayOWRRAction extends XAction implements Action {

	private static final long serialVersionUID = 1L;

	private PublicRepresentationModel model;    
    
    public DisplayOWRRAction(PublicRepresentationModel model){
    	this.model = model;
    ...
    public void xActionPerformed(ActionEvent e) throws Exception {
			OWRRPrintInformation order = 
             XhibitDelegateHelper.getResults2Delegate().getOWRRPrintInformation(XhibitSingleton.getInstance().getCourtId(), model.getLegalAidOrderValue().getId());
			...
			PreviewReportAction previewReportAction = new PreviewReportAction(order,"config/xsl/results/reports/owrr/printOWRRReport.xsl","OWRRPreview",order.buildMap(path,url));
