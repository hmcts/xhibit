...
public class DisplayOARRAction extends XAction implements Action {

	private static final long serialVersionUID = 1L;

	private PublicRepresentationModel model;    
    
    public DisplayOARRAction(PublicRepresentationModel model){
    	this.model = model;
    ...
    public void xActionPerformed(ActionEvent e) throws Exception {
		...
		OARRPrintInformation order = 
             XhibitDelegateHelper.getResults2Delegate().getOARRPrintInformation(XhibitSingleton.getInstance().getCourtId(),legalAidAmendment.getId());
		...
		PreviewReportAction previewReportAction = new PreviewReportAction(order,"config/xsl/results/reports/oarr/printOARRReport.xsl","OARRPreview",order.buildMap(path,url));
