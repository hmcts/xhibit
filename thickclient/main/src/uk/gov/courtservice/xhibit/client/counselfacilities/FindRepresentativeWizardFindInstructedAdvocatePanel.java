...
public class FindRepresentativeWizardFindInstructedAdvocatePanel extends XPanel {
	...
	private Vector<FindInstructedAdvocateTableRowModel> displayAdvocatesVec = new Vector<FindInstructedAdvocateTableRowModel>();

	private boolean choosingSubstitutedInstructedAdvocate;

	public FindRepresentativeWizardFindInstructedAdvocatePanel(FindRepresentativeWizardController controller,
			FindLegalRepresentativeModel model, Vector<FindInstructedAdvocateTableRowModel> editedAdvocatesVec,
			boolean choosingSubstitutedInstructedAdvocate) throws CSRecoverableException {
		super();
		this.controller = controller;
		this.model = model;
		this.editedAdvocatesVec = editedAdvocatesVec;
		this.choosingSubstitutedInstructedAdvocate = choosingSubstitutedInstructedAdvocate;
	...
	private void jbInit() {
		...
		this.add(getEditPostBtn(), new GridBagConstraints(0, 0, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.NONE, new Insets(2, 2, 2, 2), 0, 0));

		this.add(getResultsScrollPane(), new GridBagConstraints(0, 2, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER,
				GridBagConstraints.NONE, new Insets(2, 2, 2, 2), 0, 0));
	}
			...
			editPostBtn.addActionListener(new XAction() {
				...
				public void xActionPerformed(ActionEvent e) throws CSRecoverableException {
					...
					synchronized (model) {
						...
						FindInstructedAdvocateTableRowModel trm = displayAdvocatesVec.elementAt(row);
						if (trm == null) {
							return;
						}

						getResultsTable().clearSelection();
						EditPostModel editPostModel = new EditPostModel(model.getXac(), trm.getCrestPostNumber(),
								editedAdvocatesVec);
	...
	private void mergeChanges(EditPostModel editPostModel) {
		...
		InstructedAdvocateHelper.mergeChanges(editedAdvocatesVec, editPostModel.getInstructedAdvocates(),
	...
	protected boolean isMandatoryFieldsCompleted() {
		return model.getInstructedAdvocateTableRowModel() != null
	...
	private void moveScreenToModel() throws CSRecoverableException {
		...
		if (!choosingSubstitutedInstructedAdvocate) {
			// If we are not choosing a substituted advocate then set
			// the FindLegalRepresentativeTableRowModel and
			// InstructedAdvocateTableRowModel to the same value.
			model.setFindLegalRepresentativeTableRowModel(item);
		}
		model.setInstructedAdvocateTableRowModel(item);
	}
	...
	public void stepInitialise() throws CSRecoverableException {
		XHIBITConstant.debug("[FindRepresentativeWizardFindInstructedAdvocatePanel] stepInitialise");
		resources = XHIBITConstant.getResourceBundle(XhibitBundles.CounselFacilities);
		tb = new TitledBorder(BorderFactory.createEtchedBorder(SystemColor.controlHighlight, SystemColor.controlShadow),
	...
	public void stepDeactivate() throws CSRecoverableException {
		XHIBITConstant.debug("[FindRepresentativeWizardFindInstructedAdvocatePanel] stepDeactivate");
	}
	...
	public void stepValidate() throws CSRecoverableException, CSValidationException {
		XHIBITConstant.debug("[FindRepresentativeWizardFindInstructedAdvocatePanel] stepValidate");
		// Nothing to validate
		moveScreenToModel();
	}
	...
	public void stepUpdateViewState() throws CSRecoverableException {
		XHIBITConstant.debug("[FindRepresentativeWizardFindInstructedAdvocatePanel] stepUpdateViewState");
	...
	public void stepDeinitialise(boolean update) throws CSRecoverableException {
		XHIBITConstant.debug("[FindRepresentativeWizardFindInstructedAdvocatePanel] stepDeinitialise");
	...
	public void stepActivate() throws CSRecoverableException {
		XHIBITConstant.debug("[FindRepresentativeWizardFindInstructedAdvocatePanel] stepActivate");
		stepUpdateViewState();
	}
}