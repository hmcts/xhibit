...
public class AddIndictmentCase {
	private static final long serialVersionUID = 1L;
	private final Logger log = CSServices.getLogger(getClass());
	private Integer caseId;

	public AddIndictmentCase(XhibitApplicationController xac) throws CSRecoverableException {
		log.debug("AddIndictmentCase:");
		setCaseId(0);
		if (xac.getCaseStatus().getCaseCreateInProgressFlag() == true
			...
			if (!xac.getCaseStatus().getCaseCreateInProgressFlag()) // Accessed from outside of Case Create/Amend
				...
				int result = JOptionPane.showConfirmDialog((Component) null, "Do you want to exit the case?",
						"Add Indictments", JOptionPane.YES_NO_OPTION);
		...
		caseSearchModel.setIndictmentSearch(true);
	...
	public AddIndictmentCase(XhibitApplicationController xac, boolean overload) throws CSRecoverableException {
		setCaseId(0);
		if (xac.getCaseStatus().getCaseCreateInProgressFlag()
			...
			if (!xac.getCaseStatus().getCaseCreateInProgressFlag()) // Accessed from outside of Case Create/Amend
				...
				int result = JOptionPane.showConfirmDialog((Component) null, "Do you want to exit the case?",
						"Case Management", JOptionPane.YES_NO_OPTION);
				if ((result == JOptionPane.NO_OPTION) || (result == JOptionPane.CLOSED_OPTION)) {
					return;
				}
			}
