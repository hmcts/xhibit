...
public class CaseListingDeleteFixturePanel extends XPanel {
	...
	public CaseListingDeleteFixturePanel(CaseListingDeleteFixtureDialog parent, CaseListingDeleteFixtureModel model)
	...
	public void stepInitialise() throws CSRecoverableException {
		...
		isPreDefinedReasonMandatory = isPredefinedReasonMandatory();
	}
	...
	public void stepDeinitialise(boolean update) throws CSRecoverableException {
		if (!update) {
			...
			if(buttonPanel.okButton.equals(getDeinitialiseSource())){
				XhibitDelegateHelper.getListingsDelegate().deleteCaseDiaryFixture(model.getFixture(), 
	...
	private void setReasonLabel(boolean isMandatory) {
		String text = isMandatory ? getResourceBundle("caseListingDeleteFixtureMandatoryInd") : "";
		if (reasonLabel == null) {
	...
	private JPanel initReasonPanel() {
    	...
    	panel.setBorder(BorderFactory.createTitledBorder( getResourceBundle("caseListingDeleteFixturePanelLabel") ));
    	panel.setLayout(new GridBagLayout());
    	
    	// Label
    	gbc.gridwidth = 2;
    	String reasonLabelKey = isPreDefinedReasonMandatory ? "caseListingDeleteFixtureReasonLabelPredefinedMandatory" : "caseListingDeleteFixtureReasonLabel" ;
	...
	/**
	 * Update the CaseDiaryFixtureComplexValue model from the screen values
	 * @throws CSRecoverableException
	 */
	private void moveScreenToModel() throws CSRecoverableException {
		model.getFixture().setVacationPreDefinedRsonId( getSelectedReason() != null ? getSelectedReason().getId() : null);
		model.getFixture().setVacationFreetextReason( freeText.getText() );
		model.getFixture().setStatus("D");
		model.getFixture().setObsInd("Y");
	}
	...
	private void moveModelToScreen() {
		...
		if (model.getFixture() != null && YES.equals(model.getFixture().getObsInd())) {
			reason.setSelectedItemById(model.getFixture().getVacationPreDefinedRsonId());
			freeText.setText(model.getFixture().getVacationFreetextReason());
		}
		setMandatoryLabels();
	}
	...
	private class ReasonComboBoxAction extends XAction {
		...
		@Override
		/**
		 * Action performed when dropdown value changes (re-avaluate Ok button enablement)
		 */
	...
	private void setOkEnabled() {
		...
		// Ok is disabled if both the dropdown and the freetext fields are empty
		if ( noReasonProvided() ) {
			isValid = false;
	...
	private boolean isPredefinedReasonMandatory() throws BisRefControllerException {
		...
		if (preDefinedHearingTypes != null && !preDefinedHearingTypes.isEmpty()) {
			if ( model.getFixture().getRefHearingType() != null) {
				String hearingTypeCode = model.getFixture().getRefHearingType().getHearingTypeCode();
				result = preDefinedHearingTypes.contains(hearingTypeCode);
			}
		}
		return result;
	}
