package uk.gov.courtservice.xhibit.client.actions.courtlog;

import java.awt.Frame;
import java.awt.event.ActionEvent;

import uk.gov.courtservice.framework.exception.CSRecoverableException;
import uk.gov.courtservice.xhibit.client.courtlog.MediumEventMLDialog;
import uk.gov.courtservice.xhibit.client.courtlog.MediumEventMLModel;
import uk.gov.courtservice.xhibit.client.util.UserCancelException;

/**
 * <p>
 * Title: XHIBIT 2
 * </p>
 * <p>
 * Description:
 * </p>
 * <p>
 * Copyright: Copyright (c) 2002
 * </p>
 * <p>
 * Company: EDS
 * </p>
 * 
 * @author Unknown
 * @version $Revision: 1.8 $
 */
public class MediumMLEventAction extends CourtLogAction {
    protected void displayScreen(ActionEvent e) throws CSRecoverableException, UserCancelException {
        log.debug("MediumMLEventAction - displayScreen(e);");

        final MediumEventMLModel model = (MediumEventMLModel) cloneModel();
        final MediumEventMLDialog med = new MediumEventMLDialog((Frame) getController(), model);

        displayDialog(med);
    }
}
