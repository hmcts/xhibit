...
public class DisplayDEFSSReportAction extends SynchXAction {
	private static final long serialVersionUID = 1L;
	private Frame parentFrame = null;
	
	 public DisplayDEFSSReportAction(){
		 populateFromBundle("DEFSSReport");
     }
	 
	 public DisplayDEFSSReportAction(Frame parentFrame){
	 ...
	 public void synchActionPerformed(ActionEvent parm1) throws java.lang.Exception {
		//Call Midtier to get result set
	        ADJSSReportList reportList = XhibitDelegateHelper.getResults2Delegate().getListOfDefendantsPutBackReport(XhibitSingleton.getInstance().getCourtId(), "DS", XHIBITConstant.getResource(XhibitBundles.XhibitActionResources, "DEFSSReportActionName"));
	        if(reportList.getADJSSDefendantValues().size() > 0)
	        {
	        	DisplayActionReportHelper helper = new DisplayActionReportHelper();
	   	        ...
	   	        reportList.setShortReportCode("DEFSS");
	        	
	   	        PreviewReportAction previewReportAction = new PreviewReportAction(reportList,"config/xsl/results/reports/defss/printDEFSSReport.xsl","DEFSSReportPreview");
