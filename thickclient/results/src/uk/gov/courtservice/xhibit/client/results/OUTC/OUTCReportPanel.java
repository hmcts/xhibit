...
import uk.gov.courtservice.xhibit.client.actions.results.Reports.DisplayOUTCReportAction;
 ...
 * @version 1.0
 */
public class OUTCReportPanel extends XPanel {
	...
	public OUTCReportPanel(RunOUTCReportDialog runOUTCReportDialog, OUTCReportModel model) throws CSRecoverableException {
		this.model = model;
		this.parent = runOUTCReportDialog;
		stepInitialise();
		jbInit();
	}
	...
	public void stepDeinitialise(boolean update) throws CSRecoverableException {	
		if(update){
			if (validateTimeEstimate()) {
				DisplayOUTCReportAction xaction = new DisplayOUTCReportAction(model,parent.getParentFrame()); 
				xaction.actionPerformed(new ActionEvent(model.getXac(),0,"call DisplayOUTCReportAction "));
			}
	...
	private boolean validateTimeEstimate() {
		...
		if ( (from == null || from.equals("")) && (to != null && !to.equals(""))
			...
			messageBoxTitle = XHIBITConstant.getResource(XhibitBundles.ErrorText, "Reports.OUTC.alert.errorTitle");
        	errorMessage = XHIBITConstant.getResource(XhibitBundles.ErrorText, "Reports.OUTC.alert.timeEstimateIncompleteMessage");
			...
			messageBoxTitle = XHIBITConstant.getResource(XhibitBundles.ErrorText, "Reports.OUTC.alert.errorTitle");
        	errorMessage = XHIBITConstant.getResource(XhibitBundles.ErrorText, "Reports.OUTC.alert.timeEstimateInvalidRangeMessage");
			XMessageBox.alert(parent, messageBoxTitle, true, XMessageBox.ICONERROR, 
					errorMessage, XMessageBox.OK_ONLY, XMessageBox.DEFAULTOK);
			valid = false;
		}
