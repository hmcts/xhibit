...
/**
 * This is the component that forms the Calendar Grid. The pluggable look and
 * feel demands a separate UI Delegate, an implementation is provided in the laf
 * package. The actual class is determined dynamically by the actual look and
 * feel used. This class therefore is the controller in the MVC model for the
 * calenar grid. It implements TableModelListener because the Model is a
 * TableModel. (the first generation of MDateSelector had the calendar grid as a
 * JTable)
 * 
 * @author M Newstead
 */
public class CalendarPanel extends JComponent {
    ...
    /**
     * Returns true if the point passed in is in the month that the Calendar is
     * currently is displaying. [The user could click on a cell that is at the
     * end of the preceeding month or start of the next month]
     * 
     * @param row
     *            the row
     * @param col
     ...
     * 
     * @param pnt
     *            the point to make the selected date
     */
     ...
     * 
     * @param cell
     *            the cell for last day of the month
     */
     ...
     * 
     * @param cell
     *            the cell for first day of the month
     */
    ...
    /**
     * @return the background of cell at column d, row w. If the specified cell
     *         represents the current date (today) the current date background
     *         colour will be returned if specified other wise the colour for
     *         the day of week is returned.
     */
    ...
    /**
     * @return the foreground of cell at column d, row w. If the specified cell
     *         represents the current date (today) the current date foreground
     *         colour will be returned if specified other wise the colour for
     *         the day of week is returned.
     */
 ...
 * Revision 1.3  2006/06/05 12:31:43  bzjrnl
 * Change: TI901
 * Comment: Weblogic Upgrade - Standadise code formatting tab fix
 * Revision 1.2 2006/05/31 14:26:04 bzjrnl Change:
 * TI901 Comment: Weblogic Upgrade - Standadise code formatting Revision 1.1
 * 2004/04/02 15:43:58 sz0t7n Adding mseries to the build so we can bug fix it
 * from now on
 * 
 * Revision 1.7 2002/12/21 22:53:05 martin ** empty log message ***
 * 
 * Revision 1.6 2002/08/17 20:13:37 martin Reformatted code using intellij
 * 
 * Revision 1.5 2002/02/09 12:54:39 martin Partial support for 'Special Days'
 * 
 * Revision 1.4 2002/02/03 12:49:09 martin Added support for curret date
