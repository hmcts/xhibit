 ...
 * object that implements this interface. Applications will probably build an
 * immutable implementaion so that all instances appear and behave the same.
 * 
...
public interface MDateSelectorConstraints {
    ...
    /**
     * @return the colour of the current date background, the background in only
     *         drawn when no image file has been set
     */
     ...
     *         foreground for the day of the week represented by the index. 0 is
     *         Sunday, 6 is Saturday. Null entries will inherit the foreground
     *         given by getForeground()
     */
    public Color[] getForegrounds();

    /**
     * @return an array of colors, each not null value is the colour for the
     *         background for the day of the week represented by the index. 0 is
     *         Sunday, 6 is Saturday. Null entries will inherit the background
     *         given by getBackground()
     */
     ...
     *         <p>
     *         Usually the date is echoed by catching the events, so the date in
     *         the textfield changes as the user moves through the calendar
     *         using the TAB and arrow keys. Switching the events off causes the
     *         textfield to remain unchanged until (perhaps) the clanedar is
     *         dismissed. A value of false makes the ESCAPE and RETURN/ENTER
     *         keys more effective for the pull down calendar. In this mode, ESC
     *         will cause the value in the date field to remain unchanged
     *         regardless of the setting in the calendar. RETURN/ENTER selects
     *         the value.
     */
 ...
 * Revision 1.2 2006/05/31 14:26:05
 * bzjrnl Change: TI901 Comment: Weblogic Upgrade - Standadise code formatting /*
 * Revision 1.1 2004/04/02 15:44:00 sz0t7n /* Adding mseries to the build so we
 * can bug fix it from now on /* /* Revision 1.6 2002/02/24 12:33:26 martin /* A
 * SpecialDayModel can be passed using the constraints /* /* Revision 1.5
 * 2002/02/16 18:12:51 martin /* The eens to update the text field are
