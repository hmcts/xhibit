 ...
 * that this is subclasses to provide something more acceptable. A Windows and
 * Metal one are provided.
 * 
...
public class BasicCalendarPanelUI extends ComponentUI {
    ...
    private void installKeyboardActions() {
        calendarPanel.registerKeyboardAction(new ArrowKeyAction(-1, true), KeyStroke.getKeyStroke(KeyEvent.VK_LEFT, 0),
                JComponent.WHEN_FOCUSED);
        calendarPanel.registerKeyboardAction(new ArrowKeyAction(1, true), KeyStroke.getKeyStroke(KeyEvent.VK_RIGHT, 0),
                JComponent.WHEN_FOCUSED);
        calendarPanel.registerKeyboardAction(new ArrowKeyAction(-1, false), KeyStroke.getKeyStroke(KeyEvent.VK_UP, 0),
                JComponent.WHEN_FOCUSED);
        calendarPanel.registerKeyboardAction(new ArrowKeyAction(1, false), KeyStroke.getKeyStroke(KeyEvent.VK_DOWN, 0),
                JComponent.WHEN_FOCUSED);
    }
    ...
    private void doPaint(Graphics g, JComponent c) {
        ...
        /*
         * if(isOpaque()) { g.setColor(background); g.fillRect(0, 0, cellWidth,
         * cellHeight); }
         */
    ...
    /*
     * This is where we would draw/remove borders, focus highlights, colours
     * etc. Override this method in a subclass to render the selected date.
     */
     ...
     * selected when the calendar panel has focus. Over ride this method in a
     * subclass to change the appearance.
     */
    protected void drawFocusedEffect(Graphics g, boolean focused, boolean selected) {
        /*
         * int width, height;
         * 
         * width = getCellSize().width; height = getCellSize().height; if
         * (focused) { g.setColor(foreground); g.drawLine(3, height-5, width-3,
         * height-5); //drawDashedRect(g, 3, , width-5, height-5); }
         */
    }
    ...
    /**
     * KeyHandler factory method, this implementation handles the push of the
    ...
    /**
     * Calculates the next cell that contains a valid date and therefore can be
     * selected
     * 
     * @param inc
     *            minus -1 moves back one day, +1 forward wrapping around at
     *            both the start and end of the month
     * @param roll
     *            true if the selection rols to the next row at the end
     */
    ...
    private void move(int inc, boolean roll) {
        ...
        /*
         * Now move within the calendar, rolling around at the end/start of each
         * row
         */
        selectedCell.x += inc;
...
/*
 * $Log: BasicCalendarPanelUI.java,v $
 ...
 * 1.1 2004/04/02 15:44:04 sz0t7n Adding mseries to the build so we can bug fix
 * it from now on
 * 
 * Revision 1.7 2002/12/21 23:06:30 martin ** empty log message ***
 * 
 * Revision 1.6 2002/03/02 19:33:54 martin Made some methods private to assist
 * Obfuscation
 * 
 * Revision 1.5 2002/03/02 18:11:10 martin Made some methods private to assist
 * Obfuscation
 * 
 * Revision 1.4 2002/02/16 09:53:44 martin no message
 * 
 * Revision 1.3 2002/02/03 12:49:17 martin Added support for curret date
