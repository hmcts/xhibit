 ...
 * resizing and placement policies are inherited from FormLayout. Components are
 * added to their container using FormConstraints and this layout manager then
 * ignores many of the values !. The intention is that it is easy for the
 * application programmer to use this layout manager, it does not allow anything
 * that could not be achieved with careful use of FormLayout. It is designed to
 * be used to manage a small group of components that should ideally be placed
 * in the same cell in a FormLayout. The image below shows a FormLayout with
 * superimposed grid lines, the cell (1,1) contains 3 components, two
 * JTextFields and a JLabel.
 * <p>
 ...
 * automatically calculates the insets for its child components, the left most
 * component gets zero left insets, the right most one zero right insets, all
 * components get zero top and bottom insets. These values can not be over
 * ridden by the application program so the FormCellLayout may not be the most
 * appropriate Layout Manager to use outside of a FormLayout. (Remember the cell
 * itself has insets)
 * <p>
 * As components are added to the container, their x coordinate is automatically
 * calculated, the first component to be added gets gridx=0, the next 1 etc. The
 * gridy value is always zero as this layout manager can deal with only one row.
 * <p>
 * The following code shows the usage of the FormCellLayout to produce the
 * effects shown in the sample image above:
 * 
 * <pre>
