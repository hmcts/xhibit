...
/**
 * The <code>FormLayout</code> class is a flexible layout manager that aligns
 * components vertically and horizontally, without requiring that the components
 * be of the same size. Each <code>FormLayout</code> object maintains a
 * dynamic rectangular grid of cells, with each component occupying one cell,
 * called its <em>display area</em>.
 * <p>
 * Each component managed by a form layout is associated with an instance of
 * {@link FormConstraints} that specifies how the component is laid out within
 * its display area.
 * <p>
 * How a <code>FormLayout</code> object places a set of components depends on
 * the <code>FormConstraints</code> object associated with each component, and
 * on the minimum size and the preferred size of the components' containers.
 * <p>
 * To use a form layout effectively, you must customize one or more of the
 * <code>FormConstraints</code> objects that are associated with its
 * components. You customize a <code>FormConstraints</code> object by setting
 * one or more of its instance variables:
 * <p>
 * <dl>
 * <dt>{@link FormConstraints#gridx}, {@link FormConstraints#gridy}
 * <dd>Specifies the cell at the upper left of the component's display area,
 * where the upper-left-most cell has address <code>gridx&nbsp;=&nbsp;0</code>,
 * <code>gridy&nbsp;=&nbsp;0</code>. Use
 * <code>FormConstraints.RELATIVE</code> (the default value) to specify that
 * the component be just placed just to the right of (for <code>gridx</code>)
 * or just below (for <code>gridy</code>) the component that was added to the
 ...
 * between the component and the edges of its display area. This is defaulted to
 * four pixels on each side.
 * <dt>{@link FormConstraints#anchor}
 * <dd>Used when the component is smaller than its display area to determine
 * where (within the display area) to place the component. Valid values are
 * <code>FormConstraints.CENTER</code> (the default),
 * <code>FormConstraints.NORTH</code>, <code>FormConstraints.NORTHEAST</code>,
 * <code>FormConstraints.EAST</code>, <code>FormConstraints.SOUTHEAST</code>,
 * <code>FormConstraints.SOUTH</code>, <code>FormConstraints.SOUTHWEST</code>,
 ...
 * preferred width width will be used if the container (not the column) is wide
 * enough
 * </dl>
 * <p>
 * The following figure shows 16 components managed by a form layout:
 * <p>
 * <img src="doc-files/FormPanel.gif" ALIGN=center HSPACE=10 VSPACE=7>
 * <hr>
 * <p>
 * Here is the code that implements the example shown above, the creation of the
 * components and setting of the border have been omitted.
 * <p>
 * 
 * <pre>
 * 
...
public class FormLayout implements LayoutManager2, java.io.Serializable {
     ...
     * that can be laid out by the form layout. Current value is 20
     */
     ...
     * gridbox constraints. The Keys in comptable are the components and the
    ...
    /**
     * This field holds a gridbox constraints instance containing the default
     * values, so if a component does not have gridbox constraints associated
     ...
     * contains the working attributes for the whole layout.
     */
     ...
     * <code>FormConstraints</code> object is returned.
     * 
     * @param comp
     *            the component to be queried.
     * @return the constraint for the specified component in this form layout; a
     ...
     * is not a copy, but is the actual <code>FormConstraints</code> object
     ...
     * participating in a GridBoxLayout.
     */
     ...
     * according to the in built rules and the constraints of each component
    ...
    protected void calculateGridSize(Container parent, Dimension size) {
        ...
        do {
            /*
             * Attempt to resize the columns (& rows) by calculating the amount
             * that should be removed from each column. Initially we can remove
             * from every column as we assume the preferred width is larger than
             * the minimum width and that the preferred width can be displayed.
             * As columns are shrunk to their minimums space can not be removed
             * from them so the excess space is saved and further passes are
             * done to remove it from those columns that have not yet reached
             * their minimum. Do this until all space has been removed from the
             * columns or until all columns are as small as the layout manager
             * allows i.e. the minimum (MINWIDTH)
             */
            if (spare > 0) {
                /*
                 * The parent is smaller than our panel some shrinking takes
                 * place, divide the shortage up equally among all columns
                 */
            ...
            x = y = -1; // Start at -1 so we can increment the co-ords in the
            // loop
            ...
            for (int compindex = 0; compindex < components.length; compindex++) {
                ...
                /*
                 * If we can make the column narrower then we will calculate a
                 * new value if it makes the column reach its minimum we record
                 * the amount which could not be removed so we can remove it
                 * from somewhere else later
                 */
                if (!info.atMin[x]) {
                    int newWidth = info.colWidth[x] - adjW;
                    info.actualWidth[x] = Math.max(
                    /* info.minWidth[x] */MINWIDTH, newWidth);
            ...
            /*
             * Lets see how wide we managed to make the components and calculate
             * if it is still larger parent container, it may be since some
             * components may have been set to the minimum so there is still
             * some space to remove.
             */
             ...
             * account for integer division which may mean the spare space is
             * not actually zero but too small to worry about
             */
    ...
    protected void setComponentBounds(Container parent) {
        ...
        for (int compindex = 0; compindex < components.length; compindex++) {
             ...
             * components have been added before the ones on their right
             */
            r.x = insets.left;
            r.y = insets.top;
     ...
     * 
     * @param constraints
     *            the FormConstraints for the cell
     * @param r
     *            the rectangle speifying the bounds of the cell
     */
...
/**
 * Helper class for FormLayout that holds working values for the container being
 * layed out using FormLayout. As the container is resized the values in here
 * are recalculated by the method calculateLayoutInfo
 */
class FormLayoutInfo implements java.io.Serializable {
    public static final String REVISION_ID = "$Id: FormLayout.java,v 1.3 2006/06/05 12:31:54 bzjrnl Exp $";
    ...
    int colWidth[]; // Preferred Width of each column, large preferred width
    ...
    int minHeight[]; // The minimum height of each rowis the largest minimum

    // height of each

    // component in the column

    int actualWidth[]; // The width that each column is set to at any moment in

    // time

    int actualHeight[]; // The height that each row is set to at any moment in
     ...
     * container based on the components that have been added. The
     * FormLayoutInfo object is populated once since the components do not
     * change but some attributes are reset if necessary
     */
    ...
    void setStaticLayoutInfo(Component[] components) {
        ...
        for (int compindex = 0; compindex < components.length; compindex++) {
            ...
            /*
             * Preferred size of each column, the size of the largest component
             * in the column/row
             */
            colWidth[curX] = Math.max(colWidth[curX], dp.width + compInsets.left + compInsets.right);
            rowHeight[curY] = Math.max(rowHeight[curY], dp.height + compInsets.top + compInsets.bottom);
             ...
             * the column/row
             */
            minWidth[curX] = Math.max(minWidth[curX], dm.width + compInsets.left + compInsets.right);
            minHeight[curY] = Math.max(minHeight[curY], dm.height + compInsets.top + compInsets.bottom);
