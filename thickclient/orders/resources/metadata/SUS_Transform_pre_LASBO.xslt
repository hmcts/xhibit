...
<xsl:stylesheet version="1.0" xmlns:bs7666="http://www.govtalk.gov.uk/people/bs7666" xmlns:cs="http://www.courtservice.gov.uk/schemas/courtservice" xmlns:nar="http://www.courtservice.gov.uk/schemas/courtservice/xhibit/orders/narrative" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ord="http://www.courtservice.gov.uk/schemas/courtservice/xhibit/orders" xmlns:apd="http://www.govtalk.gov.uk/people/AddressAndPersonalDetails" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:set="http://xml.apache.org/xslt">
	...
	<xsl:template match="nar:SUS_Title_pre_LASBO">
	...
	<xsl:template match="nar:SUS_Revision_pre_LASBO">
		<!-- Check to see if this is a revised Suspended Sentence Order -->
		<xsl:if test="$baseAll/ord:OrderHeader/ord:RevisionDetails/ord:RevisedOrder='true'">
	...
	<!-- Order title -->
	<xsl:template match="nar:SUS_OrderTitle_pre_LASBO">
		<fo:inline>
			<xsl:text>Order</xsl:text>
		</fo:inline>
	</xsl:template>
	<!-- Template to display SUS Order Text -->
	<xsl:template match="nar:SUS_Order_pre_LASBO">
	...
	<!-- Imprisonment Title -->
	<xsl:template match="nar:SUS_ImpTitle_pre_LASBO">
		<fo:inline>
			<xsl:text>1. </xsl:text><xsl:value-of select="$baseAll/ord:WarningDetentionType"/>
		</fo:inline>
	</xsl:template>
	<!-- Template to display SUS order imprisonment section -->
	<xsl:template match="nar:SUS_Imp_pre_LASBO">
...
<!--		<xsl:for-each select="$baseAll/ord:ImprisonmentTerm/ord:Term">
	...
	<!-- Requirements title -->
	<xsl:template match="nar:SUS_RequirementTitle_pre_LASBO">
		<fo:inline>
			<xsl:text>2. Requirements</xsl:text>
		</fo:inline>
	</xsl:template>
	...
	<xsl:template match="nar:SUS_Requirement_pre_LASBO">
		<fo:block space-after="10pt">
	...
	<!-- Review Title -->
	<xsl:template match="nar:SUS_ReviewTitle_pre_LASBO">
		<fo:inline>
			<xsl:if test="$baseAll/ord:ReviewType/@selected='true'">
				<xsl:text>3. Review</xsl:text>
			</xsl:if>
		</fo:inline>
	</xsl:template>
	<!--  SUS Review text -->
	<xsl:template match="nar:SUS_Review_pre_LASBO">
		<xsl:if test="$baseAll/ord:ReviewType/@selected='true'">
			<fo:list-block provisional-distance-between-starts="5mm">
				<xsl:call-template name="no_bullet">
					<xsl:with-param name="text">
	...
	<!-- Warning Title -->
	<xsl:template match="nar:SUS_WarningTitle_pre_LASBO">
		<fo:inline>
			<xsl:text>Warning</xsl:text>
		</fo:inline>
	</xsl:template>
	<!-- Template to display SUS Warning text -->
	<xsl:template match="nar:SUS_Warning_pre_LASBO">
		<fo:block>
	...
	<!-- Note title -->
	<xsl:template match="nar:SUS_NoteTitle_pre_LASBO">
		<fo:inline>
			<xsl:text>Note</xsl:text>
		</fo:inline>
	</xsl:template>
	<!-- Template to display SUS Notes text -->
	<xsl:template match="nar:SUS_Note_re_LASBO">
	...
	<xsl:template match="nar:SUS_Signed_pre_LASBO">
		<!-- call template to display signed info -->
		<xsl:call-template name="SignedInfo"/>
	</xsl:template>
	<!-- Offence title -->
	<xsl:template match="nar:SUS_OffencesTitle_pre_LASBO">
		<fo:inline>
			<xsl:text>Offences</xsl:text>
		</fo:inline>
	</xsl:template>
	<!-- Template to display SUS Offences text -->
	<xsl:template match="nar:SUS_Offences_pre_LASBO">
		<xsl:choose>
	...
	<xsl:template match="nar:SUS_AttachementsTitle_pre_LASBO">
		<fo:inline>
			<xsl:text>Attached are:</xsl:text>
		</fo:inline>
	</xsl:template>
	<xsl:template match="nar:SUS_Attachements_pre_LASBO">
		<fo:inline>
