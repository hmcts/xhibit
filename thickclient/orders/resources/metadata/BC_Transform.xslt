...
<xsl:stylesheet version="1.0" xmlns:bs7666="http://www.govtalk.gov.uk/people/bs7666" xmlns:cs="http://www.courtservice.gov.uk/schemas/courtservice" xmlns:nar="http://www.courtservice.gov.uk/schemas/courtservice/xhibit/orders/narrative" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ord="http://www.courtservice.gov.uk/schemas/courtservice/xhibit/orders" xmlns:apd="http://www.govtalk.gov.uk/people/AddressAndPersonalDetails" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:set="http://xml.apache.org/xslt">
    ...
    <!-- ************************ -->
    <!-- Bail Granted Text -->
    <xsl:template match="nar:BC_BailGranted">
        <fo:inline>
            <xsl:text>Bail granted </xsl:text>
        </fo:inline>
    </xsl:template>
    <!-- Type of Bail -->
    <xsl:template match="nar:BC_BailGrantedType">
        <fo:inline>
            <xsl:if test="$base/ord:BailOrder/ord:BailDecision='Conditional'">
                <xsl:text>conditionally</xsl:text>
            </xsl:if>
            <xsl:if test="$base/ord:BailOrder/ord:BailDecision='Unconditional'">
                <xsl:text>unconditionally</xsl:text>
            </xsl:if>
        </fo:inline>
    </xsl:template>
    <!-- Record of Decision -->
    <xsl:template match="nar:BC_RecordOfDecision">
        <fo:inline>
            <xsl:text> (record of decision)</xsl:text>
        </fo:inline>
    </xsl:template>
    <!-- Part 1 -->
    <xsl:template match="nar:BC_Part1">
        <fo:inline>
            <xsl:text>Part 1</xsl:text>
        </fo:inline>
    </xsl:template>
    <!-- Personal Details -->
    <xsl:template match="nar:BC_Part1_Title1">
        <fo:inline>
            <xsl:text>Personal details</xsl:text>
        </fo:inline>
    </xsl:template>
    <!-- Template used to display appropriate text depending on the case type -->
    <xsl:template match="nar:BC_AppellantDefendant">
        <fo:inline>The </fo:inline>
        <xsl:call-template name="AppellantDefendant"/>
    </xsl:template>
    <!-- Apply to Court -->
    <xsl:template match="nar:BC_ApplyToCourt">
        <fo:inline>
            <xsl:text>applied to this Court for bail.</xsl:text>
        </fo:inline>
    </xsl:template>
    <!-- Part 2 -->
    <xsl:template match="nar:BC_Part2">
        <fo:inline>
            <xsl:text>Part 2</xsl:text>
        </fo:inline>
    </xsl:template>
    <!-- Part 2 Title Line 1 -->
    <xsl:template match="nar:BC_Part2_Title1">
        <fo:inline>
            <xsl:text>The decision and</xsl:text>
        </fo:inline>
    </xsl:template>
    <!-- Part 2 Title Line 2 -->
    <xsl:template match="nar:BC_Part2_Title2">
        <fo:inline>
            <xsl:text>any conditions</xsl:text>
        </fo:inline>
    </xsl:template>
    <!-- Bail Act 1976 -->
    <xsl:template match="nar:BC_BailAct1976">
        <fo:inline>
            <xsl:text>The Court considered the application under the Bail Act 1976 on: </xsl:text>
        </fo:inline>
    </xsl:template>
    <!-- Was Order Amended Varied? - RFS4417 -->
    <xsl:template match="nar:BC_AmendedVaried">
    ...
    <!-- Bail Was Granted -->
    <xsl:template match="nar:BC_BailWasGranted">
        <fo:inline>
            <xsl:text>Bail was granted: </xsl:text>
        </fo:inline>
    </xsl:template>
    <!-- Bail Order Pre Condition Text -->
    <xsl:template match="nar:BC_PreConditions">
        <xsl:if test="$base/ord:BailOrder">
            <!-- Surety -->
    ...
    <xsl:template match="nar:BC_PostConditions">
        <xsl:if test="$base/ord:BailOrder">
            <xsl:for-each select="$baseAll/ord:PostConditions[@selected='true']">
                ...
                <!-- call template to display International Travel Warrant details -->
                            ...
                            <fo:block>
                                ...
                                <xsl:if test="ord:ElectronicMonitoring='true'">
								  ...
								  <xsl:if test="ord:DoorstepCondition='true'">
									<fo:block>
										<xsl:text>The Defendant is required to present him/herself at the door of his/her residence to a Police Officer on request.</xsl:text>
									</fo:block>
								</xsl:if>
                            </fo:block>
                        </fo:list-item-body>
    ...
    <!-- Part 3 -->
    <xsl:template match="nar:BC_Part3">
        <fo:inline>
            <xsl:text>Part 3</xsl:text>
        </fo:inline>
    </xsl:template>
    <!-- Part 3 Title Line 1 -->
    <xsl:template match="nar:BC_Part3_Title1">
        <fo:inline>
            <xsl:text>Reason(s) for imposing</xsl:text>
        </fo:inline>
    </xsl:template>
    <!-- Part 3 Title Line 2 -->
    <xsl:template match="nar:BC_Part3_Title2">
        <fo:inline>
            <xsl:text>the above conditions</xsl:text>
        </fo:inline>
    </xsl:template>
    <!-- Template to display Reasons Text -->
    <xsl:template match="nar:BC_Reasons">
    ...
    <!-- Part 4 -->
    <xsl:template match="nar:BC_Part4">
        <fo:inline>
            <xsl:text>Part 4</xsl:text>
        </fo:inline>
    </xsl:template>
    <!-- Part 4 Title Line 1 -->
    <xsl:template match="nar:BC_Part4_Title1">
        <fo:inline>
            <xsl:text>The next appearance</xsl:text>
        </fo:inline>
    </xsl:template>
    <!-- Shall appear -->
    <xsl:template match="nar:BC_ShallAppear">
        <fo:inline>
            <xsl:text> shall appear </xsl:text>
        </fo:inline>
    </xsl:template>
    <!-- Bail Order specific court text -->
    <xsl:template match="nar:BC_CourtText">
        <xsl:choose>
