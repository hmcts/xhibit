...
<xsl:stylesheet version="1.0" xmlns:bs7666="http://www.govtalk.gov.uk/people/bs7666" xmlns:cs="http://www.courtservice.gov.uk/schemas/courtservice" xmlns:nar="http://www.courtservice.gov.uk/schemas/courtservice/xhibit/orders/narrative" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ord="http://www.courtservice.gov.uk/schemas/courtservice/xhibit/orders" xmlns:apd="http://www.govtalk.gov.uk/people/AddressAndPersonalDetails" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:set="http://xml.apache.org/xslt">
    ...
    <xsl:template match="nar:CPRO_Text1">
        <xsl:choose>
            ...
            <xsl:when test="$baseAll/ord:CommittingCourt/@selected='true'">
                <fo:block>
                    at <xsl:call-template name="CommunityCommittingCourt"/> and was committed for sentence to the Crown Court.
                </fo:block>
            </xsl:when>
            <xsl:when test="$baseAll/ord:Breach/@selected='true'">
                ...
                <fo:block space-after="12pt">
                    This Court
                    was satisfied that the defendant has failed to comply with a requirement of the above order
                    <xsl:if test="$baseAll/ord:OriginalOrderRevoked/@selected='true' and $baseAll/ord:OriginalOrderRevoked/ord:BreachCourt='no'">
    ...
    <!-- the court -->
    <xsl:template match="nar:CPRO_TheCourt">
        <fo:inline>
    ...
    <!-- CPRO text 2-->
    <!-- used within the Community PUNISHMENT AND REHAB Order -->
    <xsl:template match="nar:CPRO_Text2">
            ...
            <fo:list-block provisional-distance-between-starts="5mm">
                <fo:list-item>
                    ...
                    <fo:list-item-body start-indent="body-start()">
                        <fo:block>
                            being of the opinion that it was appropriate to make a Community Rehabilitation Order, explained to the defendant the effect of this order (including any additional requirements shown below), the consequences which may follow failure to comply with the requirements of the order, and that the Court has the power to review this order on the application of either the defendant or the supervising officer.
                        </fo:block>
                    </fo:list-item-body>
                </fo:list-item>
                <fo:list-item>
                    ...
                    <fo:list-item-body start-indent="body-start()">
                        <fo:block>
                            after considering a report by a probation officer or social worker of a local authority social services department about the defendant and the defendant's circumstances <xsl:call-template name="HearingOption2"/><xsl:text> </xsl:text> was satisfied that the defendant is a suitable person to perform community punishment work and that provision can be made for the defendant to do so in the above local justice area.
                        </fo:block>
                    </fo:list-item-body>
                </fo:list-item>
            </fo:list-block>
                    ...
                    <fo:list-item>
                        ...
                        <fo:list-item-body start-indent="body-start()">
                            <fo:block>
                                ...
                                from the date of this order be under the supervision of a probation officer in the local justice area of <xsl:call-template name="PettySessionalArea"/><xsl:text>.</xsl:text>
                            </fo:block>
                        </fo:list-item-body>
                    </fo:list-item>
                    <fo:list-item>
                    ...
                    <fo:list-item>
                        ...
                        <fo:list-item-body start-indent="body-start()">
                            ...
                            <xsl:for-each select="$baseAll/ord:CommunityPunishment/ord:Period">
                                ...
                                from the date of this order perform <xsl:call-template name="CommunityPunishmentHours"/> hour(s) of community punishment work at such times as the probation officer may instruct. 
    ...
    <!-- Template to display hearing option details -->
    <xsl:template name="HearingOption2">
    ...
    <!-- *******************************************************************-->
    <!-- COMMUNITY PUNISHMENT REHAB ORDER (CPRO) END -->
    <!-- *******************************************************************-->
</xsl:stylesheet>
