...
public class OrderSignedDialog extends XDialog {
    ...
    public OrderSignedDialog(Frame f, String title, boolean judgeToSign, XhibitApplicationController xac)
            throws CSRecoverableException {
        super(f, title, true, XDialog.OKCANCEL, XDialog.DEFAULTOK);
        this.judgeToSign = judgeToSign;

        signedVO = new SignedDetailsVO();
        signedVO.setController(xac);
    ...
    private XPanel buildPanel() throws CSRecoverableException {
        OrderSignedListener signListener = new OrderSignedListener(signedVO, getOKButton());
     ...
     */
    public SignedDetailsVO getSignDetails() throws CancelledByUserException {
        if (signedVO.getCancelledBy() == true) {
