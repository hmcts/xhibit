...
/* Add Version Number and last updated time columns to all tables in line with XHIBIT */
alter table CJI_DOCUMENT add VERSION NUMBER(5) null;
alter table CJI_DOCUMENT add LAST_UPDATE_DATE DATE null;

alter table CJI_EVENT add VERSION NUMBER(5) null;
alter table CJI_EVENT add LAST_UPDATE_DATE DATE null;

/* Add Triggers to update Version Number and last updated time */

CREATE OR REPLACE TRIGGER CJI_DOCUMENT_BIR_TR
  BEFORE INSERT
  ON CJI_DOCUMENT
  FOR EACH ROW
...
CREATE OR REPLACE TRIGGER CJI_EVENT_BIR_TR
  BEFORE INSERT
  ON CJI_EVENT
  FOR EACH ROW
...
CREATE OR REPLACE TRIGGER CJI_DOCUMENT_BUR_TR
  BEFORE UPDATE
  ON CJI_DOCUMENT
  FOR EACH ROW
...
CREATE OR REPLACE TRIGGER CJI_EVENT_BUR_TR
  BEFORE UPDATE
  ON CJI_EVENT
  FOR EACH ROW
...
/* Add a Sent_Date to the CJI_DOCUMENT table */

alter table CJI_DOCUMENT add SENT_DATE DATE null;


/* Remove unused Column  BITS_DOCUMENT_COMMENT */
alter table CJI_DOCUMENT drop Column BITS_DOCUMENT_COMMENT;



/* Add a Sent_Date to the CJI_DOCUMENT table - which is the date and time that the document was sent (successfully) to the CJSE */
/* Use Received_Date only for the the date and time when the event is added to the database. */

alter table CJI_EVENT add SENT_DATE DATE null;
