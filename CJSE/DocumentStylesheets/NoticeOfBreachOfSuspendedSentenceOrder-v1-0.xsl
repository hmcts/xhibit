...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:cs="http://www.courtservice.gov.uk/schemas/courtservice" xmlns:apd="http://www.govtalk.gov.uk/people/AddressAndPersonalDetails" xmlns:n1="http://www.govtalk.gov.uk/people/bs7666" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:util="http://www.courtservice.gov.uk/transforms/courtservice/gcsUtility" xmlns:date="http://xsltsl.org/date-time" xmlns:str="http://xsltsl.org/string" xmlns:doc="http://xsltsl.org/xsl/documentation/1.0" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:xso="http://www.w3.org/1999/XSL/Transform" exclude-result-prefixes="fo" extension-element-prefixes="util date str doc xsd n1 apd cs">
	...
	<doc:reference xmlns="">
		...
		<title>Notice Of Breach of Suspended Sentence Order Stylesheet</title>
		<para>File name : NoticeOfBreachOfSuspendedSentenceOrder-v1-0.xsl</para>
		<partintro>
			<section>
				<title>Introduction</title>
				<para>This module produces the Notice of Breach of Suspended Sentence Order in html format</para>
			</section>
		</partintro>
	</doc:reference>
	<!-- Version Information -->
	...
	<xsl:variable name="stylesheet" select="'NoticeOfBreachOfSuspendedSentenceOrder-v1-0.xsl'"/>
		...
		<html>
			...
			<body>
				...
				<xsl:call-template name="util:UniversalOrderHeader">
					...
					<xsl:with-param name="OrderHeaderRoot" select="/cs:NoticeBreachSuspendedSentenceOrder /cs:OrderHeader"/>
				</xsl:call-template>
				<!-- +++++++++ following template processes the Personal Details+++++++++ -->
				<xsl:apply-templates select="/cs:NoticeBreachSuspendedSentenceOrder/cs:OrderHeader/cs:Defendant/cs:PersonalDetails"/>
				<xsl:call-template name="NBSS_ForAttention"/>
				<xsl:call-template name="NBSS_OriginalSentence"/>
				<xsl:call-template name="NBSS_Offences"/>
				<xsl:call-template name="NBSS_Order"/>
	...
	<xsl:template name="NBSS_ForAttention">
		<h2>For Attention</h2>
		<xsl:text>The Chief Clerk</xsl:text>
		<br/>
		<xsl:value-of select="/cs:NoticeBreachSuspendedSentenceOrder/cs:AddresseeCourt/cs:CourtHouseName"/>
		<br/>
		<xsl:for-each select="/cs:NoticeBreachSuspendedSentenceOrder/cs:AddresseeCourt/cs:CourtHouseAddress/apd:Line">
			<xsl:if test=".!='-'">
				<xsl:value-of select="."/>
				<br/>
			</xsl:if>
		</xsl:for-each>
		<xsl:value-of select="/cs:NoticeBreachSuspendedSentenceOrder/cs:AddresseeCourt/cs:CourtHouseAddress/apd:PostCode"/>
		<br/>
		<xsl:value-of select="/cs:NoticeBreachSuspendedSentenceOrder/cs:AddresseeCourt/cs:CourtHouseDX"/>
		<br/>
	</xsl:template>
	
	<!-- Template to display Original Sentence text -->
	<xsl:template name="NBSS_OriginalSentence">
		<h2>Original Sentence</h2>
		<xsl:text>Date suspended sentence order imposed:</xsl:text>
		<br/>
		<xsl:value-of select="/cs:NoticeBreachSuspendedSentenceOrder/cs:SuspendedSentenceImposedDate"/>
	</xsl:template>
	
	<!-- Template to display Original Sentence text -->
	<xsl:template name="NBSS_Offences">
		<h2>Subsequent Offences</h2>
		<xsl:text>The offender named above has been dealt with by this Court in respect of a suspended sentence order imposed by your Court.</xsl:text>
		<br/>
		<xsl:value-of select="/cs:NoticeBreachSuspendedSentenceOrder/cs:SubsequentOffences"/>
		<br/>
		<xsl:text>Date dealt with by this Court: </xsl:text>
		<xsl:value-of select="/cs:NoticeBreachSuspendedSentenceOrder/cs:DateOfConviction"/>
	</xsl:template>
	
	<!-- Template to display Court Ordered text -->
	<xsl:template name="NBSS_Order">
		<h2>Order of this Court</h2>
		<xsl:if test="/cs:NoticeBreachSuspendedSentenceOrder/cs:CourtOrdered/cs:LesserTerm ='SuspendedSentenceOriginalTerm'">
		...
		<xsl:if test="/cs:NoticeBreachSuspendedSentenceOrder/cs:CourtOrdered/cs:LesserTerm !='SuspendedSentenceOriginalTerm'">
			...
			<xsl:call-template name="util:decodeModifiedDuration">
				<xsl:with-param name="duration" select="/cs:NoticeBreachSuspendedSentenceOrder/cs:CourtOrdered/cs:LesserTerm"/>
			</xsl:call-template>
			<br/>
		</xsl:if>
		<xsl:if test="/cs:NoticeBreachSuspendedSentenceOrder/cs:CourtOrdered/cs:Fine">
			<xsl:text>Fine of £ </xsl:text>
			<xsl:value-of select="/cs:NoticeBreachSuspendedSentenceOrder/cs:CourtOrdered/cs:Fine"/>
			<br/>
		</xsl:if>
		<xsl:if test="/cs:NoticeBreachSuspendedSentenceOrder/cs:CourtOrdered/cs:Requirements">
			<xsl:text>More onerous community requirement(s) imposed : </xsl:text>
			<xsl:value-of select="/cs:NoticeBreachSuspendedSentenceOrder/cs:CourtOrdered/cs:Requirements"/>
			<br/>
		</xsl:if>
		<xsl:if test="/cs:NoticeBreachSuspendedSentenceOrder/cs:CourtOrdered/cs:SupervisionalPeriod">
			<xsl:text>Supervision period of the suspended sentence order extended: </xsl:text>
			<xsl:call-template name="util:decodeModifiedDuration">
				<xsl:with-param name="duration" select="/cs:NoticeBreachSuspendedSentenceOrder/cs:CourtOrdered/cs:SupervisionPeriod"/>
			</xsl:call-template>
			<br/>
		</xsl:if>
		<xsl:if test="/cs:NoticeBreachSuspendedSentenceOrder/cs:CourtOrdered/cs:OperationalPeriod">
			<xsl:text>Operational period of the suspended sentence order extended: </xsl:text>
			<xsl:call-template name="util:decodeModifiedDuration">
				<xsl:with-param name="duration" select="/cs:NoticeBreachSuspendedSentenceOrder/cs:CourtOrdered/cs:OperationalPeriod"/>
			</xsl:call-template>
			<br/>
		</xsl:if>
	</xsl:template>
