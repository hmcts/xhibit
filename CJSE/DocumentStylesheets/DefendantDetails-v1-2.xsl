...
<xsl:stylesheet version="1.2" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:cs="http://www.courtservice.gov.uk/schemas/courtservice" xmlns:apd="http://www.govtalk.gov.uk/people/AddressAndPersonalDetails" xmlns:n1="http://www.govtalk.gov.uk/people/bs7666" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:util="http://www.courtservice.gov.uk/transforms/courtservice/gcsUtility" xmlns:doc="http://xsltsl.org/xsl/documentation/1.0" xmlns:date="http://xsltsl.org/date-time" xmlns:str="http://xsltsl.org/string" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:xso="http://www.w3.org/1999/XSL/Transform" exclude-result-prefixes="fo" extension-element-prefixes="util date str doc xsd n1 apd cs">
	...
	<doc:reference xmlns="">
		...
		<para>File name : DefendantDetails-v1-2.xsl</para>
		<partintro>
			<section>
	...
	<xsl:variable name="stylesheet" select="'defendantdetails-v1-2.xsl'"/>
	<xsl:variable name="last-modified-date" select="'2011-09-09'"/>
	...
	<xsl:template name="details">
		...
		<xsl:choose>
			...
			<b>
				<xsl:choose>
						<xsl:when test="$personalDetails/cs:Sex">
							<xsl:choose>
					...
					<xsl:otherwise>
						<xsl:text>- data removed -</xsl:text>
					</xsl:otherwise>
				</xsl:choose>
			</b>
			</xsl:when> 
		...
		<xsl:choose>
			...
			<b>
				<xsl:choose>
					<xsl:when test="$personalDetails/cs:DateOfBirth">
						<xsl:call-template name="util:ukdate_mon">
		...
		<xsl:choose>
			...
			<b>
				<xsl:choose>
					<xsl:when test="$personalDetails/cs:Nationality">
		...
		<xsl:choose>
			<xsl:when test="//cs:DefendantDetails/cs:SectionsAmended/cs:SectionAmended = 'MaskedDetails'">
				...
				<b>
					<xsl:choose>
						<xsl:when test="$personalDetails/cs:MaskedName">
		...
		<xsl:choose>
			...
			<b>
				<xsl:choose>
					<xsl:when test="//cs:DefendantDetails/cs:Defendant/cs:ASNs/cs:ASN">
		...
		<xsl:choose>
			...
			<b>
				<xsl:choose>
					<xsl:when test="//cs:DefendantDetails/cs:Defendant/cs:URN">
		...
		<xsl:choose>
			...
			<b>
				<xsl:choose>
					<xsl:when test="//cs:DefendantDetails/cs:Defendant/cs:LastConvictionDate">
						<xsl:call-template name="util:ukdate_mon">
