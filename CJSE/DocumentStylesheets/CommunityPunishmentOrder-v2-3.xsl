...
<doc:reference xmlns="">
	...
	<partintro>
		<section>
			...
			<para>PR57824 - Changed on date format</para>
			<para>PR57857 - Misisng Satisified sentence</para>
			<para>PR58661 - Added to comma to failComplyAndHearingFrom - removed comma from FailedToComply</para>
		</section>
	</partintro>
</doc:reference>
...
<xsl:variable name="minorVersion" select="'3i'" />
<xsl:variable name="stylesheet" select="'CommunityPunishmentOrder-v2-3.xsl'" />
<xsl:variable name="last-modified-date" select="'2006-06-14'" />
...
<xsl:output method="html" indent="yes"/>
		...
		<html>
			...
			<body>
				...
				<xsl:call-template name="util:UniversalOrderHeader"> 
					...
					<xsl:with-param name="OrderHeaderRoot" select="/cs:CommunityPunishmentOrder/cs:OrderHeader"/>
				</xsl:call-template>
				
				<!-- +++++++++ following template processes the Personal Details+++++++++ -->	
	   			<xsl:apply-templates select="/cs:CommunityPunishmentOrder/cs:OrderHeader/cs:Defendant/cs:PersonalDetails" />
				
				<!-- +++++++++ following template process each section of the CP order +++++++++ -->	
	...
	<xsl:template match="cs:CommittingCourt" >
		...
		<!-- PR 57383 
		<xsl:text> and was committed for sentence to be dealt with. </xsl:text>
		-->
	...
	<xsl:template match="cs:Breach" >
		...
		<xsl:if test="../cs:OriginalOrderRevoked/cs:BreachCourt='yes'">
			...
			<xsl:text> to deal with the defendant in some other manner for the offences for which it made the order</xsl:text>
			<!-- KN 20060210 - PR58273 End-->
			<xsl:if test="cs:OriginatingCourt/cs:Date">
				<xsl:text> on </xsl:text>
				<xsl:call-template name="util:ukdate_mon">
					<xsl:with-param name="inDate" select="cs:OriginatingCourt/cs:Date" />
	...
	<xsl:template match="cs:FailedToComply">
		<xsl:if test=". = 'yes' " > 
			...
			<xsl:if test="//cs:OriginalOrderRevoked/cs:CurrentCourt = 'yes' ">
				<xsl:text> and this Court was satisfied that it was in the interests of justice to revoke the order </xsl:text>
			</xsl:if>
		...
		<xsl:if test=". = 'no' " > 
			<xsl:text> </xsl:text>
			<xsl:if test="//cs:OriginalOrderRevoked/cs:CurrentCourt = 'yes' ">
				<xsl:text> this Court was satisfied that it was in the interests of justice to revoke the order </xsl:text>
			</xsl:if>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template name="failComplyAndHearingFrom">
			<br />
			<xsl:text>On </xsl:text>
			<!-- KN 20060210 - PR 57824 -->
			<!-- <xsl:call-template name="util:ukdate_fullMonth"> -->
			<xsl:call-template name="util:ukdate_mon">
			<!-- KN 20060210 - PR 57824 End-->
				<xsl:with-param name="inDate" select="//cs:OrderHeader/cs:OrderDate"/>
			</xsl:call-template>
			<!-- PR58661 added new comma-->
			<xsl:text>,</xsl:text>
			...
			<xsl:text> was satisfied that the 
					  defendant is a suitable person to perform community punishment work and 
					  that provisions can be made for the defendant to do so in the local justice area of 
			</xsl:text>
	...
	followed by any conditions that apply.</refpurpose>
 	...
 	<xsl:template name="orderConditions">
		<xsl:if test="//cs:CommunityPunishment" >
			...
			<table width="100%">
				...
				<xsl:if test="//cs:CommunityPunishment" >
					...
					<xsl:variable name="text2" >
						...
						<xsl:text> of community punishment work at such time as that officer may instruct.</xsl:text>
			...
			<xsl:if test="//cs:Conditions/cs:Condition">
	...
	<xsl:template name="footer">
		<small>
			<xsl:text>(For the use of the probation service only)  </xsl:text>
		</small>
