...
<doc:reference xmlns="">
	...
	<para>File name : WarrantAfterFailureOrder5061B-v1-0.xsl</para>
	<partintro>
		<section>
			<title>Introduction</title>
			<para>This module produces the Bench Warrant in html format</para>
			<para>Warrant for arrest after failure to comply with Community Order/Suspended Sentence Order</para>
		</section>
	</partintro>
</doc:reference>
...
<xsl:variable name="stylesheet" select="'WarrantAfterFailureOrder5061B-v1-0.xsl'" />
...
<xsl:output method="html" indent="yes"/>
			...
			<body>
				...
				<xsl:call-template name="util:UniversalOrderHeader"> 
					...
					<xsl:with-param name="OrderTitle">	
						<xsl:text>Warrant for arrest after failure to comply with</xsl:text> 
				...
				<xsl:apply-templates select="cs:WarrantAfterFailureOrder/cs:NextAppearance" /> 
	...
	<xsl:template match="cs:PersonalDetails">
		<table width="100%" >
			...
			<tr>				
				<td colspan="100%">				
					<xsl:text>has failed to comply with a </xsl:text>
					<xsl:if test="//cs:WarrantAfterFailureOrder/cs:AssociatedOrderType='communityOrder'">
                        <xsl:choose>
					...
					<xsl:text> imposed by this court.</xsl:text>
				</td>
			</tr>			
		</table>
	</xsl:template>
			...
			<tr>
				<td width="100%" colspan="100%">				
					<strong>
						<xsl:text>On </xsl:text>
						<xsl:call-template name="util:ukdate_mon">
								<xsl:with-param name="inDate" select="//cs:WarrantAfterFailureOrder/cs:OrderHeader/cs:OrderDate"/>
						</xsl:call-template>
			...
			<tr>				
				<td colspan="100%">	
					...
					<xsl:choose>
						...
						<!-- Unconditional -->
						<xsl:when test="$conditionality = 'Conditional' and not (cs:WarrantAfterFailureOrder/cs:PreConditions | cs:WarrantAfterFailureOrder/cs:PostConditions) ">
	...
	<xsl:template match="cs:PostConditions" >
		...
		<table width="100%" >
			...
			<tr>			
				...
				<td width="90%">		
						<xsl:call-template name="util:bulletRow" >
							<xsl:with-param name="bulletText" select="//cs:PersonalDetails/cs:Name"/>
						</xsl:call-template>
				</td>
			</tr>
		</table>			
