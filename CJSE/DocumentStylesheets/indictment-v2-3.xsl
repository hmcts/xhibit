...
<doc:reference xmlns="">
	...
	<para>File name : indictment-v2-3.xsl</para>
	<partintro>
		<section>
...
<xsl:variable name="stylesheet" select="'indictment-v2-3.xsl'" />
<xsl:variable name="last-modified-date" select="'2004-09-17'" />
...
<xsl:variable name="courtInfo" >
	...
	<xsl:call-template name="str:to-upper">
		<xsl:with-param name="text" select="$court/cs:CourtHouseType"/>
	</xsl:call-template> 
...
<xsl:output method="html" indent="yes"/>
	...
	<doc:template name="Charges" xmlns="">
		...
		<xsl:for-each select="//cs:Defendant/cs:PersonalDetails[../cs:Charges/cs:Charge/cs:CRESToffenceNumber = $chargeNumber]">
			<tr>
				...
				<td width="30%">
					...
					<xsl:value-of select="//cs:Defendant/cs:URN" />
				</td>
			</tr>
		</xsl:for-each>
		</table>
	...
	<!-- processes the header information - constructs the initial header information for the 
		...
		<table class="emphasis" WIDTH="100%">
			<tr>
				...
				<td WIDTH="20%"><h1><xsl:value-of select="$outputType" /></h1></td>
