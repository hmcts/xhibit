...
<doc:reference xmlns="">
	...
	<para>File name : ImprisonmentOrder-v2.xsl</para>
	<partintro>
		<section>
...
<xsl:variable name="stylesheet" select="'ImprisonmentOrder-v2.xsl'" />
<xsl:variable name="last-modified-date" select="'2003-11-21'" />
...
<xsl:output method="html" indent="yes"/>
	...
	<xsl:template match="/">
		<html>
			...
			<body>
				...
				<xsl:apply-templates select="//cs:CustodialSentence[cs:CustodialTerm/cs:ExtendedPeriod]" mode="extension"/>
	...
	<!-- processes the OrderHeader node - constructs the initial header information for the output -->
		...
		<center>
			<h1>
			<strong>
				<xsl:value-of select="'Order for imprisonment'"/>
			</strong>
			</h1>
		</center>
		<hr />
		...
		<table width="100%" >
			...
			<tr>
				...
				<td>
					<strong>
					<xsl:choose>
					<xsl:when test="./cs:ForLife">
						<xsl:text>imprisonment for life.</xsl:text>
					</xsl:when>
					<xsl:otherwise>
						<xsl:call-template name="util:decodeDuration">
							<xsl:with-param name="duration" select="cs:CustodialTerm/cs:Term"/>
						</xsl:call-template>
			...
			<tr>
				<td />
				<td>
					<xsl:if test="not (./cs:ForLife)and
					              cs:CustodialTerm/cs:Term/@TermType" >
						<xsl:text>This sentence was ordered to be </xsl:text>
						<xsl:variable name="termType" >
							<xsl:call-template name="str:to-lower">
								<xsl:with-param name="text" select="cs:CustodialTerm/cs:Term/@TermType" />
							</xsl:call-template>
						</xsl:variable>
						<xsl:choose>
	...
	<xsl:template match="cs:CustodialSentence[cs:CustodialTerm/cs:ExtendedPeriod]" mode="extension">
		<hr />
		<table width="100%" >
			<tr>
				<td width="20%" valign="top">
			...
			<tr>
				...
				<td>
					...
					<xsl:call-template name="util:decodeDuration">
						<xsl:with-param name="duration" select="cs:CustodialTerm/cs:TotalTerm"/>
					</xsl:call-template>
			...
			<tr>
				...
				<td>
					...
					<xsl:call-template name="util:decodeDuration">
						<xsl:with-param name="duration" select="cs:CustodialTerm/cs:ExtendedPeriod"/>
					</xsl:call-template>
					<xsl:text>.</xsl:text>
				</td>
			</tr>
			<xsl:if test="cs:CustodialTerm/cs:ExtendedPeriod/@TermType">
				<tr>
					<td />
					<td>
						<xsl:if test="cs:CustodialTerm/cs:ExtendedPeriod/@TermType">
							...
							<xsl:variable name="termType" >
								<xsl:call-template name="str:to-lower">
									<xsl:with-param name="text" select="cs:CustodialTerm/cs:ExtendedPeriod/@TermType" />
								</xsl:call-template>
							</xsl:variable>
							<xsl:choose>
								<xsl:when test="$termType = 'concurrent'">
			...
			<xsl:if test="cs:CustodialTerm/cs:Section44 = 'yes'">
				<tr>
					<td />
					<td>
