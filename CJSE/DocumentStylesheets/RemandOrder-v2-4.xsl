...
<doc:reference xmlns="">
	...
	<para>File name : RemandOrder-v2.3xsl</para>
	<partintro>
		<section>
...
<xsl:variable name="stylesheet" select="'RemandOrder-v2-4.xsl'" />
<xsl:variable name="last-modified-date" select="'2014-08-22'" />
...
<xsl:output method="html" indent="yes"/>
	...
	<xsl:template match="/">
		<html>
			...
			<body>
						...
						<p>
							<xsl:choose>
								<xsl:when test="/cs:RemandOrder/cs:RemandOrderType/@RemandType = 'LocalAuthority'">
									<xsl:text>Remand Order - Local Authority Accomodation</xsl:text>
								</xsl:when>
								<xsl:when test="/cs:RemandOrder/cs:RemandOrderType/@RemandType = 'YouthDetention'">
									<xsl:text>Remand Order - Youth Detention Accomodation</xsl:text>
								</xsl:when>
								<xsl:otherwise>
					...
					<xsl:with-param name="OrderAddressee">
						<p>
							<xsl:choose>
								...
								<xsl:when test="/cs:RemandOrder/cs:RemandOrderType/@RemandType = 'YouthDetention'">
									<xsl:text>To conveyor / custodian &amp; Manager of Youth Detention Accomodation</xsl:text>
								</xsl:when>
								<xsl:otherwise>
				...
				<xsl:apply-templates select="/cs:RemandOrder/cs:RemandOrderType/cs:RequirementsOnLocalAuthority" />
	...
	<xsl:template name="remandDetails" >
		...
		<xsl:choose>
			...
			<xsl:when test="/cs:RemandOrder/cs:RemandOrderType/@RemandType = 'YouthDetention'">
				<xsl:text>be remanded into Youth Detention Accomodation at </xsl:text>
			</xsl:when>
			<xsl:otherwise>
	...
	<xsl:template match="cs:RequirementsOnLocalAuthority" >
		<tr>
			<td >
				<h3>
					<xsl:text>Requirements of the court on the Local Authority</xsl:text>
				</h3>
				<xsl:value-of select="/cs:RemandOrder/cs:RemandOrderType/cs:RequirementsOnLocalAuthority" />
			</td>
		</tr>
	</xsl:template>
