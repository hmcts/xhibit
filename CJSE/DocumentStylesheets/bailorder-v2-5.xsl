...
<doc:reference xmlns="">
		...
		<para>File name : bailorder-v2-4.xsl</para>
		<partintro>
			<section>
				<title>Introduction</title>
				<para>This module produces the Bail Order in html format</para>
				<para>Change made for CR28</para>
				<para>Added CR27</para>
				<para>Added PR57240</para>
				<para>Added PR57272</para>
				<para>v2.4 - Chanhges for RFS4417</para>
			</section>
		</partintro>
	</doc:reference>
...
<xsl:variable name="stylesheet" select="'bailorder-v2-4.xsl'" />
<xsl:variable name="last-modified-date" select="'2015-04-23'" />
...
<xsl:output method="html" indent="yes"/>
		...
		<html>
			...
			<body>
				...
				<xsl:call-template name="util:UniversalOrderHeader"> 
					...
					<xsl:with-param name="OrderHeaderRoot" select="/cs:BailOrder/cs:OrderHeader"/>
					<xsl:with-param name="DisplayASN">YES</xsl:with-param>
				</xsl:call-template>

				<xsl:apply-templates select="//cs:OrderHeader/cs:Defendant/cs:PersonalDetails" />
	...
	<xsl:template name="decisionAndConditions" >
		...
		<p>
			...
			<xsl:if test="//cs:BailOrder/cs:BailChanges">
				<xsl:text>. On </xsl:text>
				<xsl:call-template name="util:ukdate_mon">
					<xsl:with-param name="inDate" select="//cs:BailOrder/cs:BailChanges/cs:AmendedVariedDate"/>
				</xsl:call-template>
				<xsl:text> this Order was </xsl:text>
				<xsl:choose>
					<xsl:when test="//cs:BailOrder/cs:BailChanges/cs:ChangeType='Amended'">
						<xsl:text>amended </xsl:text>
					</xsl:when>
					<xsl:otherwise>
						<xsl:text>varied </xsl:text>
					</xsl:otherwise>
				</xsl:choose>
				<xsl:text>to the following conditions: </xsl:text>
			</xsl:if>
		</p>
		<p>
	...
	<doc:template name="preConditions" xmlns="">
		...
		<refdescription>
			...
			<para>
		  	...
		  	<itemizedlist>
				<listitem>
			  		<para>Note: Show as section A if present</para>
			  	</listitem>
			  	<listitem>
			  	...
			  	<listitem>
			  		<para>Note: The template util:bulletRow is called to display each condition as a bulleted item in a row in a table.</para>
		  	</listitem>
		  	</itemizedlist>
		</refdescription>
	</doc:template>
			...
			<xsl:for-each select="cs:Condition">
				<xsl:variable name="text" >
					...
					<xsl:if test="cs:Surety">
						...
						<!-- KN 20050429 - CR 28 update -->
						<!-- <xsl:text> to custody at the time and place directed (recognizance of </xsl:text> -->
						<xsl:text> to custody at the time and place directed (recognizance(s) of </xsl:text>
						<!-- End of Change KN 20050429 - CR 28 update -->
						<xsl:value-of select="normalize-space(cs:Description)"/>
						<xsl:text> endorsed on Form 5102D: 'Bail: recognizance of a surety').</xsl:text>
					</xsl:if>
	...
	<xsl:template name="nextAppearance">
		...
		<table width="100%">
			<xsl:call-template name="util:bulletRow">
				...
				<xsl:with-param name="bulletText" select="'If you do not surrender to custody you will commit an offence.'"/>
			</xsl:call-template>
			<xsl:call-template name="util:bulletRow">
				<xsl:with-param name="bulletWidth" select="'10'"/>
