...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:cs="http://www.courtservice.gov.uk/schemas/courtservice" xmlns:apd="http://www.govtalk.gov.uk/people/AddressAndPersonalDetails" xmlns:n1="http://www.govtalk.gov.uk/people/bs7666" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:util="http://www.courtservice.gov.uk/transforms/courtservice/gcsUtility" xmlns:date="http://xsltsl.org/date-time" xmlns:str="http://xsltsl.org/string" xmlns:doc="http://xsltsl.org/xsl/documentation/1.0" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:xso="http://www.w3.org/1999/XSL/Transform" exclude-result-prefixes="fo" extension-element-prefixes="util date str doc xsd n1 apd cs">
	...
	<doc:reference xmlns="">
		...
		<para>File name : SuspendedSentenceOrder-v1-0.xsl</para>
		<partintro>
			<section>
	...
	<xsl:variable name="stylesheet" select="'SuspendedSentenceOrder-v1-0.xsl'"/>
	<xsl:variable name="last-modified-date" select="'2010-03-04'"/>
	...
	<xsl:template match="/">
		<html>
			...
			<body>
				...
				<xsl:apply-templates select="//cs:Imprisonment"/>
	...
	<xsl:template name="SSO_Order">
		...
		<xsl:variable name="total-requirements">
			<xsl:value-of select="count(//cs:OrderRequirements/*)"/>
		</xsl:variable>
		<xsl:text>There are </xsl:text>
		
		<xsl:choose>
			<xsl:when test="($total-requirements &gt; 0)">
				<xsl:value-of select="$total-requirements"/>		
			</xsl:when>
			<xsl:otherwise>
	...
	<xsl:template match="//cs:Imprisonment">
		<h2>1. Imprisonment</h2>
		<xsl:text>You have been sentenced to a term of imprisonment lasting </xsl:text>
		<xsl:call-template name="util:decodeDuration">
			<xsl:with-param name="duration" select="//cs:Imprisonment/cs:Term"/>
		</xsl:call-template>
		<xsl:text>.</xsl:text>
		<br/>
		<br/>
		<xsl:text>If you</xsl:text>
		  <ul>
			...
			<li>commit any other offence during the <b>operational period</b> of 
				<xsl:choose>
					<xsl:when test="//cs:OperationalPeriod/@SpecifiedInWeeks ='yes'">
						<!-- As specified in weeks will need to divide number of days by 7 -->
						<xsl:variable name="StripP" select="substring-after(//cs:OperationalPeriod,'P')" />
					...
					<xsl:otherwise>
						<xsl:call-template name="util:decodeDuration">
							<xsl:with-param name="duration" select="//cs:OperationalPeriod"/>
						</xsl:call-template>
					</xsl:otherwise>
				</xsl:choose>				
				, you will be liable to serve the <b>custodial period</b> in prison.
           </li>
       </ul>
	</xsl:template>
	...
	<xsl:template match="//cs:OrderRequirements">
		...
		<xsl:text>For the supervision period of which is the next </xsl:text>		
		<xsl:choose>
			<xsl:when test="//cs:SupervisionPeriod/@SpecifiedInWeeks ='yes'">
				<!-- As specified in weeks will need to divide number of days by 7 -->
				<xsl:variable name="StripP" select="substring-after(//cs:SupervisionPeriod,'P')" />
				<xsl:variable name="day" select="substring-before($StripP,'D')" />
				<!-- KN 2005-11-17  Mercator provide the data as a Week number rather than a day  so remove div 7 -->
				<!-- <xsl:variable name="week" select="$day div 7" /> -->
				<xsl:variable name="week" select="$day" />
		...
		<xsl:call-template name="bullet">
			...
			<xsl:value-of select="$Officer1"/>
			<xsl:text> as your Probation Officer tells you</xsl:text>
			</xsl:with-param>	
		</xsl:call-template>		
		<xsl:call-template name="bullet">
			<xsl:with-param name="text">			
			<xsl:text>tell your Probation Officer if you change your address</xsl:text>			
			</xsl:with-param>	
		</xsl:call-template>	
		<xsl:call-template name="bullet">
			<xsl:with-param name="text">			
		...
		<xsl:if test="//cs:OrderRequirements/cs:UnpaidWorkRequirement">
			<xsl:call-template name="bullet">
				<xsl:with-param name="text">
					...
					<xsl:text> as you are told within the next twelve months </xsl:text>
					<xsl:text>. </xsl:text>
			...
			<xsl:call-template name="bullet">
				<xsl:with-param name="text">
					...
					<xsl:if test="//cs:OrderRequirements/cs:ActivityRequirement/cs:Activity">
						...
						<xsl:text> in the way you are told by your Probation Officer. </xsl:text>
		...
		<xsl:if test="//cs:OrderRequirements/cs:CurfewRequirement">
			<xsl:call-template name="bullet">
				<xsl:with-param name="text">
					...
					<xsl:text>. See separate Electronic Monitoring sheet for details.</xsl:text>
				</xsl:with-param>
			</xsl:call-template>
		</xsl:if>
		<!-- Section F -->
			...
			<xsl:call-template name="bullet">
				<xsl:with-param name="text">
					...
					<xsl:if test="//cs:OrderRequirements/cs:ResidenceRequirement/cs:AlternativeLocation/cs:Site">
						...
						<xsl:text> with the prior approval of your Probation Officer. </xsl:text>												
					</xsl:if>
					<!-- Additional Req -->
		...
		<xsl:if test="//cs:OrderRequirements/cs:SupervisionRequirement">
			<xsl:call-template name="bullet">
				<xsl:with-param name="text">
					<xsl:text>attend appointments with your Probation Officer or another person at the times and places your Probation Officer says. </xsl:text>
					<!-- Additional Req -->
	...
	<xsl:template name="SSO_Review">
		<h2>3. Review</h2>
		<xsl:text>The court will review this order </xsl:text>
		<xsl:value-of select="//cs:Review/cs:Periodicity"/>
		<xsl:text>. The first review will be held on </xsl:text>
		<xsl:call-template name="util:ukdate_mon">
			<xsl:with-param name="inDate" select="//cs:Review/cs:DateTime"/>
		</xsl:call-template>
		<xsl:text> at </xsl:text>
		<xsl:call-template name="util:time">
			<xsl:with-param name="inTime" select="//cs:Review/cs:DateTime"/>
		</xsl:call-template>
		<xsl:text> at </xsl:text>
		<xsl:value-of select="//cs:Review/cs:Location/cs:CourtHouseName"/>
		<xsl:text>, </xsl:text>
		<xsl:call-template name="util:address_oneline_court">
			<xsl:with-param name="courtDetails" select="//cs:Review/cs:Location"/>
		</xsl:call-template>			
		<xsl:text>.You must attend the hearing.</xsl:text>
	...
	<xsl:template name="SSO_Note">
		<h2>Note</h2>
		<xsl:text>Either you or your Probation Officer can ask the court to look again at this order and the court can then change it or cancel it if the court feels that is the right thing to do.  The court may also pass a different sentence for the original offence(s).  If you wish to ask the court to look at your order again you should get in touch with the court at the address above.</xsl:text>
	</xsl:template>
