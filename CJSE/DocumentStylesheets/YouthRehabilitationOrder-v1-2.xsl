...
<xsl:stylesheet version="1.2"
  ...
  exclude-result-prefixes="fo" extension-element-prefixes="util date str doc xsd n1 apd cs">
	...
	<doc:reference xmlns="">
		<referenceinfo>
			<releaseinfo role="meta">Version 1.2</releaseinfo>
			<author>
				<surname>Hingston</surname>
				<firstname>Brian</firstname>
			</author>
		</referenceinfo>
		<title>Youth Rehabilitation Order Stylesheet</title>
		<para>File name : YouthRehabilitationOrder-v1-2.xsl</para>
		<partintro>
			<section>
				<title>Introduction</title>
				<para>This module produces the Youth Rehabilitation Order in html format</para>
				<para>New Order type created as part of L-R-4410-01</para>
				<para>Update for changes from L-R-4851-01</para>
				<para>Update for changes for PCSC bill</para>
			</section>
		</partintro>
	</doc:reference>
	...
	<xsl:variable name="stylesheet" select="'YouthRehabilitationOrder-v1-2.xsl'"/>
		...
		<html>
			...
			<body>
				...
				<xsl:call-template name="util:UniversalOrderHeader"> 
					...
					<xsl:with-param name="OrderHeaderRoot" select="/cs:YouthRehabilitationOrder /cs:OrderHeader"/>

				</xsl:call-template>
			
				<xsl:call-template name="YRO_Revision"/>
				
				<!-- +++++++++ following template processes the Personal Details+++++++++ -->	
	   			<xsl:apply-templates select="/cs:YouthRehabilitationOrder/cs:OrderHeader/cs:Defendant/cs:PersonalDetails" />
			
				<xsl:call-template name="YRO_Order"/>				
				<xsl:apply-templates select="//cs:OrderRequirements"/>
				<xsl:call-template name="YRO_AdditionalNotes"/>	
				<xsl:call-template name="YRO_Warning"/>				
				<xsl:call-template name="YRO_Note"/>				
				<xsl:call-template name="util:orderSignatory"/>				
				<xsl:call-template name="YRO_Offences"/>	
	...
	<xsl:template name="YRO_Revision">
	...
	<xsl:template match="//cs:OrderRequirements">
		...
		<!-- Section H -->
		<xsl:if test="//cs:OrderRequirements/cs:LocalAuthorityResidenceRequirement">
			
				<xsl:call-template name="bullet">
					<xsl:with-param name="text">
						<xsl:text>must reside in accommodation  provided by or on behalf of </xsl:text>
						<xsl:value-of select="//cs:OrderRequirements/cs:LocalAuthorityResidenceRequirement/cs:Location/cs:Site"/>
						<xsl:text> for a period of </xsl:text>
						<xsl:call-template name="util:decodeDuration">
							<xsl:with-param name="duration" select="//cs:OrderRequirements/cs:LocalAuthorityResidenceRequirement/cs:Duration"/>
						</xsl:call-template>
						<xsl:text>.  </xsl:text>
						<!-- Additional Req -->
						<xsl:if test="//cs:OrderRequirements/cs:LocalAuthorityResidenceRequirement/cs:AdditionalInformation">
							<xsl:value-of select="//cs:OrderRequirements/cs:LocalAuthorityResidenceRequirement/cs:AdditionalInformation"/>
						</xsl:if>
					</xsl:with-param>
				</xsl:call-template>
		...
		<!-- Section I -->
		<xsl:if test="//cs:OrderRequirements/cs:FosteringRequirement">
			
				<xsl:call-template name="bullet">
					<xsl:with-param name="text">
						<xsl:text>following placement by </xsl:text>
						<xsl:value-of select="//cs:OrderRequirements/cs:FosteringRequirement/cs:LocalAuthority"/>
						<xsl:text> you must reside with a local authority foster parent for </xsl:text>
						<xsl:call-template name="util:decodeDuration">
							<xsl:with-param name="duration" select="//cs:OrderRequirements/cs:FosteringRequirement/cs:Duration"/>
						</xsl:call-template>
						<xsl:text> beginning on </xsl:text>
						<xsl:call-template name="util:ukdate_mon">
							<xsl:with-param name="inDate" select="//cs:OrderRequirements/cs:FosteringRequirement/cs:BeginDate"/>
						</xsl:call-template>
						<xsl:text>. </xsl:text>
						<!-- Additional Req -->
						<xsl:if test="//cs:OrderRequirements/cs:FosteringRequirement/cs:AdditionalInformation">
							<xsl:value-of select="//cs:OrderRequirements/cs:FosteringRequirement/cs:AdditionalInformation"/>
						</xsl:if>
					</xsl:with-param>
				</xsl:call-template>
		...
		<!-- Section L -->
		<xsl:if test="//cs:OrderRequirements/cs:DrugTestingRequirement">
			
				<xsl:call-template name="bullet">
					<xsl:with-param name="text">
						<xsl:text>during the drug treatment period you must provide samples to test if there is a drug in your body. The results of the test should be sent to  </xsl:text>
						<xsl:value-of select="$Officer2"/>
						<xsl:text>.  You must provide the samples </xsl:text>
						<xsl:if test="//cs:OrderRequirements/cs:DrugTestingRequirement/cs:ResponsibleOfficerInstructions = 'yes'">
							<xsl:text>according to instructions given by your responsible officer. </xsl:text>
						</xsl:if>
						<xsl:if test="//cs:OrderRequirements/cs:DrugTestingRequirement/cs:MinimumOccasions">
							<xsl:text>on a minimum number of </xsl:text>
							<xsl:value-of select="//cs:OrderRequirements/cs:DrugTestingRequirement/cs:MinimumOccasions"/>
							<xsl:text> occasions each month. </xsl:text>
						</xsl:if>
						<xsl:if test="//cs:OrderRequirements/cs:DrugTestingRequirement/cs:Requirements">
							<xsl:text>according to the following requirements: </xsl:text>
							<xsl:value-of select="//cs:OrderRequirements/cs:DrugTestingRequirement/cs:Requirements"/>
							<xsl:text>. </xsl:text>
						</xsl:if>
						<!-- Additional Req -->
						<xsl:if test="//cs:OrderRequirements/cs:DrugTestingRequirement/cs:AdditionalInformation">
							<xsl:value-of select="//cs:OrderRequirements/cs:DrugTestingRequirement/cs:AdditionalInformation"/>
						</xsl:if>
					</xsl:with-param>
				</xsl:call-template>
		...
		<!-- Section M -->
		<xsl:if test="//cs:OrderRequirements/cs:IntoxicatingSubstanceRequirement">
			
				<xsl:call-template name="bullet">
					<xsl:with-param name="text">
						<xsl:text>submit to treatment for intoxicating substance(s) by or under the direction of </xsl:text>
						<xsl:value-of select="//cs:OrderRequirements/cs:IntoxicatingSubstanceRequirement/cs:TreatmentDirector"/>
						<!-- Clinic -->
						<xsl:if test="//cs:OrderRequirements/cs:IntoxicatingSubstanceRequirement and //cs:OrderRequirements/cs:IntoxicatingSubstanceRequirement/cs:TreatmentLocation">
							<xsl:text> at </xsl:text>
							<xsl:value-of select="//cs:OrderRequirements/cs:IntoxicatingSubstanceRequirement/cs:TreatmentLocation/cs:Site"/>
							<xsl:text> as a </xsl:text>
							<xsl:value-of select="//cs:OrderRequirements/cs:IntoxicatingSubstanceRequirement/cs:TreatmentLocation/@ResidencyStatus"/>
						</xsl:if>
						<xsl:text> for </xsl:text>
						<xsl:call-template name="util:decodeDuration">
							<xsl:with-param name="duration" select="//cs:OrderRequirements/cs:IntoxicatingSubstanceRequirement/cs:Duration"/>
						</xsl:call-template>
						<xsl:text>.  </xsl:text>
						<!-- Additional Req -->
						<xsl:if test="//cs:OrderRequirements/cs:IntoxicatingSubstanceRequirement/cs:AdditionalInformation">
							<xsl:value-of select="//cs:OrderRequirements/cs:IntoxicatingSubstanceRequirement/cs:AdditionalInformation"/>
						</xsl:if>
					</xsl:with-param>
				</xsl:call-template>
		...
		<!-- Section O -->
		<xsl:if test="//cs:OrderRequirements/cs:EducationRequirement">
			
				<xsl:call-template name="bullet">
					<xsl:with-param name="text">
						<xsl:text>must comply with arrangements for your education which have been made by </xsl:text>
						<xsl:value-of select="//cs:OrderRequirements/cs:EducationRequirement/cs:ArrangedBy"/>
						<xsl:text> and approved by </xsl:text>
						<xsl:value-of select="//cs:OrderRequirements/cs:EducationRequirement/cs:LocalAuthority"/>
						...
						<xsl:call-template name="util:decodeDuration">
							<xsl:with-param name="duration" select="//cs:OrderRequirements/cs:EducationRequirement/cs:Duration"/>
						</xsl:call-template>
						<xsl:text>. </xsl:text>
						<!-- Additional Req -->
						<xsl:if test="//cs:OrderRequirements/cs:EducationRequirement/cs:AdditionalInformation">
							<xsl:value-of select="//cs:OrderRequirements/cs:EducationRequirement/cs:AdditionalInformation"/>
						</xsl:if>
					</xsl:with-param>
				</xsl:call-template>
		...
		<!-- Section O -->         
            <xsl:if test="//cs:OrderRequirements/cs:ExtendedActivityRequirement">
			<xsl:call-template name="bullet">
				<xsl:with-param name="text">
					<xsl:if test="//cs:OrderRequirements/cs:ExtendedActivityRequirement/cs:Activity">
						<xsl:text>Undertake </xsl:text>
						<xsl:value-of select="//cs:OrderRequirements/cs:ExtendedActivityRequirement/cs:Activity"/>
						<xsl:text> for </xsl:text>
						<xsl:call-template name="util:decodeDuration">
							<xsl:with-param name="duration" select="//cs:OrderRequirements/cs:ExtendedActivityRequirement/cs:ActivityDuration"/>
						</xsl:call-template>
					</xsl:if>
					<!-- Additional Req -->
					<xsl:if test="//cs:OrderRequirements/cs:ExtendedActivityRequirement/cs:AdditionalInformation">
						<xsl:value-of select="//cs:OrderRequirements/cs:ExtendedActivityRequirement/cs:AdditionalInformation"/>
					</xsl:if>
				</xsl:with-param>
			</xsl:call-template>
		</xsl:if>
		...
		<!-- Section P -->         
            <xsl:if test="//cs:OrderRequirements/cs:ElectronicWhereaboutsRequirement">
			<xsl:call-template name="bullet">
				<xsl:with-param name="text">
					<xsl:text>Be Electronically Whereabouts Monitored for </xsl:text>
					<xsl:call-template name="util:decodeDuration">
						<xsl:with-param name="duration" select="//cs:OrderRequirements/cs:ElectronicWhereaboutsRequirement/cs:WhereaboutsDuration"/>
					</xsl:call-template>
					<!-- Additional Req -->
					<xsl:if test="//cs:OrderRequirements/cs:ElectronicWhereaboutsRequirement/cs:AdditionalInformation">
						<xsl:value-of select="//cs:OrderRequirements/cs:ElectronicWhereaboutsRequirement/cs:AdditionalInformation"/>
					</xsl:if>
				</xsl:with-param>
			</xsl:call-template>
		</xsl:if>
	...
	<!-- Template to display YRO Additonal Notes text -->
	<xsl:template name="YRO_AdditionalNotes">
		<xsl:if test="//cs:YouthRehabilitationOrder/cs:AdditionalNotes">
			<h2>Additional Notes</h2>

			<xsl:value-of select="//cs:YouthRehabilitationOrder/cs:AdditionalNotes"/>
	...
	<xsl:template name="YRO_Offences">
		<h2>Offences</h2>
		<xsl:choose>
