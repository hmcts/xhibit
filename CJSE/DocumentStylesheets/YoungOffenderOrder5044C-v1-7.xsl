...
<doc:reference xmlns="">
	...
	<para>File name : YoungOffenderOrder5044C-v1-7.xsl</para>
	<partintro>
		<section>
			<title>Introduction</title>
			<para>This module produces the Young Offender Order 5044C in html format</para>
		</section>
	</partintro>
</doc:reference>
...
<xsl:variable name="stylesheet" select="'YoungOffenderOrder5044C-v1-7.xsl'" />
<xsl:variable name="last-modified-date" select="'2022-10-06'" />
			...
			<xsl:call-template name="util:showLogo" />
				...
				<xsl:call-template name="util:UniversalOrderHeader"> 
					<xsl:with-param name="OrderTitle">
						<p>
							<xsl:text>Custodial order for persons under 18 years old</xsl:text>
						</p>
					</xsl:with-param>					
				...
				<xsl:apply-templates select="//cs:CustodialTerm" mode="block"/>
				...
				<xsl:apply-templates select="//cs:IndeterminateSentence" />
...
<xsl:template match="cs:CustodialTerm" mode="block">
	<xsl:choose>
               <xsl:when test="./@Section91 = 'yes'">	
					<xsl:call-template name="cs:CustodialTerm" />
				</xsl:when>
				<xsl:when test="./@Section250 = 'yes'">	
					<xsl:call-template name="cs:CustodialTerm" />
				</xsl:when>
				...
				<xsl:when test="./cs:ExtendedPeriod">
					<!--Display nothing for Extended Period-->
				</xsl:when>
				<xsl:when test="./cs:s235-236">
					<xsl:call-template name="cs:CustodialTermDefault" />
				</xsl:when>
				<xsl:otherwise>
					<xsl:choose>
						<xsl:when test="/cs:CustodialOrder/cs:CustodialTerm/cs:s235-236">
							<!--stops display of 2 custodial sections when secton 252a option selected-->
						</xsl:when>
						<xsl:otherwise>
								<xsl:call-template name="cs:CustodialTermDefault" />
						</xsl:otherwise>
	...
	be concurrent, consecutive or before any other periods of imprisonmemnt which may have been ordered.  
		...
		<itemizedlist>
			...
			<listitem>
				<para>Note: Section250 = 'yes' - display 'long sentence provisions of Section 250..' paragraph.</para>
			</listitem>
			<listitem>
				<para>Note: HMPleasure = 'yes' - display paragraph relating to His Majesty's pleasure.	</para>
			</listitem>
			<listitem>
		...
		<table width="100%" >
			<tr>				
				...
				<td width="80%">
					...
					<xsl:if test="./cs:LifeSentence/@HMPleasure = 'yes' and ./@SectionType = 's226'">
			...
			<xsl:choose>
				...
				<xsl:when test="@Section91 = 'yes'">			
					<tr>									
						<td/>
						<td width="80%">						
							<xsl:text>under the long sentence provisions of Section 91 of the Powers of Criminal Courts (Sentencing) Act 2000</xsl:text>			
						</td>
					</tr>					
					<!-- Section 91 + Life -->		
					<xsl:if test="./cs:LifeSentence/cs:MinimumLifeTerm = 'Section 91 Life'">
						<tr>									
							<td/>
							<td width="80%">			
								<xsl:text>for life or for public protection for serious offences under section 226 of the Criminal Justice Act 2003</xsl:text>
							</td>
						</tr>	
					</xsl:if>	
					<!-- Section 91 + Detention -->		
					<xsl:if test="contains(./cs:Term, 'P')">
						<tr>									
							<td/>
							<td width="80%">							
						...
						<tr>									
							<td/>
							<td width="80%">				
								<!-- Section 91 + Detention + Extended -->					
								<xsl:if test="./cs:ExtendedPeriod">
									<xsl:text>extended under section 228 of the Criminal Justice Act 2003</xsl:text>
								</xsl:if>
							</td>
						</tr>						
					</xsl:if>
				...
				<xsl:when test="@Section250 = 'yes'">			
					...
					<!-- Section 91 + Life -->		
					<xsl:if test="./cs:LifeSentence/cs:MinimumLifeTerm = 'Section 250 Life'">
						<tr>									
							<td/>
							<td width="80%">			
					...
					<xsl:if test="contains(./cs:Term, 'P')">
						...
						<tr>									
							<td/>
							<td width="80%">				
								<!-- Section 250 + Detention + Extended -->					
								<xsl:if test="./cs:ExtendedPeriod">
									<xsl:text>extended under section 228 of the Criminal Justice Act 2003</xsl:text>
								</xsl:if>
							</td>
				...
				<!-- HM Pleasure-->
				<xsl:when test="./cs:LifeSentence/@HMPleasure = 'yes'">		
					<tr>									
						<td/>
						<td width="80%">	
						<xsl:choose>
							<xsl:when test="./@SectionType = 's226'">
								<xsl:text>for life.</xsl:text>
							</xsl:when>
							<xsl:otherwise>
					...
					<xsl:if test="./cs:LifeSentence/cs:MinimumLifeTerm">
						<tr>									
							<td/>
							<td width="80%">	
							<xsl:choose>
								...
								<xsl:when test ="not (//cs:CustodialTerm/cs:LifeSentence/cs:MinimumLifeTerm = 'P0Y0M0W0D')">
									<xsl:text>The Court specified the defendant must serve a minimum term of </xsl:text>
									<xsl:call-template name="util:decodeModifiedDuration">
			...
			<tr>
				...
				<td width="80%">	
					<xsl:if test="./cs:Term/@TermType  " >
						...
						<xsl:text> to any other periods of </xsl:text>
						<xsl:value-of select="$detentionType"/>
						<xsl:text> to any other periods of detention to which the defendant was subject prior to the making of this order.</xsl:text>
					</xsl:if>
				</td>
			</tr>
	...
	<doc:template name="CustodialTerm mode='default'" xmlns="">
		<refpurpose>Outputs the details of the custodial sentence.</refpurpose>
		<refdescription>Gives the details of the custodial sentence imposed on the defendant by the court.
	The custodial sentence can include sdetention under 252A Sentencing act 2020. Where it is for a specific period of time then 
	additional information (@TermType) may be present which indicates wether this sentence is to
	be concurrent, consecutive or before any other periods of imprisonmemnt which may have been ordered.  
	</refdescription>
		<para>
	...
	<xsl:template name="cs:CustodialTermDefault">	
		<hr/>
		<table width="100%" >
			<tr>				
				<td width="20%" valign="top">
				...
				<td width="80%">
					...
					<xsl:choose>
						...
						<xsl:otherwise>
							<xsl:text>, under Section 252A Sentencing Act 2020, to serve a special custodial sentence of </xsl:text>
				...
				<xsl:when test="./cs:Term">
					...
					<tr>	
						<xsl:if test="./cs:s235-236">					
							<td/>
							<td width="80%">					
							<!-- Detention + Section 252A -->			
							<xsl:if test="./cs:s235-236/cs:Section='section252a'">
								<xsl:text>and, under Section 252A Sentencing Act 2020, to serve a special custodial sentence of </xsl:text>
							</xsl:if>
							<xsl:call-template name="util:decodeModifiedDuration">
									<xsl:with-param name="duration" select="./cs:TotalTerm"/>
								</xsl:call-template>
		...
		<table width="100%" >
			<tr>
				...
				<td width="80%">
					...
					<xsl:choose>
						<xsl:when test="./@SectionType='s226b'">
							<xsl:text>226B</xsl:text>
							<xsl:text> of the Criminal Justice Act 2003 to </xsl:text>
						</xsl:when>
						<xsl:when test="./@SectionType='s228'">
							<xsl:text>228</xsl:text>
							<xsl:text> of the Criminal Justice Act 2003 to </xsl:text>
						</xsl:when>
						<xsl:when test="./@SectionType='s254'">
							<xsl:text>254</xsl:text>
							<xsl:text> of the Sentencing Act 2020 to </xsl:text>
						</xsl:when>
						<xsl:when test="./@SectionType='s268a'">
							<xsl:text>268A</xsl:text>
							<xsl:text> of the Sentencing Act 2020 to </xsl:text>
						</xsl:when>
						<xsl:otherwise>
						</xsl:otherwise>
 ...
 <xsl:template name="cs:PeriodOfImprisonment" >
	...
	<doc:template name="CreditForRemand" xmlns="">
		<refpurpose>Outputs Credit for Time Served on Remand paragraph if this element is present</refpurpose>
	</doc:template>	
	...
	<xsl:template match="cs:CreditForBail">
			...
			<table width="100%">
				<tr>
					...
					<td width="80%">
						<xsl:choose>
							<xsl:when test="//cs:CreditForBail/@Section='section240a'">
		...
		<table width="100%" >
			...
			<tr>
				...
				<td width="80%">
					<xsl:text>The court applied its powers under section 116 of the Powers of Criminal Courts
					          (Sentencing) Act 2000 and ordered that the defendant be returned to a young offenders institution for </xsl:text>
					<xsl:choose>
			...
			<xsl:if test="cs:TotalPeriodOfReturn">
				<tr>
					<td/>
					<td width="80%">
						<xsl:text>The total of the period of return (and of any custodial term for a new offence) is </xsl:text>
						<xsl:call-template name="util:decodeDuration">
	...
	<xsl:template name="cs:IndeterminateSentence">
		...
		<table width="100%">
			<tr>
				...
				<td width="80%">
					<xsl:text>The court ordered that the provisions of section 226 of the Criminal Justice Act 2003 should apply to the defendant. The court specified the defendant must serve a minimum term of</xsl:text>
					<xsl:call-template name="util:decodeDuration">
						<xsl:with-param name="duration" select="cs:Term"/>
					</xsl:call-template>
