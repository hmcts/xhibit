...
<doc:reference xmlns="">
	...
	<para>File name : CommunityRehabOrder-v2-3.xsl</para>
	<partintro>
		<section>
			...
			<para>PR 57319 - XHIBIT/CJSE Differences</para>
			...
			<para>PR58273 - other manner for the offences for which it made the order on</para>
			<para>PR58273 - Additional Requirements</para>
			<para>PR57824 - Changed on date format</para>
			<para>PR57857 - Misisnf Satisified sentence</para>
		</section>
	</partintro>
</doc:reference>
...
<xsl:variable name="minorVersion" select="'3h'" />
<xsl:variable name="stylesheet" select="'CommunityRehabOrder-v2-3.xsl'" />
<xsl:variable name="last-modified-date" select="'2006-02-28'" />
...
<xsl:output method="html" indent="yes"/>
		...
		<html>
			...
			<body>
				...
				<xsl:call-template name="util:UniversalOrderHeader"> 
					...
					<xsl:with-param name="OrderHeaderRoot" select="/cs:CommunityRehabOrder/cs:OrderHeader"/>
				</xsl:call-template>
				
				<!-- +++++++++ following template processes the Personal Details+++++++++ -->	
	   			<xsl:apply-templates select="/cs:CommunityRehabOrder/cs:OrderHeader/cs:Defendant/cs:PersonalDetails" />
	...
	<xsl:template match="cs:OriginalOrderRevoked/cs:CurrentCourt">
		<xsl:if test=". = 'yes' and not(//cs:FailedToComply = 'yes')" > 
			...
			<xsl:text>This Court was satisfied that the defendant has failed to comply with a requirement of the 
		          above order and this Court was satisfied that it was in the interests of justice to revoke the order.</xsl:text>
			<br />
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="cs:FailedToComply">
		...
		<xsl:if test=". = 'no' " > 
			<xsl:text>, </xsl:text>
			<xsl:if test="//cs:OriginalOrderRevoked/cs:CurrentCourt = 'yes' ">
				<xsl:text> this Court was satisfied that it was in the interests of justice to revoke the order and </xsl:text>
			</xsl:if>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template name="rehabilitation">
			...
			<xsl:text> this Court, being of the opinion that it was appropriate to make a Community Rehabilitation Order,
					   ...
					   defendant or the supervising officer to apply for the review of the order.
			</xsl:text>
			<br />
	</xsl:template>
			...
			<table width="100%" >
				<xsl:variable name="text" >
					...
					<xsl:if test="//cs:PettySessionalArea">
						...
						<xsl:text> for the local justice area of </xsl:text>
