...
<doc:reference xmlns="">
	...
	<para>File name : CommunityPunishmentRehabOrder-v2-2.xsl</para>
	<partintro>
		<section>
...
<xsl:variable name="stylesheet" select="'CommunityPunishmentRehabOrder-v2-2.xsl'" />
			...
			<tr>
				...
				<td>
					<xsl:text>being of the opinion that it was appropriate to make a Community Rehabilitation Order,
							  explained to the defendant the effect of this order (including the 
		...
		<xsl:if test="//cs:CommunityPunishment" >
			...
			<xsl:text> that the defendant shall: </xsl:text>
				...
				<xsl:variable name="text2" >
					<xsl:text>Keep in touch with the probation officer in accordance with instructions as may from time
							  to time be given by that officer and notify the officer of any change of address.
					</xsl:text>
				</xsl:variable>
				<xsl:call-template name="util:bulletRow" >
