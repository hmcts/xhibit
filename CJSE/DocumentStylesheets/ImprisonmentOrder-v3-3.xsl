...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:cs="http://www.courtservice.gov.uk/schemas/courtservice" xmlns:apd="http://www.govtalk.gov.uk/people/AddressAndPersonalDetails" xmlns:n1="http://www.govtalk.gov.uk/people/bs7666" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:util="http://www.courtservice.gov.uk/transforms/courtservice/gcsUtility" xmlns:date="http://xsltsl.org/date-time" xmlns:str="http://xsltsl.org/string" xmlns:doc="http://xsltsl.org/xsl/documentation/1.0" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:xso="http://www.w3.org/1999/XSL/Transform" exclude-result-prefixes="fo" extension-element-prefixes="util date str doc xsd n1 apd cs">
	...
	<doc:reference xmlns="">
		<referenceinfo>
			<releaseinfo role="meta">Version 3-3</releaseinfo>
			<author>
		...
		<para>File name : ImprisonmentOrder-v3-3.xsl</para>
		<partintro>
			<section>
				...
				<para>Changes made to Custodial/Extended sentence section to reflect Legal Aid Sentencing Bill</para>
				<para>v2.8. Updates for L-R-4410-01 to cover S236A</para>
				<para>v2.9. Updates for L-R-4851-01 to cover S236A revisions</para>
				<para>v3.0 Updates to include Prisoner Voting Rights text (SR0638)</para>
				<para>v3.1 updates for 2020 Sentencing Act changes</para>
				<para>v3.2 updates for Terrorism and Sentencing Act changes</para>
				<para>v3.3 updates for PCSC Act changes (Whole life)</para>
			</section>
		</partintro>
	</doc:reference>
	<!-- Version Information -->
	<xsl:variable name="majorVersion" select="'3'"/>
	<xsl:variable name="minorVersion" select="'3'"/>
	<xsl:variable name="stylesheet" select="'ImprisonmentOrder-v3-3.xsl'"/>
	<xsl:variable name="last-modified-date" select="'2022-06-14'"/>
	...
	<xsl:variable name="sectionType">
			<xsl:choose>
				<xsl:when test="cs:ImprisonmentOrder/cs:CustodialTerm/@Section236A='yes'">section 236A Criminal Justice Act 2003</xsl:when>
				<xsl:when test="cs:ImprisonmentOrder/cs:CustodialTerm/@Section278='yes'">section 278 Sentencing Act 2020</xsl:when>	
			</xsl:choose>	
	</xsl:variable>
	<xsl:variable name="shortSectionType">
		<xsl:choose>
			<xsl:when test="cs:ImprisonmentOrder/cs:CustodialTerm/@Section236A='yes'">S236A</xsl:when>
			<xsl:when test="cs:ImprisonmentOrder/cs:CustodialTerm/@Section278='yes'">S278</xsl:when>
		</xsl:choose>		
	</xsl:variable>
	...
	<xsl:template match="/">
		<html>
			...
			<body>
				...
				<xsl:apply-templates select="//cs:RiskVulnerabilityFactors"/>
				<xsl:apply-templates select="/cs:ImprisonmentOrder/cs:CustodialTerm" mode="block"/>
				<xsl:apply-templates select="//cs:CustodialTerm[./cs:ExtendedPeriod]" mode="extension"/> 
				<!--CCN 1338 -->
				<xsl:apply-templates select="//cs:CreditOnRemand"/>
				<xsl:apply-templates select="//cs:CreditOnBail"/>								
  ...
  <xsl:template name="cs:PeriodOfImprisonment" >
		<hr/>
		<table width="100%">
			<tr>
				<td width="20%" valign="top">
					...
					<xsl:if test="./cs:Term">
						...
						<xsl:choose>
							<xsl:when test="./cs:Term/@Concurrent = 'yes'">
								...
								<xsl:value-of select="$shortSectionType"/>
				...
				<td>
					<xsl:choose>
						<xsl:when test="./cs:Term/@TermType" >
							...
							<xsl:text> to any other periods of imprisonment to which the</xsl:text>
	...
	<xsl:template match="cs:ImprisonmentOrder/cs:CustodialTerm" mode="block">
    <xsl:choose>
               <xsl:when test="./@IndeterminateSentence = 'yes'">		
					<xsl:call-template name="cs:IndeterminateSentence"/>															
				</xsl:when>
				<xsl:when test="./cs:LifeSentence">			
					<xsl:call-template name="cs:CustodialTerm"/>
				</xsl:when>
				<xsl:when test="./cs:ExtendedPeriod">
					<!--Don't show anything when extended period-->
				</xsl:when>
				<xsl:otherwise>
					<xsl:call-template name="cs:CustodialTerm" />
				</xsl:otherwise>
	...
	<xsl:template name="cs:CustodialTerm" >
		<hr/>
		<table width="100%">
			<tr>
				<td width="20%" valign="top">
				...
				<td width="80%">
					...
					<xsl:choose>
						<xsl:when test="./@SectionType = 's224a'">
							<xsl:text>, under section 224a of the Criminal Justice Act 2003,</xsl:text>
						</xsl:when>
						<xsl:when test="./@SectionType = 's225'">
							<xsl:text>, under section 225 of the Criminal Justice Act 2003,</xsl:text>
						</xsl:when>
						<xsl:when test="./@SectionType = 's283'">
							<xsl:text>, under section 283 of the Sentencing Act 2020,</xsl:text>
						</xsl:when>
						<xsl:when test="./@SectionType = 's285'">
							<xsl:text>, under section 285 of the Sentencing Act 2020,</xsl:text>
						</xsl:when>
					</xsl:choose>
			...
			<tr>
				...
				<td>
						<xsl:choose>
							<xsl:when test="./cs:LifeSentence">
								<xsl:text> to imprisonment for life. </xsl:text>
								<xsl:choose>		
									<xsl:when test="./cs:LifeSentence/cs:MinimumLifeTerm = 'No Term Specified'">
										<xsl:text>The court did not specifiy a minimum term. </xsl:text>
									</xsl:when>
									<xsl:when test="not (./cs:LifeSentence/cs:MinimumLifeTerm = 'P0Y0M0W0D')">
										<xsl:text>The court specified that the defendant must serve a minimum term of </xsl:text>
										<xsl:call-template name="util:decodeModifiedDuration">
											<xsl:with-param name="duration" select="./cs:LifeSentence/cs:MinimumLifeTerm"/>
										</xsl:call-template>
									</xsl:when>
								</xsl:choose>
								<br/>
								<xsl:choose>
									<xsl:when test="./cs:LifeSentence/@AutomaticLife">
										<xsl:text> The sentence is Automatic Life.</xsl:text>
									</xsl:when>
									<xsl:when test="./cs:LifeSentence/@MandatoryLife">
										<xsl:text> The sentence is Mandatory Life.</xsl:text>
									</xsl:when>
									<xsl:when test="./cs:LifeSentence/@DiscretionaryLife">
										<xsl:text> The sentence is Discretionary Life.</xsl:text>
									</xsl:when>
									<xsl:when test="./cs:LifeSentence/@WholeLife">
										<xsl:text> Total custodial period is life. Early release provisions do not apply - sentence is a whole life order.</xsl:text>
									</xsl:when>
								</xsl:choose>
								<br/>
								<xsl:if test="./cs:LifeSentence/cs:MinimumLifeTerm/@TermType">
									<xsl:text>This sentence was ordered to be </xsl:text>
									<xsl:variable name="termType">
										<xsl:call-template name="str:to-lower">
											<xsl:with-param name="text" select="./cs:LifeSentence/cs:MinimumLifeTerm/@TermType"/>
										</xsl:call-template>
									</xsl:variable>
									<xsl:choose>
			...
			<tr>
				<td/>
				<td>
					<xsl:if test="not (./cs:LifeSentence/@MandatoryLife)">
						<xsl:choose>
							<xsl:when  test="./cs:Term/@TermType">
								<xsl:text>This sentence was ordered to be </xsl:text>
								<xsl:variable name="termType">
				...
				<td width="80%">
					...
					<xsl:choose>
						<xsl:when test="./@SectionType='s226a'">
							<xsl:text>226A of the Criminal Justice Act 2003, to </xsl:text>
						</xsl:when>
						<xsl:when test="./@SectionType='s227'">
							<xsl:text>227 of the Criminal Justice Act 2003, to </xsl:text>
						</xsl:when>
						<xsl:when test="./@SectionType='s279'">
							<xsl:text>279 of the Sentencing Act 2020, to </xsl:text>
						</xsl:when>
						<xsl:when test="./@SectionType='s282a'">
							<xsl:text>282A of the Sentencing Act 2020, to </xsl:text>
						</xsl:when>
					</xsl:choose>
				</td>
			</tr>
	...
	<!-- **************************************** -->	
	<doc:template name="CreditOnRemand" xmlns="">
		<!-- With Legal Aid Sentencing Bill this is changed to output Time Served in Custody in a foreign jurisdiction -->
		<refpurpose>Outputs Time Served in custody in a foreign jurisdiction paragraph if this element is present</refpurpose>
	</doc:template>	
	<xsl:template match="cs:CreditOnRemand">
			<hr/>
			<table width="100%">
				<tr>
					<td width="20%" valign="top">
					   
						   <xsl:text>Time served in custody in a foreign jurisdiction</xsl:text>						
                       					
					</td>
					<td width="80%">
						<xsl:text>The court specifies that the defendant served </xsl:text>
						<xsl:call-template name="util:decodeDuration">
							<xsl:with-param name="duration" select="cs:CreditPeriod"/>
						</xsl:call-template>
						<xsl:text> in custody while awaiting extradition.</xsl:text>
					</td>
				</tr>
			</table>	
	</xsl:template>
	...
	<xsl:template match="cs:CreditOnBail">
			...
			<table width="100%">
				<tr>
					...
					<td width="80%">
						<xsl:choose>
							<xsl:when test="//cs:CreditOnBail/@Section='section240a'">
							<xsl:text>Under section 240A of the Criminal Justice Act 2003, the court directs that </xsl:text>
							</xsl:when>
							<xsl:otherwise>
							<xsl:text>Under section 325 of the Sentencing Act 2020, the court directs that </xsl:text>
							</xsl:otherwise>
						</xsl:choose>
						<xsl:call-template name="util:decodeDuration">
							<xsl:with-param name="duration" select="cs:CreditPeriod"/>
	...
	<!-- **************************************** -->	
	<doc:template name="IndeterminateSentence" xmlns="">
		<refpurpose>Outputs Indeterminate Sentence paragraph if this element is present</refpurpose>
	</doc:template>	
	<xsl:template name="cs:IndeterminateSentence">
		<hr/>
		<table width="100%">
			<tr>
				<td width="20%" valign="top">
	...
	<xsl:template name="orderNotesPart2">
		<hr/>
		<!--	Removed as part of Legal AId Sentencing Bill Changes	
	...
	<!-- ******************************************-->
	<xsl:template match="cs:RiskVulnerabilityFactors" >
		<hr/>
		<table width="100%">
			<tr>
				<td width="20%" valign="top">
				...
				<td width="80%">
					<xsl:value-of select="//cs:RiskVulnerabilityFactors"/>
				</td>
			</tr>
		</table>
	</xsl:template>
