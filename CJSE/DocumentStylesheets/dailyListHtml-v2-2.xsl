...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:cs="http://www.courtservice.gov.uk/schemas/courtservice" xmlns:apd="http://www.govtalk.gov.uk/people/AddressAndPersonalDetails" version="1.1">
	...
	<xsl:variable name="stylesheet" select="'dailylist-v2-1.xsl'"/>
	<xsl:variable name="last-modified-date" select="'2016-02-17'"/>
	...
	<xsl:template match="cs:CourtLists">
		<xsl:for-each select="cs:CourtList">
			...
			<xsl:for-each select="cs:Sittings/cs:Sitting">
				...
				<xsl:sort select="cs:CourtRoomNumber" data-type="number"/>
				<!--
				...
				<xsl:choose>
					...
					<xsl:when test="not(cs:SittingPriority = 'F')">
						<table width="100%" style="font-size:8pt">
							<tr>
								<td>		Court <xsl:value-of select="cs:CourtRoomNumber"/>
									<xsl:choose>
	...
	<xsl:template match="cs:CrownCourt">
			...
			<table width="100%">
				<tr>
					<td align="center">
						<font size="5pt">
							<b>
								<xsl:text>Daily List for </xsl:text>
		...
		<xsl:variable name="reportingRestriction">
			<xsl:for-each select="//cs:DailyList/cs:CourtLists/cs:CourtList/cs:Sittings/cs:Sitting/cs:Hearings/cs:Hearing">
				<xsl:if test="substring(cs:ListNote,1,10) = 'Order made'">
					Yes
				</xsl:if>
			</xsl:for-each>
		</xsl:variable>
		<xsl:if test="contains($reportingRestriction, 'Yes')">
			<b>
				...
				<xsl:variable name="defendant">
					...
					<xsl:if test="../../cs:PrisonerID">
						...
						<xsl:if test="../cs:Sex">
							<xsl:text> </xsl:text>
							<xsl:call-template name="toUpper">
								<xsl:with-param name="content" select="../cs:Sex"/>
							</xsl:call-template>
							<xsl:text> </xsl:text>
						</xsl:if>
						<xsl:text> </xsl:text>
			...
			<!--<xsl:choose>
				...
				<xsl:otherwise>
					<xsl:choose>
						...
						<xsl:otherwise>
							<xsl:call-template name="TitleCase">
								<xsl:with-param name="text" select="cs:Prosecution/cs:ProsecutingReference"/>
							</xsl:call-template>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:otherwise>
