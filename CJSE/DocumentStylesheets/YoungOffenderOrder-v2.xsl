...
<doc:reference xmlns="">
	...
	<para>File name : YoungOffenderOrder-v2.xsl</para>
	<partintro>
		<section>
			<title>Introduction</title>
			<para>This module produces the Young Offender Order in html format</para>
		</section>
	</partintro>
</doc:reference>
...
<xsl:variable name="stylesheet" select="'YoungOffenderOrder-v2.xsl'" />
<xsl:variable name="last-modified-date" select="'2003-11-25'" />
...
<xsl:output method="html" indent="yes"/>
	...
	<xsl:template match="/">
		<html>
			...
			<xsl:call-template name="util:showLogo" />
				...
				<xsl:apply-templates select="//cs:CustodialSentence" mode="custodial"/>
				<xsl:apply-templates select="//cs:ChapterIII" />
				<xsl:apply-templates select="//cs:CustodialSentence[./cs:Term/cs:CustodialTerm/cs:ExtendedPeriod]" mode="extension"/>
	...
	<!-- processes the OrderHeader node - constructs the initial header information for the output -->
		...
		<table WIDTH="100%">
			<tr>
				...
				<td WIDTH="15%"><xsl:text>Case Number</xsl:text></td>
				<td WIDTH="15%"><xsl:copy-of select="$caseNums"/></td>
			</tr>
			<tr>
				<td>
					<xsl:value-of select="concat('Court Number  ',cs:CourtHouse/cs:CourtHouseCode)" />
				</td>
		...
		<center>
			<h3>
			<strong>
				<xsl:value-of select="'Order for commitment to a young offender institution'"/>
			</strong>
			</h3>
		</center>
		...
		<table width="100%" >
			...
			<tr>
				...
				<td width="80%">
					...
					<xsl:text> that the defendant serve a period of </xsl:text>
					<xsl:value-of select="$detentionType"/>
					<xsl:text>, details of which follow.</xsl:text>
				</td>
			</tr>
			<tr>
				<td />
				<td>
					<xsl:if test="$detentionType = 'detention'" >
	...
	<doc:template name="CustodialSentence mode='custodial'" xmlns="">
	...
	<xsl:template match="cs:CustodialSentence" mode="custodial">
		<hr />
		<table width="100%" >
			<tr>
				<td width="20%" valign="top">
			...
			<xsl:if test="./cs:Section91 = 'yes'">
				<tr>
					<td />
					<td>
						<xsl:text>under the long sentence provisions of Section 91 of 
			...
			<xsl:choose>
			<xsl:when test="./cs:HMPleasure = 'yes'">
				<tr>
					<td />
					<td>
			...
			<xsl:otherwise>
				<xsl:if test="not (./cs:Term/cs:ForLife or ./cs:Section9394 or ./cs:HMPleasure) " >
					<tr>
						<td />
						<td>
							
							<xsl:text>in a young offender institution</xsl:text>
						</td>
					</tr>
				</xsl:if>
			</xsl:otherwise>
			...
			<tr>
				...
				<td>
					<xsl:choose>
					<xsl:when test="./cs:Term/cs:ForLife">
						<xsl:text>for life </xsl:text>
					</xsl:when>
					<xsl:when test="./cs:Section9394">
						<xsl:text>under the life sentence provisions for young offenders of Sections 93 and 94 of 
								  the Powers of Criminal Courts (Sentencing) Act 2000
						</xsl:text>
					</xsl:when>
					<xsl:when test="cs:Term/cs:CustodialTerm/cs:Term">
						<xsl:text>for </xsl:text>
						<xsl:call-template name="util:decodeDuration">
							<xsl:with-param name="duration" select="cs:Term/cs:CustodialTerm/cs:Term"/>
						</xsl:call-template>
					</xsl:when>
					</xsl:choose>
				</td>
			...
			<tr>
				<td />
				<td>
					<xsl:if test="./cs:Term/cs:CustodialTerm/cs:Term/@TermType  " >
						<xsl:text>This sentence was ordered to be </xsl:text>
						<xsl:call-template name="str:to-lower">
							<xsl:with-param name="text" select="cs:Term/cs:CustodialTerm/cs:Term/@TermType"/>
						</xsl:call-template>
						<xsl:text> to any other periods of </xsl:text>
						<xsl:value-of select="$detentionType"/>
						<xsl:text> to which the defendant was subject prior to the making of this order.</xsl:text>
					</xsl:if>
				</td>
			</tr>
		</table>
	...
	<doc:template name="CustodialSentence mode='extension'" xmlns="">
	...
	<xsl:template match="cs:CustodialSentence[./cs:Term/cs:CustodialTerm/cs:ExtendedPeriod]" mode="extension">
		<hr />
		<table width="100%" >
			<tr>
				<td width="20%" valign="top">
			...
			<tr>
				...
				<td>
					...
					<xsl:call-template name="util:decodeDuration">
						<xsl:with-param name="duration" select="cs:Term/cs:CustodialTerm/cs:TotalTerm"/>
					</xsl:call-template>
			...
			<tr>
				...
				<td>
					...
					<xsl:call-template name="util:decodeDuration">
						<xsl:with-param name="duration" select="cs:Term/cs:CustodialTerm/cs:ExtendedPeriod"/>
					</xsl:call-template>
					<xsl:text>.</xsl:text>
				</td>
			</tr>
			<xsl:if test="cs:Term/cs:CustodialTerm/cs:ExtendedPeriod/@TermType">
				<tr>
					<td />
					<td>
						<xsl:text>This sentence was ordered to be </xsl:text>
						<xsl:call-template name="str:to-lower">
							<xsl:with-param name="text" select="cs:Term/cs:CustodialTerm/cs:ExtendedPeriod/@TermType"/>
						</xsl:call-template>
						<xsl:text> to any other periods of </xsl:text>
						<xsl:value-of select="$detentionType"/>
						<xsl:text> to which the defendant was subject prior to the making of this order.</xsl:text>
			...
			<xsl:if test="cs:Term/cs:CustodialTerm/cs:Section44 = 'yes'">
				<tr>
					<td />
					<td>
						<xsl:text>The provisions of section 44 of the Criminal Justice Act 1997, as substituted by section 59
	...
	<xsl:template match="cs:ReturnToImprisonment">
		...
		<table width="100%" >
			<tr>
				<td width="20%" valign="top">
					<xsl:text>Return of defendants to a young offender institution or prison</xsl:text>
				</td>
				<td width="80%" valign="top">
			...
			<tr>
				...
				<td>
					<xsl:text>This date was, or appeared to be, earlier than the date on which the defendant would, 
							  ...
							  have completed serving the full sentence imposed by the 
					</xsl:text>
			...
			<tr>
				...
				<td>
					<xsl:text>The court applied its powers under section 116 of the Powers of Criminal Courts
					          (Sentencing) Act 2000 and ordered that the defendant be returned to 
					</xsl:text>
					<xsl:choose>
					<xsl:when test="cs:ReturnPeriod/@DetentionType = 'Detention'">
						<xsl:text>young offender institution</xsl:text>
					</xsl:when>
					<xsl:otherwise>
			...
			<xsl:if test="cs:ReturnPeriod/@TermType">
				<tr>
					<td />
					<td>
						<xsl:text>This period of </xsl:text>
						<xsl:value-of select="$detentionType"/>
						<xsl:text> was ordered to be served </xsl:text>
						<xsl:choose>
						...
						<xsl:text> any other periods of </xsl:text>
						<xsl:value-of select="$detentionType"/>
						<xsl:text> imposed by the court on the same occasion when this 
								   order was made.
						</xsl:text>
					</td>
				</tr>
	...
	<doc:template name="AdditionalNotes" xmlns="">
		<refpurpose>Outputs the additional notes if any..</refpurpose>
	</doc:template>
	...
	<xsl:template name="orderSignatory">
		<xsl:call-template name="util:altOrderSignatory" />
	</xsl:template>
