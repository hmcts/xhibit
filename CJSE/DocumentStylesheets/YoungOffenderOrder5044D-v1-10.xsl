...
<doc:reference xmlns="">
	<referenceinfo>
		<releaseinfo role="meta">Version 1-10</releaseinfo>
		<author>
	...
	<para>File name : YoungOffenderOrder5044D-v1-10.xsl</para>
	<partintro>
		<section>
			...
			<para>v1.8 Updates for 2020 Sentencing act changes</para>
			<para>v1.9 Updates for Terrorism and Sentencing act changes</para>
			<para>v1.10 Updates for PCSC act changes (whole life)</para>
		</section>
	</partintro>
</doc:reference>
...
<xsl:variable name="stylesheet" select="'YoungoffenderOrder5044D-v1-10.xsl'" />
...
<xsl:output method="html" indent="yes"/>
	...
	<xsl:template match="cs:CustodialOrder/cs:CustodialTerm" mode="custodial">
	   <xsl:choose>
			...
			<xsl:when test="//cs:CustodialTerm/cs:LifeSentence/@MandatoryLife = 'yes' or //cs:CustodialTerm/cs:LifeSentence/@AutomaticLife = 'yes' or //cs:CustodialTerm/cs:LifeSentence/@DiscretionaryLife = 'yes' or //cs:CustodialTerm/cs:LifeSentence/@WholeLife = 'yes'">
							...
							<xsl:if test="./cs:Term">
								...
								<xsl:choose>
									<xsl:when test="./cs:Term/@Concurrent = 'yes'">
										<xsl:choose>
											<xsl:when test="./cs:s235-236/cs:Section='section252a'">
												<xsl:text>(concurrent to S252A sentence below)</xsl:text>
											</xsl:when>
											<xsl:otherwise>
												<xsl:text> (concurrent to S265 sentence below)</xsl:text>
											</xsl:otherwise>
										</xsl:choose>
									</xsl:when>
									<xsl:when test="./cs:Term/@Consecutive = 'yes'">
										<xsl:choose>
											...
											<xsl:otherwise>
												<xsl:text> (consecutive to S265 sentence below)</xsl:text>
											</xsl:otherwise>
										</xsl:choose>
									</xsl:when>
								</xsl:choose>
							...
							<xsl:if test="./cs:s235-236">
								<xsl:choose>
									<xsl:when test="./cs:Term">
										...
										<xsl:choose>
											<xsl:when test="./cs:s235-236/cs:Section='section252a'">
												<xsl:text>252A Sentencing Act 2020 </xsl:text>
											</xsl:when>
											<xsl:otherwise>
												<xsl:text>265 Sentencing Act 2020 </xsl:text>
											</xsl:otherwise>
										</xsl:choose>
									</xsl:when>
									<xsl:otherwise>
										...
										<xsl:choose>
											<xsl:when test="./cs:s235-236/cs:Section='section252a'">
												<xsl:text>252A Sentencing Act 2020 to</xsl:text>
											</xsl:when>
											<xsl:otherwise>
												<xsl:text>265 Sentencing Act 2020 to</xsl:text>
											</xsl:otherwise>
										</xsl:choose>
									</xsl:otherwise>
								</xsl:choose>
			...
			<tr>
				<td/>
					...
					<xsl:choose>
						...
						<xsl:when test="//cs:CustodialTerm/cs:LifeSentence/@WholeLife">
			...
			<tr>
				...
				<td width="80%">
					...
					<xsl:choose>
						...
						<xsl:when test="./@SectionType='s266'">
							<xsl:text>266 of the Sentencing Act 2020, to </xsl:text>
						</xsl:when>
						<xsl:when test="./@SectionType='s268a'">
							<xsl:text>268A of the Sentencing Act 2020, to </xsl:text>
						</xsl:when>
					</xsl:choose>
					<xsl:call-template name="util:decodeModifiedDuration">
						<xsl:with-param name="duration" select="cs:TotalTerm"/>
