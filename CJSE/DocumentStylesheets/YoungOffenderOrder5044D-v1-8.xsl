...
<doc:reference xmlns="">
	<referenceinfo>
		<releaseinfo role="meta">Version 1-8</releaseinfo>
		<author>
	...
	<para>File name : YoungOffenderOrder5044D-v1-8.xsl</para>
	<partintro>
		<section>
...
<xsl:variable name="stylesheet" select="'YoungoffenderOrder5044D-v1-8.xsl'" />
<xsl:variable name="last-modified-date" select="'2021-08-04'" />
							...
							<xsl:if test="./cs:s235-236">
								<xsl:choose>
									<xsl:when test="./cs:Term">
										<xsl:text> and under Section 265 Sentencing Act 2020 </xsl:text>
									</xsl:when>
									<xsl:otherwise>
										<xsl:text>under Section 265 Sentencing Act 2020 to</xsl:text>
									</xsl:otherwise>
								</xsl:choose>
