...
<xsl:stylesheet version="1.1" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:cs="http://www.courtservice.gov.uk/schemas/courtservice" xmlns:apd="http://www.govtalk.gov.uk/people/AddressAndPersonalDetails" xmlns:n1="http://www.govtalk.gov.uk/people/bs7666" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:util="http://www.courtservice.gov.uk/transforms/courtservice/gcsUtility" xmlns:date="http://xsltsl.org/date-time" xmlns:str="http://xsltsl.org/string" xmlns:doc="http://xsltsl.org/xsl/documentation/1.0" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:xso="http://www.w3.org/1999/XSL/Transform" exclude-result-prefixes="fo" extension-element-prefixes="util date str doc xsd n1 apd cs">
	...
	<doc:reference xmlns="">
		...
		<para>File name : DetentionAndTrainingOrder-v1-3.xsl</para>
		<partintro>
			<section>
				...
				<param>v1.2 updated for 2020 Sentencing act</param>
				<param>v1.3 updated for Terrorism And Sentencing act</param>
			</section>
		</partintro>
	</doc:reference>
	<!-- Version Information -->
	...
	<xsl:variable name="stylesheet" select="'DetentionAndTrianingOrder-v1-3.xsl'"/>
	...
	<xsl:template match="/">
		<html>
			...
			<body>
				...
				<xsl:call-template name="util:UniversalOrderHeader">
					...
					<xsl:with-param name="OrderSubHeading">
						<xsl:choose>
							...
							<xsl:otherwise>
								<xsl:choose>
									<xsl:when test="/cs:DetentionAndTrainingOrder/cs:OrderMadeUnder = 'Section100'">
										<xsl:text>Order made under Section 100 Powers of Criminal Courts (Sentencing) Act 2000</xsl:text>
									</xsl:when>
									<xsl:when test="/cs:DetentionAndTrainingOrder/cs:OrderMadeUnder = 'Section233'">
										<xsl:text>Order made under Section 233 of the Sentencing Act 2020</xsl:text>
									</xsl:when>
								</xsl:choose>
							</xsl:otherwise>
						</xsl:choose>
				...
				<xsl:apply-templates select="/cs:DetentionAndTrainingOrder/cs:TrailMonitoringDuration"/>
	...
	<xsl:template match="cs:PersonalDetails">
		...
		<table width="100%">
			...
			<xsl:if test="//cs:RiskVulnerabilityFactors">
				<tr>
					<td width="20%" valign="top">
			...
			<tr>
				...
				<td>
					<xsl:choose>
						<xsl:when test="/cs:DetentionAndTrainingOrder/cs:Section105">
							<xsl:text>The Court ordered that under the provisions of Section </xsl:text>
							 <xsl:choose>
								<xsl:when test="/cs:DetentionAndTrainingOrder/cs:OrderMadeUnder = 'Section100'">
									<xsl:text>105 of the Powers of the Criminal Courts (Sentencing) Act 2000 </xsl:text>
								</xsl:when>
								<xsl:when test="/cs:DetentionAndTrainingOrder/cs:OrderMadeUnder = 'Section233'">
									<xsl:text>243 of the Sentencing Act 2020 </xsl:text>
								</xsl:when>
							</xsl:choose>
							 <xsl:text>the defendant be detained in secure accomodation for a further period of </xsl:text>
							<xsl:call-template name="util:decodeDuration">
	...
	<xsl:template match="cs:TrailMonitoringDuration">
		<!-- details of the dTrail Monitoring requirements -->
		<table width="100%">
			<tr>
				<td colspan="2">
					<hr/>
					...
					<table width="100%">
						<tr>
							<td width="65%">
								...
								<xsl:value-of select="//cs:TrailMonitoringDuration"/>
