...
<xsl:variable name="Event_Header_Text">
	...
	<xsl:if test="event/E40726_Ref_Verdict_Code='GL' or event/E40726_Ref_Verdict_Code='GLJ'">
		<xsl:text> Jury return with guilty verdict for a lesser offence namely </xsl:text>
    </xsl:if>
	<xsl:if test="event/E40726_Ref_Verdict_Code='GLOJ'">
		<xsl:text> Not Guilty but Guilty of Lesser Offence not charged namely (By Judge alone under DVC&amp;VA 2004) </xsl:text>
    </xsl:if>    
