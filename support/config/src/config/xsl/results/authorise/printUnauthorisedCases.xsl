...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" version="1.0">
	<xsl:import href="/config/xsl/results/authorise/printCaseUnauthorisedCase.xsl"/>
	<xsl:template match="unauthorised-cases-print-value">
			...
			<fo:page-sequence master-reference="simple">
				...
				<fo:flow flow-name="xsl-region-body">
					<fo:block font-size="15pt" font-family="sans-serif" font-weight="bold" text-align="left" space-after.optimum="15pt" break-before="page">
						<xsl:text>Unauthorised Case Status Report</xsl:text>
					</fo:block>					
					<!--<xsl:apply-templates select="."/>-->
					<xsl:call-template name="UnauthorisedCases"/>
					<fo:block id="theEnd"/>
				</fo:flow>
			</fo:page-sequence>
		</fo:root>
