...
version="1.0">
    ...
    <xsl:include href="dailyCourtRoomListFO_common.xsl"/>
    
    <!-- Version Information -->
    <xsl:variable name="majorVersion" select="'2'" />
    <xsl:variable name="minorVersion" select="'0e'" />
    <xsl:variable name="stylesheet" select="'dailyListFO_cy.xsl'" />
    <xsl:variable name="last-modified-date" select="'2005-11-09'" />
    ...
    <xsl:template  match="cs:DailyList">
        <fo:root xmlns:fo="http://www.w3.org/1999/XSL/Format">
            ...
            <xsl:call-template name="DisplayDailyList">
                ...
                <xsl:with-param name="listType">CR</xsl:with-param>
            </xsl:call-template>
        </fo:root>
    </xsl:template>
</xsl:stylesheet>