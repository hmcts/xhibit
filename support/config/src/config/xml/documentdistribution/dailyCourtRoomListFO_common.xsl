...
version="1.0">
    ...
    <xsl:template name="PageSetUp">
        <fo:layout-master-set>
            ...
            <fo:simple-page-master master-name="first" page-height="297mm" page-width="210mm" margin-top="10mm" margin-bottom="10mm" margin-left="10mm" margin-right="10mm">
                ...
                <fo:region-before extent="10mm" region-name="header-first"/>
            ...
            <fo:simple-page-master master-name="rest" page-height="297mm" page-width="210mm" margin-top="10mm" margin-bottom="10mm" margin-left="10mm" margin-right="10mm">
                <fo:region-body margin-bottom="20mm"/>
                <fo:region-before extent="10mm" region-name="header-rest"/>
                <fo:region-after extent="20mm"/>
            </fo:simple-page-master>
    ...
    <xsl:template match="cs:CourtLists">
        ...
        <xsl:for-each select="cs:CourtList">
            ...
            <xsl:for-each select="cs:Sittings/cs:Sitting">
				...
				<xsl:variable name="CurrentCourtRoomNumber">
					<xsl:value-of select="cs:CourtRoomNumber"/>
				</xsl:variable>
				<!--If this is the first sitting in this court with this court room-->
				<xsl:if test="position() != 1 and not(./preceding-sibling::cs:Sitting[cs:CourtRoomNumber=$CurrentCourtRoomNumber])">
					<!--Start a new page-->
					<fo:block break-before="page"/>
        ...
        <fo:block font-size="16pt">
            <fo:inline font-weight="bold">
                ...
                <xsl:variable name="theCC">
                    ...
                    <xsl:value-of select="$path/cs:CourtHouse/cs:CourtHouseType"/>
                </xsl:variable>
                <xsl:call-template name="getValue">
                    <xsl:with-param name="language" select="$language"/>
            ...
            <xsl:variable name="firstcourthousename">
                <xsl:value-of select="$path/cs:CourtHouse/cs:CourtHouseName"/>
            </xsl:variable>
            <xsl:call-template name="getValue">
                <xsl:with-param name="language" select="$language"/>
                    ...
                    <xsl:choose>
                        ...
                        <xsl:when test="position()=1">
                            <fo:table-row space-after="10pt">
                                ...
                                <fo:table-cell>
                                    <fo:block>
                                        <!-- Respondent Name goes here -->
										<xsl:value-of select="$respNameText"/>
                                    </fo:block>
                                </fo:table-cell>
							</fo:table-row>
							<fo:table-row>
    ...
    <xsl:template name="HeaderInfo">
        ...
        <xsl:param name="listType"/>
		<xsl:param name="path" select="."/>
        <fo:block space-before="8pt"  text-align="center" font-size="12pt" font-weight="bold">
			<xsl:call-template name="getValue">
				<xsl:with-param name="language" select="$language"/>
        ...
        <fo:block text-align="center" font-size="12pt" space-after="8pt" font-weight="bold">
			<xsl:value-of select="$path/cs:CourtHouse/cs:Description"/>
        </fo:block>
    </xsl:template>
    ...
    <xsl:template name="hearing">
        ...
        <!-- get the Respondent Name text -->
        <xsl:variable name="respondentNameText">
            <xsl:choose>
                <xsl:when test="cs:RespondentDetails/cs:Respondent">
                    <xsl:text> - v -</xsl:text>
					<!-- Surname in upper case -->
					<xsl:if test="string-length(cs:RespondentDetails/cs:Respondent/cs:RespondentName3) > 0 and cs:RespondentDetails/cs:Respondent/cs:RespondentName3 != '-'">
						<xsl:text> </xsl:text>
						<xsl:call-template name="toUpper">
							<xsl:with-param name="content" select="cs:RespondentDetails/cs:Respondent/cs:RespondentName3"/>
						</xsl:call-template>
					</xsl:if>
					<!-- First name, only the initial capitalised -->
					<xsl:if test="string-length(cs:RespondentDetails/cs:Respondent/cs:RespondentName1) > 0 and cs:RespondentDetails/cs:Respondent/cs:RespondentName1 != '-'">
						<xsl:text> </xsl:text>
						<xsl:call-template name="TitleCase">
							<xsl:with-param name="text" select="cs:RespondentDetails/cs:Respondent/cs:RespondentName1"/>
						</xsl:call-template>
					</xsl:if>
					<!-- First character of the middle name in upper case -->
					<xsl:if test="string-length(cs:RespondentDetails/cs:Respondent/cs:RespondentName2) > 0 and cs:RespondentDetails/cs:Respondent/cs:RespondentName2 != '-'">
						<xsl:text> </xsl:text>
						<xsl:call-template name="toUpper">
							<xsl:with-param name="content" select="substring(cs:RespondentDetails/cs:Respondent/cs:RespondentName2,1,1)"/>
						</xsl:call-template>
					</xsl:if>
                </xsl:when>
                <xsl:otherwise>
        ...
        <xsl:choose>
            ...
            <xsl:when test="cs:Defendants/cs:Defendant">
                ...
                <xsl:call-template name="DefendantDetails">
                    ...
                    <xsl:with-param name="respNameText" select="$respondentNameText"/>
                ...
                <fo:table table-layout="fixed" font-size="9pt">
                    ...
                    <fo:table-body>
                        <fo:table-row>
							...
							<fo:table-cell>
                                <fo:block>
                                    <xsl:value-of select="$respondentNameText"/>
                                </fo:block>
                            </fo:table-cell>
                        </fo:table-row>
                    </fo:table-body>
