...
[not(self::*/@HearingType=preceding::cs:WithFixedDate/@HearingType)]">
    ...
    <xsl:template match="cs:CourtLists" mode="satellite">
        ...
        <xsl:for-each select="cs:CourtList/cs:CourtHouse/cs:Description[. != $firstCourtHouseCode and not(. = ../../preceding-sibling::cs:CourtList/cs:CourtHouse/cs:Description)]">
            ...
            <xsl:for-each select="../../../cs:CourtList[cs:CourtHouse/cs:Description = $crtCode]/cs:WithFixedDate 
[not(self::*/@HearingType=preceding::cs:WithFixedDate[../cs:CourtHouse/cs:Description = $crtCode]/@HearingType)]">
				<!--The Hearing/ListNote will be populated with the Hearing Sequence Number so can be used to order by-->
				<xsl:sort select="./cs:Fixture/cs:Cases/cs:Case/cs:Hearing/cs:ListNote" order="ascending" data-type="number"/>
                <xsl:variable name="hearingType" select="./@HearingType"/>
                <!-- display the case details -->
            ...
            <xsl:for-each select="../../../cs:CourtList[cs:CourtHouse/cs:Description = $crtCode]/cs:WithoutFixedDate 
 [not(self::*/@HearingType=preceding::cs:WithoutFixedDate[../cs:CourtHouse/cs:Description = $crtCode]/@HearingType)]">
 				<!--The Hearing/ListNote will be populated with the Hearing Sequence Number so can be used to order by-->
				<xsl:sort select="./cs:Fixture/cs:Cases/cs:Case/cs:Hearing/cs:ListNote" order="ascending" data-type="number"/>
                <xsl:variable name="hearingType" select="./@HearingType"/>
                <!-- display the case details -->
    ...
    <xsl:template match="cs:CrownCourt">
        ...
        <fo:block text-align="left">
            ...
            <fo:block font-size="15pt" space-after="8pt" font-weight="normal">
                <xsl:call-template name="getValue">
                    <xsl:with-param name="language" select="$language"/>
            ...
            <fo:block font-size="16pt" text-align="center" font-weight="bold">
                ...
                <fo:block font-size="12pt" text-align="center" font-weight="normal">
                    <xsl:choose>
    ...
    <!--
        ...
        ***********************
    -->
    <!-- template to format text into bullet points -->
                        ...
                        <xsl:choose>
                                    ...
                                    <xsl:choose>
                                        <xsl:when test="starts-with(. , 'no later than')">
                                            ...
                                            <xsl:variable name="nolaterthantime">
                                                <xsl:value-of select="substring-before(substring-after($bulletText,'no later than '),' on ')"/>
                                            </xsl:variable>
                                            ...
                                            <xsl:variable name="nolaterthanday">
                                                <xsl:value-of select="substring-before(substring-after($bulletText,' on '),' ')"/>
                                            </xsl:variable>
                                            <xsl:if test="string-length($nolaterthanday) > 0">
                                                <xsl:value-of select="$nolaterthanday"/>
                                                <xsl:text> </xsl:text>
                                                <xsl:variable name="nolaterthanmonth">
                                                    <xsl:value-of select="substring-before(substring-after($bulletText,concat(' on ',$nolaterthanday,' ')),' ')"/>
                                                </xsl:variable>
                                                <xsl:call-template name="getValue">
                                                    <xsl:with-param name="language" select="$language"/>
                                                ...
                                                <xsl:variable name="nolaterthanyear">
                                                    <xsl:value-of select="substring-after($bulletText,concat($nolaterthanmonth,' '))"/>
                                                </xsl:variable>
                            ...
                            <xsl:otherwise>
								<xsl:call-template name="format-text-for-wrapping">
									<xsl:with-param name="str" select="$bulletText"/>
								</xsl:call-template>
                            </xsl:otherwise>
                        </xsl:choose>
                    </fo:block>
                    ...
                    <fo:table-cell>
                        ...
                        <fo:block font-size="10pt">
                            <xsl:variable name="satelliteCourtHouseNames">
                                <xsl:call-template name="getValue">
                                    <xsl:with-param name="language" select="$language"/>
                                    <xsl:with-param name="key" select="cs:WarnedList/cs:CrownCourt/cs:CourtHouseName"/>
                                </xsl:call-template>
                            </xsl:variable>
    ...
    <xsl:template name="WarnedListDetails">
        <xsl:param name="language"/>
        <fo:block text-align="left" font-size="11pt" space-after="10pt">
            ...
            <fo:list-block start-indent="5mm" provisional-distance-between-starts="5mm" font-size="10pt">
        ...
        <fo:block space-after="10pt" text-align="center">
            <fo:leader leader-pattern="rule" space-before.optimum="12pt" space-after.optimum="12pt" leader-length="80mm"/>
        </fo:block>
